...
function readAbleCode(params, code, id) {
  ...
  walk.simple(ast, {
    FunctionDeclaration(n) {
      if (n.params.length) functions.push({ start: n.start, end: n.end });
    },
  ...
  walk.simple(ast, {
    Identifier(node) {
      ...
      if (node.name === params[0]) {
        edits.push({ start: node.start, end: node.end, replacement: "module" });
  ...
  walk.simple(ast, {
    Identifier(node) {
      ...
      if (
        ...
        edits.push({ start: node.start, end: node.end, replacement: mpf[1] });
        return;
      }
    },
  });
  walk.simple(ast, {
    Identifier(node) {
      if (isInsideFunc(node)) return;
      if (mappings[node.name] && typeof mappings[node.name] === "string") {
        edits.push({
          start: node.start,
          end: node.end,
          replacement: mappings[node.name],
