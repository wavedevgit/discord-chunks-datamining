/** Chunk was on 31667 **/
/** chunk id: 880229, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  ImportBenefitsFromRoleModal: () => W,
  ImportBenefitsFromSubscriptionListingModal: () => L
}), require("./388685.js"), require("./953529.js"), require("./361932.js"), require("./187205.js");
var Chunk951288 = require("./951288.js"),
  Chunk647438 = require("./647438.js"),
  Chunk120356 = require("./120356.js"),
  o = require.n(Chunk120356),
  Chunk793030 = require("./793030.js"),
  Chunk442837 = require("./442837.js"),
  Chunk902704 = require("./902704.js"),
  Chunk477690 = require("./477690.js"),
  Chunk481060 = require("./481060.js"),
  Chunk596454 = require("./596454.jsx"),
  Chunk211266 = require("./211266.js"),
  Chunk471445 = require("./471445.js"),
  Chunk91218 = require("./91218.jsx"),
  Chunk313201 = require("./313201.js"),
  Chunk339085 = require("./339085.js"),
  Chunk518738 = require("./518738.js"),
  Chunk131704 = require("./131704.js"),
  Chunk345162 = require("./345162.js"),
  Chunk324067 = require("./324067.js"),
  Chunk485386 = require("./485386.js"),
  Chunk430824 = require("./430824.js"),
  Chunk259580 = require("./259580.jsx"),
  Chunk624138 = require("./624138.js"),
  Chunk817460 = require("./817460.js"),
  Chunk166803 = require("./166803.js"),
  Chunk629262 = require("./629262.js"),
  Chunk686807 = require("./686807.jsx"),
  Chunk981631 = require("./981631.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk916029 = require("./916029.js");
let z = (0, Chunk313201.hQ)(),
  E = (0, Chunk313201.hQ)(),
  P = (0, Chunk624138.Mg)(Chunk477690.Z.IMPORT_BENEFITS_MODAL_ROLE_ICON_SIZE);

function R(e) {
  var t;
  let {
    emojiId: n,
    emojiName: r,
    className: l
  } = e, o = (0, s.e7)([j.ZP], () => null != n ? j.ZP.getCustomEmojiById(n) : true);
  return (0, i.jsx)(h.Z, {
    emojiId: n,
    emojiName: r,
    animated: null != (t = null == o ? true : o.animated) && t,
    className: l
  })
}

function O(e) {
  let {
    icon: t,
    children: n,
    checked: r,
    disabled: l = false,
    onChange: a
  } = e, s = r || l;
  return (0, i.jsxs)("label", {
    className: o()(B.row, {
      [B.checked]: s,
      [B.disabled]: l
    }),
    children: [(0, i.jsx)("div", {
      className: B.rowIconWrapper,
      children: t
    }), (0, i.jsx)("div", {
      className: B.rowText,
      children: n
    }), (0, i.jsxs)("div", {
      className: B.rowCheckbox,
      children: [(0, i.jsx)(m.tEY, {
        children: (0, i.jsx)("input", {
          type: "checkbox",
          checked: s,
          disabled: l,
          className: B.checkboxInput,
          onChange: function(e) {
            a(e.target.checked)
          }
        })
      }), (0, i.jsx)("div", {
        className: B.checkboxDisplay,
        children: (0, i.jsx)(m.dz2, {
          size: "md",
          color: "currentColor",
          className: B.checkboxIcon,
          "aria-hidden": true
        })
      })]
    })]
  })
}

function D(e) {
  let {
    title: t,
    selectAllText: n,
    benefits: r,
    dupeBenefits: l,
    selectedBenefits: o,
    onToggleBenefit: a
  } = e, s = r.every(e => o.has(e));
  return r.length > 0 ? (0, i.jsxs)("div", {
    className: B.section,
    children: [(0, i.jsx)(m.Heading, {
      variant: "heading-md/semibold",
      color: "header-primary",
      children: t
    }), r.length - l.size > 1 && (0, i.jsx)("div", {
      className: B.list,
      children: (0, i.jsx)(O, {
        icon: (0, i.jsx)(m.avL, {
          size: "md",
          color: "currentColor",
          className: B.rowIcon
        }),
        checked: s,
        onChange: function(e) {
          for (let t of r) l.has(t) || a(t, e)
        },
        children: (0, i.jsx)(m.Text, {
          color: "header-primary",
          variant: "text-md/normal",
          children: n
        })
      })
    }), (0, i.jsx)("div", {
      className: B.list,
      children: r.map(e => (0, i.jsxs)(O, {
        icon: (0, i.jsx)(R, {
          emojiId: e.emoji_id,
          emojiName: e.emoji_name,
          className: B.rowIcon
        }),
        checked: o.has(e),
        disabled: l.has(e),
        onChange: t => a(e, t),
        children: [(0, i.jsx)(m.Text, {
          color: "header-primary",
          variant: "text-md/semibold",
          children: (0, T.Z)(e)
        }), (0, i.jsx)(m.Text, {
          color: "interactive-normal",
          variant: "text-sm/normal",
          children: e.description
        })]
      }, (0, k.ab)(e)))
    })]
  }) : null
}

function L(e) {
  let {
    transitionState: t,
    fromSubscriptionListing: n,
    existingChannelBenefits: l,
    existingIntangibleBenefits: o,
    onSubmit: s,
    onClose: d
  } = e, h = n.role_benefits.benefits, [u, x] = r.useState(() => new Set(h)), f = r.useMemo(() => h.filter(k.rC), [h]), g = (0, S.Z)(f), j = r.useMemo(() => h.filter(k.lL), [h]), p = r.useMemo(() => new Set(g.filter(e => l.some(t => t.ref_id === e.ref_id))), [g, l]), _ = r.useMemo(() => new Set(j.filter(e => o.some(t => (0, c.Z)(e, t)))), [o, j]);

  function v(e, t) {
    x(n => {
      let i = new Set(n);
      return t ? i.add(e) : i.delete(e), i
    })
  }

  function b(e) {
    e.preventDefault();
    let t = h.filter(e => u.has(e));
    s(t.filter(k.rC).filter(e => !p.has(e)), t.filter(k.lL).filter(e => !_.has(e))), d()
  }
  return (0, i.jsx)("form", {
    onSubmit: b,
    children: (0, i.jsxs)(a.Modal, {
      transitionState: t,
      onClose: d,
      title: M.intl.format(M.t["7zpiYG"], {
        tierName: n.name
      }).toString(),
      actions: [{
        variant: "secondary",
        text: M.intl.string(M.t["ETE/oK"]),
        onClick: d
      }, {
        variant: "primary",
        text: M.intl.string(M.t["27a219"]),
        disabled: 0 === u.size,
        onClick: b
      }],
      children: [0 === g.length && 0 === j.length && (0, i.jsx)(m.Text, {
        variant: "text-md/normal",
        children: M.intl.string(M.t["I+16xs"])
      }), (0, i.jsx)(D, {
        title: M.intl.string(M.t.S2kKzM),
        selectAllText: M.intl.string(M.t["5Q/fLC"]),
        benefits: g,
        dupeBenefits: p,
        selectedBenefits: u,
        onToggleBenefit: v
      }), (0, i.jsx)(D, {
        title: M.intl.string(M.t.RdwKw8),
        selectAllText: M.intl.string(M.t.aJPLVF),
        benefits: j,
        dupeBenefits: _,
        selectedBenefits: u,
        onToggleBenefit: v
      })]
    })
  })
}

function H(e) {
  var t;
  let {
    guildId: n,
    role: r,
    channelCount: l,
    onSelect: o
  } = e, a = (0, p.p9)({
    guildId: n,
    roleId: r.id,
    size: P
  });
  return (0, i.jsxs)(m.P3F, {
    className: B.roleRow,
    onClick: o,
    children: [(0, i.jsx)("div", {
      className: B.rowIconWrapper,
      children: null != a ? (0, i.jsx)(f.Z, function(e) {
        for (var t = 1; t < arguments.length; t++) {
          var n = null != arguments[t] ? arguments[t] : {},
            i = Object.keys(n);
          "function" == typeof Object.getOwnPropertySymbols && (i = i.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
            return Object.getOwnPropertyDescriptor(n, e).enumerable
          }))), i.forEach(function(t) {
            var i;
            i = n[t], t in e ? Object.defineProperty(e, t, {
              value: i,
              enumerable: true,
              configurable: true,
              writable: true
            }) : e[t] = i
          })
        }
        return e
      }({}, a)) : (0, i.jsx)(m.lZ8, {
        size: "custom",
        width: 23,
        height: 23,
        color: null != (t = r.colorString) ? t : Z.Pbq,
        className: B.roleRowIconDefault
      })
    }), (0, i.jsxs)("div", {
      className: B.rowText,
      children: [(0, i.jsx)(m.Text, {
        color: "header-primary",
        variant: "text-md/normal",
        className: B.__invalid_benefitTitle,
        children: r.name
      }), (0, i.jsx)(m.Text, {
        color: "header-secondary",
        variant: "text-sm/normal",
        children: M.intl.format(M.t["2KzH8/"], {
          num: l
        })
      })]
    }), (0, i.jsx)("div", {
      className: B.rowCheckbox,
      children: (0, i.jsx)(I.Z, {
        direction: I.Z.Directions.RIGHT
      })
    })]
  })
}

function F(e) {
  let {
    guild: t,
    onSelect: n
  } = e, l = (0, s.e7)([C.Z], () => C.Z.getSortedRoles(t.id)), o = (0, s.e7)([b.Z], () => b.Z.getCategories(t.id)), a = r.useMemo(() => o._categories.flatMap(e => o[e.channel.id].filter(e => {
    let {
      channel: t
    } = e;
    return _.zS.has(t.type)
  }).map(e => {
    let {
      channel: t
    } = e;
    return t
  })), [o]), c = r.useMemo(() => l.flatMap(e => {
    var t;
    if ((0, v.fI)(e) || (null == (t = e.tags) ? true : t.subscription_listing_id) != null || (0, v._N)(e, Z.TC2)) return [];
    let n = (0, y.Z)(a, e).length;
    return n > 0 ? {
      role: e,
      channelCount: n
    } : []
  }), [l, a]);
  return c.length > 0 ? (0, i.jsx)("div", {
    className: B.list,
    children: c.map(e => (0, i.jsx)(H, {
      guildId: t.id,
      role: e.role,
      channelCount: e.channelCount,
      onSelect: () => n(e.role)
    }, e.role.id))
  }) : (0, i.jsx)(m.Text, {
    variant: "text-md/normal",
    children: M.intl.string(M.t.u09gKi)
  })
}

function K(e) {
  let {
    onClose: t,
    guildId: n,
    onSelect: r
  } = e, l = (0, s.e7)([w.Z], () => w.Z.getGuild(n));
  return null == l ? null : (0, i.jsxs)(i.Fragment, {
    children: [(0, i.jsxs)(m.xBx, {
      "data-migration-pending": true,
      children: [(0, i.jsx)(m.Heading, {
        id: z,
        variant: "heading-md/semibold",
        children: M.intl.string(M.t["3JrZ/P"])
      }), (0, i.jsx)(m.olH, {
        "data-migration-pending": true,
        className: B.modalCloseButton,
        onClick: t
      })]
    }), (0, i.jsx)(m.hzk, {
      "data-migration-pending": true,
      className: B.modalContent,
      children: (0, i.jsx)(F, {
        guild: l,
        onSelect: r
      })
    })]
  })
}

function G(e) {
  let {
    onClose: t,
    guildId: n,
    role: l,
    onCancel: o,
    onImport: a
  } = e, s = r.useRef(l);
  null != l && (s.current = l);
  let c = null != l ? l : s.current,
    d = (0, u.Z)(() => {
      let e = b.Z.getCategories(n);
      return e._categories.flatMap(t => e[t.channel.id].filter(e => {
        let {
          channel: t
        } = e;
        return _.zS.has(t.type)
      }).map(e => {
        let {
          channel: t
        } = e;
        return t
      }))
    }),
    h = r.useMemo(() => (0, y.Z)(d, c), [d, c]),
    [f, g] = r.useState(() => new Set(h.map(e => e.id)));

  function j(e, t) {
    g(n => {
      let i = new Set(n);
      return t ? i.add(e) : i.delete(e), i
    })
  }
  return (0, i.jsxs)("form", {
    onSubmit: function(e) {
      e.preventDefault(), a(h.map(e => e.id).filter(e => f.has(e))), t()
    },
    className: B.slideForm,
    children: [(0, i.jsxs)(m.xBx, {
      "data-migration-pending": true,
      children: [(0, i.jsx)(m.Heading, {
        id: E,
        variant: "heading-md/semibold",
        children: M.intl.format(M.t.iuPodH, {
          roleName: c.name
        })
      }), (0, i.jsx)(m.olH, {
        "data-migration-pending": true,
        className: B.modalCloseButton,
        onClick: t
      })]
    }), (0, i.jsxs)(m.hzk, {
      "data-migration-pending": true,
      className: B.modalContent,
      children: [h.length > 1 && (0, i.jsx)("div", {
        className: B.list,
        children: (0, i.jsx)(O, {
          icon: (0, i.jsx)(m.avL, {
            size: "md",
            color: "currentColor"
          }),
          checked: f.size === h.length,
          onChange: function(e) {
            for (let t of h) j(t.id, e)
          },
          children: (0, i.jsx)(m.Text, {
            color: "header-primary",
            variant: "text-md/normal",
            children: M.intl.string(M.t["5Q/fLC"])
          })
        })
      }), (0, i.jsx)("div", {
        className: B.list,
        children: h.map(e => {
          var t;
          let n = null != (t = (0, x.Th)(e.type)) ? t : m.VL1;
          return (0, i.jsx)(O, {
            icon: (0, i.jsx)("div", {
              className: B.rowIcon,
              children: (0, i.jsx)(n, {
                size: "custom",
                color: "currentColor",
                width: "100%",
                height: "100%"
              })
            }),
            checked: f.has(e.id),
            onChange: t => j(e.id, t),
            children: (0, i.jsx)(m.Text, {
              color: "header-primary",
              variant: "text-md/normal",
              className: B.__invalid_benefitTitle,
              children: e.name
            })
          }, e.id)
        })
      })]
    }), (0, i.jsx)(m.mzw, {
      "data-migration-pending": true,
      children: (0, i.jsxs)(m.ButtonGroup, {
        direction: "horizontal-reverse",
        children: [(0, i.jsx)(m.Button, {
          variant: "primary",
          text: M.intl.string(M.t["1j8So6"]),
          type: "submit"
        }), (0, i.jsx)(m.Button, {
          variant: "secondary",
          text: M.intl.string(M.t["ETE/oK"]),
          onClick: o
        })]
      })
    })]
  })
}

function W(e) {
  let {
    transitionState: t,
    guildId: n,
    onClose: l,
    onImport: o
  } = e, [a, s] = r.useState();
  return (0, i.jsx)(m.Y0X, {
    "data-migration-pending": true,
    transitionState: t,
    "aria-labelledby": null == a ? z : E,
    parentComponent: "ImportBenefitsModal",
    children: (0, i.jsxs)(m.MyZ, {
      contentDisplay: "flex",
      activeSlide: +(null != a),
      width: 440,
      children: [(0, i.jsx)(m.Mi4, {
        id: 0,
        children: (0, i.jsx)(K, {
          onClose: l,
          guildId: n,
          onSelect: s
        })
      }), (0, i.jsx)(m.Mi4, {
        id: 1,
        children: (0, i.jsx)(G, {
          onClose: l,
          guildId: n,
          role: a,
          onCancel: () => s(true),
          onImport: o
        })
      })]
    })
  })
}