...
function y(A) {
  ...
  return <O src={(0, l.x)(t)} aria-hidden={true} size={e} />
}
...
function x(A) {
  ...
  return <O size={e} src={null == g ? true : g.getAvatarURL(true, 40)} status={n} isMobile={f} isTyping={v} aria-label={null == g ? true : g.username} statusTooltip={true} />
}
let L = A => {
    let {
      className: t,
    ...
    if (l && null != i && !C) a = <Q.Z size={Q.E.SMALL_32} className={P.iconContainerWithGuildIcon} iconClassName={P.iconWithGuildIcon} channel={g} guild={i} locked={v} hasActiveThreads={f} />;
    else {
  ...
  X = 12633 == require.j ? Chunk73800.forwardRef(function(A, t) {
    ...
    return <h.tEY focusTarget={Av} ringTarget={An} offset={{
        top: 2,
        bottom: 2,
        right: 4
      }}><div ref={t} className={n()(G, null != O || null != x || null != y || U ? P.wrapper : P.notInteractive, (() => {
        ...
        }(null != S ? S : Y))} onMouseUp={A => null == x ? true : x(A, l)} onMouseDown={A => null == y ? true : y(A, l)} onContextMenu={A => null == X ? true : X(A, l)} onMouseEnter={N} onMouseLeave={K}>{Q || !M ? null : <div className={n()(P.unread, At ? P.unreadImportant : true)} />}{null != (B = null == m ? true : m(AC)) ? B : AC}</div></h.tEY>
  }) : null