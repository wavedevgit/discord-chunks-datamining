...
function T(e) {
  ...
  return i.useMemo(() => {
    if (null == A) return;
    let e = null;
    return k ? e = <div className={v.container}>{<f.Z defaultAnimationState={l.SR.LOOP} giftStyle={x} className={v.seasonalGiftBoxHeaderIcon} />}{<s.olH onClick={T} className={v.closeButton} data-migration-pending={true} />}</div> : j ? e = n(null != N ? N : null, T, A) : R === O.GZ.ONE_TIME ? e = <p.t step={A} onClose={T} /> : G && (a()(I(S, y.y7), "invalid sku id: ".concat(S)), e = <_.Z currentStep={null != A ? A : true} purchaseState={C} premiumType={y.y7[S]} onClose={T} showTrialBadge={Z} showDiscountBadge={H} isGift={L} giftRecipient={M} useWinterTheme={K} isEligibleForTrial={Z} enablePremiumBrandRefresh={w} isDisplayingWowMomentConfirmation={D} />), e
