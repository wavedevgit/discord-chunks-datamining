...
let v = function(e) {
  ...
  }, N = <n.Fragment>{<div className={j.verification}>{<div className={j.header}>{<l.X6q variant={"text-sm/bold"} color={"header-primary"}>{y.intl.string(y.t.cgBTyM)}</l.X6q>}{null != w && <b.H className={j.copyIcon} chunks={w} color={l.TVs.colors.INTERACTIVE_NORMAL} onCopy={k} />}</div>}{<p.b className={j.code} chunks={w} columns={m.WK} />}</div>}{<l.Text className={j.footer} variant={"text-sm/normal"} color={"text-muted"}>{y.intl.format(y.t["H3+ktr"], {
        helpArticle: (0, d.uV)()
      })}</l.Text>}</n.Fragment>;
  return _ ? <i.Modal{...g(O({}, S), {
    onClose: x,
    actions: [],
    children: N
  })} /> : <h.Z{...g(O({}, S), {
