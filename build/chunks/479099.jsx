...
function I(e) {
  ...
  }, B = <r.Fragment>{x ? <d.Z className={a()(g.emoji, {
        [g.small]: M
      })} emojiId={N} emojiName={C} animated={!!(null == D ? true : D.animated)} size={"reaction"} /> : null}{P && R && <div className={g.closeCircle}><c.Dio size={"md"} color={"currentColor"} className={g.close} /></div>}{<c.Text variant={M ? "text-xs/semibold" : "text-sm/semibold"} lineClamp={1} color={"currentColor"}>{A}</c.Text>}</r.Fragment>, Z = {
    key: t.id,
    className: a()(g.pill, {
      [g.disabled]: E,
      [g.clickable]: L,
...
function T(e) {
  ...
  return <c.ua7 aria-label={m.intl.string(m.t["P/y+sr"])} text={(0, r.jsx)(r.Fragment, {
