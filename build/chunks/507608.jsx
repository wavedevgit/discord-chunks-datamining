...
function G(e) {
  ...
  }, [g, t.id, v]), <div className={C.wrapper}>{<l.j title={t.name} iconSrc={u} backgroundSrc={m} animatedBackgroundSrc={f} prefersReducedMotion={j} />}{<B appId={t.id} subscriptions={o} otps={s} guildId={n} />}</div>
}
...
function B(e) {
  ...
  return <div className={C.productsContainer}>{n.length > 0 && <div>{<A.r subscriptions={n} />}{<div className={C.productSection}>{n.map(e => (0, r.jsx)(z, {
          appId: t,
          guildId: l,
          listing: e,
          subscriptionType: (0, f.KW)(e.skuFlags) ? "user" : "guild"
        }, e.id))}</div>}</div>}{i.length > 0 && <div>{<u.X6q variant={"heading-lg/semibold"}>{_.intl.string(_.t.yUGTs7)}</u.X6q>}{<div className={C.productSection}>{i.map(e => (0, r.jsx)(q, {
          skuId: e.skuId,
          appId: t
        }, e.id))}</div>}</div>}</div>
}
...
function z(e) {
  ...
  let {
  ...
  }, [o, d.benefits]), K = i.useMemo(() => null != d.thumbnail ? (0, E.q)(o, d.thumbnail, 256) : true, [o, d.thumbnail]), J = i.useCallback(e => <R.pV{...L(F({}, e), {
    appId: o,
    subscriptionType: p,
    skuId: d.skuId,
    subscriptionPlan: P,
    ...
    if (q) return <r.Fragment>{null != k && <b.Z guild={k} size={b.Z.Sizes.MINI} />}{_.intl.string(_.t.l8ufqK)}</r.Fragment>
  }, [null == N ? true : N.currentPeriodEnd, k, D, G, q, A]), X = null != W ? <u.Text color={"header-secondary"} variant={"text-sm/normal"} className={C.subscriptionCardSubtext}>{W}</u.Text> : true;
  return <s.Uj title={d.summary} subtitle={X} maxBenefits={null != X ? s.ZI - 1 : s.ZI} description={null != (l = d.description) ? l : true} imgSrc={K} onPurchase={B} renderPurchaseButton={J} benefitItems={null != V ? V.map(e => (0, r.jsx)(s.Gm, {
      description: e.title,
      icon: e.icon
    }, e.id)) : true} onDetails={null != v ? v : null != V ? () => {
      (0, u.ZDy)(async () => {
        let {
...
function q(e) {
  ...
  return <o.I title={O} description={P} headerImage={I} availabilityLabel={E} PurchaseButton={e => (0, r.jsx)(R.YG, L(F({}, e), {
      appId: b,
      sku: g
