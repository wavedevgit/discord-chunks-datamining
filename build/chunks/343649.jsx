...
function ec(e) {
  ...
  if (eh) t = <g.Vq onClose={e9} />;
  else if (ef && eO && eB && null != tr && "" !== tr)
    if ((null == e_ ? true : e_.isPausedOrPausePending) && !e_.isPausedAllowsUpdatesButNotResume) t = <m.hzk><p className={et.copy}>{ee.intl.string(ee.t.mOWsFx)}</p></m.hzk>;
    else if (null != e_ && null != e_.renewalMutations) t = <m.hzk><p className={et.copy}>{ee.intl.string(ee.t.npfhh4)}</p></m.hzk>;
    ...
    t = <$.Z premiumSubscriptionPlan={eg} analyticsLocation={_} analyticsSourceLocation={er} onClose={e9} onBack={() => e0(L.h8.PLAN_SELECT)} onSkip={() => e0(null != eI || eU ? L.h8.REVIEW : L.h8.ADD_PAYMENT_STEPS)} onSubscriptionConfirmation={eE} priceOptions={e} />
  } else {
    let e, n, l, i;
    ...
    switch (eq) {
      case L.h8.PLAN_SELECT:
        c()(null != ec, "Missing guildId"), c()(null != eg, "Missing nextPremiumSubscriptionPlan"), e = <K.CP premiumSubscriptionPlan={eg} numGuildBoosts={eG} setNumGuildBoosts={ez} setForceDisableSubmitButton={e6} premiumSubscription={e_} existingAvailableSlots={ex} onClickPremiumSubscriptionLink={() => {
            if (__BILLING_STANDALONE__) {
              window.location.href = "discord://app/settings/nitro";
              return
            }
            e9(), null != s && s(), (0, C.z)()
          }} guildId={ec} priceOptions={o} />, te && null != e_ && null != e_.paymentGateway && (e = <r.Fragment>{<m.kzN className={et.externalErrorBlock}>{ee.intl.format(ee.t["/m3Y3t"], {
              paymentGatewayName: Q.Vz[e_.paymentGateway]
            })}</m.kzN>}{e}</r.Fragment>), l = <E.zx look={E.zx.Looks.LINK} color={(0, d.ap)(eb) ? E.zx.Colors.PRIMARY : E.zx.Colors.WHITE} onClick={e9}>{ee.intl.string(ee.t.oEAioK)}</E.zx>, i = <m.zxk variant={"primary"} text={ee.intl.string(ee.t["3PatS0"])} type={"submit"} disabled={e5 || 0 === eG || te} onClick={() => {
      ...
      case L.h8.AWAITING_AUTHENTICATION:
        e = <h.F className={et.__invalid_body} />;
        break;
      case L.h8.REVIEW:
        c()(null != eg, "Missing nextPremiumSubscriptionPlan"), e = <K.Gq paymentSources={eT} priceOptions={o} currentPremiumSubscription={e_} premiumSubscriptionPaymentSourceId={eI} premiumSubscriptionPlan={eg} newAdditionalPlans={eH} onPaymentSourceChange={e => eR(null != e ? e.id : null)} onPaymentSourceAdd={() => {
            e0(L.h8.ADD_PAYMENT_STEPS), eR(null)
          }} onPurchaseTermsChange={eW} legalTermsNodeRef={tt} hasLegalTermsFlash={tn} />, n = L.h8.PLAN_SELECT, i = eF ? <m.zxk variant={"active"} text={ee.intl.string(ee.t.eUEeCg)} type={"submit"} loading={eD} onClick={async () => {
          ...
          }} /> : <m.ua7 text={ee.intl.string(ee.t.XdvBLS)}>{e => (0, r.jsx)(m.zxk, el(en({
            variant: "active",
      ...
      case L.h8.CONFIRM:
        ...
        e = <K.R7 guild={a} guildBoostQuantity={eG + ex.length} onClose={e9} withAnimation={false} paymentSourceType={u} didPurchaseOnFractionalPremium={p} />
    }
    let _ = null != ev && null == (0, L.ly)(ev) ? ev : ew;
    t = eq === L.h8.ADD_PAYMENT_STEPS ? tp : <D.Z hideBreadcrumbs={eq === L.h8.CONFIRM} steps={e1} currentStep={eq} paymentError={_} purchaseErrorBlockRef={eZ} hasCurrencies={ts.length > 1} body={e} footer={eq !== L.h8.CONFIRM ? (0, r.jsxs)(m.mzw, {
  ...
  } else t = <div className={et.loadingWrapper}><m.$jN /></div>;
  let td = null;
  return eh || eq === L.h8.PREMIUM_UPSELL || (td = <X.Z onClose={e9} currentStep={eq} purchaseState={e7} />), <m.Y0X transitionState={n} className={o()({
      [et.planSelectStep]: eq === L.h8.PLAN_SELECT
    })} parentComponent={"GuildBoostPurchaseModal"}>{td}{t}</m.Y0X>
}
...
function eu(e) {
  ...
  return <N.Gt value={n}><M.PaymentContextProvider activeSubscription={t} stepConfigs={[]} skuIDs={[]}><ec{...en({}, e)} /></M.PaymentContextProvider></N.Gt>
}