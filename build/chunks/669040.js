/** Chunk was on web.js **/
"use strict";
n.d(t, {
  Z: () => el
}), n(47120);
var r = n(200651),
  i = n(192379),
  o = n(120356),
  a = n.n(o),
  s = n(392711),
  l = n.n(s),
  c = n(873546),
  u = n(442837),
  d = n(336317),
  f = n(477690),
  _ = n(481060),
  p = n(788911),
  h = n(2052),
  g = n(906732),
  m = n(218867),
  E = n(806966),
  v = n(28546),
  b = n(691251),
  y = n(98528),
  O = n(551058),
  S = n(536442),
  I = n(140465),
  T = n(775412),
  N = n(104494),
  A = n(639119),
  C = n(53691),
  R = n(75145),
  P = n(631771),
  D = n(695346),
  w = n(594174),
  L = n(626135),
  x = n(111361),
  M = n(74538),
  j = n(624138),
  k = n(543241),
  U = n(883661),
  G = n(880949),
  B = n(784222),
  F = n(172800),
  V = n(149203),
  Z = n(981631),
  H = n(957825),
  W = n(474936),
  Y = n(388032),
  K = n(711287);
let z = 0,
  q = 0,
  Q = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_LIST_PADDING_TOP),
  X = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_LIST_PADDING_RIGHT),
  J = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_LIST_PADDING_BOTTOM),
  $ = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_LIST_SEARCH_RESULTS_PADDING_TOP),
  ee = (0, j.Mg)(f.Z.EXPRESSION_PICKER_CONSTANTS_EXPRESSION_PICKER_LIST_SECTION_HEADING_HEIGHT),
  et = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_SECTION_MARGIN_BOTTOM),
  en = 16,
  er = (0, j.Mg)(f.Z.EMOJI_PICKER_CONSTANTS_EMOJI_CONTAINER_PADDING_VERTICAL),
  ei = 41,
  eo = 33,
  ea = e => e + 2 * er,
  es = e => {
    let {
      collapsedSections: t,
      diversitySurrogate: n,
      emojiGrid: o,
      emojiSize: s,
      onEmojiSelect: c,
      onSelectSoundmoji: f,
      searchQuery: p,
      sectionDescriptors: g,
      setCollapsedSections: m,
      getEmojiItemProps: y,
      getEmojiRowProps: T,
      isScrolling: N,
      isUsingKeyboardNavigation: A,
      allowAnimatedEmoji: C,
      channelGuildId: P,
      channelId: D,
      messageId: w,
      isBurstReaction: x,
      listHeaderClassName: M,
      activeSectionIndex: j,
      upsellGlowOpacity: W,
      emojiListRef: z
    } = e, er = (0, h.O)(), ea = i.useRef(Z.xAR), es = i.useMemo(() => "" !== n ? d.Z.convert.toCodePoint(n) : "", [n]), el = i.useCallback(e => {
      let t = g[e],
        n = g[e + 1];
      return e >= g.length - 1 ? t.isNitroLocked : t.isNitroLocked && !n.isNitroLocked
    }, [g]), ec = i.useCallback(e => {
      if (e >= g.length - 1) return !1;
      let t = g[e],
        n = g[e + 1];
      return !t.isNitroLocked && n.isNitroLocked
    }, [g]), eu = i.useCallback(function(e) {
      let n = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : null,
        r = new Set(t),
        i = t.has(e);
      i ? r.delete(e) : r.add(e), null != n && L.default.track(Z.rMx.EXPRESSION_PICKER_CATEGORY_COLLAPSE_TOGGLED, {
        location: er.location,
        tab: H.X1.EMOJI,
        collapsed: !i,
        guild_id: n.id
      }), e === V.UX.SOUNDMOJI && L.default.track(Z.rMx.SOUNDMOJI_EMOJI_PICKER_SECTION_TOGGLED, {
        collapsed: !i
      }), m(r)
    }, [er, t, m]), ed = i.useCallback(e => {
      let {
        columnIndex: t,
        visibleRowIndex: n
      } = e, r = e.type === B.ld.SOUNDMOJI ? null : e.type === B.ld.EMOJI ? e.emoji.allNamesString : e.name;
      ea.current !== Z.xAR && window.cancelAnimationFrame(ea.current), ea.current = window.requestAnimationFrame(() => {
        E.kJ.setInspectedExpressionPosition(t, n, b.u.MOUSE_EVENT), E.kJ.setSearchPlaceholder(r), ea.current = Z.xAR
      })
    }, []), ef = (0, u.e7)([S.qc], () => S.qc.hasHotspot(S.v6.FAVORITE_EMOJI_TOOLTIP), []), e_ = i.useMemo(() => l().memoize((e, t) => {
      let n = o[e],
        i = g[t.sectionIndex],
        a = () => {
          var e;
          null === (e = z.current) || void 0 === e || e.scrollToSectionTop(t.sectionIndex, {
            animate: !0
          })
        };
      return (0, r.jsx)(F.Z, {
        emojiDescriptors: n,
        emojiSize: s,
        surrogateCodePoint: es,
        onInspect: ed,
        onSelect: c,
        onSelectSoundmoji: f,
        getEmojiItemProps: y,
        getEmojiRowProps: T,
        isScrolling: N,
        isUsingKeyboardNavigation: A,
        rowIndex: e,
        allowAnimatedEmoji: C,
        showEmojiFavoriteTooltip: ef,
        channelGuildId: P,
        category: i.sectionId,
        selectedItemClassName: K.__invalid_selectedItem,
        channelId: D,
        messageId: w,
        isBurstReaction: x,
        inNitroLockedSection: i.isNitroLocked,
        handleScrollUpOnSectionCollapse: a
      }, e)
    }), [o, g, s, es, ed, c, f, y, T, N, A, C, ef, P, D, w, x, z]);
    i.useEffect(() => () => {
      var e, t;
      return null === (t = e_.cache) || void 0 === t ? void 0 : null === (e = t.clear) || void 0 === e ? void 0 : e.call(t)
    }, [e_]);
    let ep = i.useMemo(() => l().memoize(e => {
        let n = g[e];
        if (null == n) return;
        let {
          guild: i,
          categoryId: o,
          type: s,
          sectionId: l
        } = n;
        if (s === V.En.SEARCH_RESULTS) return;
        let c = s === V.En.GUILD ? null != i ? (0, r.jsx)(G.Z, {
            guild: i,
            height: en,
            width: en
          }) : null : null != o ? (0, r.jsx)(U.Z, {
            categoryId: o,
            height: en,
            width: en,
            size: "custom"
          }) : null,
          u = null == i ? void 0 : i.toString();
        null != o && (u = (0, k.Y1)(o, null == i ? void 0 : i.toString()));
        let d = j === e,
          f = n.isNitroLocked,
          p = f && !d,
          h = f && d,
          m = s === V.En.SOUNDMOJI ? (0, r.jsx)(_.P3F, {
            className: K.soundmojiViewMore,
            onClick: () => {
              (0, v.hr)(H.X1.SOUNDBOARD), L.default.track(Z.rMx.SOUNDMOJI_EMOJI_PICKER_VIEW_ALL_CLICK)
            },
            children: Y.NW.string(Y.t.rUEjBQ)
          }) : void 0;
        return (0, r.jsx)(O.Z, {
          className: a()(K.header, M, {
            [K.inactiveNitroHeader]: p,
            [K.activeNitroHeader]: h
          }),
          icon: c,
          isCollapsed: t.has(l),
          onClick: () => eu(l, i),
          trailing: m,
          children: u
        }, l)
      }), [g, t, eu, M, j]),
      eh = (0, I.B4)({
        location: "EmojiPickerList-useListRenderers"
      }),
      eg = i.useCallback((e, n) => {
        let i = g[e],
          o = ec(e),
          s = el(e);
        return (0, r.jsx)("div", {
          className: a()(K.categorySection, {
            [K.categorySectionLast]: e === g.length - 1,
            [K.categorySectionCollapsed]: t.has(i.sectionId),
            [K.categorySectionNitroLocked]: i.isNitroLocked,
            [K.categorySectionNitroDivider]: o || s
          }),
          children: n
        }, e)
      }, [t, g, ec, el]),
      em = i.useCallback(e => ec(e) ? eh ? (0, r.jsx)(R.fi, {}) : (0, r.jsx)(R.ZP, {
        glowOpacity: W
      }) : el(e) ? (0, r.jsx)("div", {
        className: K.nitroBottomDivider
      }) : null, [W, ec, el, eh]),
      eE = i.useCallback(e => ec(e) ? ei : el(e) ? eo : 0, [ec, el]),
      ev = i.useCallback(e => {
        var n;
        return e === g.length - 1 || t.has(null === (n = g[e]) || void 0 === n ? void 0 : n.sectionId) || ec(e) || el(e) ? 0 : et
      }, [t, g, ec, el]);
    return {
      listPadding: i.useMemo(() => ["" !== p ? $ : Q, X, J, q], [p]),
      renderRow: e_,
      renderSection: eg,
      renderSectionHeader: ep,
      renderSectionFooter: em,
      sectionMarginBottom: ev,
      sectionHeaderHeight: i.useCallback(e => g[e].type === V.En.SEARCH_RESULTS ? 0 : ee, [g]),
      sectionFooterHeight: eE
    }
  },
  el = i.memo(function(e) {
    let {
      diversitySurrogate: t,
      emojiGrid: n,
      emojiListRef: o,
      emojiSize: a,
      onEmojiSelect: s,
      onSelectSoundmoji: l,
      setUpsellConfigs: d,
      sectionDescriptors: f,
      rowCountBySection: _,
      collapsedSections: b,
      setCollapsedSections: O,
      getEmojiItemProps: S,
      getEmojiRowProps: I,
      rowCount: R,
      isUsingKeyboardNavigation: j,
      channelGuildId: k,
      channelId: U,
      messageId: G,
      isBurstReaction: F,
      listHeaderClassName: V
    } = e, H = i.useRef(!1), q = E.kJ.useStore(e => e.activeCategoryIndex), Q = (0, v.Iu)(e => e.searchQuery), X = D.Yk.useSetting(), J = (0, u.e7)([w.default], () => w.default.getCurrentUser()), $ = (0, x.I5)(J), {
      location: ee
    } = (0, h.O)(), {
      analyticsLocations: et
    } = (0, g.ZP)(), [en, er] = i.useState(0), [ei, eo] = i.useState(!1), [el, ec] = i.useState(0), eu = (0, A.N)(), ed = (0, N.Ng)(), ef = ei && !$ && !c.tq, e_ = null != eu || null != ed, {
      enabled: ep
    } = P.G.useExperiment({
      location: "Premium Emoji Roadblock"
    }, {
      autoTrackExposure: ef && !e_,
      disable: !ef || e_
    }), eh = ep || ef && null != eu, {
      listPadding: eg,
      renderRow: em,
      renderSection: eE,
      renderSectionHeader: ev,
      renderSectionFooter: eb,
      sectionMarginBottom: ey,
      sectionHeaderHeight: eO,
      sectionFooterHeight: eS
    } = es({
      collapsedSections: b,
      diversitySurrogate: t,
      emojiGrid: n,
      emojiSize: a,
      onEmojiSelect: s,
      onSelectSoundmoji: l,
      searchQuery: Q,
      sectionDescriptors: f,
      setCollapsedSections: O,
      getEmojiItemProps: S,
      getEmojiRowProps: I,
      isScrolling: H,
      isUsingKeyboardNavigation: j,
      allowAnimatedEmoji: X,
      channelGuildId: k,
      channelId: U,
      messageId: G,
      isBurstReaction: F,
      listHeaderClassName: V,
      activeSectionIndex: en,
      upsellGlowOpacity: el,
      emojiListRef: o
    }), eI = e => {
      er(e), "" === Q && E.kJ.setActiveCategoryIndex(e)
    }, eT = (0, y.Uf)({
      listRef: o,
      searchQuery: Q,
      nitroLockedSectionStates: f,
      setShowUpsell: eo,
      setUpsellGlowOpacity: ec
    }), eN = (0, y.Qs)({
      activeCategoryIndex: q,
      isScrolling: H,
      listRef: o,
      onActiveCategoryIndexChange: eI,
      scrollOffset: z,
      searchQuery: Q,
      disableForSearch: !1
    });
    (0, y.Xs)({
      searchQuery: Q,
      activeCategoryIndex: q,
      listRef: o
    });
    let eA = i.useCallback(e => {
        eT(e), eN(e)
      }, [eT, eN]),
      eC = f.length > 0;

    function eR() {
      if (!eh) return Y.NW.string(Y.t.EtmOFh);
      {
        let e = (0, M.Px)(W.p9.TIER_2);
        return Y.NW.format(Y.t.gMVjeX, {
          nitroTierName: e,
          onClick: () => {
            d({
              type: B.Bg.PREMIUM,
              emojiDescriptor: void 0
            })
          }
        })
      }
    }

    function eP() {
      if (eh) {
        var e;
        let t = (0, T.J0)();
        return null !== (e = (0, M.fr)(t)) && void 0 !== e ? e : Y.NW.string(Y.t.BmJkbW)
      }
      return Y.NW.string(Y.t.BmJkbW)
    }
    return i.useEffect(() => {
      ei && L.default.track(Z.rMx.PREMIUM_UPSELL_VIEWED, {
        type: W.cd.EMOJI_PICKER_FLOATING_UPSELL,
        location: ee,
        location_stack: et
      })
    }, [ee, et, ei]), i.useEffect(() => {
      "" !== Q && (ec(0), eo(!1))
    }, [Q]), (0, r.jsxs)("div", {
      className: K.listWrapper,
      children: [eC ? (0, r.jsx)(m.Z, {
        role: "none presentation",
        className: K.list,
        listPadding: eg,
        onScroll: eA,
        renderRow: em,
        renderSection: eE,
        renderSectionHeader: ev,
        renderSectionFooter: eb,
        rowCount: R,
        rowCountBySection: _,
        rowHeight: ea(a),
        sectionHeaderHeight: eO,
        sectionMarginBottom: ey,
        sectionFooterHeight: eS,
        stickyHeaders: !0,
        ref: o
      }) : (0, r.jsx)(p.Z, {
        message: Y.NW.string(Y.t.IxxiKC),
        className: K.noSearchResultsContainer
      }), $ || c.tq ? null : (0, r.jsx)(C.p, {
        showUpsell: ei,
        text: eR(),
        button: eP(),
        buttonAnalyticsObject: {
          section: Z.jXE.EMOJI_PICKER_FLOATING_UPSELL
        }
      })]
    })
  })