...
function _(e) {
  ...
  if (D) return <i.Modal title={v.intl.string(v.t.fuFvw8)} aria-label={v.intl.string(v.t.fuFvw8)} transitionState={w.transitionState} onClose={_} actions={[]}><a.$jN className={x.spinnerContainer} /></i.Modal>;
  if (B) return <i.Modal title={v.intl.string(v.t.fuFvw8)} aria-label={v.intl.string(v.t.fuFvw8)} transitionState={w.transitionState} onClose={_} actions={[{
      text: v.intl.string(v.t.cpT0Cg),
      onClick: _,
      variant: "primary"
    }]}>{v.intl.string(v.t.yAk8ZW)}</i.Modal>;
  let $ = q.length > 0 ? <g.F paddingBottom={16} paddingTop={16} rowData={q} handleToggleDestination={J} selectedDestinations={A} disableSelection={R} originDestination={Z} /> : <l.Fragment>{<img className={x.noResultsImg} src={j} alt={""} />}{<a.Text variant={"text-md/normal"} color={"text-muted"}>{v.intl.string(v.t.V6nAfH)}</a.Text>}</l.Fragment>,
  ...
  return R && (et = v.intl.format(v.t["/KhyPT"], {
    count: 5
  })), <i.Modal title={ee.toString()} subtitle={et} aria-label={ee.toString()} transitionState={w.transitionState} onClose={G} size={"md"} actions={[{
      text: v.intl.string(v.t.cpT0Cg),
      onClick: G,
      variant: "secondary"
    }, {
