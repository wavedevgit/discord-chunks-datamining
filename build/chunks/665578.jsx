...
function E(e) {
  let {
    ...
    return a.fS(t, u.BO) && (t = C.Plq.ADMINISTRATOR), n => <e{...Z(w({}, n), {
  ...
  return <r.Fragment>{q > 0 ? <c.Wn messageType={c.QYI.WARNING}>{S.intl.format(S.t["B/dFBA"], {
        removeCount: q
      })}</c.Wn> : null}{L ? <div className={I.requiredPermissionsBanner}>{<c.d3s size={"sm"} color={"currentColor"} className={I.icon} />}{<span className={I.message}>{S.intl.format(S.t["2889Gh"], {})}</span>}{<c.zxk variant={"secondary"} size={"sm"} onClick={K} text={S.intl.string(S.t["HO/oXl"])} />}</div> : null}{X.map((e, t) => {
      let n = t => <c.zxk{...Z(w({}, t), {
          variant: "secondary",
          disabled: Y || !M,
        ...
        r = null;
      return M ? Y && (r = S.intl.string(S.t.XTwtW1)) : r = null != l ? S.intl.string(S.t.tybdam) : S.intl.string(S.t.z2hjk5), <_.Z bar={(0, i.jsx)(c.ua7, {
          tooltipClassName: I.tooltip,
          text: r,
          shouldShow: null != r,
          children: e => n(e)
        })} inModal={v} title={e.title}><div className={v ? true : I.listContainer}><O.Z guildId={j} commandId={l} noneSelectedText={e.noneSelectedText} overwrites={e.overwrites} editPermissions={z} hasAccessToMutatePermissions={M} /></div></_.Z>
    })}</r.Fragment>
}
