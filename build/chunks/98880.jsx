...
function J(e) {
  ...
  return <$ application={n} onClick={o} sectionName={c} resultsPosition={u} disabled={p} tracksImpression={_} look={i} sectionOverallPosition={d}>{"icon" === i ? (0, r.jsx)(ee, {
      application: n,
      imageStyle: a,
      children: l
    }) : (0, r.jsx)(et, {
...
function $(e) {
  ...
  return _ ? <div ref={f ? b : true} className={y}>{o}</div> : <c.P3F innerRef={f ? b : true} className={y} onClick={n} onContextMenu={m} aria-label={G.intl.formatToPlainString(G.t["zLhr9/"], {
      applicationName: g,
...
function ee(e) {
  ...
  return <c.ua7 tooltipContentClassName={B.tooltipContent} text={s}>{e => {
      var t = H({}, Y(e));
...
function et(e) {
  ...
  return <r.Fragment>{C ? <div onMouseEnter={S} onFocus={S} onMouseLeave={R} onBlur={R} className={a()(B.bannerImageContainer, {
        [B.mediumBanner]: "medium_banner" === n,
        [B.largeBanner]: "large_banner" === n
      })}>{<span className={o}><ei application={t} fallbackColor={y} showVideo={v} disableFadeIn={u} overrideImageUrl={_} /></span>}{P || A || D !== d.ww.NONE ? <div className={B.bannerUpperRightContainer}>{A && <div className={B.promotedLabelWrapperBanner}><c.X6q variant={"text-xxs/medium"} color={"header-primary"}>{G.intl.string(G.t["/eVltr"])}</c.X6q></div>}{P && <en />}{D !== d.ww.NONE && <O.Z labelType={D} />}</div> : null}{<div className={B.bannerImageChildContainer}>{f}</div>}</div> : null}{<div className={a()(B.appDetailsContainer, {
        [B.appDetailsRowContainer]: "row" === n
      })}>{<k.Z src={m} className={a()(B.icon, {
          [B.rowIcon]: "row" === n
        })} aria-hidden={true} rendersPlaceholder={true} />}{<div className={B.appDetails}>{<div className={B.appDetailsHeaderContainer}>{<c.X6q variant={"heading-md/semibold"} color={"header-primary"} lineClamp={1}>{g}</c.X6q>}{!C && A ? <div className={B.promotedLabelWrapperNonBanner}><c.Text variant={"text-xxs/semibold"} color={"text-muted"}>{G.intl.string(G.t["/eVltr"])}</c.Text></div> : null}{<er application={t} />}</div>}{<c.Text variant={"text-sm/normal"} color={"text-secondary"} lineClamp={1}>{b}</c.Text>}</div>}{"row" === n ? <div className={B.rowDivider} /> : null}</div>}</r.Fragment>
}

function en() {
  return <Chunk481060.ua7 text={Chunk388032.intl.string(Chunk388032.t.CfTySU)}>{e => (0, r.jsx)("div", z(W({
...
function er(e) {
  ...
  let o = (0, E.fD)(t, i);
  return null == o ? null : <c.ua7 text={o}>{e => (0, r.jsx)("img", W({
...
function ei(e) {
  ...
  if (null != s) return <img src={s} alt={(0, w.BQ)(t) ? t.name : ""} className={a()(B.bannerImage, {
      [B.disableFadeIn]: o
    })} />;
  if ((0, w.BQ)(t)) {
    if ((0, w.ye)(t)) return <eo application={t} showVideo={i} disableFadeIn={o} />;
    if (null != t.bot) return <ea bot={t.bot} fallbackColor={n} disableFadeIn={o} />
  }
  return <es fallbackColor={n} disableFadeIn={o} />
}
...
function eo(e) {
  ...
  return <r.Fragment>{null != l && c ? <div className={B.activityVideoContainer}><div className={a()(B.activityVideo, {
          [B.videoFadeOut]: !n
        })} onAnimationEnd={() => n ? null : u(false)}><T.Z src={l} mediaLayoutType={U.hV.MOSAIC} loop={true} autoPlay={true} muted={true} /></div></div> : null}{<y.Z imageBackground={s} applicationName={t.name} imageClassName={d} imageNotFoundClassName={d} />}</r.Fragment>
}
...
function ea(e) {
  ...
  return null == l ? <es fallbackColor={n} disableFadeIn={i} /> : <img src={l} alt={""} className={a()(B.bannerImage, {
      [B.disableFadeIn]: i
    })} />
}
...
function es(e) {
  ...
  return <div className={a()(B.bannerImage, {
      [B.disableFadeIn]: n
    })} style={{
      backgroundColor: t
    }} />
...
function eu(e) {
  ...
  return y === E.JS.START || y === E.JS.JOIN ? u ? <J{...z(W({}, f), {
    sectionName: a,
    application: n,
    onClick: b,
    disabled: p,
