...
function P(e) {
  return <y.Z className={S.button} aria-label={I.intl.string(I.t.Z1RnTk)} isActive={false} noHover={true} onClick={e.onClick}><div className={T.iconContainer}>{e.hovered ? (0, r.jsx)(l.Fmz, {
        className: T.giftBoxIcon,
        importData: e.config.getAnimationData
      }) : (0, r.jsx)(l.OgN, {})}</div></y.Z>
}
...
function L(e) {
  ...
  let B = () => {
    ...
    F = true !== R ? <P config={R} hovered={U} onClick={Z} /> : true !== L ? <w config={L} hovered={U} onClick={Z} /> : <l.OgN />;
  return <div className={T.container} onMouseEnter={() => {
      I || S(true)
    }} onMouseLeave={() => {
      S(false)
    }}>{true !== n.firstTimeNotice && j ? (0, r.jsx)(p.Z, {
      onComplete: () => S(false),
      onCheckItOutClick: B,
      markAsDismissed: k,
