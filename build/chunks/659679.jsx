...
function g(e) {
  ...
  return <a.Z className={m.systemMessageContainer} contentClassName={m.systemMessageContent} iconNode={(0, r.jsx)(l.$Eu, {
      className: m.icon,
      color: "currentColor",
      size: "sm"
    })} iconContainerClassName={m.iconContainer} timestampClassName={m.timestamp} timestamp={n} timestampFormat={"L"} compact={true}><span className={m.boostMessage}>{<l.PUh className={m.boostMessageUser} variant={"text-md/semibold"} name={g} colorString={null != x ? x : null} colorStrings={v} />}{<o.xv variant={"text-md/medium"} lineClamp={1}><s.Z>{f.intl.string(p.default.plwH8f)}</s.Z></o.xv>}</span></a.Z>
}