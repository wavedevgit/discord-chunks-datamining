...
function j(e) {
  ...
  let z = i.useCallback(e => n(e, x), [n, x]),
      ...
      switch (e) {
        case 0:
          return <div style={{
              ...
              [C.headingTopbar]: !R
            })}>{!R && <div className={C.headingFilters}>{<O.Z loadId={t} />}{<y.Z loadId={t} />}</div>}{A && <E.Z loadId={t} />}</div>;
        case 1:
          return <c.$jN className={a()(C.spinner, {
    ...
    J = i.useCallback((e, t, n, i) => {
      ...
      {
        let e = s[t];
        return <div style={n}><v.ZP guildId={e} onClick={async e => await Q(e, t, x, T)} onView={e => z(e)} /></div>
      }
    }, [s, Q, z, x]);
  i.useEffect(() => {
    let e = V.current;
  ...
  let $ = i.useMemo(() => (0, o.debounce)(() => {
    ...
    ee = R ? S : N;
  return <div className={C.container} ref={F}><c.GMG ref={V} className={C.masonryList} sections={W} columns={M} itemGutter={16} padding={ee} renderItem={J} renderSection={Y} getSectionHeight={K} getItemKey={q} getItemHeight={X} chunkSize={24} onScroll={$} renderAccessory={e => R ? (0, r.jsx)("div", {
        className: C.sidebar,
        style: {
          height: e
        },
