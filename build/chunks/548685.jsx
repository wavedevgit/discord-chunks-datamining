...
let P = e => {
  ...
  return null == N ? null : <div className={i()(T.popularPicksSection, T.centeredSection)}>{<div className={T.blockHeader}>{<div className={T.headerLeft}>{<s.X6q variant={"heading-lg/semibold"}>{a}</s.X6q>}{M && <s.ua7 text={x.intl.string(x.t["3taPdn"])} position={"top"} aria-label={x.intl.string(x.t["3taPdn"])}>{e => {
            var t, n;
          ...
          }}</s.ua7>}</div>}{k ? <div className={i()(T.headerRight, {
          [j.shopTakeOver]: A
        })}>{<s.Text variant={"text-md/medium"}>{x.intl.string(x.t.uaX709)}</s.Text>}{<s.PhF className={T.sortSelect} options={D} select={X} isSelected={e => e === B} serialize={e => e} popoutClassName={i()({
            [j.shopTakeOver]: A
          })} />}{<s.zxk variant={"secondary"} text={x.intl.string(x.t.X3tnc3)} buttonRef={Y} onClick={() => {
            G({
              isShuffling: true,
              onOutroComplete: F,
              returnRef: Y
          ...
          }} disabled={z !== E.g.MOUNTED && z !== E.g.FINISHED} />}</div> : null}</div>}{<div className={T.feed}>{n ? (0, r.jsx)(r.Fragment, {
