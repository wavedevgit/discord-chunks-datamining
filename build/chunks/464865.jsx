/** Chunk was on 22988 **/
/** chunk id: 464865, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  V: () => j
}), require("./953529.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk442837 = require("./442837.js"),
  Chunk481060 = require("./481060.js"),
  Chunk497321 = require("./497321.js"),
  Chunk158638 = require("./158638.js"),
  Chunk246946 = require("./246946.js"),
  Chunk63063 = require("./63063.js"),
  Chunk621319 = require("./621319.js"),
  Chunk44550 = require("./44550.js"),
  Chunk436879 = require("./436879.js"),
  Chunk586382 = require("./586382.js"),
  Chunk859481 = require("./859481.js"),
  Chunk981631 = require("./981631.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk50986 = require("./50986.js");

function j(e) {
  let {
    guild: t
  } = e, n = i.useRef(null), j = (0, o.g1)(t.id, "GuildSettingsBoostPerks"), v = (0, l.e7)([c.Z], () => c.Z.hideInstantInvites);
  (0, g.Z)(n, f.KsC.BOOST_PERKS_VANITY_URL);
  let {
    vanityURLCode: _,
    vanityURLUses: O,
    originalVanityURLCode: y,
    errorDetails: C,
    fetched: N
  } = (0, l.cj)([m.Z], () => ({
    vanityURLCode: m.Z.vanityURLCode,
    vanityURLUses: m.Z.vanityURLUses,
    originalVanityURLCode: m.Z.originalVanityURLCode,
    errorDetails: m.Z.errorDetails,
    fetched: m.Z.fetchedVanityURL
  })), I = null == t ? true : t.id;
  return (i.useEffect(() => {
    null == I || N || (0, u.U5)(I)
  }, [N, I]), null == t) ? null : v ? <s.Z /> : <a.hjN ref={n}>{<a.vwX className={x.title}>{b.intl.string(b.t["5XZKy8"])}{<p.xl guild={t} />}</a.vwX>}{<a.Text variant={"text-sm/medium"} color={"text-secondary"} className={x.description}>{j ? b.intl.format(b.t["J/v84O"], {
        helpCenterArticle: d.Z.getArticleURL(f.BhN.GUILD_VANITY_URL)
      }) : b.intl.format(b.t.koklFB, {
        helpCenterArticle: d.Z.getArticleURL(f.BhN.GUILD_VANITY_URL)
      })}</a.Text>}{<h.z guild={t} vanityURLCode={_} vanityURLUses={O} originalVanityURLCode={y} errorDetails={C} />}</a.hjN>
}