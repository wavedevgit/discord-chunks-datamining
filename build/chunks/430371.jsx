...
function x(e) {
  ...
  return <div className={a()(I.root, v.flexCenter, Z)}>{<div className={I.tileWrapper} style={{
        opacity: ei && J ? 0 : 1
      }}>{<s.animated.div className={I.videoFrame} style={{
          top: eO.value.to(e => -e * P / 2)
        }}>{<s.animated.div style={{
            width: ev.value
          }} className={I.videoWrapper}><div className={v.videoSizer} style={{
              aspectRatio: e_
            }}>{eT((e, t, n) => {
              let {
                key: i
            ...
            })}</div></s.animated.div>}{ec ? <s.animated.div className={a()(I.actionRow, {
            [I.idle]: Y
          })} style={{
            bottom: eI.value
          }}><d.Z channelId={U.id} isParticipantsOpen={B} isVertical={true} /></s.animated.div> : null}</s.animated.div>}{<s.animated.div className={I.participantsWrapperAnimated} style={{
          translateY: ey.value.to(e => e * P / 2),
          opacity: ey.value,
          visibility: ey.value.to(e => 0 === e ? "hidden" : "visible")
        }}><b.ZP channel={U} onClick={o} onContextMenu={T} onDoubleClick={A} participants={eN} participantTileWidth={eC} selectedParticipantId={G.id} inCall={j} popoutWindow={k} paused={ee || !B} /></s.animated.div>}</div>}{eA.length > 0 ? <g.Z onContextMenuParticipant={T} width={F} height={V} channel={U} participants={eA} onSelectParticipant={o} /> : null}</div>
}