...
function j(e) {
  ...
  return j ? U = <o.Text variant={"text-sm/semibold"} className={E.hintText} color={"text-feedback-positive"}>{A.intl.string(A.t.seV8ys)}</o.Text> : b && (U = <o.Text variant={"text-sm/semibold"} className={E.hintText} color={"text-danger"}>{A.intl.string(A.t.tYX2pq)}</o.Text>), <div className={E.accountCard}><div className={E.userDetails}>{<o.qEK src={v.getAvatarURL(true, 40)} size={o.EFr.SIZE_40} aria-label={s.username} />}{<div className={l()(E.usernameSection, {
          [E.hasActionMaxWidth]: !j
        })}>{<div className={E.username}>{<o.Text variant={"text-md/semibold"} color={"header-primary"} lineClamp={1}>{g.ZP.getUserTag(v, {
              mode: "username",
              identifiable: r ? "never" : "always"
            })}</o.Text>}{<o.Text color={"header-secondary"} variant={"text-sm/normal"}>{I}</o.Text>}</div>}{U}</div>}{<div className={E.userActions}>{!j && <o.zxk variant={"secondary"} text={b ? A.intl.string(A.t["DSN+h4"]) : t} onClick={function() {
          ...
          }} />}{<o.hU icon={o.xhG} onClick={function(e) {
            (0, u.vq)(e, e => {
              let {
                onSelect: t
              } = e;
          ...
          }} aria-label={A.intl.string(A.t.PdRCRk)} variant={"icon-only"} />}</div>}</div></div>
}
...
function b(e) {
  ...
  return <div className={E.list}>{n ? (0, i.jsx)(o.$jN, {
      className: E.spinner
