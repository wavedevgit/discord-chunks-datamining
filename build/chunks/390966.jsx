  ...
  switch (e) {
    ...
    case 3:
      return null != i && "" !== i ? h.intl.format(h.t.vHYgJS, {
        start: n,
        startHook: e => <u.Text color={"text-feedback-positive"} variant={"text-sm/semibold"} className={m.liveEventEndTime}>{e}</u.Text>,
        end: i
      }) : null != n ? n : "";
    default:
      return t
...
function S() {
  return <Chunk481060.IGR className={Chunk923167.newBadge} color={Chunk692547.Z.unsafe_rawColors.BRAND_260.css} text={(0, Chunk255367.jsx)(Chunk481060.Text, {
      className: Chunk923167.newBadgeText,
...
function A(e) {
  ...
  return <div className={a()(m.eventStatusContainer, n)}><u.ua7 position={"right"} text={i} shouldShow={null != i}>{e => (0, r.jsx)("div", y(E({}, e), {
...
function N(e) {
  ...
  let B = i.useMemo(() => n === p.p1.CANCELED || (null == D ? true : D.is_canceled) ? 5 : n === p.p1.ACTIVE ? 3 : p.$I.has(n) ? 4 : M ? 2 : +!!k, [n, null == D ? true : D.is_canceled, M, k]),
    Z = I(B, G, g, x, j),
    {
      Icon: F,
      iconColor: V,
  ...
  return <A className={a()(s, {
      [m.isRecurring]: null != W
    })} tooltipText={Y}>{b && n === p.p1.SCHEDULED ? <S /> : <F color={V} size={"custom"} width={O} height={O} />}{<div className={m.eventStatusLabel}>{<u.Text color={H} variant={E}>{Z}</u.Text>}{null != W && <u.Text color={"header-secondary"} variant={"text-xs/normal"}>{W}</u.Text>}</div>}</A>
}