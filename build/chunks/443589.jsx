...
function P(e) {
  ...
  let R = i.useCallback(e => {
    ...
    U = i.useRef(null),
    G = () => L ? <u.P3F innerRef={U} aria-label={M()} onClick={D} className={a()(N.actionButton, S.popoutButton, {
        [N.highlight]: P
      })} onMouseEnter={R} onMouseLeave={w}><o.animated.div className={S.spriteContainer}><div className={S.sprite} /></o.animated.div></u.P3F> : <c.zx buttonRef={U} onClick={D} onMouseEnter={R} onMouseLeave={w}><div className={S.pillContentContainer}>{<u.OgN size={"custom"} width={T.width} height={T.height} color={"currentColor"} />}{<u.Text variant={"text-sm/medium"} color={"always-white"} className={S.pillButtonText}>{M()}</u.Text>}</div></c.zx>;
  return <u.yRy targetElementRef={U} animation={u.yRy.Animation.FADE} nudgeAlignIntoViewport={false} closeOnScroll={true} shouldShow={x} position={"right"} align={"top"} spacing={9} onRequestClose={() => {
      A(false)
