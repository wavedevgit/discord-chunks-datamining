...
function U(t) {
  ...
  return ((0, c.ZP)(() => {
    ...
    (null == y || null == M) && N()
  }, [M, y, N]), null == y || null == M) ? null : <r.Modal transitionState={x} title={m.intl.formatToPlainString(m.t.OhsOy8, {
      user: O.ZP.getName(M, null, y)
      ...
      children: m.intl.string(C.default["8yIKen"])
    })} onClose={N}><u.Kqy gap={16}>{<u.Kqy gap={8}>{<u.X6q variant={"heading-sm/semibold"}>{m.intl.string(m.t["9XsExs"])}</u.X6q>}{<o.hE buttons={h.map(t => (function(t, e, n) {
            let {
              value: a,
              label: i
            } = t;
          ...
          })(t, t.value === R, k))} />}</u.Kqy>}{<u.Kqy gap={8}>{<u.X6q variant={"heading-sm/semibold"}>{m.intl.string(m.t.ewHW19)}</u.X6q>}{<u.Kx8 value={j} onChange={t => B(t)} placeholder={m.intl.string(m.t.GakiHx)} rows={4} maxLength={S.GN} />}</u.Kqy>}</u.Kqy></r.Modal>
}