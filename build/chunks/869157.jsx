...
function k(e) {
  ...
  return w.length > 0 ? w.map(e => <A guild={f} commandId={t} onChange={t => P(e.id, e.type, t)} onRemove={() => Z(e.id, e.type)} overwrite={e} integration={k} canShowMigrationTooltip={R} hasAccessToMutatePermissions={g} />) : <div className={E.noItemsSelected}><s.Text color={"text-muted"} variant={"text-sm/normal"}>{d}</s.Text></div>
}
...
function A(e) {
  ...
  let D = r.useRef(null),
    L = <s.ua7 tooltipClassName={E.tooltip} text={o} shouldShow={N} position={"left"} hideOnClick={false}>{e => (0, i.jsx)("div", T(P({}, e), {
        ref: D,
    ...
    M = <s.yRy targetElementRef={D} renderPopout={() => (0, i.jsx)(s.P3F, {
        onClick: e => {
      ...
      })} position={"bottom"} align={"center"} animation={s.yRy.Animation.TRANSLATE} onRequestClose={() => m.Z.dismissToggleTooltip(a.id, j)} shouldShow={A} closeOnScroll={true}>{() => L}</s.yRy>;
  return <div className={E.entryItem}>{<div className={E.entryName}><S.Z guild={a} id={x.id} type={x.type} isLocked={N} lockTooltipText={o} /></div>}{<div className={E.entryActions}>{<R commandId={c} isSentinel={_} isDisabled={!y} onRemove={h} />}{M}</div>}</div>
}
...
function R(e) {
  ...
  return r ? null : null == t && n ? null : <div className={E.removeActions}><s.P3F className={E.removeContainer} aria-label={Z.intl.string(Z.t.mT0CQE)} onClick={l}><s.XHJ size={"md"} color={"currentColor"} className={E.removeIcon} /></s.P3F></div>
}