...
function z(e) {
  ...
  if (t.isThread() || t.isForumPost()) {
    ...
    u = <div className={k.threadSubLabel}>{<e color={f.TVs.colors.TEXT_SECONDARY} className={k.subLabelIcon} />}{<f.Text variant={"text-xs/medium"} color={"text-secondary"} lineClamp={1}>{s}</f.Text>}{null != o ? <r.Fragment>{<f.Text className={k.subLabelSeparator} variant={"text-xs/medium"} color={"text-secondary"}>{"•"}</f.Text>}{<f.Text variant={"text-xs/medium"} color={"text-secondary"}>{(0, N.Xf)(c()(o))}</f.Text>}</r.Fragment> : null}</div>
  }
  return <G{...U(D({}, l), {
    icon: (0, r.jsx)(v.Z, {
      size: v.E.SMALL_32,
...
function K(e) {
  ...
  let h = a.useMemo(() => [t.length], [t.length]),
    ...
    b = a.useCallback(e => {
      ...
      return u === p.h8.USER ? <F{...D({
        user: d
      }, b)} /> : u === p.h8.GROUP_DM ? <V{...D({
        channel: d
      }, b)} /> : u === p.h8.TEXT_CHANNEL || u === p.h8.VOICE_CHANNEL ? <z{...D({
        channel: d
      }, b)} /> : void(0, I.vE)(u)
    }, [c, o, l, i, t, n, g]),
    y = a.useRef(null),
    v = (0, _.Z)("forward-modal", y);
  return <u.bG navigator={v}><u.SJ>{e => {
        var {
          ref: t
        } = e, n = W(e, ["ref"]);
