...
function h(e) {
  ...
  return (0, s.ZP)(() => {
    null != D.current && D.current.focus()
  }), <a.Modal title={k ? y.intl.string(y.t.zeVg5e) : y.intl.string(y.t["/jubeH"])} subtitle={y.intl.string(y.t["3v8kZG"])} transitionState={t} onClose={h} actions={[{
      variant: "secondary",
      ...
      onClick: () => {
        k && (v.Z.deleteForumTag(x, O.id), h())
    ...
    }) : true}>{<div className={f.inputContainer}>{<div className={f.emojiButtonContainer}><u.yRy targetElementRef={N} renderPopout={e => {
            let {
              closePopout: n
          ...
          }} position={"left"} animation={u.yRy.Animation.NONE} align={"bottom"}>{(e, n) => {
            var t, l;
            let {
              isShown: o
            } = n;
          ...
          }}</u.yRy></div>}{<r.Is inputRef={D} maxLength={20} value={P} inputClassName={f.input} placeholder={y.intl.string(y.t["5vpeb2"])} onChange={R} autoFocus={true} onKeyDown={e => {
          e.keyCode === g.yXg.ENTER && P.length > 0 && (P.length > 0 && z(), e.preventDefault())
        }} />}{P.length > 0 || null != I ? <r.zx data-migration-pending={true} aria-label={y.intl.string(y.t.o8lsHR)} className={f.clearButton} onClick={() => {
          _(""), B(null)
        }} look={r.zx.Looks.BLANK} size={r.zx.Sizes.NONE}><u.k$p size={"md"} color={"currentColor"} className={f.clearIcon} /></r.zx> : null}</div>}{<u.LZC size={16} />}{<u.XZJ type={u.XZJ.Types.INVERTED} size={18} value={null != E && E} onChange={(e, n) => Z(n || (null == O ? true : O.moderated) == null && true)}><u.Text variant={"text-sm/normal"}>{y.intl.string(y.t["rMH+rq"])}</u.Text></u.XZJ>}</a.Modal>
}