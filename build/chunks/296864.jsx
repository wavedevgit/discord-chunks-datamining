...
function C(e) {
  ...
  return <a.Modal transitionState={t} onClose={y} title={C.name} subtitle={j.intl.format(j.t.UMajoq, {
      privacyLevel: A,
      ...
      onClick: X,
      loading: G
    }]}>{<x.Z guild={E} channel={b} name={C.name} description={null != (n = C.description) ? n : true} imageSource={(0, m.Z)(C)} isActive={false} isUserLurking={O} speakers={[]} speakerCount={0} rsvped={true} guildEvent={C} />}{S && <s.XZJ className={p.verticalSpacing} type={s.XZJ.Types.INVERTED} value={Z} onChange={e => {
        let {
          currentTarget: n
        } = e;
        return T(n.checked)
      }}><s.Text variant={"text-sm/normal"}>{j.intl.string(j.t.dGNtgI)}</s.Text></s.XZJ>}{null != P && null != P.getAnyErrorMessage() ? <s.Text color={"text-danger"} variant={"text-sm/normal"} className={p.errorMessage}>{P.getAnyErrorMessage()}</s.Text> : null}</a.Modal>
}