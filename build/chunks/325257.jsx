...
let k = Chunk73800.memo(function(e) {
  ...
  let ew = e_ || eb ? <S.Z guild={H} show={e_} active={W} onAnimationStart={function() {
        eO(e_)
    ...
    eZ = ed ? <x.OG><P.Z /></x.OG> : <a.animated.div ref={$ ? e => {
        eh(e)
      } : true} data-dnd-name={H.name} style={{
        scale: null == er ? 1 : er
      }} data-drop-hovering={e_} className={o()(A.blobContainer, {
        [A.sorting]: ee,
        [A.wobble]: e_,
        [A.selected]: e_ || W
      })}><h.aRk selected={true} upperBadge={ea} lowerBadge={ec} lowerBadgeSize={eu}>{ew}</h.aRk></a.animated.div>;
  return <N.H ref={eP}>{<E.Z hovered={!ed && em} selected={!ed && W} unread={!ed && K} className={A.pill} />}{<I.Z guild={H} disabled={ee} isDragging={ed}>{eZ}</I.Z>}{$ ? <x.ZP name={H.name} targetNode={U} onDragOverChanged={eI} /> : null}</N.H>
})