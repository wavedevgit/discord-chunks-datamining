...
function m(e) {
  ...
  return <o.yRy targetElementRef={b} renderPopout={e => {
      let {
        closePopout: a
      } = e;
      return (0, r.jsx)(d.Z, {
...
function h(e) {
  ...
  (0, c.s)(y, I);
  let O = 0 !== h;
  return <div className={l()(p.container, t)} ref={y}>{<div className={p.measurements}>{i.map((e, t) => <s.AJ index={t} onItemLayout={b}><o.njP.Item id={e.id} aria-label={e.label} className={p.tab}><o.Text variant={"text-md/medium"}>{e.label}</o.Text></o.njP.Item></s.AJ>)}{<div ref={x}><m tabs={j} onTabSelect={d} selectedTab={n} /></div>}</div>}{O && <o.njP type={"top"} look={"brand"} selectedItem={n} onItemSelect={d} className={p.tabs}>{C.map(e => <o.njP.Item id={e.id} look={"brand"} aria-label={e.label} className={p.tab}>{e.label}</o.njP.Item>)}{0 !== j.length ? <m tabs={j} onTabSelect={d} selectedTab={n} /> : null}</o.njP>}</div>
}