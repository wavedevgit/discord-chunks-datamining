...
function et(e, t, n, i) {
  ...
  return null != o && "" !== o ? <img className={n} src={o} alt={e.name} /> : <span className={i}>{e.name}</span>
}
let en = e => {
    let {
      emoji: t,
      ...
      return <I.Z className={H.__invalid_effect} effect={e} emojiSize={I.M.LARGE} onComplete={() => l(false)} />
    }, [c, t, n, o]);
    return i.useEffect(() => {
      let e = false;
    ...
    }, [t]), <div className={H.burstEmojiSection}>{d}{<_.Z className={a()(H.burstEmoji, {
          [H.hideEmoji]: s
        })} emojiId={t.id} emojiName={t.name} animated={t.animated} size={"reaction"} />}{<f.Text variant={"text-md/medium"}>{ee(t)}</f.Text>}</div>
  },
  er = Chunk73800.memo(function(e) {
    ...
    return <f.ua7 position={"left"} text={d}>{e => {
        let {
          onMouseEnter: t,
          onMouseLeave: i
        } = e;
...
function ei(e) {
  ...
  return <g.Z className={H.reactorDefault} onContextMenu={e => (0, Z.Pv)(e, n, o)} align={g.Z.Align.CENTER}>{<g.Z.Child wrap={true} grow={0} shrink={0} className={Y.marginReset}><h.Z user={null != _ ? _ : n} size={f.EFr.SIZE_24} /></g.Z.Child>}{<g.Z.Child><f.Text tag={"strong"} variant={"text-md/normal"} className={H.name}>{null != b && "" !== b && <span className={n.hasUniqueUsername() ? "" : H.nickname}>{b}</span>}{<p.Z user={n} className={null != b && "" !== b ? H.tagFaded : null} usernameClass={H.username} discriminatorClass={H.discriminator} forceUsername={true} />}</f.Text></g.Z.Child>}{!c && E && <f.P3F onClick={y}><f.Dio size={"xs"} color={"currentColor"} className={H.remove} /></f.P3F>}</g.Z>
}
class eo extends Chunk73800.PureComponent {
  ...
  render() {
    ...
    return 0 === exports.length && this.state.loadingMore ? a.push(length) : (a.push(exports.length), module && a.push(1)), <div className={Chunk111932.reactorsContainer}>{Chunk120356 === Chunk566006.O.BURST && <en emoji={require.emoji} channelId={Chunk73800.getChannelId()} messageId={Chunk73800.id} />}{<Chunk481060.aVo className={Chunk111932.reactors} fade={true} ref={this.scrollerRef} sections={a} sectionHeight={0} rowHeight={this.getRowHeight} renderRow={this.renderRow} renderSection={this.renderSection} onScroll={module ? this.handleScroll : true} />}</div>
  }
  constructor(...e) {
    super(...e), W(this, "scrollerRef", i.createRef()), W(this, "state", {
      ...
      return 1 === t ? 0 === n ? <f.$jN className={H.spinnerMore} /> : null : 0 === l.length && this.state.loadingMore ? <f.$jN className={H.spinner} /> : null != d && <ei message={i} emoji={s.emoji} guildId={o} channel={a} user={d} reaction={s} reactionType={c} disableManage={u} />
    })
  }
}
...
function ec(e) {
  ...
  if (i.useEffect(() => {
    ...
    }, [s, h.length, S, I]), null == I || null == S) return <f.$jN />;
  if (null == _) throw Error("MessageReactions.render: Message does not have a channelId");
  return <O.Z.Provider value={null != p ? p : true}><f.Y0X aria-label={c} transitionState={l} size={f.CgR.DYNAMIC} className={H.container} parentComponent={"MessageReactions"}>{a ? null : <f.Ttm className={H.scroller} fade={true}>{g.map(e => {
          var t;
        ...
        })}</f.Ttm>}{<ea message={t} reaction={S} guildId={p} channel={_} reactionType={I.reactionType} disableManage={o} />}</f.Y0X></O.Z.Provider>
}
