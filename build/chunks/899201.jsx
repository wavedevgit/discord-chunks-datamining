...
function H(e) {
  ...
  return null == W ? <m.$jN /> : <u.Modal{...t = function(e) {
...
function V(e) {
  ...
  switch (t.type) {
    case A.bm.DM:
    case A.bm.FRIEND:
      s = <m.qEK size={m.EFr.SIZE_40} src={t.item.getAvatarURL(null, 128, false)} aria-label={t.item.username} />, o = k.ZP.getName(t.item), c = k.ZP.getUserTag(t.item);
      break;
    case A.bm.GROUP_DM: {
      let e = (0, w.x)(t.item),
        n = (0, _.F6)(t.item, Z.default, j.Z);
      s = <m.qEK src={e} aria-label={n} size={m.EFr.SIZE_40} />, o = (0, _.F6)(t.item, Z.default, j.Z);
      break
    }
    case A.bm.CHANNEL: {
      let e = t.item,
      ...
      if (o = "#".concat((0, _.F6)(e, Z.default, j.Z)), c = n.name, null != n.icon) {
        ...
        s = <m.qEK src={t} aria-label={o} size={m.EFr.SIZE_40} />
      } else {
        let e = (0, U.Zg)(n.name);
        s = <div className={B.acronym} aria-hidden={true}><m.Text variant={"text-md/semibold"}>{e}</m.Text></div>
      }
    }
  }
  return <m.P3F onClick={n} className={l()(B.rowContainer, {
      [B.disabled]: r
    })}>{<div className={B.rowLeft}>{<div className={B.rowAvatar}>{s}</div>}{<div className={B.rowNameContainer}>{<m.Text variant={"text-md/semibold"} className={l()(B.rowName, {
            [B.disabled]: r
          })}>{o}</m.Text>}{<m.Text variant={"text-xs/medium"} className={l()(B.rowSubName, {
            [B.disabled]: r
          })}>{c}</m.Text>}</div>}</div>}{<m.XZJ disabled={r} value={i} type={m.XZJ.Types.INVERTED} displayOnly={true} className={B.rowRight} />}</m.P3F>
}