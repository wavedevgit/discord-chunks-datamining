/** Chunk was on web.js **/
/** chunk id: 532901, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Z: () => f
}), require("./125548.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk25209 = require("./25209.js"),
  Chunk794295 = require("./794295.js"),
  Chunk865427 = require("./865427.js"),
  Chunk853425 = require("./853425.js"),
  Chunk49012 = require("./49012.js"),
  Chunk773352 = require("./773352.js");
let d = 1024;

function f(e) {
  return {
    react(t, n, f) {
      if (e.enableBuildOverrides && (0, s._G)(t.target)) return <i.Fragment><l.Z url={t.target} /></i.Fragment>;
      let _ = n(t.content, f),
        p = "string" == typeof t.title && 0 !== t.title.length ? t.title : (0, o.Rp)(t.content),
        h = () => (0, u.r)(t),
        m = (null == e ? true : e.mustConfirmExternalLink) ? e => (null == e || e.stopPropagation(), null == e || e.preventDefault(), (0, c.q)({
          href: t.target,
          shouldConfirm: true,
          messageId: f.messageId,
          channelId: f.channelId
        }), null) : true;
      if (f.previewLinkTarget && !(0, u.r)(t)) {
        let e = "\n\n(".concat(t.target, ")");
        p.length + e.length > d && (e = "..." + e, p = (p = p.substr(0, d - e.length)).trimEnd()), p += e
      }
      return f.noStyleAndInteraction ? <span title={p}>{_}</span> : <a.Z title={p} href={t.target} trusted={h} onClick={m} messageId={f.messageId} channelId={f.channelId}>{_}</a.Z>
    }
  }
}