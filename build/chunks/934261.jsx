...
let I = e => {
    ...
    return null != r ? <div className={t ? S.effectDescriptionNoGradient : S.effectDescriptionBorderWithGradient}><div className={S.effectDescriptionContainer}>{<o.Text color={"header-primary"} variant={"text-sm/semibold"} className={S.effectName}>{(0, i.isEmpty)(I) ? Z : I}</o.Text>}{null != f ? null : <o.Text color={"text-default"} variant={"text-sm/normal"} className={S.effectDescription}>{D}</o.Text>}{null != C && <o.Text variant={"text-xxs/normal"} color={"text-muted"} className={S.effectPurchasedAt}>{_.intl.format(_.t.Io7ozs, {
            days: C.days.toString()
          })}</o.Text>}{null != f && <o.Text variant={"text-xxs/normal"} color={"text-muted"} className={S.effectPurchasedAt}>{_.intl.format(_.t.gW9R4O, {
            date: f.purchasedAt.toLocaleDateString(h, {
              month: "long",
              year: "numeric"
            })
          })}{null != f.expiresAt && <n.Fragment>{<br />}{_.intl.format(_.t.eZSTa2, {
            ...
            })}</n.Fragment>}</o.Text>}{y && <o.Text variant={"text-xxs/normal"} color={"text-muted"} className={S.effectPurchasedAt}>{_.intl.string(_.t.nKdAlJ)}</o.Text>}</div></div> : null
  },
  ...
  Z = e => {
    ...
    return <div className={S.previewContainer}>{<m.Z{...r = C({}, U), i = i = {
        disabledInputs: true,
      ...
      }), r} />}{false}{<I effectIsOwned={c} pendingProfileEffectRecord={s} product={a} purchase={u} userIsPremium={A} />}</div>
  }