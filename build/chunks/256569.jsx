...
function p(e) {
  ...
  return <div onMouseDown={l} className={g.resizeHandle} />
}
let h = Chunk73800.forwardRef(function(e, t) {
  ...
  return <s.P3F className={g.textArea} onFocus={l} onBlur={a} onClick={o} innerRef={c} ignoreKeyPress={true} style={{
      minHeight: d
    }}>{<s.Den className={g.innerScroller} style={{
        minHeight: d - 2
      }} ref={t}>{n}</s.Den>}{<p resizableNode={c} onResize={m} onResizeEnd={e => {
        m(e), null == l || l()
      }} />}</s.P3F>
});
...
function f(e) {
  ...
  return <div className={a()(g.mainContainer, f)} ref={j} tabIndex={0} onKeyUp={N}>{<h ref={v} onClick={G}>{Z.map((e, t) => <d.Z value={e} onChange={E(t)} onBlur={U} onFocus={B(t)} onRemove={() => I(t)} isSelected={D.includes(e)} isSelecting={A} error={u[e]} forceShowErrorTooltip={!L && t === Z.length - 1} />)}{<input className={a()(g.mainTextInput, {
          [g.isEditingOtherNodes]: L
        })} ref={x} onChange={y} onKeyDownCapture={C} onPaste={O} onBlur={w} placeholder={0 === Z.length ? p : true} value={R} />}</h>}{null != b && <s.Text variant={"text-xs/normal"} color={"text-muted"} className={g.maxTags}>{null != (t = null == Z ? true : Z.length) ? t : 0}{"/"}{b}</s.Text>}</div>
}