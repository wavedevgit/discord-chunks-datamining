...
let M = () => <div className={Chunk595532.editorHeader}><div className={Chunk595532.bannerUpsell}>{<Chunk587446.Z className={Chunk595532.premiumIcon} />}{<Chunk481060.X6q variant={"heading-md/bold"} color={"always-white"}>{Chunk388032.intl.string(Chunk388032.t.POSLGR)}</Chunk481060.X6q>}</div></div>,
  k = e => {
    let {
      markAsDismissed: t
    } = e, n = (0, C.q)(), i = () => {
    ...
    return <div className={x.editorHeader}>{<f.P3F className={x.closeCircleButton} onClick={i}><f.k$p size={"md"} color={"currentColor"} className={x.closeCircle} /></f.P3F>}{<f.X6q variant={"heading-lg/extrabold"}>{L.intl.string(L.t["xSR+a2"])}</f.X6q>}</div>
  },
  j = e => {
    var t, n, i;
    let {
    ...
    return <O.Z size={d.zx.Sizes.MEDIUM} textOptions={{
      ...
      }} subscriptionTier={D.Si.TIER_2} onSubscribeModalClose={m} />
  },
  U = e => {
    let {
      markAsDismissed: t,
    ...
    return <div className={x.editorFooter}>{o && <j onSubscribeSuccess={() => {
          c(true)
        }} markAsDismissed={t} />}{<u.z fullWidth={true} onClick={f} variant={"secondary"} text={_} />}</div>
  };
...
function G(e) {
  ...
  return i.useEffect(() => {
    if (s && !u) return _
  }, [s, u, _]), <E.Gt value={o}><div ref={p} data-app-right-panel={true} className={a()(x.themeEditor, c ? x.editorAnimate : null)}><f.y5t>{n ? <M /> : <k markAsDismissed={t} />}{<f.Ttm className={x.editorBody}><R.ZP type={R.yH.EDITOR}>{!s && <R.ZP.Basic className={x.selectionGroup} />}{<R.ZP.Gradient className={x.selectionGroup} isCoachmark={n} />}</R.ZP></f.Ttm>}{<U markAsDismissed={t} isCoachmark={n} />}</f.y5t></div></E.Gt>
}