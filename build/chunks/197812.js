          ...
          if (!f) {
            ...
            if (a.is.und(o.duration))
              if (o.decay) {
                ...
                for (var b = null == c.lastVelocity ? m : c.lastVelocity, y = o.precision || (h == d ? .005 : Math.min(1, .001 * Math.abs(d - h))), O = o.restVelocity || y / 10, v = o.clamp ? 0 : o.bounce, I = !a.is.und(v), S = h == d ? c.v0 > 0 : h < d, T = !1, A = 1, N = Math.ceil(e / 1), C = 0; C < N && !(!(Math.abs(b) > O) && (f = Math.abs(d - _) <= y)); ++C) {
                  I && (T = _ == d || _ > d == S) && (b = -b * v, _ = d);
                  var P = (-(1e-6 * o.tension) * (_ - d) + -(.001 * o.friction) * b) / o.mass;
                  b += P * A, _ += b * A
                } else {
                  var R = o.progress || 0;
...
}), t.BailSignal = er, t.Controller = ev, t.FrameValue = eo, t.Interpolation = eW, t.Spring = eZ, t.SpringContext = ew, t.SpringHandle = eL, t.SpringValue = ef, t.Trail = eH, t.Transition = eY, t.config = v, t.inferTo = H, t.interpolate = ez, t.to = eK, t.update = eq, t.useChain = O, t.useSpring = ek, t.useSprings = ex, t.useTrail = eM, t.useTransition = eF