/** Chunk was on web.js **/
/** chunk id: 243487, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  ZP: () => q,
  kI: () => K
}), require("./388685.js"), require("./539854.js");
var Chunk951288 = require("./951288.js"),
  Chunk647438 = require("./647438.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk442837 = require("./442837.js"),
  Chunk481060 = require("./481060.js"),
  Chunk586902 = require("./586902.js"),
  Chunk835473 = require("./835473.js"),
  Chunk289823 = require("./289823.jsx"),
  Chunk933557 = require("./933557.js"),
  Chunk552282 = require("./552282.js"),
  Chunk763296 = require("./763296.js"),
  Chunk565799 = require("./565799.js"),
  Chunk501655 = require("./501655.js"),
  Chunk470956 = require("./470956.js"),
  Chunk199902 = require("./199902.js"),
  Chunk314897 = require("./314897.js"),
  Chunk592125 = require("./592125.js"),
  Chunk944486 = require("./944486.js"),
  Chunk606304 = require("./606304.js"),
  Chunk594174 = require("./594174.js"),
  Chunk979651 = require("./979651.js"),
  Chunk938475 = require("./938475.js"),
  Chunk237997 = require("./237997.js"),
  Chunk823379 = require("./823379.js"),
  Chunk136015 = require("./136015.js"),
  Chunk426563 = require("./426563.js"),
  Chunk145597 = require("./145597.js"),
  Chunk444295 = require("./444295.js"),
  Chunk804570 = require("./804570.jsx"),
  Chunk388627 = require("./388627.js"),
  Chunk532658 = require("./532658.jsx"),
  Chunk501787 = require("./501787.js"),
  Chunk981631 = require("./981631.js"),
  Chunk65154 = require("./65154.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk213285 = require("./213285.js");

function B(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function Z(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      B(e, t, n[t])
    })
  }
  return e
}

function F(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function V(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : F(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}
let H = e => {
  let t = (null != e ? e : 0) % R.Z.DEFAULT_AVATARS.length;
  return R.Z.DEFAULT_AVATARS[t]
};

function Y(e) {
  var t, n;
  let {
    hiddenVoiceStates: i,
    voiceStateMap: o,
    size: c,
    locked: u,
    flipped: f
  } = e, _ = (0, L.e5)(c), p = (0, s.Wu)([I.default, y.Z], () => i.map(e => {
    var t;
    let n = I.default.getUser(e);
    if (null == n) return;
    let r = y.Z.getChannel(null == (t = o.get(e)) ? true : t.voiceState.channelId);
    return n.getAvatarURL(null == r ? true : r.getGuildId(), _, false)
  }).filter(C.lm), [i, o, _]), h = null != (t = p[0]) ? t : H(0), m = null != (n = p[1]) ? n : H(1);
  return (0, r.jsxs)("div", {
    className: a()(G.hiddenVoiceStates, u && G.locked, f && G.flipped),
    children: [(0, r.jsx)("div", {
      className: G.hiddenVoiceStatesAvatar,
      children: (0, r.jsx)(d.Z, {
        size: c,
        backSrc: h,
        frontSrc: m,
        isTyping: false
      })
    }), (0, r.jsx)("div", {
      className: G.hiddenVoiceStatesText,
      children: (0, r.jsxs)(l.Text, {
        variant: "text-xs/medium",
        children: ["+", i.length, " others in voice"]
      })
    })]
  })
}

function W(e) {
  let [t, n] = e;
  return new Set(n)
}
let K = Chunk647438.memo(function(e) {
  let {
    id: t,
    context: n = M.Yn.DEFAULT,
    channel: o,
    overlayVoiceStates: a,
    displayNameMode: c,
    displayUserMode: u,
    locked: d,
    pinned: f,
    guildId: _,
    showEmpty: p = true,
    isPreviewingInGame: h,
    isSettingsPreview: m = false,
    anchorLeft: g,
    avatarSizeMode: b,
    maxDisplayedVoiceStates: y = j.At
  } = e, O = (0, w.ee)(() => W(a), [a]), [v, I] = a, T = I.length > 0 && null != n && null != o && (!d || f), S = (0, s.Wu)([E.Z], () => Array.from(O).map(e => E.Z.getStreamForUser(e, _)).filter(C.lm).map(e => e.ownerId), [_, O]);
  i.useEffect(() => {
    T && (0, w.zi)(k.Odu.VOICE_V3, {
      locked: A.default.isInstanceLocked(),
      shownUserIds: Array.from(O),
      liveUserIds: S,
      contentInventoryIds: []
    })
  }, [O, S, T]);
  let N = () => d || !p ? null : (0, r.jsx)(D.E, {
      emptyText: U.intl.string(U.t.hEh0l5),
      icon: l.gj8
    }),
    R = y !== j.Og ? I.slice(0, y) : I,
    P = y !== j.Og ? I.slice(y) : [],
    x = v.get(P[0]),
    B = u === k.OYC.ALWAYS && y !== j.Og;
  return T ? (0, r.jsxs)("div", {
    className: G.voiceUserContainer,
    children: [R.map(e => {
      let i = v.get(e);
      return null == i ? null : (0, r.jsx)(L.ZP, {
        sortedVoiceState: i,
        channel: o,
        widgetId: t,
        flipped: !g,
        isPreviewingInGame: h,
        isSettingsPreview: m,
        context: n,
        size: b,
        displayNameMode: c,
        displayUserMode: u,
        locked: d
      }, e)
    }), B && 1 === P.length && null != x && (0, r.jsx)(L.ZP, {
      sortedVoiceState: x,
      channel: o,
      widgetId: t,
      flipped: !g,
      isPreviewingInGame: h,
      isSettingsPreview: m,
      context: n,
      size: b,
      displayNameMode: c,
      displayUserMode: u,
      locked: d
    }, P[0]), B && P.length > 1 && (0, r.jsx)(Y, {
      hiddenVoiceStates: P,
      voiceStateMap: v,
      size: (0, L.px)(b),
      locked: d,
      flipped: !g
    })]
  }) : N()
});

function z(e, t) {
  let n = (0, s.e7)([O.Z, y.Z], () => y.Z.getChannel(O.Z.getVoiceChannelId())),
    [r] = (0, s.e7)([S.ZP, h.Z, O.Z, y.Z], () => {
      let e = y.Z.getChannel(O.Z.getVoiceChannelId());
      return null == e ? [
        [], false
      ] : e.isGuildStageVoice() ? [h.Z.getMutableParticipants(e.id, m.pV.SPEAKER), h.Z.getParticipantsVersion(e.id)] : [S.ZP.getVoiceStatesForChannel(e), S.ZP.getVoiceStateVersion(e.getGuildId())]
    }, [], N.Q),
    o = (0, g.Es)(null == n ? true : n.id, r),
    a = i.useMemo(() => new Map(o.map(e => [e.user.id, e])), [o]),
    l = (0, s.e7)([b.default], () => b.default.getId());
  return [a, (0, s.Wu)([T.Z, O.Z, v.Z, p.Z], () => {
    let n = Array.from(o.map(e => {
      let {
        user: t
      } = e;
      return t.id
    }));
    if (t === j.Og || n.length <= t) return n;
    let r = new Map;
    for (let t of n) {
      let n = (0, c.O)({
          userId: t,
          checkSoundSharing: true,
          checkSoundboardSounds: false,
          checkIsMuted: true,
          context: e
        }, [T.Z, O.Z, v.Z, p.Z]),
        i = t === l;
      (n || i) && r.set(t, {
        isSpeaking: n,
        isCurrentUser: i
      })
    }
    let i = [],
      a = [],
      s = [];
    for (let e = 0; e < n.length; e++) {
      let o = n[e],
        l = r.get(o);
      e < t ? i.push(o) : l && (l.isSpeaking || l.isCurrentUser) ? s.push(o) : a.push(o)
    }
    if (s.length > 0) {
      let e = i.slice(0, t - s.length),
        n = i.slice(t - s.length);
      return [...e, ...s, ...n, ...a]
    }
    return n
  }, [o, e, l, t])]
}

function q(e) {
  var t;
  let {
    isSettingsPreview: n
  } = e, i = (0, s.e7)([O.Z, y.Z], () => y.Z.getChannel(O.Z.getVoiceChannelId())), o = (0, f.ZP)(i), a = z(M.Yn.DEFAULT, e.maxDisplayedVoiceStates), l = (0, s.e7)([E.Z], () => E.Z.getStreamerActiveStreamMetadata()), c = (0, x.II)(), d = (0, u.q)(null == c ? true : c.id), p = (0, s.cj)([E.Z, A.default], () => {
    let e = E.Z.getCurrentUserActiveStream();
    return {
      displayUserMode: A.default.getDisplayUserMode(),
      displayNameMode: A.default.getDisplayNameMode(),
      avatarSizeMode: A.default.getAvatarSizeMode(),
      streamApplication: (null == l ? true : l.pid) === (0, P.getPID)() ? (0, _.Z)(c) : null,
      stream: e
    }
  });
  return (0, r.jsx)(K, V(Z(V(Z({}, p), {
    application: d
  }), e), {
    overlayVoiceStates: a,
    channel: i,
    guildId: null == i ? true : i.guild_id,
    title: null != o ? o : "",
    streamMetadata: l,
    streamApplication: null != (t = p.streamApplication) ? t : {
      id: null,
      name: null == l ? true : l.sourceName
    },
    isSettingsPreview: n
  }))
}