...
e = require.nmd(module), (function() {
  ...
  function I(e) {
    ...
    if (j && (t = /(?:[ab]|dp|pre|[ab]\d+pre)(?:\d+\+?)?$/i.exec(j) || /(?:alpha|beta)(?: ?\d)?/i.exec(e + ";" + (M && a.appMinorVersion)) || /\bMinefield\b/i.test(e) && "a") && (x = /b/i.test(t) ? "beta" : "alpha", j = j.replace(RegExp(t + "\\+?$"), "") + ("beta" == x ? C : N) + (/\d+\+?/.exec(t) || "")), "Fennec" == U || "Firefox" == U && /\b(?:Android|Firefox OS|KaiOS)\b/.test(Z)) U = "Firefox Mobile";
      ...
      L.unshift("embedded")
    } else(/\bBlackBerry\b/.test(G) || /\bBB10\b/.test(e)) && (t = (RegExp(G.replace(/ +/g, " *") + "/([.\\d]+)", "i").exec(e) || 0)[1] || j) ? (Z = ((t = [t, /BB10/.test(e)])[1] ? (G = null, B = "BlackBerry") : "Device Software") + " " + t[0], j = null) : this != g && "Wii" != G && (M && P || /Opera/.test(U) && /\b(?:MSIE|Firefox)\b/i.test(e) || "Firefox" == U && /\bOS X (?:\d+\.){2,}/.test(Z) || "IE" == U && (Z && !/^Win/.test(Z) && j > 5.5 || /\bWindows XP\b/.test(Z) && j > 8 || 8 == j && !/\bTrident\b/.test(e))) && !c.test(t = I.call(g, e.replace(c, "") + ";")) && t.name && (t = "ing as " + t.name + ((t = t.version) ? " " + t : ""), c.test(U) ? (/\bIE\b/.test(t) && "Mac OS" == Z && (Z = null), t = "identify" + t) : (t = "mask" + t, U = w ? m(w.replace(/([a-z])([A-Z])/g, "$1 $2")) : "Opera", /\bIE\b/.test(t) && (Z = null), M || (j = null)), k = ["Presto"], L.push(t));
    ...
    }), (t = /\b(?:AMD|IA|Win|WOW|x86_|x)64\b/i.exec(D)) && !/\bi686\b/i.test(D) ? (Z && (Z.architecture = 64, Z.family = Z.family.replace(RegExp(" *" + t), "")), U && (/\bWOW64\b/i.test(e) || M && /\w(?:86|32)$/.test(a.cpuClass || a.platform) && !/\bWin64; x64\b/i.test(e)) && L.unshift("32-bit")) : Z && /^OS X/.test(Z.family) && "Chrome" == U && parseFloat(j) >= 39 && (Z.architecture = 64), e || (e = null);
    var q = {};
    return q.description = e, q.layout = k && k[0], q.manufacturer = B, q.name = U, q.prerelease = x, q.product = G, q.ua = e, q.version = U && j, q.os = Z || {
      architecture: null,
      family: null,
      version: null,
      toString: function() {
    ...
    }, q.parse = I, q.toString = z, q.version && L.unshift(j), q.name && L.unshift(U), Z && U && !(Z == String(Z).split(" ")[0] && (Z == U.split(" ")[0] || G)) && L.push(G ? "(" + Z + ")" : "on " + Z), L.length && (q.description = L.join(" ")), q
  }
  var T = I();
