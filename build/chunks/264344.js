  ...
  function I(e) {
    ...
    function V(t) {
      return y(t, function(t, n) {
        return t || RegExp("\\b" + (n.pattern || v(n)) + "\\b", "i").exec(e) && (n.label || n)
      })
    }

    function Z(t) {
      return y(t, function(t, n, r) {
        return t || (n[G] || n[/^[a-z]+(?: +[a-z]+\b)*/i.exec(G)] || RegExp("\\b" + v(r) + "(?:\\b|\\w*\\d)", "i").exec(e)) && r
      })
    }
    ...
    if (k && (t = /(?:[ab]|dp|pre|[ab]\d+pre)(?:\d+\+?)?$/i.exec(k) || /(?:alpha|beta)(?: ?\d)?/i.exec(e + ";" + (M && a.appMinorVersion)) || /\bMinefield\b/i.test(e) && "a") && (x = /b/i.test(t) ? "beta" : "alpha", k = k.replace(RegExp(t + "\\+?$"), "") + ("beta" == x ? C : N) + (/\d+\+?/.exec(t) || "")), "Fennec" == U || "Firefox" == U && /\b(?:Android|Firefox OS|KaiOS)\b/.test(F)) U = "Firefox Mobile";
    ...
    }), (t = /\b(?:AMD|IA|Win|WOW|x86_|x)64\b/i.exec(D)) && !/\bi686\b/i.test(D) ? (F && (F.architecture = 64, F.family = F.family.replace(RegExp(" *" + t), "")), U && (/\bWOW64\b/i.test(e) || M && /\w(?:86|32)$/.test(a.cpuClass || a.platform) && !/\bWin64; x64\b/i.test(e)) && L.unshift("32-bit")) : F && /^OS X/.test(F.family) && "Chrome" == U && parseFloat(k) >= 39 && (F.architecture = 64), e || (e = null);
    var q = {};
    return q.description = e, q.layout = j && j[0], q.manufacturer = B, q.name = U, q.prerelease = x, q.product = G, q.ua = e, q.version = U && k, q.os = F || {
      architecture: null,
      family: null,
      version: null,
      toString: function() {
    ...
    }, q.parse = I, q.toString = z, q.version && L.unshift(k), q.name && L.unshift(U), F && U && !(F == String(F).split(" ")[0] && (F == U.split(" ")[0] || G)) && L.push(G ? "(" + F + ")" : "on " + F), L.length && (q.description = L.join(" ")), q
  }
  var S = I();
  "function" == typeof define && "object" == typeof define.amd && define.amd ? (i.platform = S, define(function() {
    return S
  })) : o && a ? g(S, function(e, t) {
    o[t] = e
  }) : i.platform = S
}).call(this)