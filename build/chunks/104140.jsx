/** Chunk was on web.js **/
/** chunk id: 104140, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  a: () => O
...
function v(e) {
  ...
  return <div className={a()(o, u.wrapper)} style={l} onFocus={S} onBlur={A}>{<svg width={j.width} height={j.height} viewBox={"".concat(j.x, " ").concat(j.y, " ").concat(j.width, " ").concat(j.height)} className={a()(u.svg, {
        [u.noContain]: _,
        [u.shiftSVG]: !d
      })} overflow={"visible"} role={"none"}>{<defs>{_ && <path d={G} id={x} />}{<path d={G} id={D} />}{null != m ? <rect id={P} className={u.badgeStroke} x={n - U + p} y={-p} width={U} height={U} rx={U / 2} ry={U / 2} /> : null}{null != O ? <rect id={w} className={u.badgeStroke} x={n - (M + 2 * p) + p} y={n - (k + 2 * p) + p} width={M + 2 * p} height={k + 2 * p} rx={h / 2} ry={h / 2} /> : null}</defs>}{<mask id={R} fill={"black"} x={0} y={0} width={n} height={n}>{_ && <use href={"#".concat(x)} fill={"black"} />}{<use href={"#".concat(D)} fill={"white"} className={a()({
            [u.isHighlighted]: _
          })} />}{N ? <use href={"#".concat(P)} fill={"black"} /> : null}{C ? <use href={"#".concat(w)} fill={"black"} /> : null}</mask>}{N || C ? <mask id={L}>{<rect width={"150%"} height={"150%"} x={"-25%"} y={"-25%"} fill={"white"} />}{N ? <use href={"#".concat(P)} fill={"black"} /> : null}{C ? <use href={"#".concat(w)} fill={"black"} /> : null}</mask> : null}{I ? <r.Fragment>{<g className={u.focusStroke} mask={"url(#".concat(L, ")")}><use href={"#".concat(D)} /></g>}{<g className={u.focusFill}>{null != m ? <use href={"#".concat(P)} /> : null}{null != O ? <use href={"#".concat(w)} /> : null}</g>}</r.Fragment> : null}{_ && <path d={G} stroke={s.Z.BRAND_500} stroke-width={E} className={u.highlight} mask={"url(#".concat(L, ")")} />}{<foreignObject mask={"url(#".concat(R, ")")} x={0} y={0} className={c} width={n} height={n}>{t}</foreignObject>}</svg>}{null != O ? <div className={u.lowerBadge}>{O}</div> : null}{null != m ? <div className={u.upperBadge}>{m}</div> : null}</div>
}