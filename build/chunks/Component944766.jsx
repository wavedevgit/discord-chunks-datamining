/** Chunk was on web.js **/
/** chunk id: 944766, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  A: () => g
}), require("./896048.js"), require("./864466.js"), require("./443073.js"), require("./114821.js"), require("./339614.js");
var Chunk627968 = require("./627968.js"),
  Chunk64700 = require("./64700.js"),
  Chunk503698 = require("./503698.js"),
  s = require.n(Chunk503698),
  Chunk397927 = require("./397927.js"),
  Chunk607470 = require("./607470.jsx"),
  Chunk418126 = require("./418126.js"),
  Chunk814550 = require("./814550.js"),
  Chunk572808 = require("./572808.js"),
  Chunk9045 = require("./9045.js"),
  Chunk806931 = require("./806931.js"),
  Chunk447196 = require("./447196.js");
let h = 13e3,
  m = (e, t) => e.type === p.lp.USER && e.speaking && e.user.id !== t,
  g = e => {
    let {
      seats: t,
      claimedSeat: n,
      participant: a,
      participants: g,
      channel: E,
      flip: b,
      handleHover: y,
      enableAnimations: O = true
    } = e, A = i.useRef(null), v = i.useRef(new Set), S = (0, f.A)(a), {
      setAsset: I,
      sampleAsset: T,
      animationState: C,
      updateAnimationState: N,
      characterAssets: R
    } = (0, u.u)(null == S ? true : S.name), w = i.useMemo(() => Object.values(null != R ? R : {}).flat(), [R]), P = g.find(e => e.type === p.lp.USER && e.user.id === a.userId), D = i.useMemo(() => g.filter(e => m(e, a.userId)), [g, a.userId]), x = i.useMemo(() => D.flatMap(e => {
      let n = t.find(t => t.claimedBy === e.user.id);
      return null != n ? [n] : []
    }), [D, t]), [L, j] = i.useState(false), [M, k] = i.useState(0), [U, G] = i.useState(0);
    return i.useEffect(() => {
      j(false)
    }, [S, w]), i.useEffect(() => {
      let e = v.current;
      return () => {
        [...e].forEach(e => window.clearTimeout(e))
      }
    }, []), i.useEffect(() => {
      if (!O) return;
      let e = A.current;
      if (null == e || true === L) return;
      let t = T(),
        n = w.findIndex(e => e === t);
      I(t), G(n), [...e.children].forEach((e, t) => {
        if ("VIDEO" !== e.nodeName) return;
        let r = e;
        r.currentTime = 0, t === n && (r.play(), j(true))
      })
    }, [L, w, T, I, O]), i.useEffect(() => {
      O && ((null == P ? true : P.speaking) ? N(u.f.TALKING) : C === u.f.TALKING && N(u.f.IDLE))
    }, [P, C, N, O]), i.useEffect(() => {
      !O || x.some(e => {
        switch (n.id) {
          case 1:
          case 2:
          case 3:
            return (null == e ? true : e.id) < n.id;
          case 4:
          case 5:
          case 6:
            return (null == e ? true : e.id) > n.id;
          default:
            returnfalse
        }
      }) && M + h < Date.now() && (N(u.f.HEAD_TURN), k(Date.now()))
    }, [x, n, M, N, O]), (0, r.jsxs)(o.DUT, {
      className: _.iE,
      style: {
        width: S.width,
        top: S.offset.top,
        left: b ? -S.offset.left : S.offset.left
      },
      onMouseEnter: () => y(true),
      onMouseLeave: () => y(false),
      onClick: e => {
        e.preventDefault(), e.stopPropagation();
        let t = (a.avatar + 1) % d.oo.length;
        0 === t && (t = 1), c.Ay.update(E.id, {
          avatar: t,
          seat: null == n ? true : n.id
        })
      },
      children: [(0, r.jsx)("div", {
        className: _.Fk,
        style: {
          height: 1.25 * S.height,
          width: 1.25 * S.width
        }
      }), (null == R ? true : R.decoration) != null && (0, r.jsx)(l.A, {
        src: R.decoration[0],
        className: s()(_.GG, {
          [_.UU]: b
        }),
        autoPlay: O,
        loop: O,
        style: {
          width: S.width
        }
      }), (0, r.jsx)("div", {
        ref: A,
        children: w.map((e, t) => (0, r.jsx)(l.A, {
          style: {
            opacity: +(t === U),
            width: S.width
          },
          src: e,
          className: s()(_.GG, {
            [_.UU]: b
          }),
          onEnded: () => {
            var t, n;
            if (C === u.f.HEAD_TURN && (null == R || null == (t = R.headTurn) ? true : t.includes(e))) {
              N(u.f.HEAD_TURN_BACK);
              let e = setTimeout(() => {
                j(false), v.current.delete(e)
              }, 2e3);
              v.current.add(e)
            } else C === u.f.HEAD_TURN_BACK && (null == R || null == (n = R.headTurnBack) ? true : n.includes(e)) && N(u.f.IDLE), j(false)
          },
          autoPlay: false,
          loop: false
        }, e))
      })]
    })
  }