...
function T(e) {
  ...
  let S = (0, s.e7)([p.Z, _.default], () => !!a()(p.Z.getMessages(n.id).toArray()).reverse().find(e => e.author.id !== _.default.getId() && e.state === b.yb.SENT && !(0, E.Z)(e))),
    ...
    w = null != o && "" !== o ? <l.Text className={O.error} color={"text-danger"} variant={"text-sm/normal"}>{o}</l.Text> : null;
  return S ? <div className={O.containerCompact}>{<l.P3F className={null != o && "" !== o ? O.compactButtonDisabled : O.compactButton} aria-label={y.intl.string(y.t.pJObYG)} onClick={R}>{<f.Z sticker={C} size={24} />}{<l.Text className={O.text} variant={"text-md/medium"}>{P}</l.Text>}</l.P3F>}{w}</div> : <div className={O.containerExpanded}>{<f.Z sticker={C} size={160} className={O.stickerExpanded} />}{<l.zxk fullWidth={true} variant={"primary"} size={"md"} onClick={R} disabled={!!o} text={P} />}{w}</div>
}