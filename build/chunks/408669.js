/** Chunk was on web.js **/
/** chunk id: 408669, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  A: () => m
});
...
let a = RegExp("(?:由|從|自)?(?:(今|明|前|大前|後|大後|聽|昨|尋|琴)(早|朝|晚)|(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|後|大後|聽|昨|尋|琴)(?:日|天)(?:[\\s,，]*)(?:(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(Chunk725198.uk).join("") + "]+)(?:\\s*)(?:點|時|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(Chunk725198.uk).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(Chunk725198.uk).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i"),
  s = RegExp("(?:^\\s*(?:到|至|\\-|\\–|\\~|\\〜)\\s*)(?:(今|明|前|大前|後|大後|聽|昨|尋|琴)(早|朝|晚)|(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|後|大後|聽|昨|尋|琴)(?:日|天)(?:[\\s,，]*)(?:(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(Chunk725198.uk).join("") + "]+)(?:\\s*)(?:點|時|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(Chunk725198.uk).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(Chunk725198.uk).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i"),
  o = 1,
  l = 2,
  c = 3,
  u = 4,
...
class m extends Chunk355418.c {
  ...
  innerExtract(e, t) {
    ...
    if (a >= 12 && (g = 1), t[h]) {
      ...
      var A = t[d][0];
      "上" == A || "朝" == A || "早" == A || "凌" == A ? (g = 0, 12 == a && (a = 0)) : ("下" == A || "晏" == A || "晚" == A) && (g = 1, 12 != a && (a += 12))
    }
    ...
    if (a = 0, m = 0, g = false, v[_]) {
      let e = parseInt(v[_]);
      if (isNaN(e) && (e = (0, i.CT)(v[_])), e >= 60) return null;
      n.end.assign("second", e)
    }
    if (isNaN(a = parseInt(v[f])) && (a = (0, i.CT)(v[f])), v[p] ? "半" == v[p] ? m = 30 : "正" == v[p] || "整" == v[p] ? m = 0 : isNaN(m = parseInt(v[p])) && (m = (0, i.CT)(v[p])) : a > 100 && (m = a % 100, a = Math.floor(a / 100)), m >= 60 || a > 24) return null;
    if (a >= 12 && (g = 1), v[h]) {
      if (a > 12) return null;
    ...
    return n.text = n.text + v[0], n.end.assign("hour", a), n.end.assign("minute", m), g >= 0 ? n.end.assign("meridiem", g) : n.start.isCertain("meridiem") && 1 == n.start.get("meridiem") && n.start.get("hour") > a ? n.end.imply("meridiem", 0) : a > 12 && n.end.imply("meridiem", 1), n.end.date().getTime() < n.start.date().getTime() && n.end.imply("day", n.end.get("day") + 1), n
  }
}