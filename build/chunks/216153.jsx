/** Chunk was on 75708 **/
/** chunk id: 216153, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  Y: () => u
});
var Chunk255367 = require("./255367.js");
require("./73800.js");
var Chunk704215 = require("./704215.js"),
  Chunk231757 = require("./231757.js"),
  Chunk63063 = require("./63063.js"),
  Chunk249881 = require("./249881.js"),
  Chunk981631 = require("./981631.js"),
  Chunk388032 = require("./388032.js"),
  Chunk195794 = require("./195794.js");
let u = () => {
  let e = Chunk63063.Z.getArticleURL(Chunk981631.BhN.XBOX_CONNECTION);
  return <Chunk249881.t title={Chunk388032.intl.string(Chunk388032.t["2okkZW"])} body={Chunk388032.intl.format(Chunk388032.t.OnERSU, {
      help_article: module
    })} img={(0, Chunk255367.jsx)("img", {
      src: Chunk195794,
      width: "auto",
      height: "45",
      alt: ""
    })} newIndicatorDismissibleContent={Chunk704215.z.XBOX_ONE_WAY_RECONNECT} onClick={() => (0, Chunk231757.Z)({
      platformType: Chunk981631.ABu.XBOX,
      location: "Xbox two way upsell"
    })} />
}