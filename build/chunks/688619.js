...
! function(t, n) {
  ...
  tC.prototype.lab = function() {
    ...
    test: function() {
      for (var e = [], t = arguments.length; t--;) module[t] = arguments[t];
      if ("array" === tT(e = tS(module, "lab")) && 3 === module.length) return "lab"
    }
  });
  var tP = d.unpack,
    tw = d.RAD2DEG,
  ...
  for (var r7 = rJ, r9 = d.type, ie = d.clip_rgb, it = d.TWOPI, ir = Math.pow, ii = Math.sin, ia = Math.cos, io = y, is = function(e, t, n, r, i) {
      ...
      if ("c" === t.substr(0, 1) && (o.push(r), o.push(i)), "e" === t.substr(0, 1)) {
        ...
        for (O.push(i); b;) {
          ...
          for (var S = 0; S < m; S++)
            for (var T = a[S], A = Number.MAX_VALUE, C = true, N = 0; N < n; N++) {
              var R = iE(O[N] - T);
              R < A && (A = R, C = N), E[C]++, g[S] = C
            }
      ...
      return o
    }, iO = ib, iv = iy, iI = E, iS = function(e, t) {
      e = new iI(e), t = new iI(t);
      var n = e.luminance(),
        r = t.luminance();
      return n > r ? (n + .05) / (r + .05) : (r + .05) / (n + .05)
    }, iT = E, iA = Math.sqrt, iC = Math.pow, iN = Math.min, iR = Math.max, iP = Math.atan2, iw = Math.abs, iD = Math.cos, ix = Math.sin, iL = Math.exp, ij = Math.PI, iM = function(e, t, n, r, i) {
      ...
      var s = Array.from(e.lab()),
        ...
        R = A - T;
      R = 180 >= iw(R) ? R : A <= T ? R + 360 : R - 360, R = 2 * iA(y * O) * ix(o(R) / 2);
      var P = f - l,
        w = O - y,
  ...
  var iz = iH,
    iq = y;
  return iq.average = rG, iq.bezier = rX, iq.blend = r7, iq.cubehelix = is, iq.mix = iq.interpolate = rs, iq.random = i_, iq.scale = rH, iq.analyze = iO, iq.contrast = iS, iq.deltaE = iM, iq.distance = iU, iq.limits = iv, iq.valid = iB, iq.scales = iV, iq.colors = t4, iq.brewer = iz, iq
})