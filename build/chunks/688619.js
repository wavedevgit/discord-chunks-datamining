  ...
  for (var r7 = rJ, r9 = d.type, ie = d.clip_rgb, it = d.TWOPI, ir = Math.pow, ii = Math.sin, ia = Math.cos, io = y, is = function(e, t, n, r, i) {
      ...
      if ("c" === t.substr(0, 1) && (o.push(r), o.push(i)), "e" === t.substr(0, 1)) {
        ...
        for (O.push(i); b;) {
          ...
          for (var I = 0; I < h; I++)
            for (var T = a[I], A = Number.MAX_VALUE, C = true, N = 0; N < n; N++) {
              var P = iE(O[N] - T);
    ...
    }, iT = E, iA = Math.sqrt, iC = Math.pow, iN = Math.min, iP = Math.max, iR = Math.atan2, iD = Math.abs, iw = Math.cos, ix = Math.sin, iL = Math.exp, ij = Math.PI, iM = function(e, t, n, r, i) {
      ...
      var s = Array.from(e.lab()),
        ...
        g = .5 * (1 - iA(iC(h, 7) / (iC(h, 7) + iC(25, 7)))),
        E = c * (1 + g),
        b = p * (1 + g),
        y = iA(iC(E, 2) + iC(u, 2)),
        O = iA(iC(b, 2) + iC(_, 2)),
        ...
        N = 1 - .17 * iw(o(C - 30)) + .24 * iw(o(2 * C)) + .32 * iw(o(3 * C + 6)) - .2 * iw(o(4 * C - 63)),
        P = A - T;
      P = 180 >= iD(P) ? P : A <= T ? P + 360 : P - 360, P = 2 * iA(y * O) * ix(o(P) / 2);
      var R = f - l,
        D = O - y,
        w = 1 + .015 * iC(m - 50, 2) / iA(20 + iC(m - 50, 2)),
        x = 1 + .045 * v,
        L = 1 + .015 * v * N,
        j = 30 * iL(-iC((C - 275) / 25, 2)),
        M = -(2 * iA(iC(v, 7) / (iC(v, 7) + iC(25, 7)))) * ix(2 * o(j));
      return iP(0, iN(100, iA(iC(R / (n * w), 2) + iC(D / (r * x), 2) + iC(P / (i * L), 2) + D / (r * x) * M * (P / (i * L)))))
