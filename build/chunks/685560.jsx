...
function P() {
  ...
  if (null != exports) return <Chunk481060.zJl className={Chunk444048.container}>{<div className={Chunk444048.section}><Chunk481060.X6q variant={"heading-md/normal"}>{"Current guild: ".concat(require)}</Chunk481060.X6q></div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Reset Notification Indicators"}</Chunk481060.Text>}{<Chunk481060.zxk variant={"primary"} text={"Reset"} onClick={() => {
          (0, Chunk713081.Qh)()
        }} />}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Reset Progress Bar State"}</Chunk481060.Text>}{<Chunk481060.zxk variant={"primary"} text={"Reset"} onClick={Chunk667815.q} />}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"}>{"Set Warning State"}</Chunk481060.Text>}{<div>{<Chunk481060.zxk variant={"primary"} text={"Set Half Boosts expiring in 1 day"} onClick={() => {
            E(exports, Chunk544891.slice(Math.floor(Chunk544891.length / 2)), false)
          }} />}{<Chunk481060.zxk variant={"primary"} text={"Reset End Date"} onClick={() => {
            E(exports, Chunk544891, true)
          }} />}</div>}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Reset User Level DCs"}</Chunk481060.Text>}{y.map(e => <u.u className={_.formSwitch} contents={l} content={o.z[e]} onChange={() => {}} />)}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Reset Guild Level DCs"}</Chunk481060.Text>}{C.map(e => <c.j7V className={_.formSwitch} onChange={a => {
          a ? (0, x.Qd)(e, t, false) : ((0, p.Z1)(o.z.GUILD_POWERUP_NOTIFICATION), (0, x.c7)(e, t))
        }} value={r.includes(e)}><c.Text variant={"text-sm/normal"}>{T(e)}</c.Text></c.j7V>)}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"}>{"Reset Server Tag Guild Level DCs"}</Chunk481060.Text>}{N.map(e => <c.j7V className={_.formSwitch} onChange={a => {
          a ? (0, x.Qd)(e, t, false) : ((0, p.Z1)(o.z.GUILD_POWERUP_NOTIFICATION), (0, x.c7)(e, t))
        }} value={r.includes(e)}><c.Text variant={"text-sm/normal"}>{T(e)}</c.Text></c.j7V>)}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Reset Powerup Rollback DCs"}</Chunk481060.Text>}{O.map(e => <u.u className={_.formSwitch} contents={l} content={o.z[e]} onChange={() => {}} />)}</div>}{<div className={Chunk444048.section}>{<Chunk481060.Text variant={"eyebrow"} className={Chunk444048.header}>{"Send Powerups System Message"}</Chunk481060.Text>}{<Chunk481060.zxk variant={"primary"} text={"Send System Message"} onClick={() => {
          S(exports)
        }} />}</div>}</Chunk481060.zJl>
}