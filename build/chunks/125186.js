...
async function d(e) {
  ...
  try {
    ...
    try {
      ...
      }), o.width = f.width, o.height = f.height, d.drawImage(f, 0, 0)
    } finally {
    ...
    if (null == p) throw Error("failed to convert to webp");
    {
      ...
      let c = await d.getImageData(0, 0, o.width, o.height),
        ...
        _ = u(c),
        h = f === _;
      if (t = performance.now() - n, s.verbose("[WebP] Pixel hash results: " + "fileName=".concat(e.name, " ") + "fileLength={".concat(e.size, "} ") + "width=".concat(a.width, " ") + "height=".concat(a.height, " ") + "pixelHash=".concat(f, " ") + "mezzanineFileLength={".concat(p.size, "} ") + "mezzaninePixelHash=".concat(_, " ") + "match=".concat(h, " ") + "elapsed_ms=".concat(Math.round(t))), !h) return i("pixel_hash_mismatch")
    }
