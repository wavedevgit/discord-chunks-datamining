...
async function c(e) {
  ...
  if (e.size < a) return r.verbose("[WebP] File too small: ".concat(e.size, " < ").concat(a)), {
    should: !1,
    reason: "file_too_small"
  };
  if ("image/webp" === e.type) return r.verbose("[WebP] File already WebP format"), {
    should: !1,
    reason: "already_webp"
  };
  if (!["image/png"].includes(e.type)) return r.verbose("[WebP] Unsupported format: ".concat(e.type)), {
    should: !1,
    reason: "unsupported_format"
  };
  try {
    if (await l(e)) return r.verbose("[webp] png is animated (apng) - skipping conversion"), {
      should: !1,
      reason: "animated_image"
    }
  } catch (e) {
    if (e instanceof Error && e.message.includes("Corrupted PNG")) return r.warn("[WebP] PNG corruption detected: ".concat(e.message)), {
      should: !1,
      reason: "corrupted_file"
    };
    return r.warn("[WebP] Unexpected error during PNG analysis:", e), {
      should: !1,
...
async function u(e) {
  ...
  try {
    ...
    let g = e.name.lastIndexOf("."),
      ...
      y = performance.now() - n;
    return r.verbose("[WebP] Conversion successful: ".concat(b.name, " in ").concat(Math.round(y), "ms")), {
      success: !0,
      originalFile: e,
      convertedFile: b,
      sizeBefore: e.size,
