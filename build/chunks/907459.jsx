...
let w = () => {
    ...
    return <Chunk767714.Z size={Chunk755721.zx.Sizes.MEDIUM} textOptions={{
  ...
  D = e => {
    ...
    return <div className={P.editorHeader}>{<u.P3F className={P.closeCircleButton} onClick={() => n()}><u.k$p size={"md"} color={"currentColor"} className={P.closeCircle} /></u.P3F>}{<div className={P.title}>{<u.X6q variant={"heading-lg/extrabold"}>{R.intl.string(R.t["hb/wEx"])}</u.X6q>}{<b.Z className={P.premiumIcon} />}</div>}</div>
  },
  L = e => {
    let {
      isCoachmark: t,
    ...
    return <div className={P.editorFooter}>{i && <w />}{<l.z onClick={o} variant={"secondary"} fullWidth={true} text={i || t ? R.intl.string(R.t["dqH+qq"]) : R.intl.string(R.t.Olc2Ky)} />}</div>
  };
...
function x(e) {
  ...
  return (i.useEffect(() => {
    if (c && !g) return I.O
  }, [c, g]), null == l) ? null : <h.Gt value={o}><div ref={E} className={a()(P.editor, d ? P.editorAnimate : null)} data-app-right-panel={true}><u.y5t>{<D markAsDismissed={n} />}{<u.Ttm className={P.editorBody}><S.Z className={P.selectionGroup} isEditor={true} /></u.Ttm>}{<L markAsDismissed={n} isCoachmark={t} />}</u.y5t></div></h.Gt>
}