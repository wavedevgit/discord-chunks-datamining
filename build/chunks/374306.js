/** Chunk was on 16318 **/
n.d(t, {
  Z: () => m
});
var r = n(255367);
n(73800);
var i = n(442837),
  l = n(481060),
  a = n(961675),
  o = n(883429),
  s = n(238349),
  c = n(109434),
  u = n(456269),
  d = n(432771),
  p = n(920303),
  h = n(665149),
  f = n(388032);

function m(e) {
  let {
    channel: t
  } = e, n = (0, u.r_)(t), m = (0, i.e7)([a.Z], () => a.Z.hasHidden(t.id)), g = (0, d.c)(t.id), {
    sortOrder: b,
    tagFilter: _,
    tagSetting: C
  } = (0, c.H)(t.id), x = (0, i.e7)([s.Z, p.Z], () => !!(s.Z.getThreadIds(t.id, b, _, C).length > 0) || !!(p.Z.getThreads(t.id, b, _, C).length > 0), [t.id, b, _, C]), y = t.isMediaChannel();
  if (!n || g || y && x) return null;
  let v = m ? y ? f.intl.string(f.t["WP/IEx"]) : f.intl.string(f.t.zfq9V1) : y ? f.intl.string(f.t.p60yFx) : f.intl.string(f.t.SNOqYG);
  return (0, r.jsx)(h.JO, {
    tooltip: v,
    icon: l.d3s,
    onClick: () => o.Z.hideAdminOnboarding(t.id, !m),
    selected: !m
  })
}