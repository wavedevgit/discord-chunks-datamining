...
function E(e) {
  ...
  let I = [{
      ...
      icon: <div className={j.icon}><o.aNP /></div>,
      text: p.intl.string(p.t["+4O9nZ"]),
      className: j.row
    }],
    b = v ? <r.Fragment>{p.intl.string(p.t.cpgfFh)}{<br />}{p.intl.string(p.t.UKQ4Cg)}</r.Fragment> : <r.Fragment>{p.intl.string(p.t["xj3j4+"])}{<br />}{p.intl.string(p.t.wWueRU)}</r.Fragment>;
  return <f.Z headerText={p.intl.string(p.t["1/gpFh"])} descriptionText={b} infoRows={I} onDismissAndStay={() => {
    ...
    }} leaveButtonText={p.intl.string(p.t["Y56/oK"])} stayButtonText={p.intl.string(p.t["Z+/hfX"])} transitionState={E} onClose={C} impression={{
