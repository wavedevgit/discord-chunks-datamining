/** Chunk was on web.js **/
/** chunk id: 687131, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  z: () => u
}), require("./997841.js");
var Chunk797258 = require("./797258.js"),
  Chunk630388 = require("./630388.js"),
  Chunk358085 = require("./358085.js"),
  Chunk535246 = require("./535246.js"),
  Chunk212517 = require("./212517.js"),
  Chunk981631 = require("./981631.js");

function c(e) {
  let {
    platform: t
  } = e;
  return !!(0, s.Dp)(e) || ((0, a.isAndroid)() ? t === l.M7m.ANDROID : (0, a.isIOS)() ? t === l.M7m.IOS : !!((0, a.isDesktop)() || (0, a.isWeb)()) && t === l.M7m.DESKTOP)
}

function u(e) {
  var t, n, a;
  if (!(0, o.v)() || null == e) return null;
  let s = e.application_id;
  if (null == s || !(0, i.yE)(null != (t = e.flags) ? t : 0, l.xjy.SUPPORTS_GATEWAY_ACTIVITY_ACTION_JOIN)) return null;
  let u = r.Z.getRemoteApplicationActivity(s);
  return null == u || c(u) ? null : (0, i.yE)(null != (n = u.flags) ? n : 0, l.xjy.SUPPORTS_GATEWAY_ACTIVITY_ACTION_JOIN) && null != (a = u.platform) ? a : null
}