...
let L = function(e) {
  ...
  }, [M, A]), H = i.useMemo(() => A === S.pJs.PENDING && (s.filter(S.pJs.SPAM).length > 0 || s.filter(S.pJs.PENDING_IGNORED).length > 0), [s, A]), V = i.useMemo(() => s.filter(A, M[A]), [s, M, A]), F = A === S.pJs.PENDING, Y = i.useMemo(() => {
    if (!F) return x;
    let e = [];
    return V.forEach(t => {
      let {
