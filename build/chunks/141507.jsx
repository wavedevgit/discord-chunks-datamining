...
let P = e => {
  ...
  w = Chunk73800.memo(function(e) {
    ...
    if (R(t)) {
      ...
      w = "" === e ? <s.Text variant={"text-md/normal"} className={N.glyphEmoji}>{"surrogates" in t ? t.surrogates : null}</s.Text> : <img alt={null != (X = t.allNamesString) ? X : ""} src={e} className={N.emoji} />
    } else if ("CREATE_EMOJI" === t.type) w = <s.oFk size={"md"} color={"currentColor"} className={N.icon} colorClass={N.icon} />;
    else if ("EXPAND_OR_COLLAPSE_EMOJI" === t.type) {
      let e = n(187119),
        i = n(39874),
        o = n(853871),
        s = n(63149);
      w = W.has(t.guildId) ? <img className={N.icon} src={(0, a.wj)(q) ? o : s} alt={""} /> : <img className={N.icon} src={(0, a.wj)(q) ? e : i} alt={""} />
    }
    let Q = null != Z ? <I.Z className={N.__invalid_guildIcon} guild={Z} shouldAnimate={!V && F} /> : null;
    ...
    return <p.Z className={L} graphicPrimary={w} graphicSecondary={Q} titlePrimary={D} titleSecondary={J} isFavorite={Y} emojiSubCategory={z} />
  })