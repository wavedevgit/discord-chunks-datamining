...
function P(e) {
  ...
  return w.length + j.length === 0 ? null : <a.Ttm className={x.container}>{<N channel={o} isChannelSelected={b} voiceStatesCount={null != (l = null == g ? true : g.length) ? l : 0} />}{<div className={x.headerDivider} />}{j.map((e, t) => {
      var n;
      return <y.Z embeddedApp={e} presenceActivity={null != (n = e.presenceActivity) ? n : true} channel={o} onAction={f} />
    })}{w.map((e, t) => {
      let {
        members: n,
        activity: i
      } = e;
      return <y.Z presenceActivity={i} channel={o} members={n} onAction={f} />
    })}{_ && <r.Fragment>{<div className={x.headerDivider} />}{<div className={x.settingNudgeText}><a.Text variant={"text-xs/normal"} tag={"span"}>{S.intl.format(S.t.ePyoY2, {
            onClick: () => {
              (0, a.ZDy)(async () => {
                let {
                  default: e
...
function N(e) {
  ...
  return null == (0, p.KS)(t, u) ? null : <div className={o()(x.popoutHeaderContainer, I.popoutHeaderContainer)}>{<C.Z channel={t} />}{c ? <v.Z userCount={i} video={a} channel={t} /> : <j.Z userCount={i} />}</div>
}