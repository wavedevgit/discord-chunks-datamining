...
class I extends i.Z {
  syncHeartbeats(e, t) {
    for (let n of ("VOICE_STATE_UPDATES" !== t && "PASSIVE_UPDATE_V2" !== t && O.log("~ syncHeartbeats -> syncing heartbeats for taskTypes: ".concat(e.join(", "), " (triggered by: ").concat(t, ")")), e)) {
  ...
  getActivelyProgressingPlayOnDesktopQuestIds() {
    ...
    for (let r of (O.log("~ getActivelyProgressingPlayOnDesktopQuestIds -> Running games: ", t), t))
      if (null != r.id)
        for (let t of n.values()) v(t) && (0, p.Nj)({
          quest: t
  ...
  getActivelyProgressingStreamOnDesktopQuestIds() {
    ...
    let r = n.id;
    if (null == r) return e;
    for (let t of _.Z.quests.values()) v(t) && (0, p.Dr)({
      quest: t
    }) && t.config.application.id === r && e.add(t.id);
    return O.log("~ getActivelyProgressingStreamOnDesktopQuestIds -> Actively progressing questIds: ", e), e
  }
  getActivelyProgressingActivityQuestIds() {
    ...
    let r = _.Z.quests;
    for (let n of t.keys())
      for (let t of r.values()) v(t) && (0, p.pO)(t) && t.config.application.id === n && e.add(t.id);
    for (let t of r.values()) v(t) && (0, p.KM)(t) && n && e.add(t.id);
    return O.log("~ getActivelyProgressingActivityQuestIds -> Actively progressing questIds: ", e), e
  }
  constructor(...e) {
    super(...e), g(this, "heartbeats", {
      ...
      let i = () => {
        if (this.getActivelyProgressingQuestIds(t).has(e)) {
          if (t === r.X.STREAM_ON_DESKTOP) {
            ...
            if (null == n) {
              O.log("~ initiateHeartbeat -> Attempted to beat for stream quest but no active stream, terminating heartbeat for questId: ".concat(e)), this.terminateHeartbeat(e, t);
              return
            }
            let r = (0, s.V9)(n);
            O.log("~ initiateHeartbeat -> Sending heartbeat for questId: ".concat(e)), (0, f.m0)({
              questId: e,
              streamKey: r
            })
      ...
      let n = this.heartbeats[t],
        ...
        i = n.get(e);
      if (null != i) {
        O.log("~ terminateHeartbeat -> Terminating heartbeat for questId: ".concat(e)), window.clearTimeout(i), n.delete(e);
        let t = r.get(e);
        null != t && v(t) && (O.log("~ terminateHeartbeat -> Sending terminal heartbeat for questId: ".concat(e)), (0, f.m0)({
          questId: e,
          terminal: !0
        }))
      }
      ...
      if (O.log("~ handleSendHeartbeatSuccess -> Heartbeat succeeded for questId: ".concat(t, ")")), null != n.completedAt)
        for (let e of (O.log("~ handleSendHeartbeatSuccess -> Quest ".concat(t, " completed, terminating any heartbeats for it")), Object.keys(this.heartbeats))) this.terminateHeartbeat(t, e)
    }), g(this, "handleSendHeartbeatFailure", e => {
      let {
        questId: t
      } = e;
      O.log("~ handleSendHeartbeatFailure -> Heartbeat failed for questId: ".concat(t))
    }), g(this, "actions", {
