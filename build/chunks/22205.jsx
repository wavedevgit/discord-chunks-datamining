...
function x(e) {
  ...
  let T = i.useCallback(() => {
    ...
    k = g ? <div>{<f.Text variant={"text-md/semibold"} color={"text-primary"}>{R.intl.string(R.t.mn4eXF)}</f.Text>}{<div className={P.stackTraceCode}>{<f.Text variant={"text-sm/normal"} color={"text-secondary"}>{o.message}</f.Text>}{<f.Text variant={"text-xxs/normal"} color={"text-secondary"}><code className={P.code}><pre>{o.stack}</pre></code></f.Text>}</div>}</div> : null,
    j = g ? null : R.intl.string(R.t.oEJEFh),
    U = null != (t = null == I ? true : I.isStaff()) && t || null != (n = null == I ? true : I.isStaffPersonal()) && n;
  return <A.Y title={R.intl.string(R.t.U38qZm)} body={k} hint={U ? j : true} confirmText={R.intl.string(R.t.a3HlgI)} cancelText={null != M ? R.intl.string(R.t.qIYnPj) : true} icon={(0, r.jsx)(b.Z, {
      width: 40,
      height: 40,
      className: P.notificationIcon
    })} onNotificationClick={D} onConfirmClick={w} onCancelClick={null != M ? x : true} onDismissClick={a} expand={true} locked={true} notificationId={"overlay-crashed"} index={0} status={C._1z.ACTIVE} containerRef={null} contentOpacity={_} className={P.errorNotificationContainer} wrapperClassName={P.errorNotificationWrapper} />
}
class M extends Chunk73800.PureComponent {
  ...
  render() {
    ...
    return null != require ? Chunk73800 ? <Chunk333031.Z className={Chunk138134.errorClickNotification}><x error={require} onLock={() => {
          var e;
        ...
        }} /></Chunk333031.Z> : null : <div className={exports}>{module}</div>
  }
  constructor(...e) {
