...
function O(e) {
  ...
  return <div className={j.container}>{<i.P3F onClick={E} className={j.close} aria-label={f.intl.string(f.t.cpT0Cg)}><i.Dio size={"md"} color={"currentColor"} /></i.P3F>}{<c.Z><div className={j.iconContainer}><i.Que size={"custom"} color={"currentColor"} height={30} width={30} className={j.icon} /></div></c.Z>}{<i.X6q variant={"heading-xl/semibold"} className={j.header}>{f.intl.string(f.t.UzNv7u)}</i.X6q>}{<i.Text variant={"text-md/normal"} color={"header-secondary"} className={j.body}>{f.intl.string(f.t.UetJjI)}</i.Text>}{<div className={j.invite}>{<o.S value={R} autoFocus={false} onCopy={e => {
          if (_) return;
          (0, g.JG)(e);
        ...
        }} />}{(null == S ? true : S.vanityURLCode) == null && <i.Text variant={"text-xs/normal"} color={"header-secondary"} className={j.inviteDetail}>{(0, h.Vg)(k, A)}</i.Text>}</div>}</div>
}