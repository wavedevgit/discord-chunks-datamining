...
function F(e) {
  ...
  if (window._ws = null, null != l)
    if (t = l.ws, l.state.gateway !== n) B.verbose("[FAST CONNECT] gatewayURL mismatch: ".concat(l.state.gateway, " !== ").concat(n)), t.close(1e3), t = null;
      ...
      var _;
      let e = j({}, l.state);
      null != e.messages && (e.messages = e.messages.map(e => null != e.data && "string" == typeof e.data ? G(j({}, e), {
...
class ee extends D.Z {
  ...
  _connect() {
    ...
    s.Z.mark("\uD83C\uDF10", "Socket._connect"), B.info("[CONNECT] ".concat(n, ", ") + "encoding: ".concat(t, ", ") + "version: ".concat(r, ", ") + "compression: ".concat(null != e ? e : "none")), null !== this.webSocket && (B.error("_connect called with already existing websocket"), this._cleanup(e => e.close(4e3))), this.connectionStartTime = Date.now(), this.helloTimeout = setTimeout(() => {
  ...
  _handleReconnect() {
    B.verbose("[RECONNECT] gateway requested I reconnect."), this._cleanup(e => e.close(4e3)), this.connectionState = A.Z.WILL_RECONNECT, this._connect()
  }
  ...
  _handleClose(e, t, n) {
    if (e = e || !1, this._cleanup(), this.emit("close", {
        code: t,
        reason: n
      }), t === K) return this.connectionState = A.Z.CLOSED, B.warn("[WS CLOSED] because of authentication failure, marking as closed."), this._reset(e, t, n);
    if (this._tryDetectInvalidIOSToken(t, n, e), this.connectionState = A.Z.WILL_RECONNECT, this.nextReconnectIsImmediate) B.info("[WS CLOSED] (".concat(e.toString(), ", ").concat(t, ", ").concat(n, ") retrying immediately.")), this._connect();
      ...
      B.info("[WS CLOSED] (".concat(e.toString(), ", ").concat(t, ", ").concat(n, ") retrying in ").concat((r / 1e3).toFixed(2), " seconds.")), this.gatewayBackoff.fails > H && this._reset(e, t, n)
    }
  }
  _tryDetectInvalidIOSToken(e, t, n) {
    (0, S.isIOS)() && null != this.token && e === Y && t === W && (this.iosGoingAwayEventCount += 1, 3 === this.iosGoingAwayEventCount && d.tn.get({
      url: M.ANM.ME,
      headers: {
      ...
      401 === t && (this.connectionState = A.Z.CLOSED, B.warn("[WS CLOSED] because of manual authentication failure, marking as closed."), this._reset(n, K, "invalid token manually detected")), v.default.track(M.rMx.IOS_INVALID_TOKEN_WORKAROUND_TRIGGERED, {
        api_status_code: t
      })
    }))
  }
  ...
  _doResume() {
    var e;
    this.connectionState = A.Z.RESUMING, this.dispatcher.resumeAnalytics = (0, P.zH)(Date.now() - this.connectionStartTime), B.info("[RESUME] resuming session ".concat(null != (e = this.sessionId) ? e : "", ", seq: ").concat(this.seq)), this.send(D.j.RESUME, {
      token: this.token,
  ...
  resetSocketOnError(e) {
    ...
    }, !0), !1 !== e.sentry && T.Z.captureException(n, {
      tags: {
