...
let A = e => {
    ...
    return <div className={k.preview}>{<div className={k.previewDark}>{null != t ? t : (0, n.jsx)("img", {
          src: Z,
          alt: D.intl.string(D.t.qOsjZm)
        })}</div>}{<div className={k.previewLight}>{null != t ? t : (0, n.jsx)("img", {
          src: R,
  ...
  B = e => {
    ...
    if (null != t) return <w.Z size={160} sticker={t} />;
    ...
    return <w.Z assetData={a} fileUri={a} size={160} sticker={{
        name: s,
        description: "",
        id: s,
        pack_id: "",
...
function L(e) {
  ...
  return <form onSubmit={ej} className={k.form}><c.I transitionState={K} onClose={M} size={"lg"}>{<f.x title={D.intl.string(D.t.yxVsBA)} subtitle={D.intl.format(ex, {
          fileSize: (0, y.IC)(C.Ht, {
            useKibibytes: true
          })
        })} />}{<m.f>{<g.hjN title={D.intl.string(D.t.gjdiKC)} className={k.formItem}><A stickerPreview={ev} /></g.hjN>}{<g.hjN className={r()(k.formItem, k.formItemRow)}>{<g.xJW title={ed ? D.intl.string(D.t.wp36s7) : D.intl.string(D.t.AM3mzc)} required={true} className={k.formItemRowChild}>{ed ? (0, n.jsx)(g.oil, {
              disabled: true,
              ...
              onFileSelect: ep
            })}</g.xJW>}{<j.Z className={k.formItemRowChild} guildId={F} emojiId={$} emojiName={et} setEmojiId={ee} setEmojiName={el} shouldUpdateBothEmojiFields={true} />}</g.hjN>}{<g.xJW title={D.intl.string(D.t["0VRh6u"])} required={true} className={k.formItem}><g.oil value={V} onChange={X} placeholder={D.intl.string(D.t["3fGttb"])} maxLength={30} /></g.xJW>}{<div className={k.formItem}>{<g.vwX>{D.intl.string(D.t.uGccen)}{<g.ua7 text={D.intl.string(D.t.S6jlDg)}>{e => (0, n.jsx)(g.d3s, function(e) {
              ...
              }, e))}</g.ua7>}</g.vwX>}{<g.Kx8 value={en} onChange={ei} placeholder={D.intl.string(D.t.zwR0fX)} maxLength={100} />}</div>}{null != ea && <g.Text className={k.formItem} variant={"text-sm/normal"} color={"text-danger"}>{ea.message}</g.Text>}</m.f>}{<d.G actions={[{
          variant: "secondary",
          ...
          disabled: !eg,
          type: "submit"
        }]} actionsFullWidth={true} />}</c.I></form>
}