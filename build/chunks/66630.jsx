...
var Chunk255367 = require("./255367.js"),
  ...
  Chunk346565 = require("./346565.js"),
  Chunk562577 = require("./562577.js"),
  Chunk926563 = require("./926563.js"),
...
function M(t) {
  ...
  let th = t => {
    ...
    tS = () => null == $ ? null : () => <d.Z className={k.emoji} emojiId={$.id} emojiName={$.name} animated={!!$.animated} />,
    tg = W ? L.intl.string(L.t.rp0aho) : L.intl.string(L.t.UcdRn5),
    tj = "custom-status-placeholder-text";
  return <s.Modal title={L.intl.string(L.t.Zx4jzM)} actionBarInput={(0, l.jsx)("div", {
      ...
      await M()
    }} transitionState={r} actions={[{
      text: L.intl.string(L.t.R3BPHx),
      variant: "primary",
      onClick: tb
    }]}>{null != K && <div className={k.profilePreview}><I.Z user={K} previewText={X} previewEmoji={$} placeHolderText={tu} transitionState={r} label={tr} /></div>}{<c.hjN className={k.formGroup} titleClassName={k.customStatusInputTitle} title={tg}>{V && <div className={k.labelSelectorContainer}><c.DY3 position={"right"} color={c.FGA.BRAND} aria-label={L.intl.string(L.t.y2b7CA)} shouldShow={to} forceOpen={to && r === c.Dvm.ENTERED} tooltipClassName={k.labelSelectorTooltip} text={L.intl.string(L.t.y2b7CA)}><D.Z currentValue={tr} onChange={t => {
              let e = t === tr ? null : t;
            ...
            }} /></c.DY3></div>}{<div className={k.inputContainer} ref={tf}>{<span ref={ta} className={k.hiddenPlaceholder} aria-hidden={"true"} />}{<div className={i()(k.emojiButtonContainer, null != tr && k.emojiButtonContainerWithLabel)}><c.yRy targetElementRef={tl} renderPopout={t => {
              let {
                closePopout: e
              } = t;
              return (0, l.jsx)(S.Z, {
            ...
            }} position={"left"} animation={c.yRy.Animation.NONE} align={"top"}>{(t, e) => {
              var n, a;
              let {
                isShown: r
              } = e;
            ...
            }}</c.yRy></div>}{<u.iS autosize={true} aria-describedby={tj} value={X} maxLength={Z.s0} rows={1} showRemainingCharacterCount={false} placeholder={td} onChange={t => {
            Q(t)
          }} onKeyDown={t => {
            "Enter" === t.key && tb()
          }} className={i()(k.input, null != tr && k.inputWithLabel)} inputRef={tn} />}{null != tr && <div className={k.inputCustomStatusLabelRow}><E.Z label={tr} /></div>}{<c.nn4 id={tj}>{"".concat(L.intl.string(L.t.EVV6ub), ": ").concat(tu)}</c.nn4>}{(X.length > 0 || null != $) && <div className={i()(k.clearButtonWrapper, null != tr && k.buttonContainerWithLabel)}><c.P3F focusProps={{
              offset: {
                top: 8,
                bottom: 8,
                left: false,
            ...
            }} aria-label={L.intl.string(L.t.wfYTHR)} className={k.clearButton} onClick={() => {
              Q(""), J(null)
            }}><c.k$p size={"md"} color={"currentColor"} className={k.clearIcon} /></c.P3F></div>}</div>}</c.hjN>}</s.Modal>
}