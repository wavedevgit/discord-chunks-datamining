/** Chunk was on web.js **/
/** chunk id: 784222, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Bg: () => Z,
  Ni: () => F,
  YH: () => B,
  ld: () => U,
  rf: () => V
}), require("./388685.js"), require("./539854.js");
var Chunk647438 = require("./647438.js"),
  Chunk392711 = require("./392711.js"),
  a = require.n(Chunk392711),
  Chunk772848 = require("./772848.js"),
  Chunk442837 = require("./442837.js"),
  Chunk80932 = require("./80932.js"),
  Chunk339085 = require("./339085.js"),
  Chunk906411 = require("./906411.js"),
  Chunk689789 = require("./689789.js"),
  Chunk407477 = require("./407477.js"),
  Chunk633302 = require("./633302.js"),
  Chunk806966 = require("./806966.js"),
  Chunk28546 = require("./28546.js"),
  Chunk357156 = require("./357156.js"),
  Chunk140465 = require("./140465.js"),
  Chunk125900 = require("./125900.js"),
  Chunk984933 = require("./984933.js"),
  Chunk430824 = require("./430824.js"),
  Chunk914010 = require("./914010.js"),
  Chunk771845 = require("./771845.js"),
  Chunk594174 = require("./594174.js"),
  Chunk176354 = require("./176354.js"),
  Chunk267642 = require("./267642.js"),
  Chunk111361 = require("./111361.js"),
  Chunk392552 = require("./392552.js"),
  Chunk543241 = require("./543241.js"),
  Chunk304852 = require("./304852.js"),
  Chunk199257 = require("./199257.js"),
  Chunk149203 = require("./149203.js"),
  Chunk185923 = require("./185923.js"),
  Chunk388032 = require("./388032.jsx");

function L(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function j(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      L(e, t, n[t])
    })
  }
  return e
}

function k(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function M(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : k(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}
var U = function(e) {
  return e[e.EMOJI = 0] = "EMOJI", e[e.CREATE_EMOJI = 1] = "CREATE_EMOJI", e[e.EXPAND_OR_COLLAPSE_EMOJIS = 2] = "EXPAND_OR_COLLAPSE_EMOJIS", e[e.SOUNDMOJI = 3] = "SOUNDMOJI", e
}({});
let G = [Chunk185923.Z5.DISALLOW_EXTERNAL, Chunk185923.Z5.DISALLOW_CUSTOM],
  B = e => {
    let {
      channel: t,
      collapsedSections: n,
      includeCreateEmojiButton: i = true,
      pickerIntention: l,
      emojiSearchResults: p,
      gridWidth: h,
      emojiPaddingHorizontal: L,
      emojiSpriteSize: k,
      shouldShowSoundmojiInEmojiPicker: U,
      showOnlyUnicode: B
    } = e, Z = (0, s.e7)([c.ZP], () => c.ZP.categories), V = r.useMemo(() => B ? _.ZP.getCategories() : Z, [Z, B]), F = (0, s.e7)([y.Z], () => y.Z.getGuild(null == t ? true : t.getGuildId()), [t]), H = null == t ? null : t.getGuildId(), Y = (0, s.e7)([c.ZP], () => c.ZP.getDisambiguatedEmojiContext(H), [H]), W = (0, N.NJ)(H), K = (0, N.wC)(H), z = (0, N.q5)(H), {
      canSplitFrecencyList: q
    } = (0, d.S)({
      location: "useEmojiGrid",
      autoTrackExposure: true
    }), X = (0, f.p3)({
      location: "useEmojiGrid",
      autoTrackExposure: true
    });
    f.Xb.useExperiment({
      location: "useEmojiGrid"
    }, {
      autoTrackExposure: true
    });
    let {
      topEmojis: Q,
      newlyAddedEmojis: J
    } = (0, P.Z)(H, l), $ = (0, s.Wu)([v.ZP], () => v.ZP.getFlattenedGuildIds(), []), ee = (0, s.e7)([c.ZP], () => c.ZP.expandedSectionsByGuildIds), {
      canCreateExpressions: et
    } = (0, m.XJ)(F), en = (0, s.e7)([I.default], () => I.default.getCurrentUser()), er = (0, A.I5)(en), ei = (0, g.B4)(), ea = Y.getCustomEmoji(), eo = e => {
      if (e.type === u.B.GUILD) return e.guildId
    }, es = r.useMemo(() => a().groupBy(ea, eo), [ea]), {
      enabled: el,
      treatment: ec
    } = C.Z.useExperiment({
      location: "EmojiPicker"
    }, {
      autoTrackExposure: true,
      disable: !et
    }), eu = (0, E.g2)({
      location: "useEmojiGrid"
    });
    return r.useMemo(() => {
      let e = [],
        r = [],
        s = [],
        c = [],
        u = 0,
        d = 0;
      if (null != Y && null != h) {
        let f = Math.floor(h / (k + 2 * L)),
          m = 3 * f,
          g = (o, _) => {
            let p = new Map,
              h = n.has(_.sectionId),
              [g, E] = a().partition(o, e => {
                let n = T.ZP.isEmojiDisabled({
                  emoji: e,
                  channel: t,
                  intention: l
                });
                return p.set(e, n), !n
              }),
              b = g.concat(E),
              y = _.guild,
              v = O.Z.getGuildId(),
              I = _.sectionId === w.En.TOP_GUILD_EMOJI,
              A = !I && null != y && v === y.id && b.length < (0, S.y4)(y),
              R = I && b.length < f && !el,
              P = null != y && I && el && ec === C.B.WITH_TOP_LIST && es[y.id].length < (0, S.y4)(y),
              D = i && et && null != y && (A || R || P);
            D && P && b.length === f && b.shift();
            let L = b.length > m && null != y && _.isNitroLocked,
              U = L && !ee.has(y.id);
            if (U) {
              let e = !el && D;
              b.splice(m - (e ? 2 : 1))
            }
            let G = el && D && !h,
              B = Math.ceil((D || L ? b.length + 1 : b.length) / f),
              Z = [];
            for (let e = 0; e < B; e++) {
              let t = 0 === e,
                n = e * f - (G && e > 0 ? 1 : 0),
                r = n + f - (G && t ? 1 : 0),
                i = b.slice(n, r).map((e, n) => {
                  var r, i;
                  return {
                    type: 0,
                    emoji: e,
                    size: k,
                    isDisabled: p.get(e),
                    rowIndex: d,
                    columnIndex: G && t ? n + 1 : n,
                    visibleRowIndex: u,
                    category: _.type,
                    subCategory: _.sectionId === w.En.TOP_GUILD_EMOJI ? (0, N.IP)(Q, J, null != (i = null != (r = e.id) ? r : e.uniqueName) ? i : e.name) : w.t0.NONE
                  }
                });
              if (G && t && !h && (i = [{
                  type: 1,
                  guildId: y.id,
                  name: x.intl.string(x.t["Z/r7IS"]),
                  size: k,
                  rowIndex: d,
                  columnIndex: 0,
                  visibleRowIndex: u
                }, ...i]), Z.push(i), !h) {
                if (!el && D && e === B - 1) {
                  let t = Z[e];
                  t.push({
                    type: 1,
                    guildId: y.id,
                    name: x.intl.string(x.t["Z/r7IS"]),
                    size: k,
                    rowIndex: e,
                    columnIndex: t.length,
                    visibleRowIndex: u
                  })
                }
                if (L && e === B - 1) {
                  let t = Z[e];
                  t.push({
                    type: 2,
                    guildId: y.id,
                    name: x.intl.string(x.t.NZI2Zm),
                    size: k,
                    rowIndex: e,
                    columnIndex: t.length,
                    visibleRowIndex: u,
                    sectionCollapsedToThreeRows: U
                  })
                }
                c.push(i.length), s.push(i), u++
              }
              d++
            }
            let V = h ? 0 : B,
              F = M(j({}, _), {
                count: o.length
              });
            r.push(F), e.push(V)
          };
        if (null != p) 0 !== p.unlocked.length && g(p.unlocked, {
          type: w.En.SEARCH_RESULTS,
          sectionId: w.En.SEARCH_RESULTS,
          count: p.unlocked.length,
          isNitroLocked: false
        }), 0 !== p.locked.length && g(p.locked, {
          type: w.En.PREMIUM_UPSELL,
          categoryId: w.UX.PREMIUM_UPSELL,
          sectionId: w.En.PREMIUM_UPSELL,
          count: p.locked.length,
          isNitroLocked: !er
        });
        else {
          if (eu && U) {
            let t = [{
              type: 3,
              rowIndex: d,
              columnIndex: 0,
              visibleRowIndex: u
            }];
            r.push({
              type: w.En.SOUNDMOJI,
              categoryId: w.UX.SOUNDMOJI,
              sectionId: "soundmoji",
              count: t.length,
              isNitroLocked: false
            });
            let i = n.has(w.UX.SOUNDMOJI);
            !i && (c.push(t.length), s.push(t), u++), e.push(+!i), d++
          }
          for (let e of V)
            if (e === w.UX.CUSTOM) {
              let n = n => {
                let r = es[n];
                if (null == r) return;
                let i = r.filter(e => !G.includes(T.ZP.getEmojiUnavailableReason({
                  emoji: e,
                  channel: null != t ? t : b.ZP.getDefaultChannel(n),
                  intention: l
                })));
                if (0 === i.length) return;
                let a = y.Z.getGuild(n),
                  s = ei || !er && T.ZP.isEmojiCategoryNitroLocked({
                    categoryEmojis: i,
                    channel: t,
                    intention: l
                  });
                g(i, {
                  categoryId: e,
                  guild: a,
                  type: w.En.GUILD,
                  sectionId: null != a ? a.id : (0, o.Z)(),
                  count: i.length,
                  isNitroLocked: s
                })
              };
              if (null != F && n(F.id), (0, D.Gt)(l))
                for (let e of $)(null == F || F.id !== e) && n(e)
            } else if (e === w.UX.TOP_GUILD_EMOJI) {
            if (null != F) {
              let {
                allEmojis: t
              } = (0, R.J)({
                topEmojis: Q,
                newlyAddedEmojis: J
              });
              t.length > 0 && g(t, {
                categoryId: e,
                guild: y.Z.getGuild(F.id),
                type: w.En.TOP_GUILD_EMOJI,
                sectionId: w.En.TOP_GUILD_EMOJI,
                count: t.length,
                isNitroLocked: false
              })
            }
          } else if (e === w.UX.RECENT) {
            let n = (l === D.Hz.REACTION && (q || X) ? z : K).filter(e => !T.ZP.isEmojiFiltered({
              emoji: e,
              channel: t,
              intention: l
            }));
            g(n, {
              categoryId: e,
              type: w.En.RECENT,
              sectionId: w.En.RECENT,
              count: n.length,
              isNitroLocked: false
            })
          } else if (e === w.UX.FAVORITES) {
            let n = W.filter(e => !T.ZP.isEmojiFiltered({
              emoji: e,
              channel: t,
              intention: l
            }));
            0 !== n.length && g(n, {
              categoryId: e,
              type: w.En.FAVORITES,
              sectionId: w.En.FAVORITES,
              count: n.length,
              isNitroLocked: false
            })
          } else {
            let t = _.ZP.getByCategory(e);
            null != t && g(t, {
              categoryId: e,
              type: w.En.UNICODE,
              sectionId: e,
              count: t.length,
              isNitroLocked: false
            })
          }
        }
      }
      return {
        columnCounts: c,
        emojiGrid: s,
        rowCountBySection: e,
        sectionDescriptors: r
      }
    }, [Y, h, k, L, p, n, el, ec, es, i, et, ee, t, l, Q, J, er, V, F, $, q, X, z, K, W, ei, eu, U])
  };
var Z = function(e) {
  return e[e.PREMIUM = 0] = "PREMIUM", e[e.ROLE_SUBSCRIPTION = 1] = "ROLE_SUBSCRIPTION", e
}({});
let V = e => {
    let {
      pickerIntention: t,
      selectedChannel: n,
      onSelectEmoji: i,
      setUpsellConfigs: a,
      emojiSelectAnalytics: o,
      trackEmojiFavorited: d
    } = e, f = (0, s.e7)([c.ZP], () => c.ZP.getDisambiguatedEmojiContext(null == n ? true : n.getGuildId()), [n]);
    return r.useCallback((e, r) => {
      switch (e.type) {
        case 1:
          return void i({
            emoji: true,
            willClose: r.isFinalSelection,
            isBurst: r.isBurst
          });
        case 0: {
          let {
            emoji: s
          } = e;
          if (null == s) return;
          let c = T.ZP.getEmojiUnavailableReason({
            emoji: s,
            channel: n,
            intention: t
          });
          if (r.toggleFavorite) return void(f.isFavoriteEmojiWithoutFetchingLatest(s) || null != c ? (0, l.Xe)(s) : (null == d || d(e), (0, l.$K)(s)));
          if (c === D.Z5.GUILD_SUBSCRIPTION_UNAVAILABLE) return;
          if (c === D.Z5.PREMIUM_LOCKED) return void a({
            type: 0,
            emojiDescriptor: e
          });
          if (c === D.Z5.ROLE_SUBSCRIPTION_LOCKED && s.type === u.B.GUILD) return void a({
            type: 1,
            guildId: s.guildId,
            emojiId: s.id
          });
          if (c === D.Z5.ROLE_SUBSCRIPTION_UNAVAILABLE) return;
          null == o || o(e), i({
            emoji: s,
            willClose: r.isFinalSelection,
            isBurst: r.isBurst
          })
        }
      }
    }, [n, t, i, a, f, o, d])
  },
  F = e => {
    let {
      emojiListRef: t
    } = e, n = (0, h.Iu)(e => e.searchQuery), i = r.useCallback(e => {
      var n;
      return null == (n = t.current) ? true : n.scrollToSectionTop(e)
    }, [t]);
    return r.useCallback(e => {
      "" !== n ? ((0, h.ql)(""), p.kJ.setActiveCategoryIndex(e)) : i(e)
    }, [i, n])
  }