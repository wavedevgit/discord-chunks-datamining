...
function l(e) {
  ...
  }, [m, h, l, b]), <div className={s.streamInfoContainer}>{<div><strong>{"Stream Info"}</strong></div>}{<div className={s.infoRow}>{<span>{"Video Codec: "}</span>}{<strong>{c}</strong>}</div>}{<div className={s.infoRow}>{<span>{"Resolution: "}</span>}{<strong>{d}</strong>}</div>}{m && <div className={s.infoRow}>{<span>{"Bitrate Estimate: "}</span>}{<strong>{_}</strong>}</div>}</div>
}