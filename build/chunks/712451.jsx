...
function S(e) {
  ...
  let ef = S === O.pC.AVATAR || S === O.pC.BANNER,
    eA = (0, h.M)(!ef);
  return <m.Gt value={er}><u.Y0X transitionState={f} size={u.CgR.MEDIUM} parentComponent={"ImageEditingModal"}>{j && !eA && <g.Z type={S} analyticsPage={k} analyticsSection={D.jXE.CROP_GIF_MODAL} isGIF={ei} banner={r} />}{<u.xBx className={w.modalHeader} separator={false}>{<u.X6q variant={"heading-lg/semibold"}>{I.intl.string(I.t.DxAYCA)}</u.X6q>}{<u.olH onClick={M} className={w.modalCloseButton} />}</u.xBx>}{<u.hzk className={w.modalContent}>{<div className={w.editingContainer}>{q ? (0, n.jsxs)("div", {
          ...
          })}</div>}{<div className={w.toolsContainer}>{<div className={w.zoomControls}>{<u.XBm size={"xxs"} color={"currentColor"} className={w.zoomIcon} />}{<u.iRW className={w.slider} initialValue={1} minValue={1} maxValue={2} keyboardStep={.025} asValueChanges={eu} disabled={J || q} equidistant={true} hideBubble={true} aria-label={I.intl.string(I.t.dnvZSk)} />}{<u.XBm size={"md"} color={"currentColor"} className={w.zoomIcon} />}</div>}{<u.ua7 text={I.intl.string(I.t.E36Wd3)} aria-label={I.intl.string(I.t.LzFiKC)}>{e => {
              var {
                onClick: t
              } = e, r = function(e, t) {
                if (null == e) return {};
            ...
            }}</u.ua7>}</div>}{j && eA && <div><p.Z uploadType={S} analyticsSource={en} showUpsell={true} position={"inline"} className={w.premiumUpsell} showShadow={false} /></div>}</u.hzk>}{<u.mzw className={w.modalFooter}>{<c.zx className={w.resetButton} look={c.zx.Looks.LINK} size={c.Ph.MIN} color={c.zx.Colors.PRIMARY} onClick={eg} disabled={!W}>{I.intl.string(I.t.yBZMsb)}</c.zx>}{<c.zx className={w.linkButton} look={c.zx.Looks.LINK} size={c.Ph.MIN} color={c.zx.Colors.PRIMARY} onClick={eC}>{I.intl.string(I.t["9TG40t"])}</c.zx>}{<u.zxk variant={"primary"} text={I.intl.string(I.t.ZSHmKC)} loading={J} onClick={eE} disabled={J || q} />}</u.mzw>}</u.Y0X></m.Gt>
}