...
function A(e) {
  ...
  if (t.isPurchasedExternally) {
    ...
    return <u.eee href={e} useDefaultUnderlineStyles={false} className={N.externalLink}><u.zxk variant={"secondary"} text={I.intl.string(I.t.SgX7RU)} fullWidth={true} /></u.eee>
  }
  if (!k || !L) return <u.$jN />;
  if (!(B.length > 0)) return <d.zx fullWidth={true} look={d.zx.Looks.FILLED} color={s ? d.zx.Colors.BRAND : d.zx.Colors.PRIMARY} onClick={q}>{I.intl.string(I.t.CpOiEB)}</d.zx>;
  {
    ...
    return <i.Fragment>{(() => {
        ...
        return <f.ZP prependOption={null == e ? {
            label: I.intl.string(I.t.iA5vAw),
            value: null
          } : null} className={l} paymentSources={B} hidePersonalInformation={Z} selectedPaymentSourceId={e} onChange={Y} onPaymentSourceAdd={q} dropdownLoading={U} disabled={D} paymentGatewayRestrictions={t.eligiblePaymentGateways} />
      })()}{null != t.paymentSourceId ? <h.b currencies={n}><div className={a()(N.currency, y.flex, y.alignCenter)}>{<div>{I.intl.string(I.t["0YjaXV"])}</div>}{<h.Z className={N.currencyDropdown} selectedCurrency={G} currencies={n} onChange={e => {
              z(true, e, H)
            }} />}</div></h.b> : null}</i.Fragment>
  }
}
let P = async (e, t, r) => {
  let s = await (0, u.ZDy)(async () => {
    ...
    return n => <s newInvoice={e} onConfirm={t} onCancel={r} modalProps={n} />
  }, {
    onCloseRequest: () => {
      null != s && (0, u.Mr3)(s), r()
    }
