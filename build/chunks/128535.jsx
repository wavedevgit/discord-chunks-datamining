...
let O = 16,
  ...
  T = e => {
    ...
    if (0 === i.length) return <c.Text variant={"text-sm/medium"} color={"text-muted"}>{b.intl.format(b.t.GXqvCw, {
        gameTitle: o
      })}</c.Text>;
    let h = [],
      g = [s];
    ...
    return <c.Text variant={"text-sm/medium"} color={"text-muted"}>{E.reduce((e, t) => [...e, t, " "], [])}</c.Text>
  },
  S = function(e) {
    ...
    return <div className={o()(y.container, {
        [y.inFlight]: A
      })}>{<div className={y.info}>{<div className={y.header}>{<c.Mgn className={o()(y.headerIcon, v(S))} size={"custom"} color={"currentColor"} width={O} height={O} />}{<c.Text variant={"text-sm/semibold"}>{I(S, t)}</c.Text>}</div>}{T({
          quest: e.quest,
          errors: S,
          gameTitle: t,
          location: e.location,
          ...
          impressionId: f,
          sourceQuestContent: e.sourceQuestContent
        })}</div>}{<div className={y.cta}><l.zx color={l.Tt.PRIMARY} onClick={N} disabled={A}><div className={y.ctaInner}>{m.render()}{b.intl.string(b.t.nPThNT)}</div></l.zx></div>}</div>
  }