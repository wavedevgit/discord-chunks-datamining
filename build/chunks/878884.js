/** Chunk was on 93602 **/
n.d(t, {
  Z: () => I
}), n(388685);
var r, l, i, o = n(442837),
  a = n(46973),
  s = n(570140),
  c = n(642047),
  u = n(189786),
  d = n(5192),
  p = n(592125),
  f = n(19780),
  m = n(594174),
  g = n(979651),
  b = n(938475),
  O = n(981631),
  h = n(354459);
let y = new c.Z,
  v = new c.Z,
  S = new Set;

function j(e, t, n) {
  let r = new u.Z({
      userId: e.id,
      channelId: n
    }),
    l = (0, b.PH)(r, null != t ? t : O.ME, e.id);
  y.set(e.id, l);
  let i = {
    type: h.fO.USER,
    user: e,
    id: e.id,
    streamId: null,
    voiceState: r,
    voicePlatform: null,
    speaking: !1,
    lastSpoke: 0,
    soundsharing: !1,
    ringing: !1,
    userNick: d.ZP.getName(t, n, e),
    localVideoDisabled: !1
  };
  v.set(e.id, i)
}

function N(e) {
  let t = y.delete(e),
    n = v.delete(e),
    r = S.delete(e);
  return t || n || r
}

function P() {
  var e;
  let t = f.Z.getChannelId();
  if (null == t) return !1;
  let n = null == (e = p.Z.getChannel(t)) ? void 0 : e.getGuildId(),
    r = !1;
  return S.forEach(e => {
    if (null != g.Z.getVoiceStateForChannel(t, e)) return void S.delete(e);
    let l = m.default.getUser(e);
    null != l && (r = !0, S.delete(e), j(l, n, t))
  }), r
}

function E() {
  y.clear(), v.clear(), S.clear()
}
class Z extends(r = o.ZP.Store) {
  initialize() {
    this.waitFor(g.Z, m.default, p.Z, f.Z), this.syncWith([m.default], P)
  }
  get desyncedVoiceStatesCount() {
    return y.size()
  }
  getDesyncedUserIds() {
    return y.keys()
  }
  getDesyncedVoiceStates() {
    return y.values()
  }
  getDesyncedParticipants() {
    return v.values()
  }
}
i = "RTCConnectionDesyncStore", (l = "displayName") in Z ? Object.defineProperty(Z, l, {
  value: i,
  enumerable: !0,
  configurable: !0,
  writable: !0
}) : Z[l] = i;
let I = new Z(s.Z, {
  CONNECTION_OPEN: function() {
    E()
  },
  VOICE_CHANNEL_SELECT: E,
  RTC_CONNECTION_STATE: function(e) {
    let {
      state: t,
      context: n
    } = e;
    if (n !== a.Yn.DEFAULT || t !== O.hes.DISCONNECTED) return !1;
    E()
  },
  VOICE_STATE_UPDATES: function(e) {
    let {
      voiceStates: t
    } = e, n = f.Z.getChannelId();
    return null != n && t.reduce((e, t) => {
      let {
        userId: r,
        channelId: l
      } = t;
      return l === n && !!N(r) || e
    }, !1)
  },
  RTC_CONNECTION_CLIENT_CONNECT: function(e) {
    let {
      userIds: t,
      guildId: n,
      channelId: r,
      context: l
    } = e;
    return l === a.Yn.DEFAULT && t.reduce((e, t) => {
      if (null != g.Z.getVoiceStateForChannel(r, t)) return e;
      let l = m.default.getUser(t);
      return null == l ? (S.add(t), e) : (j(l, n, r), !0)
    }, !1)
  },
  RTC_CONNECTION_CLIENT_DISCONNECT: function(e) {
    let {
      userId: t,
      context: n
    } = e;
    return n === a.Yn.DEFAULT && N(t)
  }
})