...
class P extends E.ZP {
  ...
  async uploadFileToCloudAsChunks(e) {
    ...
    for (let a = 0; a < i; a++) {
      S.info("Uploading chunk ".concat(a + 1, " of ").concat(i, " for file id ").concat(this.id));
      let o = a * e,
        s = Math.min(o + e, n),
      ...
      try {
        ...
        throw S.error("Error uploading chunk ".concat(a + 1, " for file id ").concat(this.id, ": ").concat(e)), e
      }
    }
  ...
  async uploadChunk(e) {
    ...
    for (let a = 1; a <= n; a++) {
      var r;
      this.uploadAnalytics.totalRequestCount = (null != (r = this.uploadAnalytics.totalRequestCount) ? r : 0) + 1, S.log("Attempt ".concat(a, " for file id ").concat(this.id, " : Uploading chunk ").concat(e.start, "-").concat(e.end - 1, " of ").concat(e.totalSize, " bytes"));
      let n = i().throttle(t => {
        let n = e.start + t.loaded;
        this.emit("progress", n, e.totalSize, n - this.loaded), this.loaded = n
      }, 50);
      try {
        if ((await o.tn.put(T(v({
            url: e.sessionUrl,
            body: e.chunk,
            headers: t,
  ...
  uploadFileToCloud(e) {
    ...
    S.log("Uploading ".concat(this.id)), this.item.platform === E.ow.REACT_NATIVE ? n = null != (t = {
      type: this.item.mimeType,
      uri: this.item.uri,
      name: this.item.filename
    }).type && "application/json" !== t.type ? t.type : "application/octet-stream" : (t = this.item.file, n = "application/octet-stream");
    let r = i().throttle(e => {
    ...
    return void 0 !== e.md5Hash && (a[N] = e.md5Hash), o.tn.put(T(v({
  ...
  async upload() {
    ...
    try {
      ...
      let e = await this.trackTime("getUploadUrlTimeMs", async () => await o.tn.post(T(v({
