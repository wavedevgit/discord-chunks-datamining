/** Chunk was on 37082 **/
/** chunk id: 613087, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  B$: () => d,
  l7: () => p,
  p: () => m
}), require("./388685.js"), require("./539854.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk867309 = require("./867309.js"),
  Chunk126663 = require("./126663.js"),
  Chunk393238 = require("./393238.js"),
  Chunk743294 = require("./743294.js");

function c(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      var r;
      r = n[t], t in e ? Object.defineProperty(e, t, {
        value: r,
        enumerable: true,
        configurable: true,
        writable: true
      }) : e[t] = r
    })
  }
  return e
}

function u(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : (function(e, t) {
    var n = Object.keys(e);
    if (Object.getOwnPropertySymbols) {
      var r = Object.getOwnPropertySymbols(e);
      n.push.apply(n, r)
    }
    return n
  })(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}
let d = Chunk73800.createContext({
    registerComponent: () => {},
    unregisterComponent: () => {},
    animatedComponents: {},
    expandedContentRef: Chunk73800.createRef(),
    collapsedContentRef: Chunk73800.createRef(),
    recalculateAnimationPositions: () => {},
    animatedComponentProps: [],
    expansionSpring: null,
    mountPoints: new Map
  }),
  p = Chunk73800.forwardRef(function(e, t) {
    var n;
    let {
      children: l,
      id: c,
      inState: u,
      isTextTransition: p = false
    } = e, {
      recalculateAnimationPositions: m,
      registerComponent: g,
      unregisterComponent: f,
      expansionSpring: x,
      mountPoints: h
    } = s.useContext(d), j = s.useRef(null), _ = s.useRef(null), b = s.useRef(true);
    s.useEffect(() => {
      m()
    }, [m]), s.useLayoutEffect(() => {
      let e = j.current;
      return null != e && g(e, c, u), () => {
        null != e && f(c, u)
      }
    }, [c, u, g, f]);
    let v = s.useCallback(e => {
      let {
        height: t
      } = e;
      b.current !== t && (m(), b.current = t)
    }, [m]);
    (0, i.PM)(j, v);
    let C = null == (n = h.get(c)) ? true : n.current,
      y = null;
    return null == C ? y = null : p && null != x ? y = <r.Fragment>{"collapsed" === u && (0, o.createPortal)(<a.animated.div style={{
          position: "absolute",
          opacity: x.to({
            range: [0, 1],
            output: [1, 0]
          })
        }}>{l(_)}</a.animated.div>, C)}{"expanded" === u && (0, o.createPortal)(<a.animated.div style={{
          position: "absolute",
          opacity: x.to({
            range: [0, 1],
            output: [0, 1]
          })
        }}>{l(_)}</a.animated.div>, C)}</r.Fragment> : "collapsed" === u && (y = (0, o.createPortal)(l(_), C)), <div style={{
        opacity: +(null == y && "collapsed" === u || null == C)
      }} ref={t}>{l(j)}{y}</div>
  }),
  m = e => {
    let {
      children: t,
      expandedContentRef: n,
      collapsedContentRef: o,
      expansionSpring: a
    } = e, [i, p] = s.useState({}), [m, g] = s.useState([]), [f, x] = s.useState(() => new Map), h = s.useCallback((e, t, n) => {
      p(r => {
        var s;
        let o = null != (s = r[t]) ? s : {
          expanded: null,
          collapsed: null
        };
        return u(c({}, r), {
          [t]: u(c({}, o), {
            [n]: e
          })
        })
      }), x(e => {
        let n = new Map(e);
        return n.set(t, s.createRef()), n
      })
    }, []), j = s.useCallback((e, t) => {
      let n = false;
      p(r => {
        var s;
        let o = null != (s = r[e]) ? s : {
          expanded: null,
          collapsed: null
        };
        return o[t] = null, n = null == o.expanded && null == o.collapsed, u(c({}, r), {
          [e]: o
        })
      }), n && x(t => {
        let n = new Map(t);
        return n.delete(e), n
      })
    }, []), _ = s.useCallback(() => {
      let e = [];
      for (let t in i) {
        if (null == i[t] || null == n.current || null == o.current) continue;
        let r = i[t].collapsed,
          s = i[t].expanded;
        if (null == r || null == s) continue;
        let a = s.getBoundingClientRect(),
          c = n.current.getBoundingClientRect(),
          u = r.getBoundingClientRect(),
          d = o.current.getBoundingClientRect(),
          p = a.top - c.top + l.Li,
          m = u.top - d.top,
          g = a.left - c.left + l.Li,
          f = u.left - d.left,
          x = -a.right + c.right + l.Li,
          h = -u.right + d.right;
        e.push({
          id: t,
          collapsedLeft: f,
          expandedLeft: g,
          collapsedRight: h,
          expandedRight: x,
          collapsedTop: m,
          expandedTop: p,
          width: a.width
        })
      }
      g(e)
    }, [i, n, o, g]);
    return <d.Provider value={{
        registerComponent: h,
        unregisterComponent: j,
        animatedComponents: i,
        expandedContentRef: n,
        collapsedContentRef: o,
        recalculateAnimationPositions: _,
        animatedComponentProps: m,
        expansionSpring: a,
        mountPoints: f
      }}>{t}</d.Provider>
  }