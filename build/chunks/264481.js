...
let x = [() => p.intl.string(p.t.madJdH), () => p.intl.string(p.t.NYmfoK), () => p.intl.string(p.t.R2PaCg), () => p.intl.string(p.t.laSR8v), () => p.intl.string(p.t.DnsJEx)],
  ...
  _ = [() => p.intl.string(p.t.AyMGXF), () => p.intl.string(p.t.aAFW7e), e => p.intl.formatToPlainString(p.t.h2g0cH, {
    name: e
...
function I(e) {
  let {
    ...
    channelId: a,
    onClose: s
  } = e, d = u.ZP.getName(n, a, t), m = p.intl.formatToPlainString(p.t.sjSitL, {
    name: d
  }), [x] = o.useState(() => {
    var e;
