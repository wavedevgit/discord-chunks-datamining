...
let v = Chunk73800.memo(function(e) {
  ...
  let G = (0, E.K)(x, U, {
    ...
    B = I(x);
  return <div className={l()(O.container, N, T, {
      [O.showEffect]: s !== g.F.PLAIN,
      [O.animated]: s === g.F.ANIMATED && !C,
      [O.loop]: S,
      [O.inProfile]: v
    })} style={G}>{<span data-username-with-effects={k} className={l()(O.innerContainer, null == B ? true : B.effectClassName)}>{j}</span>}{(null == B ? true : B.glowClassName) != null && <span className={l()(O.glowContainer, O.innerContainer, B.glowClassName)} aria-hidden={true}>{k}</span>}</div>
});
