...
function L(e) {
  ...
  let {
    ...
    quest: a,
    location: R.dr.QUESTS_BAR
  }), M = (0, c.e7)([p.Z], () => p.Z.useReducedMotion), Z = (0, c.e7)([f.Z], () => f.Z.hasLayers()), W = s.useRef(null), H = s.useMemo(() => (0, _.q8)(a), [a]), U = (null == (t = a.userStatus) ? true : t.enrolledAt) != null, F = (0, u.Z)(U), K = (null == (n = a.userStatus) ? true : n.completedAt) != null, {
    hasError: z,
    isLoading: G
    ...
    et(false), $.current || K || eu(false)
  }, [K, eu]), ef = s.useCallback(() => {
    et(false), K || eu(false), $.current = false
  }, [K, eu]), eh = s.useCallback(() => {
    er(true)
  }, []), ex = s.useCallback(() => {
    er(false), eu(false)
  ...
  s.useEffect(() => {
    ...
    K || !U || F || $.current || eu(false)
  }, [U, K, F, eu]), s.useLayoutEffect(() => {
  ...
  s.useEffect(() => {
    ...
    (!Q || L) && x.Z.clearTracking(), X && el && !G && !z && b && x.Z.stopTracking(a.id)
  }, [Q, L, X, el, G, z, b, a.id]);
  let {
    enabled: eT,
    status: ew
  ...
  if (!b || !X && el && !G || z) return z ? m.log("Not rendered due to asset error") : b || m.log("Not rendered due to ineligibility"), null;
  let eN = eP + 78 * !!K;
  return (0, r.jsx)(O.A, {
    questOrQuests: a,
    questContent: v.jn.QUEST_BAR_V2,
