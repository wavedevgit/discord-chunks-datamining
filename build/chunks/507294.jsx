...
let k = function(e) {
  ...
  return <f.Y0X transitionState={i} aria-labelledby={W} className={E.modal} parentComponent={"GuildRoleConnectionsModal"}>{<f.xBx separator={false} justify={a.k.Justify.BETWEEN}>{<div className={E.header}>{<R.Z className={E.headerIcon} size={24} />}{<f.X6q variant={"text-lg/semibold"}>{P.intl.string(P.t.ghtnsr)}</f.X6q>}</div>}{<f.olH onClick={k} />}</f.xBx>}{<f.hzk className={E.container}>{<f.X6q variant={"heading-md/medium"}>{P.intl.format(P.t["Y+TsER"], {
          helpdeskArticleUrl: _.Z.getArticleURL(w.BhN.CONNECTION_DETAILS)
        })}</f.X6q>}{<div className={E.verifiedRoles}>{L.map(e => {
