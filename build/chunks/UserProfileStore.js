...
function J(e) {
  ...
  if (null == (n = C.get(z.user.id)) || n.delete(X), A.delete(z.user.id), null != z.mutual_guilds) {
    ...
    }), k.set(z.user.id, _.ZP.getFlattenedGuildIds().filter(t => null != e[t]).map(t => ({
      guild: e[t].guild,
      nick: e[t].nick
    })))
  }
  ...
  if (null != H && H.userId === z.user.id && (Date.now() > H.expiresAtMs ? H = null : $(er, H)), D.set(z.user.id, {
      userId: z.user.id,
      ...
      pronouns: null != (S = null == (f = z.user_profile) ? true : f.pronouns) ? S : "",
...
function et(e) {
  ...
  s.fetchStartedAt = o, s.fetchEndedAt = Date.now(), s.fetchError = a, D.set(r, s), (null == a ? true : a.status) === 404 && (M.set(r, 0), j.set(r, U), k.set(r, G))
}
...
function eu(e) {
  return [...D.keys()].reduce((e, t) => e_(t) || e, false)
}
