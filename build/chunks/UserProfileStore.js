...
function G(e) {
  let t = e.data.type,
    n = e.data.games.map(e => I(O({}, e), {
      applicationId: e.game_id
    }));
  return t === s.l.FAVORITE_GAMES ? (i()(n.length > 0, "Favorite games widget has at least one game"), {
    id: e.id,
...
function F(e) {
  let t = P.get(e);
  if ((null == t ? true : t.profileEffectExpiresAt) == null) return;
  let n = 1e3 * t.profileEffectExpiresAt + E.Cm - Date.now();
  if (n <= 0) t.profileEffectId = true, t.profileEffectExpiresAt = true, D.delete(e), e_.emitChange();
  else {
    let t = D.get(e);
    if (null == t) return;
...
function V(e, t) {
  ...
  if (null == n) return;
  let r = n.get(t);
  if ((null == r ? true : r.profileEffectExpiresAt) == null) return;
  let i = L.get(e),
    o = 1e3 * r.profileEffectExpiresAt + E.Cm - Date.now();
  if (o <= 0) n.set(t, I(O({}, r), {
    profileEffectId: true,
    profileEffectExpiresAt: true
  })), null != i && i.delete(t), e_.emitChange();
  else {
    if (null == i) return;
    let n = i.get(t);
    if (null == n) return;
...
function H() {
  S.clear(), A.clear(), P.clear(), w.clear(), x.clear(), M.clear(), k.clear(), B = false
}
...
function q(e) {
  ...
  let {
    userProfile: en,
    fetchStartedAt: er
  } = e, ei = null != (R = null == (t = en.guild_member_profile) ? true : t.guild_id) ? R : T;
  if (null == (n = S.get(en.user.id)) || n.delete(ei), A.delete(en.user.id), null != en.mutual_guilds) {
    ...
    }), k.set(en.user.id, m.ZP.getFlattenedGuildIds().filter(t => null != e[t]).map(t => ({
      guild: e[t].guild,
      nick: e[t].nick
    })))
  }
  ...
  let eo = null != en.premium_since ? new Date(en.premium_since) : null,
    ...
    es = en.application,
    el = null != en.badges ? en.badges.map(e => {
      let t = (0, g.fv)(e.id);
      if ((e.id === N || null != t) && null != eo) {
        let n = b.intl.formatToPlainString(b.t["8zbGNT"], {
          date: eo
        });
        return null != t && (n = b.intl.formatToPlainString(b.t.Hu4jfn, {
          date: eo
        })), I(O({}, e), {
          description: n
        })
      }
      return e.id.startsWith(C) && null != ea ? I(O({}, e), {
        description: b.intl.formatToPlainString(b.t.IWkAq6, {
          date: ea
        })
      }) : e
    }) : [];
  if (null != Z && Z.userId === en.user.id && (Date.now() > Z.expiresAtMs ? Z = null : X(el, Z)), P.set(en.user.id, {
      userId: en.user.id,
      banner: null == (r = en.user_profile) ? true : r.banner,
      accentColor: null == (i = en.user_profile) ? true : i.accent_color,
      themeColors: null == (o = en.user_profile) ? true : o.theme_colors,
      popoutAnimationParticleType: null == (a = en.user_profile) ? true : a.popout_animation_particle_type,
      bio: null != (U = null == (s = en.user_profile) ? true : s.bio) ? U : "",
      profileEffectId: null == (d = en.user_profile) || null == (u = d.profile_effect) ? true : u.id,
      profileEffectExpiresAt: null == (p = en.user_profile) || null == (f = p.profile_effect) ? true : f.expires_at,
      pronouns: null != (B = null == (h = en.user_profile) ? true : h.pronouns) ? B : "",
      ...
      badges: el,
      widgets: null == (E = en.widgets) ? true : E.map(G)
    }), (null == (v = en.user_profile) || null == (y = v.profile_effect) ? true : y.expires_at) != null) {
    let e = new l.V7;
  ...
  if (null != en.guild_member_profile) {
    let e = {
        ...
        themeColors: null == (W = en.guild_member_profile) ? true : W.theme_colors,
        popoutAnimationParticleType: null == (z = en.guild_member_profile) ? true : z.popout_animation_particle_type,
        profileEffectId: null == (Q = en.guild_member_profile) || null == (q = Q.profile_effect) ? true : q.id,
        profileEffectExpiresAt: null == ($ = en.guild_member_profile) || null == (J = $.profile_effect) ? true : J.expires_at,
    ...
    if ((null == (et = en.guild_member_profile) || null == (ee = et.profile_effect) ? true : ee.expires_at) != null) {
      let e = new l.V7,
        t = L.get(en.user.id);
      if (null != t) t.set(en.guild_member_profile.guild_id, e);
      else {
...
function J(e) {
  ...
  s.fetchStartedAt = a, s.fetchEndedAt = Date.now(), s.fetchError = o, P.set(r, s), (null == o ? true : o.status) === 404 && (M.set(r, 0), x.set(r, j), k.set(r, U))
}
...
function en(e) {
  B = false, null != e.guild_id ? ee(e) : $(e)
}
...
function ea(e) {
  var t, n;
  let r = e.user.id;
  return !((null != (n = null == (t = S.get(r)) ? true : t.size) ? n : 0) > 0) && ed(r)
}

function es(e) {
  return [...P.keys()].reduce((e, t) => ed(t) || e, false)
}
...
class ef extends Chunk750041.Z {
  initialize() {
    this.waitFor(Chunk771845.ZP), this.syncWith([Chunk706454.default], eu)
  }
  ...
  getGuildMemberProfile(e, t) {
    var n, r;
    return null == t ? null : null != (r = null == (n = w.get(e)) ? true : n.get(t)) ? r : null
  }
  getMutualFriends(e) {
    return x.get(e)
  }
