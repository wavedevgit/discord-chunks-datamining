...
let g = {
  ...
  p = Chunk73800.memo(function(e) {
    ...
    return <div className={r()(h.container, {
        [h.initialized]: S,
        [h.disabled]: !S || i
      }, t)}>{<div className={h.playButtonContainer}>{<s.P3F className={h.playButton} onClick={S ? Z : true}>{b ? (0, l.jsx)(s.wNq, {
            size: "xs",
            color: "currentColor",
            ...
            color: "currentColor",
            className: h.playButtonIcon
          })}</s.P3F>}{<div className={h.durationContainer}><s.Text className={h.__invalid_duration} variant={"text-xs/normal"} color={E ? "text-feedback-warning" : "text-feedback-positive"}>{"".concat((0, f.my)(O).toFixed(2), "s")}</s.Text></div>}</div>}{<d.Z playing={b} onPlaybackChange={k} onPausePlayback={P} onChangePosition={M} disabled={i} />}</div>
  })