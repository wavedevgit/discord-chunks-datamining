...
function E() {
  ...
  return exports && require && (R.push(<Chunk481060.sNh id={"overrides"} label={"Overrides"} action={() => {
      Chunk230711.Z.open(Chunk981631.oAB.DEVELOPER_OPTIONS, Chunk726985.s6.DEVELOPER_OPTIONS_OVERRIDES_TAB)
    }} />), R.push(<Chunk481060.sNh id={"manual-triggers"} label={"Manual Triggers"} action={() => {
      Chunk230711.Z.open(Chunk981631.oAB.DEVELOPER_OPTIONS, Chunk726985.s6.DEVELOPER_OPTIONS_MANUAL_TRIGGERS_TAB)
    }} />), R.push(<Chunk481060.sNh id={"developer-flags"} label={"Developer Flags"} action={() => {
      Chunk230711.Z.open(Chunk981631.oAB.DEVELOPER_OPTIONS, Chunk726985.s6.DEVELOPER_OPTIONS_FLAGS_TAB)
    }}>{<Chunk481060.S89 id={"tracing-requests"} label={"Tracing Requests"} checked={h} action={() => {
        (0, Chunk179658.y)({
          trace: !h
        })
      }} />}{<Chunk481060.S89 id={"forced-canary"} label={"Forced Canary"} checked={j} action={() => {
        (0, Chunk179658.y)({
          canary: !j
        })
      }} />}{<Chunk481060.S89 id={"always-deliver"} label={"Ads auto-targeting"} checked={C} action={() => {
        Chunk695346.zY.updateSetting(!C)
      }} />}{<Chunk481060.S89 id={"source-maps"} label={"Source Maps"} checked={P} action={() => {
        (0, Chunk179658.y)({
          sourceMapsEnabled: !P
        })
      }} />}{<Chunk481060.S89 id={"idle-status"} label={"Idle Status Indicator"} checked={w} action={() => {
        (0, Chunk179658.y)({
          idleStatusIndicatorEnabled: !w
        })
      }} />}{<Chunk481060.S89 id={"accessibility-auditing"} label={"Accessibility Auditing"} checked={A} action={() => {
        (0, Chunk179658.y)({
          axeEnabled: !A
        })
      }} />}{<Chunk481060.S89 id={"prevent-popout-close"} label={"Prevent Popouts From Closing"} checked={x} action={() => {
        (0, Chunk179658.y)({
          preventPopoutClose: !x
        })
      }} />}{<Chunk481060.S89 id={"preview-collections"} label={"Preview Unpublished Collections"} checked={D} action={() => {
        (0, Chunk179658.y)({
          onlyShowPreviewAppCollections: !D
        })
      }} />}{<Chunk481060.S89 id={"disable-collections-cache"} label={"Disable Collections Cache"} checked={I} action={() => {
        (0, Chunk179658.y)({
          disableAppCollectionsCache: !I
        })
      }} />}</Chunk481060.sNh>), R.push(<Chunk481060.sNh id={"logging"} label={"Logging"} action={() => {
      Chunk230711.Z.open(Chunk981631.oAB.DEVELOPER_OPTIONS, Chunk726985.s6.DEVELOPER_OPTIONS_LOGGING_TAB)
    }}>{<Chunk481060.S89 id={"gateway-events"} label={"Gateway Events"} checked={E} action={() => {
        (0, Chunk179658.y)({
          logGatewayEvents: !E
        })
      }} />}{<Chunk481060.S89 id={"overlay-events"} label={"Overlay RPC Events"} checked={f} action={() => {
        (0, Chunk179658.y)({
          logOverlayEvents: !f
        })
      }} />}{<Chunk481060.S89 id={"analytics-events"} label={"Analytics Events"} checked={v} action={() => {
        (0, Chunk179658.y)({
          logAnalyticsEvents: !v
        })
      }} />}{<Chunk481060.S89 id={"analytics-debugger"} label={"Analytics Debugger"} checked={S} action={() => {
        (0, Chunk179658.y)({
          analyticsDebuggerEnabled: !S
        })
      }} />}</Chunk481060.sNh>), R.push(<Chunk481060.sNh id={"design-tools"} label={"Design Tools"} action={() => {
      Chunk230711.Z.open(Chunk981631.oAB.DEVELOPER_OPTIONS, Chunk726985.s6.DEVELOPER_OPTIONS_DESIGN_TOOLS_TAB)
    }}><Chunk481060.S89 id={"layout-debugging"} label={"Enable Layout Debugging"} checked={module} action={() => {
        (0, Chunk179658.y)({
          layoutDebuggingEnabled: !module
        })
      }} /></Chunk481060.sNh>), module && (R.push(<Chunk481060.II_ id={"horizontal-spacing"} label={"Horizontal Spacing"} control={(e, t) => (0, a.jsx)(i._wy, O(y({}, e), {
      ref: t,
      value: _,
      minValue: 0,
      maxValue: Z,
    ...
    }))} />), R.push(<Chunk481060.II_ id={"vertical-spacing"} label={"Vertical Spacing"} control={(e, t) => (0, a.jsx)(i._wy, O(y({}, e), {
      ref: t,
      value: T,
      minValue: 0,
      maxValue: N,
