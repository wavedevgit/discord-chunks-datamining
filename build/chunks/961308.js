...
class h extends Chunk727916.Z {
  ...
  innerExtract(e, t) {
    ...
    if (a = 0, h = 0, g = false, S[_]) {
      let e = parseInt(S[_]);
      if (isNaN(e) && (e = (0, i.zU)(S[_])), e >= 60) return null;
      n.end.assign("second", e)
    }
    if (isNaN(a = parseInt(S[f])) && (a = (0, i.zU)(S[f])), S[p] ? "半" == S[p] ? h = 30 : "正" == S[p] || "整" == S[p] ? h = 0 : isNaN(h = parseInt(S[p])) && (h = (0, i.zU)(S[p])) : a > 100 && (h = a % 100, a = Math.floor(a / 100)), h >= 60 || a > 24) return null;
    if (a >= 12 && (g = 1), S[m]) {
      if (a > 12) return null;
    ...
    return n.text = n.text + S[0], n.end.assign("hour", a), n.end.assign("minute", h), g >= 0 ? n.end.assign("meridiem", g) : n.start.isCertain("meridiem") && 1 == n.start.get("meridiem") && n.start.get("hour") > a ? n.end.imply("meridiem", 0) : a > 12 && n.end.imply("meridiem", 1), n.end.date().getTime() < n.start.date().getTime() && n.end.imply("day", n.end.get("day") + 1), n
  }
}