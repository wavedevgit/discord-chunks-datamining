/** Chunk was on web.js **/
/** chunk id: 297781, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Af: () => j,
  An: () => W,
  E6: () => Y,
  EE: () => G,
  Gk: () => Q,
  Gt: () => D,
  OV: () => U,
  PZ: () => $,
  Rg: () => X,
  U9: () => z,
  XF: () => B,
  Yl: () => q,
  ej: () => k,
  f: () => H,
  m7: () => M,
  n8: () => K,
  pQ: () => F,
  v1: () => V,
  wO: () => Z
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk913527 = require("./913527.js"),
  l = require.n(Chunk913527),
  Chunk876215 = require("./876215.js"),
  Chunk884439 = require("./884439.js"),
  Chunk705512 = require("./705512.js"),
  Chunk481060 = require("./481060.js"),
  Chunk456100 = require("./456100.js"),
  Chunk70956 = require("./70956.js"),
  Chunk823379 = require("./823379.js"),
  Chunk709054 = require("./709054.js"),
  Chunk26033 = require("./26033.js"),
  Chunk22211 = require("./22211.js"),
  Chunk561308 = require("./561308.js"),
  Chunk797342 = require("./797342.js"),
  Chunk371991 = require("./371991.js"),
  Chunk206583 = require("./206583.js"),
  Chunk388032 = require("./388032.js"),
  Chunk859589 = require("./859589.js");

function S(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function A(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      S(e, t, n[t])
    })
  }
  return e
}

function N(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function C(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : N(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}

function R(e, t) {
  if (null == e) return {};
  var n, r, i = P(e, t);
  if (Object.getOwnPropertySymbols) {
    var o = Object.getOwnPropertySymbols(e);
    for (r = 0; r < o.length; r++) n = o[r], !(t.indexOf(n) >= 0) && Object.prototype.propertyIsEnumerable.call(e, n) && (i[n] = e[n])
  }
  return i
}

function P(e, t) {
  if (null == e) return {};
  var n, r, i = {},
    o = Object.keys(e);
  for (r = 0; r < o.length; r++) n = o[r], t.indexOf(n) >= 0 || (i[n] = e[n]);
  return i
}
let w = {
  [Chunk876215.s.TOP_ARTIST]: [W],
  [Chunk876215.s.PLAYED_GAME]: [U, G, j, z, Z, H, K, V, F],
  [Chunk876215.s.TOP_GAME]: [Y],
  [Chunk876215.s.WATCHED_MEDIA]: [q, X],
  [Chunk876215.s.LAUNCHED_ACTIVITY]: [U, G, j, z, K, F]
};
var D = function(e) {
  return e[e.CARD = 0] = "CARD", e[e.POPOUT = 1] = "POPOUT", e[e.STREAMING_POPOUT = 2] = "STREAMING_POPOUT", e[e.GAME_PROFILE = 3] = "GAME_PROFILE", e[e.USER_PROFILE = 4] = "USER_PROFILE", e[e.EMBED = 5] = "EMBED", e[e.LEADERBOARD_POPOUT = 6] = "LEADERBOARD_POPOUT", e[e.OVERLAY = 7] = "OVERLAY", e[e.FRIENDS_POPOUT = 8] = "FRIENDS_POPOUT", e[e.APP_LAUNCHER = 9] = "APP_LAUNCHER", e
}({});
let L = Chunk73800.createContext({});

function x() {
  return Chunk73800.useContext(L)
}

function M(e) {
  var {
    children: t
  } = e, n = R(e, ["children"]);
  return <div{...C(A({
    className: T.badgeContainer
  }, n), {
    children: t
  })} />
}

function k(e) {
  let {
    Icon: t,
    text: n,
    iconColor: i,
    tooltipText: o,
    showTooltip: a
  } = e, {
    defaultTextColor: s,
    defaultIconColor: l,
    location: c
  } = x();
  return <f.ua7 text={o} shouldShow={a}>{e => (0, r.jsxs)(M, C(A({}, e), {
      children: [(0, r.jsx)(t, {
        size: "xxs",
        color: null != i ? i : l
      }), (0, r.jsx)(f.Text, {
        variant: "text-xs/normal",
        color: s,
        className: T.badgeLabel,
        scaleFontToUserSetting: 5 === c,
        children: n
      })]
    }))}</f.ua7>
}

function j(e) {
  let {
    entry: t
  } = e, {
    channel: n
  } = (0, E.Z)(t), {
    location: i
  } = x();
  return null == n || (0, h.Hi)(i, [1, 2, 3, 4]) ? null : <k Icon={f.gj8} text={n.name} />
}

function U(e) {
  let {
    entry: t,
    hovered: n
  } = e, {
    defaultTextColor: i,
    defaultIconColor: o,
    location: a
  } = x(), s = (0, b.Jg)(t) && (0, h.Hi)(a, [0, 4, 7, 9]), l = s ? f.TVs.colors.TEXT_FEEDBACK_POSITIVE : o, c = s ? "text-feedback-positive" : i, {
    streamPreviewUrl: u
  } = (0, E.Z)(t), {
    enabled: d
  } = _.c.useExperiment({
    location: "GameTimestampBadge"
  }, {
    autoTrackExposure: true
  }), p = null != u ? f.hGI : (0, g.Mq)(t) && !d ? f.jje : f.iWm;
  return <M>{<p size={"xxs"} color={l} />}{<O.ZP entry={t} textColor={c} hovered={n} bold={true} scaleFontToUserSetting={5 === a} />}</M>
}

function G(e) {
  let {
    entry: t
  } = e, {
    defaultTextColor: n,
    defaultIconColor: i
  } = x(), {
    state: o,
    party: a
  } = (0, y.n)(t), s = (0, b.bT)(o, a);
  return null == s ? null : <M>{<f.BFJ size={"xxs"} color={i} />}{<f.Text variant={"text-xs/normal"} color={n} lineClamp={1}>{s}</f.Text>}</M>
}

function B(e) {
  let {
    entry: t,
    hovered: n
  } = e, {
    defaultTextColor: i,
    defaultIconColor: o,
    location: a
  } = x(), s = (0, b.Jg)(t) && (0, h.Hi)(a, [0, 4]), l = s ? f.TVs.colors.TEXT_FEEDBACK_POSITIVE : o, c = s ? "text-feedback-positive" : i;
  return <div className={T.badgeContainer}>{<f.RZG size={"xxs"} color={l} />}{<O.ZP entry={t} textColor={c} hovered={n} bold={true} />}</div>
}

function Z(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x(), i = (0, h.Hi)(n, [0, 3]) ? f.TVs.colors.STATUS_POSITIVE : true;
  return (0, b.Ol)(t) ? <k Icon={f._IE} text={I.intl.string(I.t.keY6mZ)} iconColor={i} /> : null
}

function F(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x();
  if (!(0, b.V5)(t)) return null;
  let i = (0, b.kr)(t),
    o = 0 === n && !i,
    {
      text: a,
      tooltipText: s
    } = (0, b.zo)(t);
  return null == a ? null : <k Icon={f.ANZ} text={a} tooltipText={s} showTooltip={o} />
}

function V(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x(), i = 0 !== n, o = (0, b.dw)(t);
  if (null == o) return null;
  let a = (0, b.GE)(o);
  return <k Icon={f.Oe7} showTooltip={!i} tooltipText={a} text={i ? a : I.intl.string(I.t.adnLsL)} />
}

function H(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x(), i = (0, b.vU)(t);
  return (0, b.q_)(t) ? <k Icon={f.qOE} showTooltip={0 === n} text={I.intl.formatToPlainString(I.t["Klie/P"], {
      days: i
    })} tooltipText={I.intl.formatToPlainString(I.t.PwMe0t, {
      days: i
    })} /> : null
}

function Y(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x(), i = 0 !== n, o = (0, b.yA)(t);
  if (null == o) return null;
  let a = i ? " — " : ": ",
    s = i ? I.t.C0Axoa : I.t.SDRHgo;
  return <k Icon={f.rm8} text={(0, r.jsxs)(r.Fragment, {
      children: [I.intl.string(I.t["/50eHh"]), a, I.intl.format(s, {
        hours: Math.round(o / p.Z.Seconds.HOUR)
      })]
    })} />
}

function W(e) {
  var t;
  let {
    entry: n
  } = e, {
    location: i
  } = x(), o = null == (t = (0, b.PJ)(n, u.N.AGGREGATE_COUNT)) ? true : t.count;
  if (null == o) return null;
  let a = (0, h.Hi)(i, [1, 2, 5]) ? I.intl.formatToPlainString(I.t.HtifnJ, {
    count: o
  }) : I.intl.formatToPlainString(I.t["jq/Bmp"], {
    count: o
  });
  return <k Icon={f.rm8} text={a} />
}

function K(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x();
  if (3 === n) return null;
  let i = (0, b.ig)(t);
  return null == i || i === d.o.TRENDING_TYPE_UNSPECIFIED ? null : <k Icon={f.YqE} text={I.intl.string(I.t.kAlUs7)} />
}

function z(e) {
  let {
    entry: t
  } = e, {
    location: n
  } = x();
  if (3 === n) return null;
  let i = m.default.extractTimestamp(t.extra.application_id);
  return l()().diff(l()(i), "days") > v.G ? null : <k Icon={f.rIT} text={I.intl.string(I.t.vYuyWV)} />
}

function q(e) {
  let {
    entry: t,
    hovered: n
  } = e, {
    defaultTextColor: i,
    defaultIconColor: o,
    location: a
  } = x(), s = (0, b.Jg)(t) && 4 === a, l = s ? f.TVs.colors.TEXT_FEEDBACK_POSITIVE : o, c = s ? "text-feedback-positive" : i;
  return <div className={T.badgeContainer}>{<f.ARS size={"xxs"} color={l} />}{<O.ZP entry={t} textColor={c} hovered={n} bold={true} />}</div>
}

function X(e) {
  let {
    entry: t
  } = e, n = (0, b.ap)(t.extra.media_assets_large_text);
  return null == n ? null : <k Icon={f.sVe} text={n} />
}

function Q(e) {
  let t, {
    location: n,
    children: i,
    className: o
  } = e;
  return t = (0, h.Hi)(n, [1, 5, 6]) ? {
    defaultTextColor: "content-inventory-overlay-text-secondary",
    defaultIconColor: f.TVs.colors.CONTENT_INVENTORY_OVERLAY_TEXT_SECONDARY
  } : 2 === n ? {
    defaultTextColor: "interactive-normal"
  } : 3 === n ? {
    defaultTextColor: "text-muted"
  } : 4 === n ? {
    defaultTextColor: "text-secondary",
    defaultIconColor: f.TVs.colors.TEXT_SECONDARY
  } : {
    defaultTextColor: "text-secondary"
  }, <L.Provider value={A({
      location: n
    }, t)}><div className={a()(T.badgesContainer, {
        [T.badgesContainerCard]: 0 === n,
        [T.badgesContainerPopout]: (0, h.Hi)(n, [1, 2]),
        [T.badgesContainerGameProfile]: 3 === n
      }, o)}>{i}</div></L.Provider>
}

function J(e) {
  switch (e.content_type) {
    case c.s.TOP_ARTIST:
      return w[e.content_type].map((t, n) => <t entry={e} />);
    case c.s.PLAYED_GAME:
      return w[e.content_type].map((t, n) => <t entry={e} />);
    case c.s.TOP_GAME:
      return w[e.content_type].map((t, n) => <t entry={e} />);
    case c.s.WATCHED_MEDIA:
      return w[e.content_type].map((t, n) => <t entry={e} />);
    case c.s.LAUNCHED_ACTIVITY:
      return w[e.content_type].map((t, n) => <t entry={e} />);
    default:
      return null
  }
}

function $(e) {
  let {
    entry: t,
    location: n,
    className: i
  } = e, o = J(t);
  return null == o ? null : <Q location={n} className={i}>{o}</Q>
}