...
function _(e) {
  ...
  return <c.j7V value={!!r} onChange={n} note={s}>{t}</c.j7V>
}
...
function j() {
  ...
  return Chunk120356 ? <Chunk921801.F setting={Chunk726985.s6.NOTIFICATIONS_EMAILS}>{<div ref={exports} />}{<Chunk481060.hjN className={Chunk20493.marginTop40} tag={Chunk481060.RB0.H1} title={Chunk388032.intl.string(Chunk388032.t.TPchzM)}>{Chunk930441.Od.map(e => {
        let {
          category: t,
          label: r,
          subLabel: s
        } = e, a = E[t], l = <_ label={r()} subLabel={s()} enabled={n[t]} onChange={e => {
            (0, m.pR)(t, e)
          }} />;
        return null != a ? <d.F setting={a}>{l}</d.F> : l
      })}{<Chunk921801.F setting={Chunk726985.s6.NOTIFICATIONS_EMAILS_UNSUBSCRIBE_FROM_ALL_MARKETING_EMAILS}>{<Chunk755721.zx className={Chunk495288.marketingUnsubscribeButton} disabled={!j} look={Chunk755721.iL.LINK} color={Chunk755721.Tt.LINK} onClick={Chunk960412.oc}>{Chunk388032.intl.string(Chunk388032.t.Ra9Pws)}</Chunk755721.zx>}{<Chunk481060.R94 type={Chunk481060.geA.DESCRIPTION}>{Chunk388032.intl.string(Chunk388032.t.iYjQ8f)}</Chunk481060.R94>}{<Chunk481060.$i$ className={a()(Chunk20493.marginTop40, Chunk20493.marginBottom40)} />}</Chunk921801.F>}</Chunk481060.hjN>}</Chunk921801.F> : null
}