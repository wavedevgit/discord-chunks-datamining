...
function P() {
  ...
  return <Chunk481060.ubH theme={module}>{<Chunk481060.oxh darkSrc={Chunk144452} lightSrc={Chunk144452} width={106} height={100} style={{
        marginBottom: 10
      }} />}{<Chunk481060.OZU note={Chunk388032.intl.string(Chunk388032.t.bwytdn)} noteClassName={Chunk869322.emptyText} className={Chunk869322.emptyTitle}>{Chunk388032.intl.string(Chunk388032.t.vhQK3t)}</Chunk481060.OZU>}</Chunk481060.ubH>
}
...
function L(e) {
  ...
  return null != i && "" !== i ? <img className={s} src={i} alt={n.name} /> : <span className={r}>{n.name}</span>
}
...
function E(e) {
  ...
  return <d.njP look={"custom"} orientation={"vertical"} aria-label={y.intl.string(y.t["qbir+/"])} selectedItem={a} onItemSelect={s}>{t.answers.map(e => {
      var t;
...
function U(e) {
  ...
  return <div className={M.voterDefault} onContextMenu={e => (0, b.Pv)(e, t, n)}>{<div className={M.voterAvatarContainer}><m.Z user={null != a ? a : t} size={d.EFr.SIZE_24} aria-hidden={true} guildId={n.guild_id} /></div>}{<d.Text tag={"span"} variant={"text-md/normal"} className={M.name} aria-label={s}>{<span className={t.hasUniqueUsername() ? "" : M.nickname}>{s}</span>}{<u.Z user={t} className={M.tagFaded} usernameClass={M.username} discriminatorClass={M.discriminator} forceUsername={true} />}</d.Text>}</div>
}
...
function V(e) {
  ...
  let v = a.useCallback(() => {
    ...
    p = a.useCallback(e => {
      ...
      return 1 === t ? 0 === n ? <d.$jN className={M.spinnerMore} /> : null : 0 === r.length && u ? <d.$jN className={M.spinner} /> : null != a && <U channel={s} user={a} />
    }, [s, u, r]),
    f = a.useMemo(() => {
      let e = [];
      return 0 === r.length && u ? e.push(0) : (e.push(r.length), i && e.push(1)), e
    }, [i, u, r.length]);
  return <div className={M.votersListContainer}><d.aVo className={M.voters} fade={true} ref={o} sections={f} sectionHeight={0} rowHeight={x} renderRow={p} onScroll={i ? v : true} /></div>
}
...
function O(e) {
  ...
  return <o.I transitionState={u} onClose={r} size={"lg"}>{<c.x title={n.poll.question.text} subtitle={y.intl.format(y.t.XRkuoa, {
        count: x.toLocaleString()
      })} />}{<d.w0Z className={M.body}><div className={M.content}>{<d.Ttm className={M.scroller} fade={true}><E poll={n.poll} selectedAnswerId={h} onSelectAnswer={_} reactionVoteCounts={j} /></d.Ttm>}{<d.njP.Panel id={h} aria-label={C} className={M.panel}>{null != v && f > 0 ? (0, l.jsx)(V, {
            message: n,
            reaction: v,
            channel: m
          }) : (0, l.jsx)(P, {})}</d.njP.Panel>}</div></d.w0Z>}</o.I>
}