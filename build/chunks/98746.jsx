...
function C(t) {
  ...
  return <s.Modal onClose={k} transitionState={S} title={G} subtitle={null != H ? H : true} actions={(() => {
      let t = [];
      return K || t.push({
        variant: "secondary",
      ...
      children: (0, l.jsx)(u.Text, {
        ...
        children: h.intl.string(j.default["8yIKen"])
    ...
    }) : true}><u.Kqy direction={"vertical"} gap={24}>{<u.Kqy direction={"vertical"} gap={8}>{<u.xJW titleClassName={x.title} title={(0, l.jsx)(u.Text, {
            variant: "text-md/medium",
            color: "text-primary",
            children: h.intl.string(null != Z ? h.t["+KCLVl"] : h.t.w4Ivys)
          })}>{R && <u.kzN className={x.error}>{h.intl.string(h.t.IrYX19)}</u.kzN>}{<u.FXm className={x.radioGroup} value={T} options={q.map(t => {
              var e, n;
              return e = function(t) {
            ...
            })} onChange={A} withTransparentBackground={true} />}</u.xJW>}{<div className={a()(x.reasonFreeText, {
            [x.hidden]: "other" !== T
          })}><u.Kx8 maxLength={m.GNZ} onChange={B} value={null != M ? M : ""} rows={5} autoFocus={true} /></div>}</u.Kqy>}{<u.xJW titleClassName={x.title} title={(0, l.jsx)(u.Text, {
          variant: "text-md/medium",
          color: "text-primary",
          children: h.intl.string(null != Z ? h.t.ERSM8P : h.t["8l3W09"])
        })}><u.q4e options={f()} value={w} onChange={X} /></u.xJW>}</u.Kqy></s.Modal>
}