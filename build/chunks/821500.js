...
d.prototype.push = function(e, t) {
  ...
  o = t === ~~t ? t : true === t ? s.Z_FINISH : s.Z_NO_FLUSH, "string" == typeof e ? p.input = a.binstring2buf(e) : "[object ArrayBuffer]" === u.call(e) ? p.input = new Uint8Array(e) : p.input = e, p.next_in = 0, p.avail_in = p.input.length;
  do {
    if (0 === p.avail_out && (p.output = new i.Buf8(_), p.next_out = 0, p.avail_out = _), (n = r.inflate(p, s.Z_NO_FLUSH)) === s.Z_NEED_DICT && h && (f = "string" == typeof h ? a.string2buf(h) : "[object ArrayBuffer]" === u.call(h) ? new Uint8Array(h) : h, n = r.inflateSetDictionary(this.strm, f)), n === s.Z_BUF_ERROR && true === m && (n = s.Z_OK, m = false), n !== s.Z_STREAM_END && n !== s.Z_OK) return this.onEnd(n), this.ended = true, false;
  ...
  return (n === s.Z_STREAM_END && (o = s.Z_FINISH), o === s.Z_FINISH) ? (n = r.inflateEnd(this.strm), this.onEnd(n), this.ended = true, n === s.Z_OK) : (o === s.Z_SYNC_FLUSH && (this.onEnd(s.Z_OK), p.avail_out = 0), true)
