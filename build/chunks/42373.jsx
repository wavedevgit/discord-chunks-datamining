...
let v = e => {
    ...
    return s ? <div className={E.bentoBoxButton}><o.z variant={"secondary"} onClick={g} text={t} /></div> : <u.zx look={u.iL.OUTLINED} color={m} className={a()(j.bentoBoxButton, c ? j.bentoBoxButtonWhite : j.bentoBoxButtonBrand)} onClick={g}>{t}</u.zx>
  },
  S = e => {
    let t, {
        name: n,
    ...
    let V = () => <div className={a()(U.textBox, U["".concat(P)])}>{<div>{null != A && <div className={U.badge}><m.X6q variant={"eyebrow"} color={"always-white"}>{A}</m.X6q></div>}{<m.X6q variant={t} color={"header-primary"} className={a()(U.header, !R && U["".concat(P)])}>{s}</m.X6q>}</div>}{<m.Text variant={"text-md/medium"} color={R ? "text-primary" : "text-muted"} className={U.description}>{o}</m.Text>}{null != d && null != y && <v descriptionCta={d} onClick={y} debouncedOnClickAnalytics={L} enablePremiumBrandRefreshDesign={R} />}</div>,
      G = () => <div className={a()(U.boxArtContainer, U["".concat(P)])}><g.Z playsInline={true} preload={S ? "auto" : "none"} muted={true} poster={u} loop={true} className={a()(U.boxVideo, {
            [I]: null != I
          })} ref={Z}><source src={p} type={D ? _.m.MP4 : _.m.WEBM} /></g.Z></div>,
      F = T % 2 != 0,
      H = () => F ? <i.Fragment>{<V />}{<G />}</i.Fragment> : <i.Fragment>{<G />}{<V />}</i.Fragment>,
      z = {
