...
function q(e) {
  ...
  if (t.isThread() || t.isForumPost()) {
    ...
    d = <div className={M.threadSubLabel}>{<e color={f.TVs.colors.TEXT_SECONDARY} className={M.subLabelIcon} />}{<f.Text variant={"text-xs/medium"} color={"text-secondary"} lineClamp={1}>{i}</f.Text>}{null != o ? <r.Fragment>{<f.Text className={M.subLabelSeparator} variant={"text-xs/medium"} color={"text-secondary"}>{"•"}</f.Text>}{<f.Text variant={"text-xs/medium"} color={"text-secondary"}>{(0, L.Xf)(u()(o))}</f.Text>}</r.Fragment> : null}</div>
  }
  return <U{...A(k({}, l), {
    icon: (0, r.jsx)(y.Z, {
      size: y.E.SMALL_32,
...
function z(e) {
  ...
  let u = l.useMemo(() => [t.length], [t.length]),
    ...
    b = l.useCallback(e => {
      ...
      return i === h.h8.USER ? <H{...k({
        user: u
      }, p)} /> : i === h.h8.GROUP_DM ? <F{...k({
        channel: u
      }, p)} /> : i === h.h8.TEXT_CHANNEL || i === h.h8.VOICE_CHANNEL ? <q{...k({
        channel: u
      }, p)} /> : void(0, T.vE)(i)
    }, [s, a, t, f]),
    m = l.useRef(null),
    g = (0, p.Z)("share-command-modal", m);
  return <o.bG navigator={g}><o.SJ>{e => {
        var {
          ref: t
        } = e, n = I(e, ["ref"]);
