...
function x(e, t) {
  return 0 === t.length ? null : <div className={E.row}>{<e className={E.activityIcon} color={"currentColor"} />}{<h.ZP guildId={true} users={t} max={6} />}</div>
}
...
function I() {
  let e = (0, Chunk442837.Wu)([Chunk853856.Z, Chunk592125.Z], () => Chunk709054.default.keys(Chunk853856.Z.getFavoriteChannels()).map(e => m.Z.getChannel(e)).filter(Chunk823379.lm)),
    ...
    Z = 0 === Chunk392711.length ? null : <div className={Chunk714509.row}>{<Chunk481060.ewx size={"lg"} color={"currentColor"} className={Chunk714509.activityIcon} />}{<Chunk884338.ZP guildId={true} users={Chunk392711} max={3} />}{<div className={Chunk714509.stageListenerPill}>{<Chunk481060.VWR size={"xs"} color={"currentColor"} />}{<Chunk481060.Text className={Chunk714509.stageListenerCount} color={"text-default"} variant={"text-xs/medium"}>{Chunk388032}</Chunk481060.Text>}</div>}</div>,
  ...
  return <Chunk255367.Fragment>{Z}{w}{T}{R}</Chunk255367.Fragment>
}

function P() {
  return <Chunk255367.Fragment>{<div className={l()(Chunk714509.row, Chunk714509.rowGuildName)}><span className={l()(Chunk714509.guildNameText, Chunk714509.guildNameTextLimitedSize)}>{Chunk388032.intl.string(Chunk388032.t.wMWycn)}</span></div>}{<I />}</Chunk255367.Fragment>
}
...
function N(e) {
  ...
  return <c.ua7 hideOnClick={true} spacing={12} position={"right"} text={(0, r.jsx)(P, {})} aria-label={t} tooltipClassName={S.listItemTooltip} onTooltipShow={n}>{e => {
      var t, n;
