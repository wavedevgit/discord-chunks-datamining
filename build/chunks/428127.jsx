...
function O(e) {
  ...
  return <c.P3F className={o()(m.bar, m.voiceBar)} onClick={h}>{<c.gj8 size={"custom"} className={m.voiceChannelsIcon} width={14} height={14} color={"currentColor"} />}{<c.Text variant={"text-xs/semibold"} className={m.barText}>{g.intl.format(g.t["fDlr+P"], {
        count: b.length
      })}</c.Text>}{<d.Z guildId={n.id} className={m.voiceChannelsUsers} users={b.slice(0, 4)} renderMoreUsers={() => null} max={4} size={c.EFr.SIZE_16} />}</c.P3F>
}
...
function _(e) {
  ...
  return <div className={o()(m.container, {
      [m.top]: "top" === t,
      [m.bottom]: "bottom" === t
    })}><s.animated.div className={m.containerPadding} style={S} aria-hidden={E}>{C === f.x.HIDDEN ? (0, r.jsx)("div", {
