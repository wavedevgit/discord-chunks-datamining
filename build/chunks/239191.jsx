...
function f(e) {
  ...
  return <c.Kqy direction={"horizontal"} gap={8}>{t.map((e, t) => {
      var o;
      let d = (0, i.jsx)(c.Kqy, {
        gap: 16,
...
function w(e) {
  ...
  return <c.Y0X transitionState={t} size={c.CgR.DYNAMIC} className={O.root} data-modal-align={"top"} parentComponent={"DesktopRefreshOnboardingModal"}>{<div className={O.gradient} />}{<c.xBx className={O.header}><c.Kqy gap={K ? 4 : 8}>{<c.Kqy gap={K ? 8 : 16}>{<div className={O.headerImage} />}{<c.X6q variant={K ? "heading-lg/semibold" : "heading-xl/semibold"} color={"header-primary"}>{j.intl.string(j.t["7+iCGx"])}</c.X6q>}</c.Kqy>}{<c.X6q variant={K ? "heading-sm/medium" : "heading-md/medium"} color={"text-secondary"}>{j.intl.string(j.t.fZMMqK)}</c.X6q>}</c.Kqy></c.xBx>}{<c.hzk className={O.content}><c.Kqy gap={K ? 20 : 24}>{<c.Kqy gap={K ? 8 : 12}>{<c.X6q variant={"heading-md/medium"}>{j.intl.string(j.t.Ksh3io)}</c.X6q>}{<f showLabels={false} options={P} selected={N} onSelect={e => {
            ...
            })} />}</c.Kqy>}{<c.Kqy gap={K ? 8 : 12}>{<c.Kqy gap={0}>{<c.Kqy direction={"horizontal"} gap={4} align={"center"}>{<c.X6q variant={"heading-md/medium"}>{j.intl.string(j.t["C/5V0N"])}</c.X6q>}{<c.IGR color={c.TVs.colors.BG_BRAND.css} text={j.intl.string(j.t.y2b7CA)} />}</c.Kqy>}{<c.Text variant={"text-sm/medium"} color={"text-secondary"}>{j.intl.string(j.t["2w1sEB"])}</c.Text>}</c.Kqy>}{<f showLabels={true} options={_} selected={C} onSelect={e => {
              switch (e) {
                case "default":
            ...
            }} />}</c.Kqy>}</c.Kqy></c.hzk>}{<c.mzw className={O.footer}><c.zxk variant={"primary"} text={j.intl.string(j.t["1Qm829"])} onClick={n} /></c.mzw>}</c.Y0X>
}