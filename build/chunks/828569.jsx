...
let N = e => {
    ...
    return <t.Fragment>{<x.Z icon={a.SlE} iconClassName={E.iconColorPurple} description={S.intl.string(S.t.uAfKTU)} />}{<x.Z icon={a.$zw} iconClassName={E.iconColorPink} description={S.intl.formatToPlainString(S.t.sWnv5O, {
          numGuildSubscriptions: w.cb,
          discountPercent: (0, k.T3)(r, w.Rr / 100)
        })} />}{<x.Z icon={a.EO4} iconClassName={E.iconColorYellow} description={S.intl.string(S.t.pqHIf3)} />}</t.Fragment>
  },
  T = e => {
    let {
      trialOffer: r,
    ...
    return <div className={E.wrapper}>{<a.olH data-migration-pending={true} onClick={p} className={E.closeButton} />}{d && <h.dz className={E.premiumTrialBadge} />}{<img className={c()(E.heroImage, {
          [E.heroImageWithTrialOffer]: d
        })} src={n(178632)} alt={""} />}{i ? <a.$jN /> : <t.Fragment>{<a.Text variant={"text-md/normal"} className={E.heading}>{null == s ? (0, t.jsx)(a.$jN, {
            type: a.$jN.Type.PULSING_ELLIPSIS
          }) : S.intl.format(S.t.TBsJfX, {
            monthlyPrice: (0, I.T4)(s.amount, s.currency)
          })}</a.Text>}{<div className={E.perks}><N locale={u} /></div>}</t.Fragment>}</div>
  },
  L = e => {
    let {
      trialOffer: r,
    ...
    return <a.mzw data-migration-pending={true} className={E.footer}>{<a.zxk variant={"secondary"} size={"sm"} text={S.intl.string(S.t.cpT0Cg)} onClick={o} />}{<O.Z textOptions={{
          textOverride: null != r || null != n ? S.intl.string(S.t["Gd/XHB"]) : true
        }} premiumModalAnalyticsLocation={i} subscriptionTier={w.Si.TIER_2} size={s.zx.Sizes.SMALL} color={s.zx.Colors.GREEN} onClick={() => {
          o()
        }} />}</a.mzw>
  };
...
function R(e) {
  ...
  return <m.PaymentContextProvider activeSubscription={null} stepConfigs={[]} skuIDs={[]}><a.Y0X{...r = function(e) {
