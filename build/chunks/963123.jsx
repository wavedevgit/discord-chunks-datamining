...
function W() {
  ...
  return <div className={Chunk844136.pauseText} style={{
      background: (0, Chunk780384.wj)(module) ? "rgba(0, 0, 0, 0.65)" : "rgba(255, 255, 255, 0.65)"
    }}><Chunk481060.Text variant={"text-sm/normal"} color={"text-default"}>{Chunk388032.intl.string(Chunk388032.t.U7Xrb2)}</Chunk481060.Text></div>
}
...
function X(e) {
  ...
  return null == a ? null : <k.A questOrQuests={t} questContent={y.jn.VIDEO_MODAL_END_CARD} sourceQuestContent={o}>{e => (0, r.jsx)("img", {
      ref: t => {
        e.current = t
      },
      src: a.url,
...
function $(e) {
  ...
  return <f.P3F className={i()(z.endScreenPanel, z.accentOnHover, s)} onMouseEnter={d} onMouseLeave={m} onFocus={d} onBlur={m} onClick={a}><div className={z.endScreenPanelInner}>{<div className={z.endScreenPanelTextCont}>{<f.X6q variant={"heading-md/semibold"} className={z.endScreenPanelTextTitle}>{t}</f.X6q>}{<f.X6q variant={"heading-sm/normal"} className={z.endScreenPanelTextSubtitle}>{n}</f.X6q>}</div>}{<Z.Z color={"#747783"} />}{<o size={"md"} color={c ? "#FFFFFF" : "#B5BAC1"} className={z.endScreenIcon} />}</div></f.P3F>
}
...
function J(e) {
  ...
  let {
    ...
    visibleChanged: eO,
    targetRef: eb
  } = (0, F.Yy)(), [eC, eS] = l.useState(true === er ? V.rq.PLAYING : V.rq.PAUSED), [eh, e_] = l.useState(false), [ej, ey] = l.useState(false), ex = (0, _.il)(J), [eT, eD] = l.useState(ex.percentComplete), [eP, eR] = l.useState(false), [eN, eI] = l.useState(true), [ek, eA] = l.useState(false), [ew, eL] = l.useState([]), [eM, eV] = l.useState(b.Z.getEffectiveConnectionSpeed()), [eB, eZ] = l.useState(0), [eF, eU] = l.useState(0), [eq, eY] = l.useState(false), [eH, eQ] = l.useState(false), ez = l.useRef(true), eG = l.useRef(null), eK = l.useRef(null), eW = (0, x.km)(e => {
    var t;
    return null != (t = e.videoProgress[J.id]) ? t : {
      timestampSec: 0,
      duration: 10
    }
  }, c.X), eX = (0, x.km)(e => e.setVideoProgress), e$ = (0, x.km)(e => e.muted), eJ = (0, x.km)(e => e.volume), e0 = (0, m.e7)([E.Z], () => E.Z.useReducedMotion), e1 = (0, l.useRef)(null), e2 = (0, l.useRef)(null), e6 = l.useRef(true), e8 = (null == (t = J.userStatus) ? true : t.completedAt) != null, e4 = (0, x.km)(e => e.transcript), [e7, e3] = l.useState(null), [e9, e5] = l.useState(false), [te, tt] = l.useState(false), [tn, tr] = l.useState(null), tl = e8 ? null != (O = null == (n = e1.current) ? true : n.duration) ? O : 0 : Math.max(eW.maxTimestampSec, ex.progressSeconds), to = l.useMemo(() => (0, D.T)({
    quest: J,
  ...
  }(J), ts = e8 ? eW.timestampSec >= eW.duration ? 0 : eW.timestampSec : Math.max(eW.timestampSec, ex.progressSeconds);
  (0, v.Ng)(() => {
    e6.current && (e6.current = false, eV(b.Z.getEffectiveConnectionSpeed()), e8 && eW.timestampSec >= eW.duration && eX(J.id, 0, eW.duration))
  });
  let {
    videoVariant: tc,
    videoAsset: tu,
  ...
  let tR = l.useCallback(e => {
    var t;
    e8 || (null == (t = J.userStatus) ? true : t.enrolledAt) == null || (to.info("[QV] sendServerProgressUpdate: ".concat(e)), (0, _.FI)(J, e)), tv()
  }, [e8, J, to, tv]);
  l.useEffect(() => {
    el !== f.Dvm.HIDDEN && el !== f.Dvm.EXITING && el !== f.Dvm.EXITED && (null == el || !eO || eg || e8) && (!eE || ev || e8) || null == e1.current || eC !== V.rq.PLAYING || (to.info("[QV] | Pausing video | playerState: ".concat(eC, ", parentTransitionState: ").concat(el, ", visible: ").concat(eg, ", focused: ").concat(ev, ", isQuestCompleted: ").concat(e8)), tP(V.rq.PAUSED), e8 || e3(y.yE.LOST_FOCUS))
  }, [el, ev, eE, eg, eO, eC, e8, tP, tR, to]);
  ...
  let tM = !tN && (ej || eh || eC === V.rq.ENDED),
    ...
    tB = () => {
      ...
      to.info("[QV] | handleSeekBackIncrement | newTime: ".concat(e)), tF(e), eC === V.rq.ENDED && tP(V.rq.PAUSED), tb(y.jn.VIDEO_MODAL, h.jZ.SEEK_BACKWARD)
    },
    tZ = () => {
      if (null == e1.current || !t6) return;
      let e = Math.min(e1.current.currentTime + 10, tl);
      to.info("[QV] | handleSeekForwardIncrement | newTime: ".concat(e)), tF(e), eC !== V.rq.ENDED && e >= e1.current.duration && tP(V.rq.ENDED), tb(y.jn.VIDEO_MODAL, h.jZ.SEEK_FORWARD)
    };
  l.useEffect(() => {
    let e = e1.current;
    return () => {
  ...
  let tF = l.useCallback(e => {
      null != e1.current && (to.info("[QV] | seekTimeline | timeSec: ".concat(e)), tD(), eR(true), tt(false), e1.current.currentTime = e, eX(J.id, e, e1.current.duration))
    }, [e1, eX, J.id, tD, to]),
    tU = () => {
  ...
  let tJ = eC === V.rq.ENDED,
    ...
    t6 = e8 || eW.maxTimestampSec >= (null != (k = null == (o = e1.current) ? true : o.currentTime) ? k : 0) + 1,
    t8 = l.useMemo(() => null === (0, N.z0)(J, u.X.WATCH_VIDEO, N.n1.VIDEO, N.O.TRANSCRIPT), [J]);
  return <f.P3F className={z.videoCont} data-fullscreen={ep} tabIndex={false} onMouseEnter={() => {
      e_(true), tI(false), tA.current = performance.now(), null != tk.current && clearTimeout(tk.current)
    }} onMouseLeave={() => {
      e_(false), tI(false)
    }} onMouseMove={tL} onKeyDown={tL}><div className={i()(z.videoContInnerRelative, {
        [z.videoContInnerRelativePortrait]: "portrait" === es,
        [z.videoContInnerRelativeLandscape]: "landscape" === es
      })}>{tJ && (() => {
        if (!(0, P.e)({
            location: q.dr.VIDEO_MODAL
          })) return <r.Fragment>{<X quest={J} shouldShow={tJ} sourceQuestContent={ea} />}{<div className={z.videoContOverlay} />}{<$ title={ti} subtitle={ta} icon={f.d4D} className={z.endScreenPanelRight} onClick={() => t0(y.jn.VIDEO_MODAL)} />}</r.Fragment>;
        {
          var e, t, n;
          let l = J.config.taskConfigV2.tasks[u.X.WATCH_VIDEO];
          return <w.Z quest={J} title={null != (e = null == l ? true : l.messages.videoEndCtaTitle) ? e : Q.intl.string(Q.t.Ka526u)} subtitle={null != (t = null == l ? true : l.messages.videoEndCtaSubtitle) ? t : Q.intl.string(Q.t.tKMcAg)} ctaBtnLabel={null != (n = null == l ? true : l.messages.videoEndCtaButtonLabel) ? n : Q.intl.string(Q.t.iiTtpK)} onCTAClick={() => t0(y.jn.VIDEO_MODAL_END_CARD)} sourceQuestContent={ea} />
        }
      })()}{<g.Z ref={e => {
          e1.current = e, eb.current = e
        }} autoPlay={er} playsInline={true} mediaLayoutType={ep ? H.hV.STATIC : H.hV.RESPONSIVE} className={i()({
          [z.hidden]: tJ,
          [z.videoInner]: true
        })} controls={false} poster={null == t1 ? true : t1.url} disablePictureInPicture={true} onTimeUpdate={e => {
          null != e1.current && (e1.current.currentTime >= eB && (to.info("[QV] | handleTimeUpdate: capturing server time update: currentTime: ".concat(e1.current.currentTime)), eZ(e1.current.currentTime + 6 + 2 * Math.random()), tR(e1.current.currentTime)), e1.current.currentTime >= eF && (eU(e1.current.currentTime + 1), (0, j.qm)(J.id, ex.taskType, e1.current.currentTime), en(e1.current.currentTime)), eX(J.id, e1.current.currentTime, e1.current.duration), eD(e1.current.currentTime / e1.current.duration * 100))
        }} onEnded={e => {
          null != e1.current && (tR(e1.current.duration + 1), to.info("[QV] | handleEnded: sending progress update: ".concat(e1.current.duration + 1)), eX(J.id, e1.current.duration, e1.current.duration)), tP(V.rq.ENDED), eA(false)
        }} onLoadedData={e => {
          ...
          eL(t)
        }} onCanPlay={tz} onCanPlayThrough={tz} onSeeked={() => {
          to.info("[QV] | handleSeeked"), tt(true)
        }} onAbort={() => tQ(V.CY.ABORT)} onError={() => tQ(V.CY.ERROR)} onEmptied={() => tQ(V.CY.EMPTIED)} onStalled={() => tQ(V.CY.STALLED)} onClick={() => {
          to.info("[QV] | handleVideoClick"), tU()
        }} crossOrigin={"anonymous"}>{null != t2 && <track ref={e2} src={t2.url} label={"English"} kind={"captions"} srcLang={"en"} default={true} />}{null != tu && tc !== N.n1.VIDEO_HLS && <source onError={() => tQ(V.CY.SOURCE_ERROR)} src={tu.url} type={tu.mimetype} />}</g.Z>}{(eN || ek) && eC === V.rq.PLAYING && <f.$jN type={f.$jN.Type.WANDERING_CUBES} className={z.loadingSpinner} />}{eC === V.rq.PAUSED && e7 === y.yE.LOST_FOCUS && <W />}{ec && eC !== V.rq.ENDED && <r.Fragment>{<f.P3F onClick={() => eu(false)} tabIndex={false}><div className={z.transcriptOverlay} /></f.P3F>}{<s.animated.div className={i()(z.transcriptCont, {
            ...
            }} /></s.animated.div>}</r.Fragment>}{<s.animated.div className={z.videoFooterContGradient} style={{
          opacity: (0, s.to)([tG.to({
            range: [0, 1],
            output: [0, 1]
          })], e => "".concat(e))
        }} />}{<div className={i()(z.playPausePopCont, {
          [z.play]: eC === V.rq.PLAYING,
          [z.pause]: eC === V.rq.PAUSED
        })}>{eC === V.rq.PLAYING ? (0, r.jsx)(f.o1U, {
          className: z.playPausePopIcon
        }) : (0, r.jsx)(f.fpf, {
          className: z.playPausePopIcon
        })}</div>}{ed && null != tn && <s.animated.div className={z.captionContainer} ref={tW} style={{
          translateY: (0, s.to)([tG.to({
            range: [0, 1],
            output: [0, false]
          })], e => "".concat(e, "px"))
        }}><f.Text variant={"text-lg/semibold"} color={"always-white"} className={z.captionText}>{tn.text}</f.Text></s.animated.div>}{<s.animated.div className={z.videoFooterCont} style={{
          ...
          height: (0, s.to)([tG.to({
        ...
        }}>{<L.Z percent={eT} animate={true !== e6.current && !eP} interactionEnabled={e8 && eH} backgroundColor={tM ? true : "rgba(0, 0, 0, 0.0)"} preloadedBuffers={tM ? ew : true} duration={null != (Z = null == (p = e1.current) ? true : p.duration) ? Z : 1} maxSeekableTime={tM && eH ? tl : true} onClick={e => {
            tF(e), eC === V.rq.ENDED && tP(V.rq.PLAYING)
          }} onScrubBack={() => {
            tB()
          }} onScrubForward={() => {
            tZ()
          }} />}{<s.animated.div className={z.videoControlsCont} style={{
          ...
          }} onFocus={() => ey(true)} onBlur={() => ey(false)}><A.Z videoRef={e1} videoTask={ee} playerState={eC} animSpring={tG} visible={tM} seekForwardEnabled={t6} hideCaptionBtn={null == t2} hideTranscriptBtn={t8} handlePlaybackBtnClick={tU} handleTranscriptBtnClick={() => {
              ec || null != e4 && e4.questId === J.id && e4.fetchStatus !== x.iF.NONE && e4.fetchStatus !== x.iF.FAILURE || (0, j.lL)(J, u.X.WATCH_VIDEO), eu(!ec), tb(y.jn.VIDEO_MODAL, ec ? h.jZ.TRANSCRIPT_DISABLE : h.jZ.TRANSCRIPT_ENABLE)
            ...
            }} handleSeekBackBtnClick={tB} handleSeekForwardBtnClick={tZ} handleControlBarPendingInteraction={eY} onVolumeChange={tx} /></s.animated.div>}</s.animated.div>}</div></f.P3F>
}