...
function S(e, t) {
  ...
  let U = () => {
    ...
    }, Z = <o.sNh id={"automod-rules-loading"} label={g.intl.string(g.t.ZTNur6)} />;
  return R || (Z = <r.Fragment>{x && <o.sNh id={"add-first-rule"} label={g.intl.string(g.t.f72Zqa)} action={G} disabled={b} />}{L.map(e => {
      ...
      return <o.k5B id={e.id} label={e.name} subtext={(0, r.jsx)(o.Text, {
          color: "text-muted",
          className: E.actionTextHeader,
          variant: "text-xs/normal",
          children: t.slice(2)
        })} group={"automod-rule-selection"} checked={false} disabled={b} action={() => B(e)} />
    })}{M && <r.Fragment>{<o.Clw />}{<o.sNh id={"add-another-rule"} label={g.intl.string(g.t["0K5jDA"])} action={G} disabled={b} />}</r.Fragment>}</r.Fragment>), <o.sNh id={"guild-automod-add-selection"} label={g.intl.formatToPlainString(g.t.Kkjv1t, {
      keywordCount: j
    })} onFocus={U}>{Z}</o.sNh>
}