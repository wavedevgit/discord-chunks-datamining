    ...
    [f.Etm.SET_ACTIVITY]: {
      ...
      validation: e => (0, p.Z)(e).required().keys({
        ...
        activity: (0, p.Z)(e).keys({
          ...
          buttons: e.array().items((0, p.Z)(e).keys({
      ...
      handler(e) {
        ...
        if (![h.He.IPC, h.He.WEBSOCKET, h.He.POST_MESSAGE].includes(r.transport)) throw new d.Z({
          errorCode: f.lTL.INVALID_COMMAND
        }, 'command not available from "'.concat(r.transport, '" transport'));
        if (null == l && h.He.IPC === r.transport) throw new d.Z({
          errorCode: f.lTL.INVALID_COMMAND
        }, "nonzero pid required");
        if (null == p) return a.Z.dispatch({
        ...
        if (null == S && (p.type = f.IIU.PLAYING), null != y) {
          let e = i().values(y).filter(e => !!e);
          if (null != I && i().intersection(e, [I.id]).length > 0 && !g.includes(r.application.id)) throw new d.Z({
            errorCode: f.lTL.INVALID_ACTIVITY_SECRET
          }, "secrets cannot match the party id");
          if (i().uniq(e).length < e.length) throw new d.Z({
            errorCode: f.lTL.INVALID_ACTIVITY_SECRET
          }, "secrets must be unique");
          if (null != C) throw new d.Z({
            errorCode: f.lTL.INVALID_ACTIVITY_SECRET
          }, "secrets cannot currently be sent with buttons")
        }
        if (null != C && (b.button_urls = C.map(e => e.url), p.buttons = C.map(e => e.label)), p.metadata = b, null != v)
          for (let e of Object.keys(v)) Date.now().toString().length - v[e].toString().length > 2 && (v[e] = Math.floor(v[e] * u.Z.Millis.SECOND));
