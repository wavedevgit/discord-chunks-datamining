  ...
  y = {
    [_.Etm.SET_ACTIVITY]: {
      ...
      handler(e) {
        ...
        if (![b.He.IPC, b.He.WEBSOCKET, b.He.POST_MESSAGE].includes(C.transport)) throw new g.Z({
          errorCode: _.lTL.INVALID_COMMAND
        }, 'command not available from "'.concat(C.transport, '" transport'));
        if (null == S && b.He.IPC === C.transport) throw new g.Z({
          errorCode: _.lTL.INVALID_COMMAND
        }, "nonzero pid required");
        if (null == N) return a.Z.dispatch({
        ...
        if (null != Z && (0, o.Kb)(Z) && A) {
          ...
          if ((null == e ? void 0 : e.applicationId) === Z.id) {
            ...
            null != t && (N.secrets = E({
              join: null != (l = null == (r = N.secrets) ? void 0 : r.join) ? l : crypto.randomUUID()
            }, null != (m = N.secrets) ? m : {}), P.embedded_activity_instance_id = t, x |= (0, c.Z)({
              flags: x,
              embeddedActivity: e
            })), N.party = E({
        ...
        if (null == U && (N.type = _.IIU.PLAYING), null != D) {
          ...
          if (null != R && i().intersection(e, [R.id]).length > 0 && !O.includes(C.application.id)) throw new g.Z({
        ...
        if (null != M && (P.button_urls = M.map(e => e.url), N.buttons = M.map(e => e.label)), N.metadata = P, null != k)
