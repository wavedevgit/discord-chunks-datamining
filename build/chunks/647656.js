  ...
  N = {
    [_.Etm.SET_ACTIVITY]: {
      ...
      handler(e) {
        ...
        if (![b.He.IPC, b.He.WEBSOCKET, b.He.POST_MESSAGE].includes(v.transport)) throw new g.Z({
          errorCode: _.lTL.INVALID_COMMAND
        }, 'command not available from "'.concat(v.transport, '" transport'));
        if (null == C && b.He.IPC === v.transport) throw new g.Z({
          errorCode: _.lTL.INVALID_COMMAND
        ...
        if (null != Z && (0, o.Kb)(Z) && A) {
          ...
          if ((null == e ? void 0 : e.applicationId) === Z.id) {
            ...
            null != t && (S.secrets = E({
              join: null != (l = null == (r = S.secrets) ? void 0 : r.join) ? l : crypto.randomUUID()
            }, null != (m = S.secrets) ? m : {}), P.embedded_activity_instance_id = t, x |= (0, c.Z)({
              flags: x,
              embeddedActivity: e
            })), S.party = E({
        ...
        let w = (0, u.S5)(S, A);
        w > 0 && (S.flags = w | x), delete S.instance, null == (t = S.party) || delete t.privacy;
        let {
          assets: L,
          party: R,
          secrets: D,
        ...
        if (null != M && (P.button_urls = M.map(e => e.url), S.buttons = M.map(e => e.label)), S.metadata = P, null != k)
          for (let e of Object.keys(k)) Date.now().toString().length - k[e].toString().length > 2 && (k[e] = Math.floor(k[e] * f.Z.Millis.SECOND));
        if (null == L) I = Promise.resolve([]);
        else {
          if (null == v.application || null == v.application.id) throw Error();
          I = (0, h.hR)(v.application.id, [L.large_image, L.small_image])
        }
        return I.then(e => {
          var t, n;
          let [r, i] = e;
          ...
          return null != l && (s.has_match_secret = !!l.match, s.has_join_secret = !!l.join), null != L && (s.has_images = !!(L.large_image || L.small_image)), null != o && (s.party_max = null != o.size && o.size[1] > 0 ? o.size[1] : void 0, s.party_id = o.id), p.default.track(_.rMx.ACTIVITY_UPDATED, s), S
        })
      }
    }
  }