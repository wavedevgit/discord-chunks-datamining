...
let d = e => Object.values(e).sort((e, t) => e.order - t.order),
  u = e => {
    ...
    return <s.Z className={c.navBar} transparent={true}><div className={c.navBarContent}>{<div className={c.navBarSectionContentContainer}>{<r.SrA className={c.nitroWheelIcon} colorClass={c.nitroWheelIconColor} />}{<div className={c.navBarSectionContent}>{m.map(e => {
              let t = n === e.id,
                s = u[e.id];
            ...
            })}</div>}</div>}{<a.Z size={"sm"} variant={"overlay-secondary"} />}</div></s.Z>
  }