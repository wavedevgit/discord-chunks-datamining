...
let N = 40,
  C = e => {
    ...
    let S = (0, i.e7)([h.Z], () => null != t.roleId ? h.Z.getRole(n.id, t.roleId) : true),
      C = (0, f.Z)(S),
      R = C ? O.intl.formatToPlainString(O.t.t0928f, {
        name: I
      ...
      L = C ? D : w;
    return <o.VqE><E.W_><div className={v.popoutContainer}>{<div className={v.mainContent}>{<div className={v.roleIconContainer}><c.Z{...A(T({}, P), {
                enableTooltip: false,
                className: v.__invalid_roleIcon,
                enableHeight: false
              })} /></div>}{<div className={v.truncatingText}>{<o.X6q variant={"heading-md/semibold"} className={v.roleName}><l.Z>{I}</l.Z></o.X6q>}{<o.Text variant={"text-sm/normal"}>{R}</o.Text>}</div>}</div>}{<o.zxk size={"sm"} variant={"secondary"} text={O.intl.string(O.t["H930+P"])} onClick={L} icon={o.$Eu} fullWidth={true} autoFocus={true} />}</div></E.W_></o.VqE>
  }