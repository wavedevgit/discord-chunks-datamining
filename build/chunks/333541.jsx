...
function C(e) {
  ...
  return <u.yRy targetElementRef={d} renderPopout={e => {
      var t;
      let {
        closePopout: n
      } = e;
    ...
    }} position={"left"} animation={u.yRy.Animation.NONE} align={"top"}>{(e, n) => {
      var o, i;
      let {
        isShown: r
      } = n;
...
function S(e) {
  ...
  return <r.Modal onClose={a} actions={k} transitionState={i} title={O.intl.string(O.t.DD4vJC)} subtitle={O.intl.string(O.t.YVM22t)}><div className={h.form}>{<C customStatusEmoji={P} setCustomStatusEmoji={w} onClose={a} />}{<div className={h.customInputGroup}>{<u.Text variant={"eyebrow"} color={"text-muted"}>{O.intl.string(O.t["74vS//"])}</u.Text>}{<u.oil value={S} onChange={e => x(e.substring(0, v.s))} placeholder={O.intl.string(O.t.k6lfDg)} />}</div>}</div></r.Modal>
}