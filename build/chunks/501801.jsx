...
function b(e) {
  ...
  return null == n ? null : <d.Text variant={"text-md/normal"}>{n.name}</d.Text>
}

function L(e, t) {
  return <div className={C.colorHook} style={{
      backgroundColor: t
    }} />
}
class x extends Chunk73800.PureComponent {
  render() {
    ...
    return <div className={Chunk392711} />
  }
}
...
function U(e) {
  let {
      ...
      A = o().flatten(t.changes.map(s => {
        ...
        let {
          ...
          if (t.action === R.rsA.MEMBER_ROLE_UPDATE) return function(e) {
            ...
            return Array.isArray(t) ? t.map(e => <div className={C.subListItem}>{e.name}</div>) : null
          }(s);
          if (t.targetType === R.KFR.ROLE || t.action === R.rsA.CHANNEL_OVERWRITE_CREATE || t.action === R.rsA.CHANNEL_OVERWRITE_UPDATE) return function(e, t) {
            ...
            return Array.isArray(n) ? n.map((e, n) => <div className={C.subListItem}>{p.hQ(e, t)}</div>) : null
          }(s, t);
          if ((t.action === R.rsA.CHANNEL_UPDATE || t.action === R.rsA.THREAD_UPDATE) && s.key === R.zUn.FLAGS) return function(e) {
            ...
            return (Array.isArray(n) && (l = l.concat(n.map(e => <div className={C.subListItem}>{p.cT(e)}</div>))), Array.isArray(t) && (l = l.concat(t.map(e => <div className={C.subListItem}>{p.em(e)}</div>))), l.length > 0) ? l : null
          }(s);
          if ((t.action === R.rsA.ONBOARDING_PROMPT_UPDATE || t.action === R.rsA.ONBOARDING_PROMPT_CREATE) && s.key === R.zUn.OPTIONS) return function(e, t, n) {
            let {
              ...
              return (o().isEqual(e.title, n.title) || g.push("title"), o().isEqual(e.description, n.description) || g.push("description"), 0 === A.length && 0 === T.length && 0 === m.length && 0 === I.length && 0 === g.length) ? null : <li>{<div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.UPDATE} targetType={t.targetType} action={t.action} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t["WZO+iY"], {
                      title: l
                    })}</d.Text>}</div>}{<div className={C.onboardingChangeLogItemChanges}>{A.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t["R+km+f"], {
                      roles: A.join(", ")
                    })}</d.Text>}{T.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t.xIVRoa, {
                      roles: T.join(", ")
                    })}</d.Text>}{m.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t.iYowX1, {
                      channels: m.join(", ")
                    })}</d.Text>}{I.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t.JLdJpK, {
                      channels: I.join(", ")
                    })}</d.Text>}{g.map((t, l) => <d.Text variant={"text-md/normal"}>{S.intl.format(S.t["0698zc"], {
                      key: t,
                      oldValue: e[t],
                      newValue: n[t]
                    })}</d.Text>)}</div>}</li>
            ...
            return <ul className={C.onboardingChangeLogContainer}>{_.map(e => {
                let {
                  role_ids: n,
                  channel_ids: l,
                  title: s,
                ...
                return <li>{<div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.CREATE} targetType={t.targetType} action={t.action} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t.CZ1tys, {
                        title: s
                      })}</d.Text>}</div>}{<div className={C.onboardingChangeLogItemChanges}>{u.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t["gt/5z8"], {
                        roles: u.join(", ")
                      })}</d.Text>}{c.length > 0 && <d.Text variant={"text-md/normal"}>{S.intl.format(S.t["4q006e"], {
                        channels: c.join(", ")
                      })}</d.Text>}</div>}</li>
              })}{A.map(e => <li><div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.DELETE} targetType={t.targetType} action={t.action} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t["m3dt+P"], {
                      title: e.title
                    })}</d.Text>}</div></li>)}{T.map(e => u(c[e], E[e]))}</ul>
          }(s, t, n);
            ...
            switch (s) {
              case R.zUn.WELCOME_MESSAGE:
                return ((e, t) => {
                  ...
                  let o = g.default.getUser(null == t || null == (n = t.author_ids) ? true : n[0]),
                    ...
                    c = (null == o ? true : o.id) !== (null == u ? true : u.id) ? <li><div className={C.onboardingChangeLogItemTitle}><d.Text variant={"text-md/normal"}>{S.intl.format(S.t.PrOzAw, {
                    ...
                    E = (null == t ? true : t.message) !== e.message ? <li><div className={C.onboardingChangeLogItemTitle}><d.Text variant={"text-md/normal"}>{S.intl.format(S.t.oB7rBQ, {
              ...
              case R.zUn.NEW_MEMBER_ACTIONS:
                return ((e, t) => {
                  ...
                  return <ul>{c.map(e => <li><div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.CREATE} targetType={i} action={a} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t["oFSu6+"], {
                            name: e.title
                          })}</d.Text>}</div></li>)}{E.map(e => <li><div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.DELETE} targetType={i} action={a} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t.GssWam, {
                            name: e.title
                          })}</d.Text>}</div></li>)}</ul>
                })(null != n ? n : [], l);
              case R.zUn.RESOURCE_CHANNELS:
                return ((e, t) => {
                  let n = e.map(e => e.channel_id),
                    ...
                    c = t.filter(e => s.includes(e.channel_id)),
                    E = e.filter(e => u.includes(e.channel_id));
                  return <ul>{c.map(e => <li><div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.CREATE} targetType={i} action={a} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t.HlGndH, {
                            name: e.title
                          })}</d.Text>}</div></li>)}{E.map(e => <li><div className={C.onboardingChangeLogItemTitle}>{<x actionType={R.vB8.DELETE} targetType={i} action={a} />}{<d.Text variant={"text-md/normal"}>{S.intl.format(S.t["7pysg4"], {
                            name: e.title
                          })}</d.Text>}</div></li>)}</ul>
                })(null != n ? n : [], l);
              default:
        ...
        return function(e) {
          ...
          let m = S.intl.format(s, {
            ...
            newColorsHook: (e, t) => {
              ...
              return <span>{[n, s, i].filter(N.lm).map((e, n) => (0, r.jsxs)(l.Fragment, {
            ...
            oldTagHook: (e, t) => <T.Z tag={u} size={T.Z.Sizes.SMALL} />,
            newTagHook: (e, t) => <T.Z tag={c} size={T.Z.Sizes.SMALL} />,
            oldEmojiHook: (e, t) => <_.Z emojiId={u} />,
            newEmojiHook: (e, t) => <_.Z emojiId={c} />,
            applicationHook: (e, t) => {
              var n;
              return <b applicationId={null != (n = null == u ? true : u.application_id) ? n : null == c ? true : c.application_id} />
            }
          });
          return null == m ? null : <div className={C.detail} style={{
              position: "relative",
              top: 1
            }}>{<d.Text className={C.prefix} variant={"code"} color={function(e) {
                switch (e) {
                  case R.vB8.CREATE:
              ...
              }(o.actionType)}>{a < 10 ? "0".concat(a) : a}{<span className={C.dash}>{"—"}</span>}</d.Text>}{<div className={C.change}>{<div className={C.changeStr}>{m}</div>}{null != i ? <div>{i}</div> : null}</div>}</div>
        }({
    ...
    return <d.P3F onClick={s} className={i()(C.changeDetails, a)}>{A}</d.P3F>
  }, [t, n, s, a]);
  return l.useMemo(() => {
    switch (t.targetType) {
      case R.KFR.GUILD:
