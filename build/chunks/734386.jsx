...
function G(e) {
  ...
  function H() {
    if (1 !== F.length || Z.length > 0) return <R.Z guildId={t.guild_id} className={D.avatars} maxUsers={U} users={F} />;
    let e = F[0],
      n = A.ZP.getName(e);
    return <div className={D.avatars}>{<d.qEK src={e.getAvatarURL(t.guild_id, 24)} aria-label={n} size={d.EFr.SIZE_24} />}{<d.Text tag={"span"} className={D.singleUserName} variant={"text-md/normal"}>{n}</d.Text>}{"\xa0"}{<d.Text tag={"span"} variant={"text-md/normal"} color={"text-muted"}>{w.intl.string(w.t.rt0ERU)}</d.Text>}</div>
  }
  ...
  function Y() {
    return Z.map((e, n) => {
      ...
      return V ? <m.Z className={a()(D.role, {
          [D.last]: n === Z.length - 1
        })} roleName={e.name} roleColor={s} disabled={!V} verified={l} onClick={() => {
          E.Z.open(t.guild_id, P.pNK.MEMBERS), E.Z.selectRole(e.id)
        }} /> : <g.Z className={a()(D.role, {
          [D.last]: n === Z.length - 1
        })} roleName={e.name} roleColor={s} verified={l} />
    })
  }
  ...
  function K() {
    return V ? <div className={D.channelSettingButtons}>{<N.Z label={w.intl.string(w.t.dMJ3Y2)} onClick={() => o(true)} icon={(0, r.jsx)(d.BFJ, {
          size: "xs",
          color: "currentColor"
        })} />}{<N.Z label={w.intl.string(w.t["3gUsJS"])} onClick={W} icon={(0, r.jsx)(d.vdY, {
          size: "xs",
          color: "currentColor"
        })} />}</div> : null
  }
  ...
  function q() {
    return n ? <d.u_l renderModal={e => (0, r.jsx)(h.default, j(M({}, e), {
        onClose: () => (z(), e.onClose()),
        channelId: t.id
      }))} onCloseRequest={() => o(false)} /> : null
  }
  return <C.ZP channelId={t.id}>{<C.Kq locked={true} channelType={t.type} />}{<C.Ot>{w.intl.format(w.t.I3R7Vl, {
        channelName: s
      })}</C.Ot>}{<C.jz className={L.markup}>{w.intl.format(w.t.QuwqjI, {
        channelName: s,
        topicHook: () => b.Z.parseTopic(t.topic, true, {
          channelId: t.id
        })
      })}</C.jz>}{K()}{<div className={D.members}>{H()}{Y()}</div>}{q()}</C.ZP>
}