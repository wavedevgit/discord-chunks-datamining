...
function Y(e) {
  ...
  let eR = "refresh_sm",
    eP = eC ? <c.svS size={eR} color={"currentColor"} colorClass={B.attachButtonPlus} /> : <c.oFk size={eR} color={"currentColor"} colorClass={B.attachButtonPlus} />,
    ew = <c.yRy targetElementRef={Q} shouldShow={null != el} animation={c.yRy.Animation.NONE} align={"recentClips" === el ? "center" : "left"} position={"top"} positionKey={null != el ? el : "null"} onRequestOpen={() => {
  ...
  return <r.Fragment>{<div className={B.uploadInput}><u.Z ref={X} onChange={eh} multiple={V.rateLimitPerUser <= 0} tabIndex={false} aria-hidden={true} /></div>}{<div className={B.attachWrapper}><D.Z channel={V} isOmniMenuOpen={"attachMenu" === el} openOmniMenu={() => ec("attachMenu")} enabled={eT} animationContainerClassName={B.buttonAnimation} glowClassName={B.buttonAnimationGlow} trinketsClassName={B.buttonAnimationTrinkets}>{ew}</D.Z></div>}</r.Fragment>
}
