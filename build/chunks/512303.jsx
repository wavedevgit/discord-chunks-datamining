...
function y(e) {
  ...
  let w = null;
  return null != N ? w = <s.sNh className={b.interactionInfoMenuItem} disabled={true} iconLeft={() => (0, r.jsx)(f.Z, {
      guild: N,
      size: f.Z.Sizes.MINI
    })} id={"integration-owner"} label={N.name} subtext={E.intl.formatToPlainString(E.t.ShLXXF, {
      application: e.author.username
    })} /> : null != A && (w = <s.sNh action={() => (0, _.openUserProfileModal)({
      userId: A.id,
      guildId: R,
      channelId: e.channel_id,
      sourceAnalyticsLocations: v
    })} className={b.interactionInfoMenuItem} iconLeft={() => (0, r.jsx)(c.Z, {
      user: A,
      size: s.EFr.SIZE_20
    })} id={"integration-owner"} label={A.username} subtext={E.intl.formatToPlainString(E.t.ShLXXF, {
      application: e.author.username
    })} />), <s.sNh id={"view-interaction-info"} label={E.intl.string(E.t.Rjezb2)}>{w}{null != P ? <s.sNh action={() => (0, _.openUserProfileModal)({
        userId: P.id,
        guildId: R,
        channelId: e.channel_id,
        sourceAnalyticsLocations: v
        ...
        user: P,
        size: s.EFr.SIZE_20
      })} id={"interaction-user"} label={P.username} subtext={E.intl.string(E.t["04gxNj"])} /> : null}</s.sNh>
}