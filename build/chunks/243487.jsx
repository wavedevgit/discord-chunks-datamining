...
function U(e) {
  ...
  return <div className={o()(L.hiddenVoiceStates, u && L.locked, f && L.flipped)}>{<div className={L.hiddenVoiceStatesAvatar}><d.Z size={c} backSrc={m} frontSrc={O} isTyping={false} /></div>}{<div className={L.hiddenVoiceStatesText}><s.Text variant={"text-xs/medium"}>{"+"}{i.length}{" others in voice"}</s.Text></div>}</div>
}
let z = Chunk73800.memo(function(e) {
  ...
  return S ? <div className={L.voiceUserContainer}>{Z.map(e => {
      let i = P.get(e);
      return null == i ? null : <A.ZP sortedVoiceState={i} channel={l} widgetId={t} flipped={!b} isPreviewingInGame={m} isSettingsPreview={O} context={n} size={h} displayNameMode={c} displayUserMode={u} locked={d} />
    })}{D && 1 === C.length && null != V && <A.ZP sortedVoiceState={V} channel={l} widgetId={t} flipped={!b} isPreviewingInGame={m} isSettingsPreview={O} context={n} size={h} displayNameMode={c} displayUserMode={u} locked={d} />}{D && C.length > 1 && <U hiddenVoiceStates={C} voiceStateMap={P} size={(0, A.px)(h)} locked={d} flipped={!b} />}</div> : d || !g ? null : <N.E emptyText={R.intl.string(R.t.hEh0l5)} icon={s.gj8} />
});
