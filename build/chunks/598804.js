/** Chunk was on web.js **/
"use strict";
n.d(t, {
  Z: () => v
}), n(361932), n(187205), n(539854), n(388685);
var r = n(255367),
  i = n(73800),
  a = n(392711),
  o = n.n(a),
  s = n(442837),
  l = n(481060),
  c = n(208049),
  u = n(763296),
  d = n(294206),
  _ = n(592125),
  f = n(430824),
  p = n(496675),
  h = n(594174),
  m = n(74538),
  g = n(710111),
  E = n(231338),
  b = n(681162);
let y = 4;

function O(e) {
  let t = (0, s.e7)([h.default], () => m.ZP.canUseSoundboardEverywhere(h.default.getCurrentUser())),
    n = (0, s.e7)([u.Z], () => u.Z.getSoundsForGuild(g.X8)),
    r = (0, s.Wu)([f.Z], () => f.Z.getGuildIds()),
    a = i.useMemo(() => p.Z.can(E.Pl.USE_EXTERNAL_SOUNDS, e), [e]);
  return i.useMemo(() => {
    let i = [];
    if (t && a) {
      let e = r.flatMap(e => {
        var t, n;
        return null != (n = null == (t = u.Z.getSoundsForGuild(e)) ? void 0 : t.filter(e => e.available)) ? n : []
      });
      i.push(...o().sampleSize(e, y))
    } else if ((null == e ? void 0 : e.guild_id) != null) {
      var s;
      let t = null == (s = u.Z.getSoundsForGuild(null == e ? void 0 : e.guild_id)) ? void 0 : s.filter(e => e.available);
      i.push(...o().sampleSize(t, y))
    }
    return i.length < y && i.push(...o().sampleSize(null != n ? n : [], y - i.length)), i
  }, [a, null == e ? void 0 : e.guild_id, n, r, t])
}
let v = i.memo(function(e) {
  let {
    channelId: t,
    onSelectSoundmoji: n
  } = e, a = (0, s.e7)([u.Z], () => !u.Z.isFetching() && !u.Z.hasFetchedAllSounds(), []), o = (0, s.e7)([_.Z], () => _.Z.getChannel(t)), f = O(o);
  return (i.useEffect(() => {
    (0, c.w)()
  }, [a]), 0 === f.length) ? (0, r.jsx)(l.$jN, {}) : (0, r.jsx)("div", {
    className: b.rowContainer,
    children: f.map((e, t) => (0, r.jsx)(d.ZP, {
      suppressPlaySound: !0,
      enableSecondaryActions: !0,
      isSoundmoji: !0,
      sound: e,
      channel: o,
      onSelectItem: t => null == n ? void 0 : n(e, !t.shiftKey)
    }, t))
  })
})