...
function u(t) {
  ...
  return null == c && 0 === f.length ? null : <r.sNh id={"move-to-category"} label={d.intl.string(d.t.FAplmp)}>{null != c && <r.kSQ><r.sNh id={"favorite-uncategorized"} label={c.name} action={() => g(c.id)} /></r.kSQ>}{f.length > 0 && <r.kSQ>{f.map(t => {
        let {
          id: n,
          name: e
        } = t;
