...
function b(e) {
  ...
  return <div className={_.container}>{<header id={c} className={_.header}>{<l.Z src={p} className={_.appIcon} rendersPlaceholder={true} />}{<O application={t} />}{<y application={t} />}</header>}{<ul className={_.rows}>{m.map(e => (0, r.jsxs)(a.P3F, {
        tag: "li",
        className: _.row,
        onClick: () => n(e.type),
...
function y(e) {
  ...
  return null == n || 0 === n.length ? null : <div className={_.detailsContainer}><div className={_.overviewContainerNoVideo}>{null != n && n.length > 0 ? (0, r.jsx)(I, {
        description: n
      }) : null}</div></div>
}
...
function O(e) {
  ...
  return <div className={_.titleContainer}>{<a.X6q variant={"heading-xl/extrabold"} lineClamp={1}>{n}</a.X6q>}{i ? <div className={_.partnerLabelContainer}><a.Text variant={"text-xs/medium"} color={"text-default"}>{f.intl.string(f.t.LO4f0N)}</a.Text></div> : null}</div>
}
let v = 3;
...
function I(e) {
  ...
  let l = i.useMemo(() => {
    ...
    O = o || b;
  return <div className={_.descriptionContainer}>{<div ref={E} className={_.overflowHidden} onTransitionEnd={y}><a.Text ref={u} className={p.markup} variant={"text-sm/medium"} lineClamp={O ? true : v} style={{
          maxHeight: O ? true : m.minHeightOverride
        }}>{l}</a.Text></div>}{null != h && h > v ? <a.P3F className={_.expandableDescriptionClickable} onClick={() => s(e => !e)}>{<a.Text variant={"text-sm/semibold"} color={"text-brand"}>{O ? f.intl.string(f.t.u4YJ8v) : f.intl.string(f.t["N/tajI"])}</a.Text>}{O ? <a.u04 size={"sm"} color={a.TVs.colors.TEXT_BRAND} /> : <a.CJ0 size={"sm"} color={a.TVs.colors.TEXT_BRAND} />}</a.P3F> : null}</div>
}
