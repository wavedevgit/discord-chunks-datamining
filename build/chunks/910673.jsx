...
let h = t => {
  ...
  return <l.Y0X transitionState={x} aria-label={C.intl.string(C.t.xdm3XF)} parentComponent={"ExplicitMediaLearnMoreModal"}>{<l.xBx separator={false} className={m.modalHeader}><img src={A} alt={C.intl.string(C.t.id3fND)} className={m.headerImg} /></l.xBx>}{<l.hzk className={m.modalBody}>{<l.X6q variant={"heading-lg/bold"} className={m.modalInteriorHeader}>{C.intl.string(C.t.sGW77u)}</l.X6q>}{<l.Text variant={"text-md/normal"} color={"header-secondary"}>{function() {
          if (y) return C.intl.string(C.t.f3ARDQ);
          let t = C.intl.string(C.t.RUw0ZG),
            e = C.intl.string(C.t["E/oQYG"]);
          return L ? t : e
        }()}</l.Text>}{<div className={m.buttonContainer}>{y ? <l.zxk variant={"primary"} text={C.intl.string(C.t.KPGVWl)} onClick={function() {
          ...
          }} fullWidth={true} /> : L ? <l.zxk variant={"primary"} text={C.intl.string(C.t["9D+zGR"])} onClick={function() {
          ...
          }} fullWidth={true} /> : <l.zxk variant={"primary"} text={C.intl.string(C.t.hvVgAQ)} onClick={() => {
            (0, o.Z)(E.Z.getArticleURL(u.BhN.EXPLICIT_MEDIA_REDACTION))
          }} fullWidth={true} />}{<l.zxk variant={"secondary"} text={C.intl.string(C.t.bmbHPD)} onClick={function() {
            R(), M(I.Yy.EXPLICIT_MEDIA_LEARN_MORE_CLICK_DISMISS)
          }} fullWidth={true} />}{<l.Text variant={"text-sm/medium"} color={"header-secondary"}>{N()}</l.Text>}</div>}</l.hzk>}</l.Y0X>
}