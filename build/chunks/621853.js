...
function U(e, t) {
  ...
  })), null != i && i.delete(t), eo.emitChange();
  else {
    if (null == i) return;
    let n = i.get(t);
    if (null == n) return;
...
function H(e) {
  ...
  if (null == (n = O.get(J.user.id)) || n.delete($), I.delete(J.user.id), null != J.mutual_guilds) {
    ...
    }), L.set(J.user.id, _.ZP.getFlattenedGuildIds().filter(t => null != e[t]).map(t => ({
      guild: e[t].guild,
      nick: e[t].nick
    })))
  }
  ...
  if (A.set(J.user.id, {
      ...
      badges: null != J.badges ? J.badges.map(e => {
        let t = (0, p.fv)(e.id);
        ...
        return e.id.startsWith(T) && null != et ? y(E({}, e), {
