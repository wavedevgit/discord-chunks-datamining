...
let N = {
  ...
  C = {
    className: a()("mention", Chunk912861.mention)
  };
...
function P(e) {
  ...
  if (null != g.value) switch (g.type) {
    ...
    case c.jw.MENTIONABLE: {
      let e = g.value.toString(),
        t = null != I ? m.Z.getRole(I.id, e) : true;
      if (null != t) f = <_.Z>{y.ME}{t.name}</_.Z>;
      else {
  ...
  return null == f && (f = R(null == M ? true : M.toString())), [<i.Fragment>{<l.Text{...A(T({}, N), {
...
function w(e) {
  ...
  if (i.useEffect(() => {
    ...
    }, [m.id, E, b]), null == b) h = <l.$jN type={l.$jN.Type.SPINNING_CIRCLE} className={v.spinner} />;
  else {
    let e = [],
  ...
  return <div className={v.container} onCopy={C}>{<div className={v.tooltip} ref={I}>{h}</div>}{<div className={v.tooltipPointer} />}</div>
}
