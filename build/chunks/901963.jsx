...
let y = "premiumRetentionEmojiPickerNotice",
  ...
  v = e => {
    ...
    return <div className={b.premiumRetentionNotice}>{<c.SrA size={"md"} className={b.premiumRetentionNoticeIcon} color={N} />}{<div className={b.premiumRetentionNoticeContent}>{<c.Text variant={"text-xs/normal"}>{E.intl.format(E.t.bTMjiI, {
            planName: p.ZP.getDisplayPremiumType(I.planId),
            endsAt: S.toDate()
          })}</c.Text>}{<div><c.eee onClick={() => {
              (0, h.A3)(n), t(), d.Z.open(m.oAB.PREMIUM)
            }}>{E.intl.string(E.t.W3aavr)}</c.eee></div>}</div>}{<c.P3F onClick={() => {
          l.K.set(y, A), O = A, v(true)
        }}><c.Dio size={"md"} color={"currentColor"} className={b.premiumRetentionNoticeClose} /></c.P3F>}</div>
  }