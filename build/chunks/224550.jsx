...
let g = (e, t, n, r) => {
  ...
  E = function(e) {
    ...
    return <r.Fragment>{"" !== G && <div>{<div>{G}</div>}{<div className={m.divider} />}</div>}{"" !== U && <div>{U}</div>}{S === f.HeQ.PAYSAFE_CARD && <div className={m.paymentSourceNoticeCopy}>{h.intl.string(h.t.kj9VLC)}</div>}{S === f.HeQ.SOFORT && <div className={m.paymentSourceNoticeCopy}>{h.intl.string(h.t["UYy1/v"])}{" "}</div>}</r.Fragment>
  }