...
let er = Chunk998502.ZP.getEnableHardwareAcceleration() ? Chunk481060.Xo$ : Chunk481060.qEK,
  ...
  el = () => <Chunk481060.r7p size={"xs"} color={"currentColor"} className={Chunk425154.favoriteIcon} />,
  eo = () => <Chunk481060.kZF size={"xs"} color={"currentColor"} className={Chunk425154.favoriteIcon} />,
  ec = () => <Chunk481060.t6m size={"xs"} color={"currentColor"} className={Chunk425154.favoriteIcon} />,
  ed = e => {
    var {
      route: t,
      selected: n,
    ...
    return <N.Z className={a()(ee.channel, {
        [ee.fullWidth]: m.tq
      }, h)} onClick={p} role={f} focusProps={et({
        within: true
      }, es)} aria-posinset={b} aria-setsize={x} ref={_}><g.vjx as={"div"} selected={n} className={a()(l, ee.interactive, ee.linkButton, {
...
function eu(e) {
  let {
  ...
  }, eq = () => t.isSystemDM() ? <div className={ee.subtext}>{(0, v.Z)(t.id) ? $.intl.string($.t.FL5T09) : $.intl.string($.t.NnY5lZ)}</div> : t.isMultiUserDM() ? <div className={ee.subtext}>{$.intl.format($.t.CxSA5O, {
    ...
    applicationStream: j,
    voiceChannel: D
  }) ? <E.Z location={"PrivateChannel"} user={m} activities={x} voiceChannel={D} applicationStream={j} animateEmoji={J || eu || ep} textClassName={ee.activityStatusText} iconClassName={eP ? ee.mutedIcon : true} /> : null, eX = () => {
    let e = g.EFr.SIZE_32;
    if (t.isMultiUserDM())
      if (t.recipients.length >= 2 && s && null == t.icon) return <w.Z aria-hidden={true} recipients={t.recipients} size={e} isTyping={L} status={M} />;
      else return <er{...en(et({}, ej), {
        src: (0, T.x)(t),
        "aria-hidden": true,
        size: e,
    ...
    let n = null;
    return m.isSystemUser() || (n = (0, b.Z)(x) ? Q.Skl.STREAMING : M), <er{...en(et({}, ej), {
      size: g.EFr.SIZE_32,
      src: ex,
      avatarDecoration: e_,
      status: n,
  ...
  }, eJ = <P.Z userName={eD} displayNameStyles={null == m ? true : m.displayNameStyles} effectDisplayType={J || l || ep ? A.F.ANIMATED : A.F.PLAIN} loop={J} />, eQ = eS ? <i.Fragment>{eJ}{<k.ZP primaryGuild={null == m ? true : m.primaryGuild} userId={null == m ? true : m.id} inline={true} disableGuildProfile={true} className={a()(ee.clanTag, {
        [ee.clanTagMuted]: eP
      })} />}</i.Fragment> : eJ, e$ = r.useRef(null);
  return <u.mh id={t.id}>{e => {
      var {
        role: n
      } = e, r = ei(e, ["role"]);
      return (0, i.jsxs)(N.Z, {
