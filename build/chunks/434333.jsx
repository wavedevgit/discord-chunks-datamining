/** Chunk was on web.js **/
/** chunk id: 434333, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  k: () => d,
  m: () => u
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk1561 = require("./1561.jsx"),
  Chunk481060 = require("./481060.js"),
  Chunk220087 = require("./220087.js");
let u = {
    RED: Chunk220087.backgroundRed,
    BACKGROUND_TERTIARY: Chunk220087.backgroundTertiary,
    BACKGROUND_ACCENT: Chunk220087.backgroundAccent
  },
  d = Chunk73800.forwardRef(function(e, t) {
    let {
      role: n,
      children: i,
      className: o,
      onDismiss: d,
      backgroundColor: f = u.RED,
      icon: _ = l.Mgn,
      iconClassName: p
    } = e;
    return <div className={a()(c.error, o, f)} ref={t}>{<_ className={a()(c.icon, p)} color={"currentColor"} />}{<div role={n} className={c.text}>{i}</div>}{null != d ? <s.P onClick={d} className={c.errorClose}><l.Dio size={"md"} color={"currentColor"} className={c.errorCloseIcon} /></s.P> : null}</div>
  })