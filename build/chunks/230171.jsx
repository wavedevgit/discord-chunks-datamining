...
function x(e) {
  ...
  }, [Y, H, G, B, U, t.id, F, a, x, j, O, K, y]), q = O ? <k app={t} /> : <M app={t} />;
  return <A.W title={g} staticBannerSrc={T} videoBannerSrc={N} onClickBanner={W} bannerAspectRatio={L} iconSrc={v} embedUrl={n} info={q} actions={z} onClickContent={W} trackingConfig={{
      id: t.id,
      linkType: o,
      referrerId: x,
      activityCustomId: a,
...
function M(e) {
  ...
  return <r.Fragment>{null != _ && <l.Text variant={"text-xs/medium"} className={L.description} color={"none"}>{_}</l.Text>}{null != f && <l.Text variant={"text-xs/normal"} className={L.tagline} color={"none"} lineClamp={1}>{<l.Jmo size={"xxs"} color={"currentColor"} />}{f}</l.Text>}</r.Fragment>
}
...
function k(e) {
  let {
    ...
    if (d.length > 0) return <N.K activityUsers={d} guildId={c} activityText={D.intl.formatToPlainString(D.t["IJa+4+"], {
        count: d.length
      })} />;
    if (u.some(e => (0, m.ig)(e) === a.o.GLOBAL)) return <r.Fragment>{<l.YqE size={"xxs"} color={"currentColor"} />}{D.intl.string(D.t.TsWCdX)}</r.Fragment>;
    let e = o > 0 ? D.intl.format(D.t.z8EAJS, {
      count: o
    }) : D.intl.string(D.t.RjceQU);
    return <r.Fragment>{<l.tBG size={"xxs"} color={"currentColor"} />}{e}</r.Fragment>
  }, [u, d, c, o]);
  return <r.Fragment>{n.length > 0 && <l.Text variant={"text-xs/medium"} className={L.description} color={"none"} lineClamp={1}>{n.map((e, t) => (0, r.jsx)("span", {
        className: L.tag,
        children: e
      }, t))}</l.Text>}{<l.Text variant={"text-xs/normal"} className={L.tagline} color={"none"} lineClamp={1}>{f}</l.Text>}</r.Fragment>
}