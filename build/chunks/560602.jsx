...
let g = (e, t, l) => {
  ...
  x = e => {
    ...
    return <div aria-hidden={true} className={v.roleCircle} style={{
  ...
  b = e => {
    ...
    return R ? <s.$jN /> : <o.Modal title={w.intl.string(w.t.TFGnmp)} actions={[{
        variant: "secondary",
        ...
        onClick: T,
        disabled: !M
      }]} onClose={j} transitionState={O}>{<s.VcW closeOnSelect={false} className={v.roleSelector} maxVisibleItems={5} placeholder={w.intl.string(w.t["8kKqCQ"])} multi={true} value={P} options={E} onChange={e => {
          _(e)
        }} renderOptionPrefix={e => null != e ? (0, r.jsx)(x, {
          rolesRow: H.get(e.value)
        }) : null} />}{<div className={v.row}>{<s.j7V value={k} hideBorder={true} onChange={C} className={v.switchGroup}>{w.intl.string(w.t.kPwwAw)}</s.j7V>}{<s.Text variant={"text-sm/normal"}>{w.intl.format(w.t.l7Ercn, {})}</s.Text>}</div>}{G ? <div className={v.row}><s.Text className={v.error} variant={"text-sm/normal"}>{w.intl.string(w.t.mgZKZG)}</s.Text></div> : null}</o.Modal>
  }