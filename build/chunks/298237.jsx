...
function O(t) {
  ...
  return <a.Y0X transitionState={e} size={a.CgR.DYNAMIC} aria-label={g.intl.string(g.t.QpSKo6)} className={v.modal} impression={{
      impressionName: l.ImpressionNames.USER_AGE_GATE,
      impressionProperties: {
        existing_user: false
      }
    }} parentComponent={"NewUserAgeGate"}><a.hzk className={v.content}>{<div className={v.image} />}{<form onSubmit={C}>{<a.X6q className={v.title} variant={"heading-xl/semibold"}>{g.intl.string(g.t.QpSKo6)}</a.X6q>}{<a.Text color={"header-secondary"} className={v.description} variant={"text-md/normal"}>{g.intl.format(g.t.EcJBEB, {
            helpURL: p.Z.getArticleURL(b.BhN.AGE_GATE)
          })}</a.Text>}{<m.Z required={true} autoFocus={true} wrapperClassName={v.formItem} label={g.intl.string(g.t.rhBeKS)} name={"birthday"} onChange={t => S(t)} onPopulated={Z} error={A} value={O} ref={N} />}{<u.zx buttonRef={j} type={"submit"} size={u.zx.Sizes.LARGE} submitting={E} disabled={null == O} fullWidth={true}>{g.intl.string(g.t.i4jeWV)}</u.zx>}</form>}</a.hzk></a.Y0X>
}