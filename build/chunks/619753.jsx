...
function w(e) {
  ...
  let s = <r.Fragment>{<l className={P.parentChannelNameIcon} size={"xxs"} color={"currentColor"} />}{<o.Text className={P.parentChannelNameText} variant={"text-xs/medium"} color={"header-secondary"}>{t.name}</o.Text>}</r.Fragment>;
  return (0, O.Em)(t.type) ? <o.P3F className={i()(P.parentChannelName, P.parentChannelNameClickable)} onClick={e => {
      e.stopPropagation(), n(t.id)
    }}>{s}</o.P3F> : <div className={P.parentChannelName}>{s}</div>
}
...
function G(e) {
  ...
  return <A.aQ.Provider value={(0, m.Z)(k, W)}><ul role={"group"} className={P.searchResultGroup} aria-label={Q}>{<o.P3F onClick={() => z(i.id)}><div className={P.channelNameContainer}>{<K className={P.channelNameIcon} size={"xs"} color={"currentColor"} />}{<o.ua7 text={Z} shouldShow={J}>{e => (0, r.jsxs)("span", y(j({}, e), {
              ref: Y,
              className: P.channelNameText,
              children: [G && null !== B && "".concat(B, " : "), q]
            }))}</o.ua7>}{<w parentChannel={V} onSelectChannel={z} />}</div></o.P3F>}{a.map((e, t) => {
        let n = c + t;
        return <x.Z ref={e => {
            E.current[n] = e
          }} totalResults={g} scrollTo={f} renderEmbeds={O} searchOffset={b} pageResultsLength={a.length} result={e} index={n} onJump={L} listItemProps={M.getItemProps({
            index: n
          })} />
      })}</ul></A.aQ.Provider>
}
let k = Chunk73800.memo(function(e) {
  ...
  let {
    ...
    return <G channel={t} results={n} highlighter={K} startIndex={l} resultRefs={z} totalResults={D} scrollTo={i} renderEmbeds={s} offset={v} jumpToMessage={k} listNavigator={H} favoriteSearch={null != S && S} />
  });
  W.push();
  let q = l.useRef(null);
  l.useLayoutEffect(() => {
  ...
  let Y = (0, o.mFp)();
  return <r.Fragment>{<div{...y(j({
      ref: q
    }, H.getContainerProps(), Y), {
      "aria-busy": x,
      children: W
    })} />}{_ > 0 || A > 0 ? <o.P3F tag={"div"} className={P.resultsBlocked} onClick={() => {
        null == h || h(!w)
      }}>{<div className={P.resultsBlockedImage} />}{<div className={P.__invalid_resultsBlockedText}>{w ? _ > 0 && A > 0 ? M.intl.formatToPlainString(M.t["OvJs9/"], {
          count: _ + A
          ...
          count: A
        })}</div>}</o.P3F> : null}{!x && !S && <L.Z renderPageWrapper={N} onPageChange={T} offset={v} totalCount={null != I ? I : D} pageSize={U.vpv} />}</r.Fragment>
})