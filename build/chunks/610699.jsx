...
function G(e) {
  ...
  return null != n && n > 0 && o.push(<div className={x.statusWrapper}>{<i className={x.statusOnline} />}{<u.Text variant={"text-xs/normal"} className={x.count} color={i}>{L.intl.format(L.t["LC+S+v"], {
        membersOnline: n
      })}</u.Text>}</div>), null != t && o.push(<div className={x.statusWrapper}>{<i className={x.statusOffline} />}{<u.Text variant={"text-xs/normal"} className={x.count} color={i}>{L.intl.format(L.t.zRl6XV, {
        count: t
      })}</u.Text>}</div>), <div className={x.statusCounts}>{o}</div>
}
...
function B(e) {
  ...
  if (null != t && null != n) {
    let e = (0, O.KS)(t, n);
    return <div className={a()(x.channel, {
        [x.ended]: i
      })}>{null != e ? <e className={x.channelIcon} size={"xs"} color={"currentColor"} /> : null}{<v.Z><u.Text variant={"text-xs/normal"} color={o}>{L.intl.format(L.t["dc+LW1"], {
            channelName: t.name,
            serverName: n.name
          })}</u.Text></v.Z>}</div>
  }
  return null != n ? <div className={a()(x.channel, {
      [x.ended]: i
    })}><v.Z><u.Text variant={"text-xs/normal"} color={o}>{L.intl.format(L.t.u0vaDA, {
          guildName: n.name
        })}</u.Text></v.Z></div> : null
}
...
function Z(e) {
  ...
  let b = i.useCallback(() => {
    ...
    V = null != O;
  if (null == O) {
    if (null == a.guild) return <P.Z />;
    O = (0, R.Qs)(a.guild)
  }
  let H = V && !k || V && C;
  return (V && C && (o = L.intl.string(L.t.wJNK8P)), k || (o = L.intl.string(L.t.hHGrW1)), null == a.code || "" === a.code || null == v) ? null : <p.Gt value={j}><F app={v} activityUsers={G} isMember={V} channel={M} guild={O} members={h} membersOnline={m} isActivityActive={x} submitting={Z} isDisabled={H} tooltip={o} handleAcceptInvite={B} onView={b} /></p.Gt>
}
