...
function O(t) {
  ...
  return <g.Z heroImageSrc={b} heroImageAlt={f.intl.string(f.t.RVyYCA)} header={f.intl.string(f.t["mWO+ys"])} description={f.intl.string(f.t.S0XtKC)}><div className={_.buttonContainer}>{<s.zxk text={E ? f.intl.string(f.t.XyHpKC) : f.intl.string(f.t.l4EmaW)} variant={"primary"} fullWidth={true} icon={s.k$p} onClick={() => {
          E ? R() : M()
        }} />}{<s.zxk text={O ? f.intl.string(f.t.QvwOJy) : f.intl.string(f.t["7fHyEx"])} variant={"secondary"} fullWidth={true} icon={s.U65} onClick={Z} loading={N} disabled={O} />}{<s.zxk text={L} variant={"secondary"} fullWidth={true} icon={s.kBi} onClick={() => w(I)} />}</div></g.Z>
}