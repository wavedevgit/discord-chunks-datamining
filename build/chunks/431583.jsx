...
let y = [{
  ...
  C = e => {
    ...
    return <f.eee useDefaultUnderlineStyles={false} href={t} className={l()(r, N.refreshDownloadButton)} onClick={n}>{a}</f.eee>
  },
  b = e => {
    let t, {
        platform: a,
    ...
    if (Array.isArray(u)) t = u.map((e, t) => {
      let a = i ? e.url() : "";
      return <C url={a} onClick={() => n(m)} text={e.getText()} />
    });
    else {
      let e = i ? u() : true;
      t = <C url={e} onClick={() => n(m)} text={v.intl.string(v.t["1WjMbG"])} />
    }
    return <li className={l()(N.platform, {
        [N.active]: o
      })} onMouseEnter={() => r(p)}><div className={N.contentWrapper}>{<div className={N.iconWrap}>{<div className={_} />}{<div className={l()(N.active, _)} />}</div>}{<div>{<f.X6q className={N.platformName} variant={"heading-lg/semibold"}>{m}</f.X6q>}{<div className={l()(N.downloadButtons, {
...
function K(e) {
  ...
  let D = (0, _.Dt)();
  return <f.Y0X className={N.downloadApps} transitionState={r} aria-labelledby={D} parentComponent={"DownloadAppsModal"}>{<f.olH onClick={a} className={N.modalCloseButton} />}{<f.hzk className={N.inner}>{<f.nn4><f.H id={D}>{v.intl.string(v.t.BK8LKy)}</f.H></f.nn4>}{!c.tq && <div className={N.platformsWrap}>{<f.X6q variant={"heading-lg/semibold"} className={N.header}>{v.intl.string(v.t["0KK0bG"])}</f.X6q>}{<ul className={N.platforms}>{y.map(e => (0, s.jsx)(b, {
            isActive: w === e.platformKey,
            platform: e,
            onClick: K,
            onMouseEnter: k,
            claimed: p
          }, e.platformKey))}</ul>}</div>}{<div className={N.platformsWrap}>{!c.tq && <f.X6q variant={"heading-lg/semibold"} className={N.header}>{v.intl.string(v.t.RdSNWV)}</f.X6q>}{<ul className={N.platforms}>{A.map(e => (0, s.jsx)(b, {
            isActive: w === e.platformKey,
            platform: e,
            onClick: K,
            onMouseEnter: k,
            claimed: p
          }, e.platformKey))}</ul>}</div>}</f.hzk>}{<f.mzw className={N.footer}><f.Text variant={"text-sm/normal"}>{v.intl.format(v.t["RJS+1N"], {})}</f.Text></f.mzw>}</f.Y0X>
}