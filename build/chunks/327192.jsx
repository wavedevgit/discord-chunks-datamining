...
let z = e => {
  ...
  return null != (0, l.e7)([N.Z], () => N.Z.getGuild(s.guild.id), [s.guild.id]) || (r = <u.zx size={u.Ph.SMALL} onClick={function() {
      g.Z.joinServer(s.id, () => {})
    }} disabled={a}><span>{a ? V.intl.string(V.t.RXvQQk) : V.intl.string(V.t.XpeFYm)}</span></u.zx>), <div className={G.integrationWrapper}>{<div className={G.integration}>{<S.Z size={S.Z.Sizes.SMALL} guild={s.guild} className={G.guildIcon} />}{<div className={G.integrationInner}>{<m.Text variant={"text-md/semibold"} color={"header-primary"}>{s.guild.name}</m.Text>}{<m.eee href={null == (n = _.Z.get(s.type)) || null == (t = n.getPlatformUserUrl) ? true : t.call(n, s.account)}><m.Text variant={"text-xs/normal"} color={"header-secondary"}>{s.account.name}</m.Text></m.eee>}</div>}{r}</div>}{c && <m.Text variant={"text-xs/normal"} color={"text-danger"} className={G.integrationError}>{o}</m.Text>}</div>
};
...
function W(e) {
  ...
  function ee() {
    ...
    (0, Z.Z)(o) && (t = <i.Fragment>{t}{<w.Z className={G.infoBox}>{V.intl.format(V.t.COW3Xl, {
          platformName: e.name
        })}</w.Z>}</i.Fragment>), (0, m.h7j)(n => <b.default{...H({
  ...
  return <div className={G.connection}>{function(e) {
      var t;
    ...
    }(o)}{o.twoWayLink ? null : o.type === M.ABu.XBOX ? <div className={G.upsellWrapper}><L.Y /></div> : o.type === M.ABu.PLAYSTATION ? <div className={G.upsellWrapper}><k.t /></div> : null}{function(e) {
      var t;
      ...
      return a && (l = V.intl.string(V.t.i4jeWV)), r.push((0, i.jsx)(u.zx, {
    ...
    }(o)}{M.BFP.has(o.type) && (n = <m.j7V className={G.connectionOptionSwitch} hideBorder={true} value={f} onChange={function(e) {
        x(e), g.Z.setFriendSync(o.type, o.id, e)
      }}><m.Text variant={"text-sm/semibold"}>{V.intl.string(V.t["+KCMSk"])}</m.Text></m.j7V>), M.vbS.has(o.type) && (s = <m.j7V className={G.connectionOptionSwitch} hideBorder={true} value={y} onChange={function(e) {
        A(e), g.Z.setShowActivity(o.type, o.id, e)
      }}><m.Text variant={"text-sm/semibold"}>{V.intl.format(V.t["6u6J0t"], {
          platform: J.name
        })}</m.Text></m.j7V>), (null == (t = _.Z.get(o.type)) ? true : t.hasMetadata) === true && (a = <m.j7V className={G.connectionOptionSwitch} hideBorder={true} value={1 === I} onChange={function(e) {
        let {
          verified: t
        } = o, n = +!!e;
        if (e && !t) {
        ...
        N(n), g.Z.setMetadataVisibility(o.type, o.id, n)
      }} disabled={1 !== v || null == o.metadata}><m.Text variant={"text-sm/semibold"}>{V.intl.string(V.t.FYKGsL)}</m.Text></m.j7V>), <div className={G.connectionOptionsWrapper}><div className={G.connectionOptions}>{<m.j7V className={G.connectionOptionSwitch} hideBorder={true} value={1 === v} onChange={function(e) {
            let {
              verified: t
            } = o, n = +!!e;
            if (e && !t) {
          ...
          }}><m.Text variant={"text-sm/semibold"}>{V.intl.string(V.t.f7yOAQ)}</m.Text></m.j7V>}{a}{s}{n}</div></div>}{function() {
      if (o.revoked || o.integrations.length > 0) return (0, i.jsx)(m.$i$, {
        className: G.connectedAccountSeparator
      })
    }()}{o.revoked ? <m.xJW className={G.integrationsWrapper}><m.R94 className={G.integrationRevoked}>{V.intl.format(V.t["6C4lgI"], {
        ...
        })}</m.R94></m.xJW> : o.integrations.length > 0 ? <m.xJW className={G.integrationsWrapper}>{<m.vwX tag={m.RB0.H5}><m.Text variant={"text-xs/semibold"}>{V.intl.string(V.t.fOe3fX)}</m.Text></m.vwX>}{<div>{o.integrations.map(e => (0, i.jsx)(z, {
          integration: e
        }, e.id))}</div>}</m.xJW> : true}</div>
}
...
function Y() {
  ...
  }, []), <div className={Chunk741138.connectContainer}>{<Chunk481060.X6q variant={"heading-md/bold"} color={"interactive-active"} className={Chunk741138.connectHeader}>{Chunk388032.intl.string(Chunk388032.t.ZeDrUV)}</Chunk481060.X6q>}{<Chunk481060.Text variant={"text-xs/normal"} color={"header-secondary"}>{Chunk388032.intl.format(Chunk388032.t["oYc+Gx"], {
        privacyPolicyUrl: Chunk981631.EYA.PRIVACY
      })}</Chunk481060.Text>}{<q />}</div>
}
...
function q() {
  ...
  return <div className={Chunk741138.connectionsContainer}>{exports.slice(0, 10).map(e => <f.Z type={e.type} className={G.__invalid_accountButton} innerClassName={G.accountButtonInner} />)}{<Chunk481060.ua7 text={Chunk388032.intl.string(Chunk388032.t.QqTz8f)}>{t => {
        let {
          onMouseEnter: n,
          onMouseLeave: r
        } = t;
...
function X(e) {
  ...
  return t = n ? <m.$jN className={F.marginTop20} type={m.$jN.Type.SPINNING_CIRCLE} /> : 0 === r.length ? <m.ubH className={F.marginTop40} theme={s}><m.OZU note={V.intl.string(V.t.WenGZ2)}>{V.intl.string(V.t["aoLS8/"])}</m.OZU></m.ubH> : r.filter(e => _.Z.isSupported(e.type)).map((e, t) => <W theme={s} account={e} locale={a} onDisconnect={() => (function(e) {
      let {
        type: t,
        id: n
      } = e;
      g.Z.disconnect(t, n)
    })(e)} />), <m.xJW className={G.connectionList}>{t}</m.xJW>
}
let J = () => {
  ...
  return (Chunk73800.useEffect(() => {
    Chunk457330.Z.fetch()
  }, []), module) ? <Chunk497321.Z /> : <Chunk481060.hjN className={Chunk741138.__invalid_connections} tag={Chunk481060.RB0.H1} title={Chunk388032.intl.string(Chunk388032.t["3fe7U1"])}>{<Y />}{<X fetching={exports} accounts={require} theme={Chunk120356} locale={a} />}</Chunk481060.hjN>
}