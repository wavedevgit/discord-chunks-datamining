...
function _(e) {
  ...
  return <svg xmlns={"http://www.w3.org/2000/svg"} width={"372"} height={"24"} fill={"none"}>{<defs><mask id={n}><P fill={true} /></mask></defs>}{<o.animated.rect style={function(e) {
        ...
        mask: "url(#".concat(n, ")")
      }, s)} fill={"#BB45FF"} x={"0"} y={"0"} height={"24"} />}{<P stroke={true} />}</svg>
}
...
function C() {
  ...
  return (0, Chunk73800.useEffect)(() => {
    (null == Chunk126663 ? true : Chunk126663.pointsWhenUnlocked) == null && Chunk481060({
      pointsWhenUnlocked: require
  ...
  }, []), <div className={Chunk182643.leveling}>{<div className={Chunk182643.levelInfo}>{<Chunk763283.Z variant={"text-lg/bold"} color={"clicker-game-brand"}>{Chunk388032.intl.format(Chunk603571.default.IG6UGB, {
          level: Z
        })}</Chunk763283.Z>}{<Chunk763283.Z className={Chunk182643.experience} variant={"text-sm/medium"} color={"clicker-game-brand"}>{Chunk388032.intl.format(Chunk603571.default.Rfy3Cg, {
        ...
        })}</Chunk763283.Z>}</div>}{<_ percent={S / I * 100} />}{k < Z ? <Chunk144931.Z className={Chunk182643.claimButton} onClick={A}>{Chunk388032.intl.format(Chunk603571.default.IUF6np, {
        points: M,
        grassIconHook: Chunk702319.Z
      })}</Chunk144931.Z> : null}</div>
}