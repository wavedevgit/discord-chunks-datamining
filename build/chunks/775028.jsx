...
function R(e) {
  ...
  return <div className={a()(E.categoryRow, {
    ...
    }}>{<d.Text className={E.categoryText} color={"text-muted"} variant={"text-sm/medium"} lineClamp={1}>{r.name}</d.Text>}{"null" !== r.id && !s && <d.P3F className={E.selectAll} onClick={() => {
        s || i(t.id, r.id, r.id)
      }}>{<d.XZJ className={E.selectAllCheckbox} size={14} shape={d.XZJ.Shapes.SMALL_BOX} value={o} type={d.XZJ.Types.INVERTED} displayOnly={true} />}{<d.Text color={"text-muted"} variant={"text-xs/medium"}>{P.intl.string(P.t.mSQwnZ)}</d.Text>}</d.P3F>}</div>
}
let D = Chunk73800.memo(function() {
    return <div className={Chunk174612.separator} />
  }),
  L = Chunk73800.memo(function(e) {
    let {
      index: t
    } = e;
    return <d.Text variant={"text-xs/normal"} className={E.subtitleSeparator}>{"\xb7"}</d.Text>
  }),
  M = Chunk73800.memo(function(e) {
    let {
      ...
      let n = [];
      return I.TPd.GUILD_THREADS_ONLY.has(e.type) && t >= 1 && n.push(<d.Text variant={"text-xs/normal"} className={E.channelSubtitle}>{P.intl.format(P.t.z0qMLy, {
          count: t
        })}</d.Text>), B.has(e.type) || n.push(<d.Text variant={"text-xs/normal"} className={E.channelSubtitle}>{(0, y.qw)(e.id)}</d.Text>), null != e.topic && e.topic.length > 0 && n.push(<m.Z className={S.markup}><d.Text className={E.topic} variant={"text-xs/normal"}>{x.Z.parseTopic(e.topic, true, {
            channelId: e.id
          })}</d.Text></m.Z>), [n.map((e, t) => [e, <L index={t} />])].flat(2).slice(0, false)
    ...
    return <l.Fragment>{<d.ua7 text={U} delay={500} position={N}>{e => (0, l.jsxs)(d.P3F, A(T({
...
function H(e) {
  let {
    ...
    return <R channel={i.channel} guild={n} onCategoryClick={u} />
  }, [t, n, m, u]), f = r.useCallback(e => {
    let {
      section: r,
      row: i
    } = e;
    if (0 === r && false === m) return <N.Z guild={n} />;
    let a = t._categories[r + m],
      o = t[a.channel.id][i];
    return null == o ? null : <M category={a.channel} channel={o.channel} guild={n} isFirstChannel={0 === i} isLastChannel={t[a.channel.id].length - 1 === i} tooltipDirection={c ? "right" : "top"} onChannelClick={u} />
  }, [t, n, m, c, u]), g = (0, N.q)(n.id), x = (0, y.tn)(n.id, t, 64, g);
  return <d._2F className={i} innerClassName={a()(o, E.scrollerInner)} sectionHeight={48} rowHeight={e => x[e].rowHeight} renderSection={p} renderRow={f} sections={x.map(e => e.rowCount)} />
}