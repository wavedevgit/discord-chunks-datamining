...
function T(e) {
  ...
  return (o.useEffect(() => {
    ...
    null != V && null != L && (0, c.ZDy)(async () => {
      ...
      modalKey: "dismissible_content_".concat(V)
  ...
  }, [t, V, M, L]), (null == T ? true : T.powerupCatalog) == null) ? null : <div className={Z.container}>{<d.Z className={Z.toolbar} hideSearch={true} toolbar={(0, r.jsx)("div", {})}>{<d.Z.Icon icon={c.$Eu} aria-label={""} />}{<d.Z.Title>{N.intl.string(w.default.yv3DJC)}</d.Z.Title>}</d.Z>}{<div className={Z.contentContainer}>{<c.zJl className={Z.powerupsContainer}>{B.map(e => {
          let {
            type: n,
            listings: o
          } = e, {
        ...
        })}</c.zJl>}{<div className={Z.sidebarContainer}>{A ? <b.Z guildId={t} /> : <j.Z guildId={t} />}{<h.Z guildId={t} />}</div>}</div>}</div>
}