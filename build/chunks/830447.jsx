/** Chunk was on web.js **/
/** chunk id: 830447, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  T: () => C,
  v: () => N
}), require("./361932.js"), require("./187205.js"), require("./539854.js"), require("./388685.js"), require("./415506.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk348327 = require("./348327.js"),
  l = require.n(Chunk348327),
  Chunk498405 = require("./498405.js"),
  Chunk597442 = require("./597442.js"),
  Chunk21340 = require("./21340.js"),
  Chunk922770 = require("./922770.js"),
  Chunk857595 = require("./857595.js"),
  Chunk607070 = require("./607070.js"),
  Chunk828214 = require("./828214.js"),
  Chunk670596 = require("./670596.js"),
  Chunk788314 = require("./788314.js"),
  Chunk906944 = require("./906944.js");

function b(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function y(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      b(e, t, n[t])
    })
  }
  return e
}

function O(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function v(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : O(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}

function I(e) {
  return null == e ? [] : i.Children.toArray(e).flatMap(e => {
    var t;
    return null == e ? [] : Array.isArray(e) ? I(e) : e.type === i.Fragment ? null != (t = I(e.props.children)) ? t : [] : [e]
  })
}

function T(e) {
  return I(e).reduce((e, t) => {
    var n, r;
    if (t.type === h.Cl) return e.push({
      type: "separator",
      navigable: false
    }), e;
    if (t.type === h.kS) {
      let n = T(t.props.children);
      return n.length > 0 && (e.push({
        type: "groupstart",
        length: n.length,
        navigable: false,
        props: t.props
      }), e.push(...n), e.push({
        type: "groupend",
        length: n.length,
        navigable: false,
        props: t.props
      })), e
    }
    if (t.type === h.sN) return e.push(null != t.props.render ? {
      type: "customitem",
      key: t.props.id,
      navigable: null == t.props.navigable || t.props.navigable,
      render: t.props.render,
      props: t.props
    } : {
      type: "item",
      key: t.props.id,
      navigable: true,
      label: t.props.label,
      children: t.props.children ? T(t.props.children) : true,
      onChildrenScroll: t.props.onChildrenScroll,
      props: t.props,
      childRowHeight: t.props.childRowHeight,
      listClassName: t.props.listClassName,
      subMenuClassName: t.props.subMenuClassName
    }), e;
    if (t.type === h.S8) return e.push({
      type: "checkbox",
      key: t.props.id,
      navigable: true,
      props: t.props
    }), e;
    if (t.type === h.k5) return e.push({
      type: "radio",
      key: t.props.id,
      navigable: true,
      props: t.props
    }), e;
    else if (t.type === h.II) return e.push(null != t.props.control ? {
      type: "control",
      key: t.props.id,
      navigable: true,
      props: t.props
    } : {
      type: "compositecontrol",
      key: t.props.id,
      navigable: false !== t.props.interactive,
      children: t.props.children,
      props: t.props
    }), e;
    throw Error("Menu API only allows Items and groups of Items as children. Received ".concat(null != (r = null != (n = null == t ? true : t.type) ? n : t) ? r : typeof t, " (").concat(typeof t, ") instead"))
  }, [])
}

function S(e) {
  return e.reduce((e, t) => (t.navigable && e.push({
    key: t.key,
    children: "item" === t.type && null != t.children ? S(t.children) : true
  }), e), [])
}

function A(e, t, n, i) {
  let o = 0,
    a = [];
  return e.reduce((e, s, l) => {
    let c = a.length > 0 ? a[a.length - 1] : e;
    switch (s.type) {
      case "separator":
        c.push(<g.Z0 />), o = 0;
        break;
      case "groupstart":
        o > 0 && s.length > 0 && (c.push(<g.Z0 />), o = 0), a.push([]);
        break;
      case "groupend":
        a.length > 0 && e.push(<g.ZA{...y({
          contents: a.pop()
        }, s.props)} />);
        break;
      case "item": {
        let {
          children: e,
          childRowHeight: a,
          onChildrenScroll: l,
          listClassName: u,
          subMenuClassName: d
        } = s, f = null != e, _ = [...n, s.key], p = t.isFocused(_), h = null != e ? {
          "aria-haspopup": true
        } : {}, m = <g.ck{...v(y({}, s.props), {
          label: s.label,
          hasSubmenu: null != e,
          isFocused: p,
          menuItemProps: y({}, t.getItemProps({
            path: _,
            hasSubmenu: f
          }), h),
          onClose: i
        })} />;
        f ? null != a ? c.push(<g.P2{...v(y({}, s.props), {
          parentItem: m,
          isFocused: p,
          menuSubmenuProps: t.getSubmenuProps({
            path: _
          }),
          rows: A(e, t, _, i),
          rowHeight: a,
          onScroll: l,
          listClassName: u
        })} />) : c.push(<g.LY{...v(y({}, s.props), {
          subMenuClassName: d,
          parentItem: m,
          isFocused: p,
          menuSubmenuProps: t.getSubmenuProps({
            path: _
          }),
          renderSubmenu: () => A(e, t, _, i)
        })} />) : c.push(m), o++;
        break
      }
      case "customitem": {
        let e = [...n, s.key];
        c.push(<g.kq{...v(y({}, s.props), {
          isFocused: t.isFocused(e),
          menuItemProps: t.getItemProps({
            path: e,
            navigable: s.navigable
          }),
          onClose: i,
          children: s.render
        })} />), o++;
        break
      }
      case "checkbox": {
        let e = [...n, s.key];
        c.push(<g.oC{...v(y({}, s.props), {
          isFocused: t.isFocused(e),
          menuItemProps: t.getItemProps({
            path: e,
            role: "menuitemcheckbox"
          })
        })} />), o++;
        break
      }
      case "radio": {
        let e = [...n, s.key];
        c.push(<g.Rk{...v(y({}, s.props), {
          isFocused: t.isFocused(e),
          menuItemProps: t.getItemProps({
            path: e,
            role: "menuitemradio"
          })
        })} />), o++;
        break
      }
      case "control": {
        let e = [...n, s.key];
        c.push(<g.lc{...v(y({}, s.props), {
          isFocused: t.isFocused(e),
          menuItemProps: t.getItemProps({
            path: e
          }),
          onClose: i
        })} />), o++;
        break
      }
      case "compositecontrol": {
        let e = [...n, s.key];
        c.push(<g.$I{...v(y({}, s.props), {
          isFocused: t.isFocused(e),
          menuItemProps: t.getItemProps({
            path: e
          }),
          onClose: i,
          children: s.children
        })} />), o++
      }
    }
    return e
  }, [])
}

function N(e) {
  var t;
  let {
    navId: n,
    variant: o = "flexible",
    hideScroller: s = false,
    className: f,
    children: h,
    onClose: b,
    onSelect: O,
    onInteraction: I
  } = e, N = T(h), C = S(N), P = i.useRef([]);
  l()(P.current, C) || (P.current = C);
  let w = null == (t = N.find(e => null != e.key)) ? true : t.key,
    D = (0, c.ZP)({
      navId: n,
      items: P.current,
      initialFocusPath: p.Z.keyboardModeEnabled && null != w ? [w] : [],
      closeMenu: b,
      defaultIsUsingKeyboardNavigation: p.Z.keyboardModeEnabled
    });
  i.useEffect(() => {
    D.isUsingKeyboardNavigation ? p.Z.keyboardModeEnabled || (0, _.Qj)() : p.Z.keyboardModeEnabled && (0, _.rf)()
  }, [D.isUsingKeyboardNavigation]);
  let L = i.useRef(null);
  (0, u.T)(L);
  let x = s ? d.u2 : d.zJ,
    M = i.useMemo(() => ({
      onSelect: O,
      onInteraction: I
    }), [O, I]);
  return <m.p.Provider value={M}><div{...v(y({
      className: a()(E.menu, E[o], f)
    }, D.getContainerProps()), {
      ref: L,
      "aria-label": e["aria-label"],
      children: (0, r.jsxs)(x, {
        className: E.scroller,
        children: [0 === N.length && (0, r.jsx)(g.ck, {
          disabled: true,
          label: () => (0, r.jsx)(R, {}),
          menuItemProps: D.getItemProps({
            path: ["empty"]
          }),
          isFocused: false,
          onFocus: () => {},
          onClose: b
        }), N.length > 0 && A(N, D, [], b)]
      })
    })} /></m.p.Provider>
}

function C() {
  return <div className={a()(Chunk906944.menu, Chunk906944.loader, Chunk906944.flexible)}><Chunk922770.$ /></div>
}

function R() {
  let e = "♫ (つ｡◕‿‿◕｡)つ ♪",
    t = "♫ ⊂(｡◕‿‿◕｡⊂) ♪",
    [n, o] = Chunk73800.useState(module);
  return Chunk73800.useEffect(() => {
    if (Chunk607070.Z.useReducedMotion) return;
    let n = setInterval(() => {
      Chunk120356(n => n === e ? t : e)
    }, 700);
    return () => clearInterval(require)
  }, []), <div style={{
      width: "100%",
      textAlign: "center"
    }}>{require}</div>
}