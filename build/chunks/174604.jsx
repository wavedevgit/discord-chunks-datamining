...
function b(e) {
  ...
  return <l.xv tag={"span"} variant={"text-md/semibold"} className={p.highlight}>{t.title}</l.xv>
}
...
function y(e) {
  ...
  return null == h || 0 === h.length ? null : <s.animated.div className={a()(p.container, {
      [p.themeResponsiveContainer]: n
    })} style={E(m({}, O), {
      opacity: +!!g
    })}>{<l.xv tag={"span"} variant={"text-md/semibold"} className={a()(p.description, {
        [p.text]: !n,
        [p.responsiveText]: n
      })}>{I}</l.xv>}{<c.zx className={p.button} onClick={o}>{_.intl.string(f.default.E5V5KC)}</c.zx>}</s.animated.div>
}