...
function x(e) {
  ...
  return <div className={a()(D.content, {
      [D.outerGlow]: U,
      [D.contentCoachmark]: q
    })} onMouseEnter={L} onMouseLeave={x}><div className={a()(X, {
        [D.innerGlow]: U
      })}>{<div className={D.subContent}>{<div className={D.avatars}>{<p.Z className={Q} user={o} aria-label={o.username} size={R.EF.SIZE_56} />}{null != B && <p.Z className={J} user={B} aria-label={o.username} size={R.EF.SIZE_24} />}</div>}{<div className={D.textContainer}>{<d.Text variant={"text-md/medium"} color={$} lineClamp={3}>{V()}</d.Text>}{<d.Text className={ee} variant={"text-sm/normal"}>{H()}</d.Text>}</div>}</div>}{<div className={D.buttonContainer}>{null != G && <u.zx className={a()(D.button, ei)} onClick={z} size={u.zx.Sizes.MEDIUM} color={u.zx.Colors.CUSTOM} look={u.zx.Looks.OUTLINED}><div className={D.buttonContentContainer}><d.Text variant={"text-sm/medium"} className={a()(D.buttonText, eo)}>{Y()}</d.Text></div></u.zx>}{<u.zx className={D.button} onClick={K} size={u.zx.Sizes.MEDIUM} color={et}><div className={D.buttonContentContainer}>{<d.OgN color={en} size={"custom"} width={"14"} height={"14"} />}{<d.Text variant={"text-sm/medium"} className={a()(D.buttonText, er)}>{w.intl.string(w.t["wg/30t"])}</d.Text>}</div></u.zx>}</div>}{null != M && <div className={a()(D.connector, {
