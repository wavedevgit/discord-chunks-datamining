...
function C(e) {
  ...
  }, [h]), null == b ? null : <c.Gt value={h}><i.Y0X transitionState={t} size={P ? i.CgR.DYNAMIC : i.CgR.MEDIUM} parentComponent={"NameplateModal"} data-migration-pending={true}>{P ? (0, n.jsx)(i.$jN, {
        className: g.spinner,
...
function j(e) {
  ...
  return <n.Fragment>{<i.xBx separator={false} className={g.header}>{<i.X6q variant={"heading-lg/semibold"}>{y.intl.string(y.t.BwdeMz)}</i.X6q>}{<i.olH className={g.closeButton} onClick={j} />}</i.xBx>}{<i.hzk className={g.content} scrollbarType={"none"}>{<_.Z selected={I} onSelect={(e, t) => {
          A(e), T(null != t && t)
        }} onOpenShop={D} available={v} purchased={O} />}{<m.Z user={f} guildId={P} selectedNameplate={I} purchased={k} />}</i.hzk>}{<i.mzw className={g.modalFooter}>{k || null == I ? <i.zxk variant={"primary"} text={y.intl.string(y.t.Jh8fJy)} disabled={N} onClick={() => {
          null != P ? (0, p.RH)(I) : (0, o.Rx)(I), j()
        }} /> : <i.zxk variant={"primary"} onClick={() => D(null == I ? true : I.skuId)} text={y.intl.string(y.t.fYfGgI)} />}{<i.zxk variant={"secondary"} text={y.intl.string(y.t.mDcKND)} onClick={j} />}</i.mzw>}</n.Fragment>
}