...
let b = e => ({
  ...
  j = e => {
    let {
      ...
      let n = C[t.value];
      return null == n ? null : <div className={_.value}>{<s.Z guild={n} size={s.Z.Sizes.SMALLER} active={true} className={_.icon} />}{<a.Text variant={"text-sm/normal"} className={_.text}>{t.label}</a.Text>}</div>
    }, [C]), S = l.useCallback(e => {
      if (null == e || null == e.value) return null;
      let t = C[e.value];
      return null == t ? null : <div className={_.option}>{<s.Z guild={t} size={s.Z.Sizes.MEDIUM} active={true} className={_.icon} />}{<div className={_.optionLabelContainer}>{<a.Text variant={"text-sm/medium"} className={_.text}>{e.label}</a.Text>}{<a.Text variant={"text-xs/normal"} color={"text-tertiary"} className={_.text}>{p.intl.format(p.t.WkK72t, {
              count: E[e.value]
            })}</a.Text>}</div>}</div>
    }, [E, C]);
    return l.useEffect(() => {
    ...
    }, [I, t, d, n, E]), <a.q4e className={_.guildSelector} onChange={t} options={I} popoutPosition={"top"} popoutWidth={240} renderOptionLabel={S} renderOptionValue={O} value={n} aria-labelledby={j} placeholder={I.length < 1 ? p.intl.string(p.t.jHpxws) : p.intl.string(p.t["4mqeQE"])} isDisabled={I.length < 1} />
  }