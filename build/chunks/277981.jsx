...
let T = {
  ...
  renderResults(e) {
    ...
    if (n) return <s.$jN className={y.spinner} type={s.$jN.Type.SPINNING_CIRCLE} />;
    if (null != t) {
      var O, I;
      let e = false,
        n = t.map((t, n) => {
          if (t.type === E.q9n.GIF) {
            var i, a, s;
            return e = true, <_.ZP.GIFIntegration className={y.horizontalAutocomplete} onClick={f} onHover={u} selected={o === n} index={n} width={null != (i = t.meta.width) ? i : 0} height={null != (a = t.meta.height) ? a : 0} src={null != (s = t.meta.src) ? s : ""} url={t.meta.url} />
          }
        }),
      ...
      return <i.Fragment>{<_.ZP.Title title={l} />}{e ? <p.Z className={y.horizontalAutocompletes}>{n}</p.Z> : n}</i.Fragment>
    }
  },
  onSelect(e) {
    let {
