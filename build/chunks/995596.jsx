...
let C = e => [{
  ...
  _ = e => {
    ...
    return <d.v2r navId={"members-tabs-overflow-menu"} aria-label={v.intl.string(v.t.riPnr6)} hideScroller={true} onClose={a} onSelect={a}><d.kSQ>{t.map(e => {
          let {
            id: t,
            label: a
          } = e;
...
function E(e) {
  ...
  return (0, u.s)(x, O, [w]), <l.Fragment>{<b.Z.Divider className={g.divider} />}{<div className={g.tabsContainer} ref={x}>{<div className={g.measurements}>{N.map((e, t) => <o.AJ index={t} onItemLayout={P}><d.njP.Item id={e.id} aria-label={e.label}>{e.label}</d.njP.Item></o.AJ>)}{<div ref={R}><T tabs={M} onTabSelect={a} selectedTab={n} /></div>}</div>}{<d.njP aria-label={v.intl.string(v.t.tcvVXF)} selectedItem={n} type={"top-pill"} onItemSelect={a}>{y.map(e => <d.njP.Item id={e.id} aria-label={e.label}>{e.label}</d.njP.Item>)}{0 !== M.length ? <T tabs={M} onTabSelect={a} selectedTab={n} /> : null}</d.njP>}</div>}</l.Fragment>
}