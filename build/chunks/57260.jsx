/** Chunk was on web.js **/
/** chunk id: 57260, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  z: () => f
}), require("./388685.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk126663 = require("./126663.js"),
  Chunk481060 = require("./481060.js"),
  Chunk393238 = require("./393238.js"),
  Chunk397974 = require("./397974.js");
let d = 100;

function f(e) {
  let {
    children: t,
    className: n,
    collapsibleContent: o,
    isExpanded: f
  } = e, [_, p] = i.useState(false), [h, m] = i.useState(true), [g, E] = i.useState(false), b = null != f ? f : _, {
    ref: y,
    height: O = 0
  } = (0, c.ZP)(), {
    ref: v,
    height: I = 0
  } = (0, c.ZP)(), T = (0, l.q_F)({
    height: b ? O + I : I,
    config: s.config.stiff,
    onRest: () => E(true)
  }, h ? "animate-never" : "respect-motion-settings"), S = i.useCallback(() => {
    p(!_)
  }, [_, p]);
  return i.useLayoutEffect(() => {
    let e = setTimeout(() => {
      m(false)
    }, d);
    return () => clearTimeout(e)
  }, []), <div className={a()(u.collapseable, {
      [u.toggled]: b
    }, n)}><s.animated.div className={a()(u.contentExpandContainer, {
        [u.showOverflow]: b && g
      })} style={T}>{<div ref={v} className={a()(u.header, {
          [u.toggled]: b
        })}>{t({
          onClick: S
        })}</div>}{<div ref={y}>{o}</div>}</s.animated.div></div>
}