...
function _(e) {
  ...
  let {
    ...
    trailing: M
  } = e, [B, U] = r.useState(false), [W, H] = r.useState(false), G = L && !D && !W && true !== R && null != E, z = r.useRef(null), F = r.useRef(null), V = r.useCallback((e, t, n, r) => null == e || null == r ? null : n ? <m.Z align={m.Z.Align.CENTER} ref={F}>{<d.Text variant={"text-sm/normal"}>{e}</d.Text>}{n ? <d.Fbu size={"custom"} color={"currentColor"} width={10} height={10} className={y.caret} /> : null}</m.Z> : <c.zx buttonRef={z} size={c.zx.Sizes.SMALL} look={n ? c.zx.Looks.LINK : c.zx.Looks.FILLED} color={n ? c.zx.Colors.PRIMARY : c.zx.Colors.BRAND} disabled={t} onClick={r}>{e}</c.zx>, [z, F])(P, T, k, A);
  r.useEffect(() => {
    var e, t;
  ...
  let K = G ? <d.yRy targetElementRef={k ? F : z} renderPopout={() => (0, i.jsx)(d.P3F, {
        onClick: e => {
      ...
      })} position={"bottom"} align={"center"} animation={d.yRy.Animation.TRANSLATE} shouldShow={true}>{() => V}</d.yRy> : V,
    q = (0, s.Wu)([b.Z], () => {
      var e;
    ...
    $ = <m.Z>{<x.Z name={_} icon={C} imageSrc={N} iconBackgroundColor={S} iconClassName={I} iconWrapperClassName={w} details={Z} isPremium={q.length > 0} />}{<m.Z.Child shrink={0} grow={0}><m.Z align={m.Z.Align.CENTER} justify={m.Z.Justify.END}>{function(e, t, n) {
            var r, l;
          ...
          }(E, B, X)}{K}{M}</m.Z></m.Z.Child>}</m.Z>;
  return k ? <d.P3F onClick={() => {
      L && null != R && null != E && j.Z.dismissOverviewTooltip(R, E.integration), null == A || A()
    }} onFocus={() => U(true)} onBlur={() => U(false)}><d.Zbd editable={true} className={o()(y.card, y.clickable)}>{$}</d.Zbd></d.P3F> : <d.Zbd editable={true} className={y.card}>{$}</d.Zbd>
}