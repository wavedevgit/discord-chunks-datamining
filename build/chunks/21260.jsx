...
function v(e) {
  let {
    ...
    maxLines: d,
    containerWidth: p
  }), v = i.useCallback(e => <b items={e} renderItem={o} />, [o]), I = null != f ? f : v, T = i.useMemo(() => n.slice(0, h + 1), [n, h]), S = i.useMemo(() => n.slice(h + 1), [n, h]);
  return <div className={a()(t, u.items)} ref={_}>{<div aria-hidden={true} className={u.measurements}>{<div className={u.itemMeasurements}>{n.map((e, t) => (0, r.jsx)(y, {
          index: t,
          onItemLayout: m,
          children: o(e)
        }, O(e)))}</div>}{<div className={u.overflowMeasurement} ref={g}>{o(c.intl.formatToPlainString(c.t.vGclWl, {
          count: Number("1".concat(n.length))
        }))}</div>}</div>}{T.map(o)}{S.length > 0 && I(S)}</div>
}