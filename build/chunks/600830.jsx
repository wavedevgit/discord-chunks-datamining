...
function g(t) {
  let {
    ...
    return null == t || null != e ? null : <i.sNh id={"popout-call"} label={j.intl.string(j.t.JzWez8)} icon={i.rgF} action={() => s.hP(t)} />
    ...
    return null != t && t.isGuildVocal() ? <i.kSQ>{n}{o}</i.kSQ> : null
  }(e);
  return <i.v2r navId={"rtc-channel"} aria-label={j.intl.string(j.t.ogxXGh)} onClose={o.Zy} onSelect={n} onInteraction={g}>{O}{y}{<i.kSQ>{v}{<i.S89 id={"show-voice-states"} checked={h} action={() => d.Fg.updateSetting(!h)} label={j.intl.string(j.t.vkbSBw)} />}</i.kSQ>}</i.v2r>
}