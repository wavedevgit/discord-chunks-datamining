...
let N = new h.Z("AuditLogUtils"),
  S = [f.J6.DAYS, f.J6.HOURS, f.J6.MINUTES, f.J6.SECONDS],
  R = () => ({
  ...
  Y = () => x({
    ...
    [j.zUn.MAX_AGE]: AT({
      [L.intl.string(L.t.PqEzn5)]: L.t.uWrLv7
    }, L.t["Q+5kcH"]),
    [j.zUn.TEMPORARY]: AV(L.t.MWp6Hx, L.t.omiqTE),
    [j.zUn.FLAGS]: Aj({
      [a.$.IS_GUEST_INVITE]: L.t.XYZMbG
    })
  }, R()),
  W = () => ({
  ...
  J = () => x({
    [j.zUn.NAME]: AQ(L.t.ahU1o6, L.t["wxs+vb"])
  }, R()),
  q = () => x({
  ...
  return A.forEach(A => {
    let n = function(A, e) {
        switch (A.targetType) {
          ...
          case j.KFR.ROLE:
            return AH(A, j.zUn.NAME, A => I.Z.getRole(e.id, A), A => A.name);
          case j.KFR.ONBOARDING_PROMPT:
            let t = AH(A, j.zUn.ID, A => d.Z.getOnboardingPrompt(A), A => A.title);
          ...
          case j.KFR.EMOJI:
            return AH(A, j.zUn.NAME, A => E.ZP.getGuildEmoji(e.id).find(e => e.id === A), A => A.name);
          case j.KFR.STICKER:
        ...
        A.changes.forEach(n => {
          let l = function(A, e, t) {
            if (e.action === j.rsA.APPLICATION_COMMAND_PERMISSION_UPDATE) {
              ...
              switch (e.type) {
                case j.ecB.ROLE:
                  A.subtarget = Ab(e.id, A => I.Z.getRole(t.id, A), A => A.name);
                  break;
                case j.ecB.USER:
            ...
            switch (A.key) {
              ...
              case j.zUn.AUTO_MODERATION_ACTIONS:
                if (e.targetType === j.KFR.AUTO_MODERATION_RULE) return Ap(A, A => A.map(A => A.type), A => A.map(u.ZF).join(", "));
                break;
              ...
              case j.zUn.AUTO_MODERATION_TRIGGER_METADATA:
                if (e.targetType === j.KFR.AUTO_MODERATION_RULE) return Ap(A, A => null != A && "object" == typeof A ? null != A.keyword_filter && Array.isArray(A.keyword_filter) ? L.intl.formatToMarkdownString(L.t.y91UXV, {
              ...
              case j.zUn.AVAILABLE_TAGS:
                return function(A) {
                  ...
                  if (n.forEach(A => {
                    ...
                    for (let A in i)
                      if (null == l[A]) return new D.ms(j.zUn.AVAILABLE_TAG_ADD, null, AI(i[A]))
                  }
                  if (n.length > r.length) {
                    for (let A in l)
                      if (null == i[A]) return new D.ms(j.zUn.AVAILABLE_TAG_DELETE, null, AI(l[A]))
                  }
                  for (let A in l) {
                    let e = l[A],
                      t = i[A];
