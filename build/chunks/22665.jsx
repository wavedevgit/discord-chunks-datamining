/** Chunk was on web.js **/
/** chunk id: 22665, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Z: () => O
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk913527 = require("./913527.js"),
  l = require.n(Chunk913527),
  Chunk481060 = require("./481060.js"),
  Chunk739566 = require("./739566.js"),
  Chunk318713 = require("./318713.js"),
  Chunk223021 = require("./223021.js"),
  Chunk942951 = require("./942951.js"),
  Chunk594174 = require("./594174.js"),
  Chunk55935 = require("./55935.js"),
  Chunk130883 = require("./130883.js"),
  Chunk388032 = require("./388032.js"),
  Chunk537670 = require("./537670.js"),
  Chunk848697 = require("./848697.js"),
  Chunk328756 = require("./328756.js");
let O = e => {
  let {
    reportingUserId: t,
    guildId: n,
    compact: o,
    channel: s,
    messageId: O,
    reportedTimestamp: v
  } = e, I = p.default.getUser(t), T = i.useMemo(() => o ? (0, f.Z)((0, h.vc)(l()(), "LT")) : null, [o]), S = (0, _.l)({
    user: I,
    channelId: s.id,
    guildId: n,
    messageId: O
  })((0, u.JZ)(I, s)), A = null != I ? I.getAvatarURL(n, (0, c.pxk)(c.EFr.SIZE_16)) : y, N = null != I ? g.intl.format(m.default["+zqXZm"], {
    username: I.username,
    onUserClick: S
  }) : g.intl.string(m.default.xpRjfX);
  return <div className={a()(b.messageSpine, b.repliedMessage, T)}>{<c.qEK src={A} size={c.EFr.SIZE_16} className={E.avatar} aria-label={""} />}{<c.Text variant={"text-sm/medium"} color={"text-secondary"} className={E.username}>{N}</c.Text>}{<c.Text variant={"text-sm/medium"} color={"text-secondary"}>{<span className={E.timestampSeparator}>{"•"}</span>}{<d.Z timestamp={v} className={E.timestamp} />}</c.Text>}</div>
}