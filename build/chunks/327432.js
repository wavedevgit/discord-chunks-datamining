...
var b = function(e) {
    ...
    for (var u of i) {
      ...
      if (!d) break;
      var f = U(u, d);
      if ([a, o] = n ? [o, f] : [f, a], G(a, r.ZWJ) && G(o, r.ExtPict) && !(l = n ? Z(e.substring(0, s)) : Z(e.substring(0, e.length - s))) || G(a, r.RI) && G(o, r.RI) && !(c = null !== c ? !c : !!n || Y(e.substring(0, e.length - s))) || a !== r.None && o !== r.None && F(a, o)) break;
      s += u.length
    }
    return s || 1
  },
          ...
          for (var [j, U] of eg.points(t)) {
            ...
            if (null != t && null != G) t[U] = G;
              ...
              for (var [V, Z] of ei.texts(e))
                if (-1 === el.compare(Z, k)) B = [V, Z];
                else {
                  F = [V, Z];
                  break
                } var H = !1;
              B && F && (H = el.equals(F[1], k) ? !el.hasPrevious(F[1]) : el.common(B[1], k).length < el.common(F[1], k).length), B && !H ? (j.path = B[1], j.offset = B[0].text.length) : F ? (j.path = F[1], j.offset = 0) : t = null
            }
          }
        break;
      case "remove_text":
...
var eY = eH(eH(eH(eH({}, eP), ek), eF), eV)