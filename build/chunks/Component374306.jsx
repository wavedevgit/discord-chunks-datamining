...
function g(e) {
  let {
    ...
    tagFilter: _,
    tagSetting: y
  } = (0, c.H)(t.id), x = (0, i.e7)([o.Z, p.Z], () => !!(o.Z.getThreadIds(t.id, b, _, y).length > 0) || !!(p.Z.getThreads(t.id, b, _, y).length > 0), [t.id, b, _, y]), j = t.isMediaChannel();
  if (!n || m || j && x) return null;
  let O = g ? j ? f.intl.string(f.t["WP/IE1"]) : f.intl.string(f.t.zfq9V4) : j ? f.intl.string(f.t.p60yF1) : f.intl.string(f.t.SNOqYC);
  return (0, r.jsx)(h.JO, {
    tooltip: O,
    icon: l.d3s,
