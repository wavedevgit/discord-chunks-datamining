...
function g(e) {
  let {
    ...
    tagFilter: y,
    tagSetting: x
  } = (0, c.H)(t.id), _ = (0, i.e7)([o.Z, h.Z], () => !!(o.Z.getThreadIds(t.id, b, y, x).length > 0) || !!(h.Z.getThreads(t.id, b, y, x).length > 0), [t.id, b, y, x]), j = t.isMediaChannel();
  if (!n || m || j && _) return null;
  let O = g ? j ? f.intl.string(f.t["WP/IEx"]) : f.intl.string(f.t.zfq9V1) : j ? f.intl.string(f.t.p60yFx) : f.intl.string(f.t.SNOqYG);
  return (0, r.jsx)(p.JO, {
    tooltip: O,
    icon: l.d3s,
