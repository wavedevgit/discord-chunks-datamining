...
let W = 0,
  ...
  eo = e => {
    let {
      ...
      return <G.Z emojiDescriptors={n} emojiSize={s} surrogateCodePoint={es} onInspect={ed} onSelect={c} onSelectSoundmoji={f} getEmojiItemProps={O} getEmojiRowProps={v} isScrolling={A} isUsingKeyboardNavigation={N} rowIndex={e} allowAnimatedEmoji={R} showEmojiFavoriteTooltip={ef} channelGuildId={P} category={i.sectionId} selectedItemClassName={Y.__invalid_selectedItem} channelId={D} messageId={L} isBurstReaction={x} inNitroLockedSection={i.isNitroLocked} handleScrollUpOnSectionCollapse={a} />
    ...
    let ep = i.useMemo(() => l().memoize(e => {
        ...
        let c = s === B.En.GUILD ? null != i ? <j.Z guild={i} height={ee} width={ee} /> : null : null != o ? <k.Z categoryId={o} height={ee} width={ee} size={"custom"} /> : null,
          u = null == i ? true : i.name;
        null != o && (u = (0, M.Y1)(o, null == i ? true : i.name));
        let d = W === e,
          f = n.isNitroLocked,
          p = f && !d,
          h = f && d,
          g = s === B.En.SOUNDMOJI ? <_.P3F className={Y.soundmojiViewMore} onClick={() => {
              (0, b.hr)(F.X1.SOUNDBOARD), w.default.track(Z.rMx.SOUNDMOJI_EMOJI_PICKER_VIEW_ALL_CLICK)
            }}>{H.intl.string(H.t.rUEjBQ)}</_.P3F> : true;
        return <I.Z className={a()(Y.header, V, {
            [Y.inactiveNitroHeader]: p,
            [Y.activeNitroHeader]: h
          })} icon={c} isCollapsed={t.has(l)} onClick={() => eu(l, i)} trailing={g}>{u}</I.Z>
      }), [m, t, eu, V, W]),
      eh = (0, S.B4)(),
      em = i.useCallback((e, n) => {
        let i = m[e],
          o = ec(e),
          s = el(e);
        return <div className={a()(Y.categorySection, {
      ...
      eg = i.useCallback(e => ec(e) ? eh ? <C.fi /> : <C.ZP glowOpacity={et} /> : el(e) ? <div className={Y.nitroBottomDivider} /> : null, [et, ec, el, eh]),
  ...
  ea = Chunk73800.memo(function(e) {
    ...
    } = (0, m.ZP)(), [$, ee] = i.useState(0), [et, en] = i.useState(false), [er, ea] = i.useState(0), {
      listPadding: el,
      renderRow: ec,
      renderSection: eu,
    ...
    return i.useEffect(() => {
      ...
      "" !== K && (ea(0), en(false))
    }, [K]), <div className={Y.listWrapper}>{ey ? <g.Z role={"none presentation"} className={Y.list} listPadding={el} onScroll={eb} renderRow={ec} renderSection={eu} renderSectionHeader={ed} renderSectionFooter={ef} rowCount={S} rowCountBySection={_} rowHeight={ei(a)} sectionHeaderHeight={ep} sectionMarginBottom={e_} sectionFooterHeight={eh} stickyHeaders={true} ref={o} /> : <p.Z message={H.intl.string(H.t.IxxiKC)} className={Y.noSearchResultsContainer} />}{X || c.tq ? null : <N.p showUpsell={et} text={eO()} button={ev()} buttonAnalyticsObject={{
