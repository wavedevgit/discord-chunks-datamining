...
function x(e, t, n, i) {
  ...
  if (!t) {
    ...
    return <circle style={{
        opacity: a
      }} fill={e} r={t} cx={i} cy={i} />
  }
  let s = o.height + 2 * n.stroke,
    l = o.width + 2 * n.stroke,
    c = o.x - n.stroke,
    d = o.y - n.stroke;
  return <rect fill={e} height={s} width={l} style={{
      opacity: a
    }} x={c} y={d} rx={n.stroke} />
}
...
function k(e) {
  let {
    ...
    width: (0, h.px)(n),
    height: (0, h.px)(n)
  }, A = null == p || m ? true : R(p, E, y), N = T.size * g.hs, C = P(E, n, y, O), w = null != v && <svg width={N + I} height={N} viewBox={"0 0 ".concat(N + I, " ").concat(N)} className={b.avatarDecoration} aria-hidden={true}><foreignObject x={0} y={0} width={N} height={N} mask={null != C ? "url(#".concat(C, ")") : true}><div className={b.avatarStack}><img className={b.avatar} src={v} alt={" "} aria-hidden={true} /></div></foreignObject></svg>;
  return null != i || null != o ? <l.P className={a()(b.wrapper, b.pointer, f)} style={S} onClick={i} tabIndex={_} onContextMenu={c} onMouseDown={o} onKeyDown={s} onMouseEnter={null != u ? u : true} onMouseLeave={null != d ? d : true} aria-label={A} aria-hidden={m}>{t}{w}</l.P> : <div className={a()(b.wrapper, f)} style={S} onContextMenu={null != c ? c : true} onMouseEnter={null != u ? u : true} onMouseLeave={null != d ? d : true} role={"img"} aria-label={A} aria-hidden={m}>{t}{w}</div>
}
let j = Chunk73800.forwardRef((e, t) => {
  ...
  return <div ref={t} className={a()(b.avatarStack, o)}>{<img src={null != n ? n : true} alt={" "} className={b.avatar} aria-hidden={true} />}{i && <div className={b.avatarSpeaking} />}</div>
});
...
function Z(e) {
  let {
    ...
    imageClassName: U,
    pulseStatusIcon: Z
  } = e, F = (0, u.vj)(v, S), V = i.useId(), H = i.useId(), [Y] = i.useState(() => O({
    fill: o
  }, M({
    size: E,
    status: n,
    ...
    fill: $
  } = K, ee = T(K, ["avatarCutoutX", "avatarCutoutY", "avatarCutoutWidth", "avatarCutoutHeight", "avatarCutoutRadius", "fill"]), et = (0, h.px)(E), en = (0, h.UC)(E), er = en.status * h.D6, ei = en.status * h.EW, eo = null != v ? (en.status * h.D6 - en.status) / 2 : 0, ea = en.size + Math.ceil(eo), [es, el, ec] = (0, f.q_F)({
    config: {
      tension: 450,
      friction: 20,
      clamp: true
...
let V = Chunk73800.memo(function(e) {
  ...
  let {
    isMobile: a = false,
    isTyping: s = false
  } = o, l = i.useRef(n), c = i.useRef(a), d = (0, u.vj)(n, t), f = i.useRef(d), _ = i.useRef(false), p = _.current || F(s, n, l.current, a, c.current);
  return i.useLayoutEffect(() => {
    _.current = p, l.current = n, c.current = a, f.current = d
  }, [n, a, d, p]), null != n && null != l.current && p ? <Z{...I(O({}, o), {
    status: n,
    statusColor: d,
