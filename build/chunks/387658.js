/** Chunk was on 35755 **/
n.d(t, {
  Z: () => U
}), n(47120);
var i = n(200651),
  l = n(192379),
  r = n(120356),
  o = n.n(r),
  a = n(140835),
  s = n(246521),
  c = n(442837),
  u = n(481060),
  d = n(911969),
  p = n(607070),
  m = n(555573),
  f = n(213459),
  h = n(10718),
  v = n(895924),
  y = n(583027),
  b = n(585483),
  N = n(499254),
  g = n(541099),
  x = n(827498),
  j = n(496158),
  E = n(676161),
  P = n(660090),
  C = n(783097),
  O = n(46332),
  A = n(176412),
  I = n(870205),
  _ = n(981631),
  S = n(689079),
  T = n(388032),
  L = n(672289),
  Z = n(634437);
let R = "placeholder",
  w = [, , , , , ].fill(R);

function M(e) {
  var t, n;
  let {
    context: r,
    command: o,
    section: s,
    sectionName: c
  } = e, d = l.useCallback(() => {
    let e = g.Z.entrypoint();
    N.yT(x.ti.COMMAND), (0, y.Mo)({
      command: o,
      location: v.Vh.APP_LAUNCHER_APPLICATION_VIEW,
      sectionName: c
    }), "channel" === r.type && (m.Po({
      channelId: r.channel.id,
      command: o,
      section: s,
      location: v.Vh.APP_LAUNCHER_APPLICATION_VIEW,
      sectionName: c,
      source: e,
      commandOrigin: v.bB.APPLICATION_LAUNCHER
    }), b.S.dispatch(_.CkL.FOCUS_CHANNEL_TEXT_AREA, {
      channelId: r.channel.id
    }))
  }, [r, o, s, c]), p = (null !== (n = null === (t = o.options) || void 0 === t ? void 0 : t.length) && void 0 !== n ? n : 0) > 0, f = l.useMemo(() => (0, A.ae)(o.displayDescription, void 0), [o.displayDescription]), h = l.useMemo(() => (0, i.jsxs)("div", {
    className: Z.commandTextContainer,
    children: [(0, i.jsx)(u.Text, {
      variant: "text-sm/semibold",
      color: "header-primary",
      children: o.displayName
    }), (0, i.jsx)(u.Text, {
      variant: "text-xs/medium",
      color: "text-muted",
      lineClamp: 1,
      children: f
    })]
  }), [o.displayName, f]);
  return (0, i.jsxs)(u.P3F, {
    className: Z.command,
    onClick: d,
    children: [(0, i.jsx)(u.Rny, {
      className: Z.commandFocusBlock,
      children: h
    }), p ? (0, i.jsx)(a.F, {}) : (0, i.jsx)(D, {
      context: r,
      command: o,
      sectionName: c
    })]
  })
}

function k() {
  let e = (0, c.e7)([p.Z], () => p.Z.useReducedMotion),
    {
      styleLarge: t,
      styleSmall: n
    } = l.useMemo(() => ({
      styleLarge: {
        width: "".concat(10 + 20 * Math.random(), "%"),
        height: "auto"
      },
      styleSmall: {
        width: "".concat(30 + 60 * Math.random(), "%"),
        height: "auto"
      }
    }), []),
    r = l.useMemo(() => (0, i.jsxs)("div", {
      className: Z.commandTextContainerPlaceholder,
      children: [(0, i.jsx)("div", {
        className: L.textPlaceholder,
        style: t,
        children: (0, i.jsx)(u.Text, {
          className: L.hidden,
          variant: "text-sm/semibold",
          color: "header-primary",
          lineClamp: 1,
          children: "_"
        })
      }), (0, i.jsx)("div", {
        className: L.textPlaceholder,
        style: n,
        children: (0, i.jsx)(u.Text, {
          className: L.hidden,
          variant: "text-xs/medium",
          color: "text-muted",
          lineClamp: 1,
          children: "_"
        })
      })]
    }), [t, n]);
  return (0, i.jsx)("div", {
    className: o()(Z.command, L.loadingAnimation, {
      [L.noAnimation]: e
    }),
    children: r
  })
}

function D(e) {
  let {
    context: t,
    command: n,
    sectionName: r
  } = e;
  (0, f.PL)(!0, !0), (0, f.em)(t, !0, !0);
  let o = (0, j.D)(t),
    [a, c] = l.useState(!1),
    d = l.useCallback(async e => {
      if ("channel" !== t.type) return;
      e.stopPropagation();
      let i = g.Z.lastShownEntrypoint();
      try {
        await (0, O.L)({
          applicationId: n.applicationId,
          channel: t.channel,
          sectionName: r,
          location: v.Vh.APP_LAUNCHER_APPLICATION_VIEW,
          entrypoint: i,
          commandIntegrationTypes: n.integration_types
        }) && (await (0, C.Y$)({
          command: n,
          optionValues: {},
          context: o,
          sectionName: r,
          commandOrigin: v.bB.APP_LAUNCHER_APPLICATION_VIEW
        }), N.yT(x.ti.COMMAND))
      } finally {
        c(!1)
      }
    }, [n, t, r, o]);
  return (0, i.jsxs)(u.zxk, {
    type: "submit",
    onClick: d,
    disabled: a,
    size: u.PhG.ICON,
    color: u.zxk.Colors.PRIMARY,
    className: Z.commandSentCTAButton,
    innerClassName: Z.commandSentCTAButtonInner,
    "aria-label": T.NW.formatToPlainString(T.t.UXw6W1, {
      commandName: n.untranslatedName
    }),
    children: [(0, i.jsx)(u.Text, {
      variant: "text-sm/medium",
      color: "currentColor",
      children: T.NW.string(T.t.TXNS7e)
    }), (0, i.jsx)(s.U, {
      size: "xs",
      color: "currentColor"
    })]
  })
}

function W(e) {
  let {
    context: t,
    commands: n,
    section: l,
    headerName: r,
    sectionName: o,
    children: a
  } = e;
  return 0 === n.length ? null : (0, i.jsxs)(i.Fragment, {
    children: [(0, i.jsxs)("div", {
      className: Z.commandListHeader,
      children: [(0, i.jsx)(u.X6q, {
        variant: "heading-sm/semibold",
        children: r
      }), a]
    }), (0, i.jsx)("ul", {
      className: Z.commandContainer,
      "aria-label": r,
      children: n.map((e, n) => e === R ? (0, i.jsx)(k, {}, e + n) : (0, i.jsx)(M, {
        context: t,
        command: e,
        section: l,
        sectionName: o
      }, e.id))
    })]
  })
}

function U(e) {
  var t;
  let {
    context: n,
    application: r,
    sectionName: o,
    installOnDemand: a,
    setHasCommands: s
  } = e, {
    filterSection: c,
    commandsByActiveSection: u,
    sectionDescriptors: p,
    loading: m
  } = h.wi({
    context: n,
    filters: {
      commandTypes: [d.yU.CHAT]
    },
    options: {
      placeholderCount: 0,
      limit: S.tn,
      includeFrecency: !0,
      allowApplicationState: a,
      installOnDemand: a,
      applicationId: r.id
    },
    allowFetch: !0
  }), f = null !== (t = p.find(e => e.id === r.id)) && void 0 !== t ? t : null, {
    sortOrder: v,
    setSortOrder: y,
    commands: b,
    canSort: N
  } = (0, P.Z)({
    sectionId: r.id,
    commandsByActiveSection: u
  });
  l.useEffect(() => {
    c(r.id)
  }, [r.id, c]);
  let g = (0, E.Z)({
    context: n,
    commands: b,
    limit: 5
  });
  return (l.useEffect(() => {
    s(b.length > 0)
  }, [s, b]), m || 0 !== b.length) ? (0, i.jsxs)("ul", {
    className: Z.contentContainer,
    children: [(0, i.jsx)(W, {
      context: n,
      section: f,
      commands: g,
      headerName: T.NW.string(T.t.acSE0t),
      sectionName: o
    }), (0, i.jsx)(W, {
      context: n,
      section: f,
      commands: m ? w : b,
      headerName: T.NW.string(T.t.DUU9Ly),
      sectionName: o,
      children: N && (0, i.jsx)(I.Z, {
        sortOrder: v,
        onSortOptionClick: y
      })
    })]
  }) : null
}