...
function W(e) {
  ...
  return <C.Z><L.Z participant={t} maxVisibleUsers={Y} guildId={n.getGuildId()} channelId={n.id} disableInteraction={i === k.IlC.POPOUT || o === k.AEg.FULL_SCREEN} /></C.Z>
}
...
function K(e) {
  ...
  return null == t || null == i ? null : <r.Fragment>{<c.Z className={G.avatar} size={l.EFr.SIZE_24} user={t} animate={false} />}{<l.Text className={G.playingText} variant={"text-md/medium"} color={o}>{a}</l.Text>}</r.Fragment>
}
...
function z(e) {
  let t, {
    ...
    u = null != s;
  if ((null == i ? true : i.type) === M.fO.ACTIVITY) null != n && (t = <r.Fragment>{<y.ZP.Divider className={G.divider} />}{<m.Z game={n} className={G.activityIcon} />}{<l.Text className={G.playingText} variant={"text-md/normal"} color={"none"}>{(0, d.Z)(n.name)}</l.Text>}{a && <W channel={o} focusedParticipant={i} />}</r.Fragment>);
  else if ((null == i ? true : i.type) === M.fO.STREAM) {
    let e = i.user,
      n = i.stream;
    t = <r.Fragment>{<y.ZP.Divider className={G.divider} />}{<K user={e} channel={o} stream={n} color={a ? "header-primary" : "none"} />}{a && <W channel={o} focusedParticipant={i} />}</r.Fragment>
  } else u && (t = <b.Z guildEvent={s} recurrenceId={c} />);
  return <div className={G.container}>{t}</div>
}
...
function q(e) {
  ...
  let L = (0, s.e7)([_.Z], () => _.Z.getChannelStatus(t)),
    ...
    Z = (0, P.ZP)(t),
    V = k ? <l.ua7 text={U.intl.string(U.t.Mgpxi4)} delay={500} position={"bottom"} shouldShow={Z}>{e => (0, r.jsxs)(l.P3F, H(F({}, e), {
  ...
  return <div className={G.subtitleContainer}>{<l.f6W theme={j.BR.DARK}>{e => (0, r.jsxs)(y.ZP, {
        onDoubleClick: O.O,
        transparent: true,
        showToolbar: !E,
