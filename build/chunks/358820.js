...
function R(e) {
  ...
  if ((null == o ? void 0 : o.status) === y.L.DOWNLOADED) return void c.Z.dispatch(T(I({
    type: "VOICE_FILTER_FILE_READY"
  }, e), {
    fetchedFromNetwork: !1,
    analyticsContext: t
  }));
  (null == o ? void 0 : o.status) !== y.L.DOWNLOADING && (c.Z.dispatch(I({
    type: "VOICE_FILTER_DOWNLOAD_STARTED"
  }, e)), m.ZP.downloadVoiceFilterFile(n, i, t => {
    let {
      downloadedBytes: n,
      totalBytes: r
    } = t;
    ...
    if (null == t ? void 0 : t.USER_CANCELED_DOWNLOAD) A.info("User canceled the download for Voice Filter dependency", e);
      ...
      A.error(n, I({
        reason: t
      }, e)), f.default.track(O.rMx.VOICE_FILTER_ERROR, {
        error_message: n,
        cause: (0, _.X)(Error(t))
      }), h.Z.captureException(Error(n, {
        cause: t
      }), {
        tags: {
          modelId: r
...
async function w(e) {
  ...
  try {
    ...
    A.error("failed to set voice filter", t), c.Z.dispatch({
...
async function D(e) {
  ...
  await e.setCatalog(r, o), await c.Z.dispatch({
...
async function x() {
  ...
  if (!P) try {
    ...
    A.warn("Failed to refresh voice filters catalog: ".concat(e.message)), f.default.track(O.rMx.VOICE_FILTER_ERROR, {
      error_message: "Failed to refresh voice filters catalog",
      cause: (0, _.X)(e)
    }), h.Z.captureException(e), await c.Z.dispatch({
  ...
  if (!(b.Z.isNativeModuleLoaded() || b.Z.isNativeModuleLoading()) && !__OVERLAY__) {
    ...
    try {
      ...
      d.Z.getMediaEngine().on(l.aB.VoiceFiltersFailed, e => {
        A.warn("Voice Filters failed in process: ".concat(e)), f.default.track(O.rMx.VOICE_FILTER_ERROR, {
          error_message: "Voice Filters failed in process",
          cause: (0, _.X)(Error(e))
        }), h.Z.captureException(Error("Voice Filters failed in process", {
          cause: e
        }))
      })
    } catch (e) {
      A.warn("Failed to load Voice Filters module: ".concat(e.message)), f.default.track(O.rMx.VOICE_FILTER_ERROR, {
        error_message: "Failed to load Voice Filters module",
        cause: (0, _.X)(e)
      }), h.Z.captureException(e), c.Z.dispatch({
        type: "VOICE_FILTER_NATIVE_MODULE_STATE_CHANGE",
        state: y.O.FAILED
      })
    }
