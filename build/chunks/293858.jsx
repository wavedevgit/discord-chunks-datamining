...
function b(e) {
  ...
  }, [K, W]), r = null != y ? y(O, X, M) : null != E ? <c.VY /> : B ? <c.TB planId={O.id} onClose={X} /> : A.current === O.id ? <c.ZP planId={O.id} postSuccessGuild={S} onClose={X} paymentSourceType={J} hideClose={Y} startingFractionalPremiumEndsAt={R.current} /> : <c.ZP followupSKUInfo={v} startingPremiumSubscriptionPlanId={A.current} planId={O.id} onClose={X} isDowngrade={null != I && (0, u.GY)(I, O.id, o)} paymentSourceType={J} hideClose={Y} startingFractionalPremiumEndsAt={R.current} />, <t.Fragment>{<m.C3>{<L.Z />}{r}</m.C3>}{null != z && z}{Q && null != q && <h.Z onClose={X} selectedPromotionalDecoPurchaseRecord={q} selectedGiftingPromotionReward={D} />}</t.Fragment>
}