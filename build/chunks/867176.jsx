...
function C(e) {
  ...
  return <svg className={y.mask} viewBox={"0 0 ".concat(C, " ").concat(R)} style={{
      minWidth: C,
      minHeight: R
    }}>{<mask id={L}>{<rect fill={"white"} x={"0"} y={"0"} width={"100%"} height={"100%"} />}{<circle fill={"black"} cx={Y} cy={W} r={H} />}</mask>}{<foreignObject x={"0"} y={"0"} width={"100%"} height={"100%"} overflow={"visible"} mask={"url(#".concat(L, ")")}>{v}{<div className={a()(y.banner, I)} onMouseMove={() => M(true)} onMouseLeave={() => M(false)} style={{
          height: R,
          minHeight: R,
          backgroundImage: null != U ? "url(".concat(U, ")") : true,
          backgroundColor: "COMPLETE" !== G ? c.Z.unsafe_rawColors.PRIMARY_800.css : V
        }}>{!j && (0, g.F8)(U) && <p.Z className={y.gifTag} />}{v}</div>}</foreignObject>}</svg>
}
