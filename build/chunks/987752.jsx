...
function j(e) {
  ...
  return <td className={x.statColumn}>{t}</td>
}
...
function N(e) {
  ...
  return <tr className={x.statRow}>{<j><img className={x.statIcon} src={n} alt={""} /></j>}{<j><p.Z variant={"text-xs/medium"} className={x.statName}>{t}</p.Z></j>}{<j><div className={x.statValue}>{<img className={x.pointsIcon} src={v} alt={""} />}{<c.Text className={x.monospace} variant={"text-xs/medium"}>{(0, u.v)(Math.floor(s))}</c.Text>}</div></j>}{<j><c.Text className={a()(x.statValuePerSecond, x.monospace)} variant={"text-xs/medium"}>{h.intl.format(i, {
          points: Math.abs(o),
...
function P(e) {
  ...
  return <N name={o.name} valueTotal={s} valuePerSecond={a} iconSrc={o.purchaseIconSrc} />
}
...
function _(e) {
  ...
  return <c.f6W theme={b.BR.DARK} disableAdaptiveTheme={true}>{e => (0, r.jsxs)(c.Y0X, {
      className: e,
      transitionState: t,
...
function C(e) {
  ...
  return <m.O iconSrc={O} title={h.intl.string(y.default.bS8cBA)} onClick={() => {
