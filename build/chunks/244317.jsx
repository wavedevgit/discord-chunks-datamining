...
function f(e) {
  ...
  }, [t, _]), _ && false !== m) ? true === m ? <div className={p.container}>{<a.X6q variant={"heading-xxl/bold"}>{d.intl.string(d.t.csrAMD)}</a.X6q>}{<a.Text variant={"text-xs/semibold"}>{d.intl.string(d.t["m1+IBg"])}</a.Text>}{<a.zxk variant={"primary"} text={d.intl.string(d.t.fIv16O)} onClick={() => h(true)} />}</div> : <div className={p.container}>{<a.X6q variant={"heading-xxl/bold"}>{d.intl.string(d.t["Z+hCVV"])}</a.X6q>}{<a.$jN />}</div> : <s.Z{...function(e) {
