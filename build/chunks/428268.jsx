...
let v = function(e) {
  ...
  function x() {
    if (y) return null;
    let e = N ? <s.d3s className={g.infoMessageIcon} /> : <s.aNP color={o.Z.colors.STATUS_WARNING} className={g.infoMessageIcon} />,
      t = N ? m.intl.string(m.t["Ihg/Dg"]) : m.intl.string(m.t["J8U+Iy"]);
    return <div className={g.updateMessage}>{e}{<s.Text variant={"text-xs/medium"} color={"text-muted"}>{t}</s.Text>}</div>
  }
  return <r.Fragment>{<u.Z className={g.divider} />}{x()}{<a.zx className={g.joinLeaderboardButton} innerClassName={g.joinLeaderboardButtonInner} onClick={D}>{S && !y && <img alt={"Riot Games"} src={A.icon.whiteSVG} className={g.riotIcon} />}{L()}</a.zx>}</r.Fragment>
}