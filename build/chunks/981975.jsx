...
function P(e) {
  ...
  return <div className={N.headerNoConfigContainer}>{<d.X6q className={N.headerHeading} variant={"eyebrow"}>{C.intl.string(C.t.nMir29)}</d.X6q>}{<d.Text variant={"text-sm/normal"} color={"header-secondary"} className={N.headerNoConfigSubtitle}>{C.intl.format(C.t.q5f7tL, {
        helpdeskArticleUrl: p.Z.getArticleURL(O.BhN.CONNECTION_DETAILS_ADMIN)
      })}</d.Text>}{<d.zxk disabled={n} onClick={t} text={C.intl.string(C.t["OSvW5+"])} fullWidth={true} variant={"primary"} />}</div>
}
...
function w(e) {
  ...
  let B = null;
  if (0 === L.length) B = <P handleAddVerificationClicked={U} locked={l} />;
  else if (L.length > 0) {
    var F;
    let e = null;
    L.length < 10 && (e = <d.zxk disabled={l} onClick={U} text={C.intl.string(C.t["OSvW5+"])} icon={d.oFk} fullWidth={true} variant={"secondary"} />), B = <r.Fragment>{F = () => (0, f.d_)(n.id, []), <r.Fragment>{<div className={N.headerTitleContainer}>{<div className={N.headerTitleTextContainer}><d.X6q variant={"eyebrow"}>{C.intl.string(C.t.nMir29)}</d.X6q></div>}{<d.Avr variant={"critical"} onClick={F} disabled={l} text={C.intl.string(C.t.ntW1cX)} textVariant={"text-sm/semibold"} />}</div>}{<d.Text variant={"text-sm/normal"}>{C.intl.format(C.t.q5f7tL, {
            helpdeskArticleUrl: p.Z.getArticleURL(O.BhN.CONNECTION_DETAILS_ADMIN)
          })}</d.Text>}</r.Fragment>}{function(e, t, n, i, l) {
        let a = T(i, m.O1.OR);
      ...
      }(L, e => (0, f.d_)(n.id, T(e, k)), l, n.id, o)}{e}</r.Fragment>
  }
  return <d.yWw className={N.scroller} style={{
      scrollPaddingTop: g
    }} onScroll={Z}><div className={I.contentWidth}>{<div className={a()(I.header, I.stickyHeader, {
          [I.stickyHeaderElevated]: !R
        })} ref={w}><v.Z guild={t} role={n} selectedSection={_.ZI.VERIFICATIONS} setSelectedSection={s} /></div>}{(null != D ? D : 0) > 0 ? <div className={N.warningContainer}>{<d.Mgn size={"custom"} width={20} height={20} color={"currentColor"} className={N.warningIcon} />}{<d.Text variant={"text-sm/normal"} color={"text-default"}>{C.intl.string(C.t["2aFeeX"])}</d.Text>}</div> : null}{B}</div></d.yWw>
}