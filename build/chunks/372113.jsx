...
function j(e) {
  ...
  return "xs" === t ? <c.P3F tabIndex={o} className={w.learnMoreLink} tag={"span"} onClick={n}><c.Text variant={"text-sm/medium"} color={"text-link"}>{i}</c.Text></c.P3F> : <l.zx tabIndex={o} wrapperClassName={w.ctaButtonWrapper} color={l.Tt.PRIMARY} onClick={n}>{i}</l.zx>
}
...
function G(e) {
  ...
  return <c.ua7 text={O.tooltipText} tooltipContentClassName={w.ctaTooltipText}>{e => {
      var t;
...
let B = e => {
  ...
  }, ei = (0, s.e7)([d.default], () => d.default.locale), eo = (0, m.z)(n), ea = W && Y, es = _ && !V, el = Q.length > 0 && W && (0, h.$J)(n) && F && !V && !eo, ec = <v.Z autoplay={f} className={a()(w.gridImg, {
      [w.questRewardGiftInventory]: W && "lg" === u,
      [w.questRewardEmbed]: K && "lg" === u,
      [w.questRewardEmbedSm]: "sm" === u,
      [w.questRewardEmbedXs]: "xs" === u
    })} learnMoreStyle={W ? null : "text"} location={C.dr.QUESTS_CARD} quest={n} questContent={o} questContentPosition={D} sourceQuestContent={x} />, eu = () => _ && !V ? null : <G quest={n} progressState={B} isCollectibleQuest={Y} questContent={o} inGiftInventory={W} sourceQuestContent={x} />;
  return <div className={w.root}>{<div className={a()(w.outerContainer, {
        [w.outerContainerSm]: "sm" === u,
        [w.outerContainerXs]: "xs" === u,
        [w.outerContainerNoProgress]: !q
      })} style={{
        visibility: z ? "inherit" : "hidden"
      }} aria-hidden={!z}>{<c.ua7 text={es ? P.intl.string(P.t["04MTGR"]) : null} tooltipContentClassName={w.rewardTileExpirationTooltip} shouldShow={es}>{e => (0, r.jsxs)("div", M(L({
        ...
        }))}</c.ua7>}{<div className={a()(w.gridText, w.taskDetails)}>{<c.Text variant={k(o, u, F)} className={w.taskInstructions}>{_ ? P.intl.formatToPlainString(P.t["ge+AJi"], {
            questName: n.config.messages.questName
          }) : X}</c.Text>}{<c.Text variant={"lg" === u ? "text-sm/medium" : "text-xs/medium"} color={"text-muted"}>{U({
            quest: n,
            locale: ei,
            isQuestExpired: _,
            sharedQuestFields: $,
          ...
          })}</c.Text>}</div>}{en ? <div className={a()(w.ctaButtonContainer, w.gridCtaButtons)}>{<l.zx color={l.Tt.PRIMARY} disabled={true} className={w.questEnrollmentBlockedButton}>{P.intl.string(P.t.V293qq)}</l.zx>}{<l.zx color={l.Tt.BRAND} onClick={() => (0, I.openQuestMinorEnrollmentBlockModal)(n, o, x)} className={w.questEnrollmentBlockedButton}>{P.intl.string(P.t.vY9GgI)}</l.zx>}</div> : <div className={a()(w.ctaButtonContainer, w.gridCtaButtons)}>{!W && <j containerSize={u} onClick={er}>{P.intl.string(P.t.LLLLPD)}</j>}{eu()}</div>}{q && <O.Z className={w.gridProgressBar} color={V ? c.TVs.colors.TEXT_FEEDBACK_POSITIVE : c.TVs.colors.BG_BRAND} quest={n} isInventory={W} />}</div>}{el && <div className={w.microphoneContainer}>{<div className={w.separator} />}{<S.Z quest={n} location={o} sourceQuestContent={x} />}</div>}</div>
}