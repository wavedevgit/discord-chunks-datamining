...
class Z extends Chunk73800.PureComponent {
  ...
  renderSpinner(e) {
    return <p.ZP>{<p.Dx>{e}</p.Dx>}{<p.Hh />}</p.ZP>
  }
  renderExpiredInvite() {
    let {
      defaultRoute: e,
    ...
    return <Chunk388905.ZP>{<Chunk388905.Ee src={require("./167969.js")} className={Chunk20493.marginBottom8} />}{<Chunk388905.Dx className={o()(Chunk20493.marginTop8, Chunk20493.marginBottom8)}>{Chunk388032.intl.string(Chunk388032.t.KPowgo)}</Chunk388905.Dx>}{<Chunk388905.DK>{Chunk388032.intl.string(Chunk388032.t.j8734e)}</Chunk388905.DK>}{<Chunk388905.zx className={o()(Chunk20493.marginTop40, Chunk20493.marginBottom8)} onClick={() => exports(module)}>{Chunk388032.intl.string(Chunk388032.t.fIv16O)}</Chunk388905.zx>}{<Chunk388905.zx onClick={() => window.open(Chunk63063.Z.getArticleURL(Chunk981631.BhN.GIFTING), "_blank")} look={Chunk388905.zx.Looks.LINK} color={Chunk388905.zx.Colors.LINK}>{Chunk388032.intl.string(Chunk388032.t["/CjuXF"])}</Chunk388905.zx>}</Chunk388905.ZP>
  }
  renderAppOpened() {
    return <Chunk388905.ZP>{<Chunk388905.Dx className={Chunk20493.marginBottom8}>{Chunk388032.intl.string(Chunk388032.t.csrAMD)}</Chunk388905.Dx>}{<Chunk388905.DK>{Chunk388032.intl.string(Chunk388032.t["m1+IBg"])}</Chunk388905.DK>}{<Chunk388905.zx className={Chunk20493.marginTop40} onClick={() => this.setState({
          continueOnWeb: true
        })}>{Chunk388032.intl.string(Chunk388032.t["qsI+EB"])}</Chunk388905.zx>}</Chunk388905.ZP>
  }
  renderVerification(e) {
    let {
      sentVerification: t
    } = this.state;
    return <p.ZP>{<p.Ee src={n(892235)} className={A.marginBottom8} />}{<p.Dx>{C.intl.format(C.t.ivLUf3, {
          username: e.username
        })}</p.Dx>}{<p.DK className={A.marginTop20}>{C.intl.string(C.t["8Su18/"])}</p.DK>}{<p.zx disabled={t} className={A.marginTop40} onClick={this.handleResendVerification}>{t ? C.intl.string(C.t.CMa9Rk) : C.intl.string(C.t.lm1UKi)}</p.zx>}{<p.zx look={p.zx.Looks.LINK} color={p.zx.Colors.LINK} onClick={this.refreshUser} className={A.marginTop8}>{C.intl.string(C.t.Po9eBQ)}</p.zx>}</p.ZP>
  }
  renderAuthenticated(e, t) {
    let n = this.getErrorMessage(e);
    return <p.ZP>{<O.Z giftCode={e} />}{<p.zx disabled={null != n} className={A.marginTop40} onClick={this.handleAccept}>{C.intl.string(C.t.n6I6k5)}</p.zx>}{null != n ? <p.DK className={A.marginTop20}>{n}</p.DK> : <p.i_ className={A.marginTop20}>{C.intl.format(C.t.NYM08v, {
  ...
  render() {
    ...
    if (null == Chunk73800) return Chunk120356 ? this.renderExpiredInvite() : this.renderSpinner(Chunk388032.intl.string(Chunk388032.t.b3lf1d));
    if (Chunk120356) {
      if (require) {
        let e = this.state.currentUser;
        return Chunk893776 || null == module ? this.renderSpinner(Chunk388032.intl.string(Chunk388032.t.bYb2nZ)) : this.requiresVerification && null != module ? this.renderVerification(module) : this.renderAuthenticated(Chunk73800, module)
      }
      return "login" === this.getMode() ? <Chunk781428.Z giftCodeSKU={exports} giftCode={Chunk73800} transitionTo={Chunk442837} location={Chunk570140} /> : <Chunk423527.Z giftCodeSKU={exports} giftCode={Chunk73800} transitionTo={Chunk442837} location={Chunk570140} />
    }
    return null
  }
  constructor(...e) {
...
let P = Chunk442837.ZP.connectStores([Chunk82142.Z, Chunk283595.Z, Chunk314897.default, Chunk55563.Z, Chunk896797.Z, Chunk362762.Z], e => {
  ...
  return {
    giftCode: n,
    sku: r,
    libraryApplication: null != r && (null == n ? true : n.entitlementBranches) != null ? v.z2(n.entitlementBranches, r, b.Z) : null,
