...
let E = e => <div className={e.className}>{e.children}</div>;
class O extends(n = Chunk73800.PureComponent) {
  ...
  renderHeader() {
    let {
      ...
      return <s.P3F className={l()(d.headerCell, a, e.headerCellClassName, e.cellClassName, n === e.key && u, {
          [d.clickable]: this.isSortable(e)
        })} onClick={this.isSortable(e) ? () => this.handleSort(e.key) : true}><div className={d.headerCellContent}>{o}{this.isSortable(e) && n === e.key ? <_ className={d.sortIcon} /> : null}</div></s.P3F>
    });
    return <E className={l()(Chunk120356, Chunk483849.row, {
  ...
  render() {
    ...
    return <div className={module}>{exports ? this.renderHeader() : null}{this.renderBody()}</div>
  }
  constructor(...e) {
...
u(O, "SortDirection", Chunk981631.sHY), u(O, "defaultProps", {
  ...
  rowComponent: e => <div className={e.className}>{e.children}</div>,
  cellComponent: e => <div className={e.className}>{e.children}</div>,
  hasHeader: true,
  sortData: true,
  stickyHeader: false
});
