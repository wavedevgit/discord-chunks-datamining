...
function Z(t) {
  ...
  return <i.Fragment>{null != r || null != l ? <o.Z className={g.emoji} emojiId={l} emojiName={r} animated={!!(null == d ? true : d.animated)} /> : null}{e}</i.Fragment>
}
...
function p(t) {
  ...
  let C = t => {
    ...
    m = null == s ? true : s.map(t => {
      let n = g.includes(t);
      return <l.S89 id={t.id} label={(0, i.jsx)(Z, {
          tag: t
        })} disabled={p && !n} action={() => C(t)} checked={n} />
    });
  return <l.sNh id={"edit-tags"} label={v.intl.string(v.t["436ZFx"])}>{m}</l.sNh>
}