...
function ee(e) {
  ...
  return i.useEffect(() => {
    ...
    if (I && "leaf_fall" === f) {
  ...
  }, [I, y, h, f]), <r.Fragment>{<s.Ji ref={b} sprites={q} colors={W} spriteWidth={Y} spriteHeight={Y} />}{<d.P3F onClick={n} className={a()(v.easterEggAnimationClickTarget, {
      ...
      })}><d.kci animationRef={N} className={a()(v.easterEggAnimation, {
          [v.easterEggAnimationHideLeaf]: T
        })} nextScene={O} sceneSegments={C} onScenePlay={S} onSceneComplete={A} importData={X} pauseWhileUnfocused={false} /></d.P3F>}</r.Fragment>
}
  ...
  let L = i.useCallback(() => {
      if (!R.enabled)
        if (S || 0 !== Math.floor(Math.random() * N)) {
          var e;
          let t = null == (e = C.current) ? true : e.getBoundingClientRect();
          if (null == t) return;
    ...
    k = i.useCallback((e, t) => {
      (null == t ? true : t.id.startsWith(F)) && M()
    }, [M]);
  i.useEffect(() => T(k));
  let j = <d.P3F className={v.iconWrapper} innerRef={C} onClick={l}><d.$Eu color={d.TVs.unsafe_rawColors.GUILD_BOOSTING_PINK} className={v.icon} onMouseEnter={L} /></d.P3F>;
  return <b.Z iconNode={j} timestamp={n.timestamp} compact={o}>{<div onClick={l} className={v.message}>{t}</div>}{S ? <p.ZP><div className={v.cannonWrapper}><ee onAnimationComplete={x} onClick={M} /></div></p.ZP> : null}</b.Z>
}