...
function B(e) {
  ...
  return (0, b.Eu)(t, n.id) ? <l.ua7 text={w.intl.string(w.t.HFwRpq)} position={"bottom"} color={l.FGA.GREY}>{e => (0, r.jsx)("div", k(x({}, e), {
      className: D.interactive,
      children: (0, r.jsx)(l.Mgn, {
        color: l.TVs.colors.STATUS_WARNING_BACKGROUND
      })
    }))}</l.ua7> : <C.Z userId={n.user.id} />
}
...
function Z(e) {
  ...
  let eu = false;
  return o && !J && !k && q && !b && null != V && v.Z.supports(P.AN.VIDEO) ? eu && !Z ? <A.Z selected={b} width={l} /> : null != ea && null == es ? <N.Z avError={ea} userId={t.id} width={l} selected={b} /> : <m.Z onResize={L} wrapperClassName={D.videoWrapper} className={D.content} mirror={z} streamId={W} videoComponent={V} fit={C} videoSpinnerContext={Y.id === H ? u.m.SELF_VIDEO : u.m.REMOTE_VIDEO} paused={F} userId={Y.id} /> : <div className={a()(D.content, {
      [D.blockedAvatar]: x || M
    })}><h.Z aria-label={ee} src={er} avatarDecoration={ei} backgroundSrc={Y.getAvatarURL(n.guild_id, 80)} size={en} pulseSpeakingIndicator={B} speaking={K} userId={Y.id} /></div>
}