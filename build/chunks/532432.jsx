...
function E(e) {
  ...
  return <g.Z className={v} title={E} titleIcon={w ? (0, i.jsx)(c.IGR, {
      text: x.intl.string(x.t.y2b7CA),
      className: _.newBadge
    }) : true} description={P ? x.intl.format(x.t.U4tZfX, {
      recentAvatarsLimit: b.iZ,
      onClick: k
    }) : true} errors={n} disabled={S} forcedDivider={I}><div className={_.buttonsContainer}>{<A className={a()({
          [_.buttonHighlighted]: N
        })} size={o.zx.Sizes.SMALL} onClick={k}>{null != C ? C : x.intl.string(x.t["4OynCA"])}</A>}{t && <div data-button-hoisted-classname-wrapper={true} className={_.removeButton}><c.zxk variant={"secondary"} size={"sm"} text={null != O ? x.intl.string(x.t.TDjKDg) : x.intl.string(x.t.twB3f3)} onClick={() => s(null)} /></div>}</div></g.Z>
}