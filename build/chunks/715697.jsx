/** Chunk was on web.js **/
/** chunk id: 715697, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Z: () => _
}), require("./35282.js");
var Chunk255367 = require("./255367.js");
require("./73800.js");
var Chunk120356 = require("./120356.js"),
  o = require.n(Chunk120356),
  Chunk729594 = require("./729594.js"),
  Chunk442837 = require("./442837.js"),
  Chunk780384 = require("./780384.js"),
  Chunk210887 = require("./210887.js"),
  Chunk635477 = require("./635477.js"),
  Chunk902692 = require("./902692.js"),
  Chunk94488 = require("./94488.js");

function _(e) {
  var t, n;
  let {
    className: i,
    embed: {
      url: _
    }
  } = e, p = (0, s.e7)([c.Z], () => (0, l.wj)(c.Z.theme));
  if (null == _) return null;
  let h = null,
    m = null,
    g = null;
  try {
    let e = a.parse(_, true);
    h = e.host, m = e.pathname, g = null != (n = e.query.i) ? n : null, Array.isArray(g) && (g = g[0])
  } catch (e) {
    return null
  }
  if (!(0, u.L)(h) || null == m) return null;
  let E = /^\/([a-z]{2}(?:-[a-z]{2})?)\//.test(m) ? 2 : 1,
    b = null == (t = m.split("/")[E]) ? true : t.toLowerCase();
  null != g && (b = "song");
  let y = 450;
  "song" === b ? y = 175 : "music-video" === b && (y = 371), m = m.substr(1);
  let O = p ? "dark" : "light";
  return <iframe className={o()(f.embedAppleMusic, i)} src={d.P.EMBED(m, O, g)} style={{
      maxWidth: 660,
      minWidth: 300,
      width: "100%",
      height: y
    }} frameBorder={0} sandbox={"allow-forms allow-popups allow-same-origin allow-scripts allow-storage-access-by-user-activation"} allow={"encrypted-media *; fullscreen *; clipboard-write"} />
}