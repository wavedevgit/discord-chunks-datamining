/** Chunk was on web.js **/
/** chunk id: 461797, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Wt: () => c,
  jB: () => l
});
var Chunk508425 = require("./508425.js"),
  Chunk559949 = require("./559949.js"),
  Chunk985018 = require("./985018.jsx");
let s = "1144058522808614923",
  o = "1144058844004233369",
  l = {
    above_the_clouds: {
      preset: "above_the_clouds",
      themeColors: [752280, 9215590],
      avatarDecorationSkuId: "1144059132517826601",
      displayNameStyles: {
        fontId: Chunk559949.x.CHICLE,
        effectId: Chunk508425.z.POP,
        colors: [959694]
      },
      getName: () => a.intl.string(a.t["TFc+iF"]),
      getHeaderSrc: () => n(702051).A,
      getBannerSrc: e => e ? n(735161).A : n(887e3).A
    },
    cyberpunk: {
      preset: "cyberpunk",
      themeColors: [1967991, 742532],
      avatarDecorationSkuId: null,
      displayNameStyles: {
        fontId: Chunk559949.x.PIXELIFY,
        effectId: Chunk508425.z.NEON,
        colors: [2949343]
      },
      getName: () => a.intl.string(a.t["4g+5bq"]),
      getHeaderSrc: () => n(272017).A,
      getBannerSrc: e => e ? n(735271).A : n(208574).A
    },
    shadow_realm: {
      preset: "shadow_realm",
      themeColors: [0, 4458504],
      avatarDecorationSkuId: s,
      displayNameStyles: {
        fontId: Chunk559949.x.NEO_CASTEL,
        effectId: Chunk508425.z.TOON,
        colors: [0xff0000]
      },
      getName: () => a.intl.string(a.t.ycg1xj),
      getHeaderSrc: () => n(863653).A,
      getBannerSrc: e => e ? n(538263).A : n(576974).A
    },
    starlit_dream: {
      preset: "starlit_dream",
      themeColors: [5123751, 590625],
      avatarDecorationSkuId: o,
      displayNameStyles: {
        fontId: Chunk559949.x.CHERRY_BOMB,
        effectId: Chunk508425.z.TOON,
        colors: [3343795]
      },
      getName: () => a.intl.string(a.t["9WLHvr"]),
      getHeaderSrc: () => n(434361).A,
      getBannerSrc: e => e ? n(700309).A : n(135572).A
    },
    neon_space: {
      preset: "neon_space",
      themeColors: [6094952, 1007678],
      avatarDecorationSkuId: null,
      displayNameStyles: {
        fontId: Chunk559949.x.MUSEO_MODERNO,
        effectId: Chunk508425.z.NEON,
        colors: [28737]
      },
      getName: () => a.intl.string(a.t.UdNuqi),
      getHeaderSrc: () => n(301928).A,
      getBannerSrc: e => e ? n(285974).A : n(775627).A
    }
  },
  c = e => l[e]