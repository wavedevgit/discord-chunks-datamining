...
async function _(e, t) {
  ...
  let j = d.Z.getGuildIds().filter(g).map(e => i.ZP.getUsableGuildEmoji(e)).flat().filter(e => null == p.ZP.getEmojiUnavailableReason({
    emoji: e,
    channel: t,
    guildId: b,
    intention: h.Hz.CHAT
  ...
  if (j.length > 10) {
    let n = j[Math.floor(Math.pow(Math.random(), 2) * j.length)];
