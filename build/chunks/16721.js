...
(y || v || O || Chunk923646 || Chunk204954) && (h = function(e) {
  ...
  var A = I.groups,
    ...
    R = 0,
    D = T;
  if (N && (false === g(P = E(P, "y", ""), "g") && (P += "g"), D = b(T, S.lastIndex), S.lastIndex > 0 && (!S.multiline || S.multiline && "\n" !== m(T, S.lastIndex - 1)) && (w = "(?: " + w + ")", D = " " + D, R++), n = RegExp("^(?:" + w + ")", P)), v && (n = RegExp("^" + w + "$(?!\\s)", P)), y && (i = S.lastIndex), s = r(_, N ? n : S, D), N ? s ? (s.input = b(s.input, R), s[0] = b(s[0], R), s.index = S.lastIndex, S.lastIndex += s[0].length) : S.lastIndex = 0 : y && s && (S.lastIndex = S.global ? s.index + s[0].length : i), v && s && s.length > 1 && r(p, s[0], n, function() {
