...
let m = (e, t, n, r) => {
  ...
  g = function(e) {
    ...
    let D = (0, o.K)({
      ...
      M = "",
      k = "";
    if (y) M = p.NW.format(p.t.rr811N, {
      paidURL: f.EYA.PAID_TERMS
    });
    else if (O) {
      if (null != s && (null == I ? void 0 : I.subscriptionPeriodEnd) != null) M = p.NW.format(p.t["2VPTa2"], {
        subtotalRate: g
      });
      else switch (t) {
        case _.rV.MONTH:
          M = 1 === n ? p.NW.format(p.t["+ywPmZ"], {}) : p.NW.format(p.t.oBwZfn, {
            intervalCount: n
          });
          break;
        case _.rV.YEAR:
      ...
      case f.POd.GUILD_PRODUCT:
        k = L, M = p.NW.format(p.t["GEAQ+v"], {
          paidURL: f.EYA.PAID_TERMS
        });
        break;
      default:
        k = x, M = p.NW.string(p.t["9/siSU"])
    } else if (null == E || b) switch (b && (k = L), t) {
      case _.rV.MONTH:
        M = b ? p.NW.string(p.t.IjNapq) : p.NW.string(p.t["/sGXPj"]), M = b ? p.NW.string(p.t.IjNapq) : 1 === n ? p.NW.string(p.t["/sGXPj"]) : p.NW.formatToPlainString(p.t.Fqjiho, {
          intervalCount: n
        });
        break;
      case _.rV.YEAR:
      ...
      v || (e = m(C, t, n, E.id)), M = (null == R ? void 0 : R.isPausedAllowsResumeButNotUpdates) ? p.NW.format(p.t.B6oNwM, {
        primaryText: r,
        rate: e,
        paidURL: f.EYA.PAID_TERMS,
        contactLink: f.EYA.CONTACT,
        helpdeskArticle: l.Z.getArticleURL(f.BhN.BILLING)
      }) : null != R && (0, d.GY)(R, E.id, w) ? p.NW.format(p.t.LyBQUV, {
        primaryText: r,
        rate: e,
        paidURL: f.EYA.PAID_TERMS,
        contactLink: f.EYA.CONTACT,
