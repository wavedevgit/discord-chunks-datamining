...
function j(e) {
  ...
  return <_.Z targetElementRef={d} user={j} guildId={k} channelId={O.id} position={"top"} clickTrap={true}>{e => (0, r.jsx)(s.ua7, {
      text: B,
...
function x(e) {
  ...
  }(t, f, g), _ = c ? s.V_R : s.qJs, E = <s.M0o onClick={() => u(!c)} color={s.YX$.PRIMARY} size={s.tT7.SIZE_24} tooltip={c ? I.intl.string(I.t["Z/Tya2"]) : I.intl.string(I.t.XR5BAQ)} icon={(0, r.jsx)(_, {
      size: "xs",
      color: "currentColor"
    })} focusProps={T} />, y = b && c ? t : m;
  return y.length <= 0 ? null : <s.zJl className={a()(C.scroller, l)} fade={true}><div ref={h} className={a()(C.voiceUsers, !c && C.collapsed)} role={"group"} aria-label={I.intl.string(I.t["/ZgaVV"])}>{y.map(e => {
        let {
          user: t,
          nick: i,
          voiceState: l
        } = e;
        return t.id !== f ? <j channel={n} user={t} nick={i} mute={l.isVoiceMuted()} deaf={l.isVoiceDeafened()} video={l.selfVideo} serverMute={l.mute} serverDeaf={l.deaf} /> : null
      })}{b ? E : null}</div></s.zJl>
}