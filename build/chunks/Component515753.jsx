/** Chunk was on web.js **/
/** chunk id: 515753, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  Qj: () => em,
  RS: () => eg,
  ZP: () => eE,
  bU: () => ef
}), require("./388685.js");
var Chunk951288 = require("./951288.js"),
  Chunk647438 = require("./647438.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk512722 = require("./512722.js"),
  l = require.n(Chunk512722),
  Chunk799899 = require("./799899.js"),
  Chunk990547 = require("./990547.js"),
  Chunk91192 = require("./91192.jsx"),
  Chunk873546 = require("./873546.js"),
  Chunk893999 = require("./893999.js"),
  Chunk442837 = require("./442837.js"),
  Chunk481060 = require("./481060.js"),
  Chunk493683 = require("./493683.js"),
  Chunk239091 = require("./239091.js"),
  Chunk420660 = require("./420660.js"),
  Chunk468363 = require("./468363.js"),
  Chunk326255 = require("./326255.js"),
  Chunk956221 = require("./956221.js"),
  Chunk771173 = require("./771173.jsx"),
  Chunk385499 = require("./385499.jsx"),
  Chunk570908 = require("./570908.jsx"),
  Chunk702321 = require("./702321.js"),
  Chunk201895 = require("./201895.js"),
  Chunk43267 = require("./43267.js"),
  Chunk933557 = require("./933557.js"),
  Chunk163889 = require("./163889.jsx"),
  Chunk111028 = require("./111028.jsx"),
  Chunk181430 = require("./181430.js"),
  Chunk821795 = require("./821795.js"),
  Chunk892567 = require("./892567.jsx"),
  Chunk853856 = require("./853856.js"),
  Chunk93687 = require("./93687.js"),
  Chunk489618 = require("./489618.js"),
  Chunk785232 = require("./785232.jsx"),
  Chunk172751 = require("./172751.jsx"),
  Chunk850020 = require("./850020.js"),
  Chunk379839 = require("./379839.js"),
  Chunk359135 = require("./359135.js"),
  Chunk516817 = require("./516817.jsx"),
  Chunk878857 = require("./878857.js"),
  Chunk518950 = require("./518950.js"),
  Chunk199902 = require("./199902.js"),
  Chunk158776 = require("./158776.js"),
  Chunk306680 = require("./306680.js"),
  Chunk699516 = require("./699516.js"),
  Chunk111583 = require("./111583.js"),
  Chunk9156 = require("./9156.js"),
  Chunk594174 = require("./594174.js"),
  Chunk709054 = require("./709054.js"),
  Chunk998502 = require("./998502.js"),
  Chunk981631 = require("./981631.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk800525 = require("./800525.js");

function er(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function ei(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      er(e, t, n[t])
    })
  }
  return e
}

function eo(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function ea(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : eo(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}

function es(e, t) {
  if (null == e) return {};
  var n, r, i = el(e, t);
  if (Object.getOwnPropertySymbols) {
    var o = Object.getOwnPropertySymbols(e);
    for (r = 0; r < o.length; r++) n = o[r], !(t.indexOf(n) >= 0) && Object.prototype.propertyIsEnumerable.call(e, n) && (i[n] = e[n])
  }
  return i
}

function el(e, t) {
  if (null == e) return {};
  var n, r, i = {},
    o = Object.keys(e);
  for (r = 0; r < o.length; r++) n = o[r], t.indexOf(n) >= 0 || (i[n] = e[n]);
  return i
}
let ec = Chunk998502.ZP.getEnableHardwareAcceleration() ? Chunk481060.Xo$ : Chunk481060.qEK,
  eu = {
    offset: {
      top: 2,
      bottom: 2,
      right: 4
    }
  },
  ed = 500,
  ef = e => {
    var {
      nameplate: t,
      icon: n,
      forceShow: i = false,
      reducedClickTarget: o = false,
      visibleElementRef: s
    } = e, l = es(e, ["nameplate", "icon", "forceShow", "reducedClickTarget", "visibleElementRef"]);
    let c = (0, B.A)(t);
    return (0, r.jsx)(h.P3F, ea(ei({
      className: a()(en.closeButton, {
        [en.closeButtonPlated]: null != t,
        [en.closeButtonForceShow]: i,
        [en.reducedClickTarget]: o
      }),
      style: c
    }, l), {
      focusProps: {
        offset: {
          top: false,
          bottom: false,
          left: false,
          right: 6
        }
      },
      children: (0, r.jsx)("div", {
        ref: s,
        className: a()({
          [en.innerCloseButtonPlated]: null != t
        }),
        children: (0, r.jsx)(n, {
          size: "sm",
          color: "currentColor",
          className: a()(en.closeIcon, {
            [en.closeIconPlated]: null != t
          })
        })
      })
    }))
  },
  e_ = () => (0, Chunk951288.jsx)(Chunk481060.r7p, {
    size: "xs",
    color: "currentColor",
    className: Chunk800525.favoriteIcon
  }),
  ep = () => (0, Chunk951288.jsx)(Chunk481060.kZF, {
    size: "xs",
    color: "currentColor",
    className: Chunk800525.favoriteIcon
  }),
  eh = () => (0, Chunk951288.jsx)(Chunk481060.t6m, {
    size: "xs",
    color: "currentColor",
    className: Chunk800525.favoriteIcon
  }),
  em = e => {
    var {
      route: t,
      selected: n,
      icon: i,
      iconClassName: o,
      interactiveClassName: s,
      text: l,
      children: u,
      locationState: d,
      onClick: _,
      className: p,
      role: m,
      "aria-posinset": g,
      "aria-setsize": E,
      listItemRef: b
    } = e, y = es(e, ["route", "selected", "icon", "iconClassName", "interactiveClassName", "text", "children", "locationState", "onClick", "className", "role", "aria-posinset", "aria-setsize", "listItemRef"]);
    return (0, r.jsx)(R.Z, {
      className: a()(en.channel, {
        [en.fullWidth]: f.tq
      }, p),
      onClick: _,
      role: m,
      focusProps: ei({
        within: true
      }, eu),
      "aria-posinset": g,
      "aria-setsize": E,
      ref: b,
      children: (0, r.jsx)(h.vjx, {
        as: "div",
        selected: n,
        className: a()(s, en.interactive, en.linkButton, {
          [en.interactiveSelected]: n
        }),
        children: (0, r.jsxs)(c.rU, ea(ei({
          to: {
            pathname: t,
            state: d
          },
          className: en.link
        }, y), {
          children: [(0, r.jsx)(T.Z, {
            muted: false,
            avatar: (0, r.jsx)(i, {
              size: "refresh_sm",
              className: a()(en.linkButtonIcon, o),
              color: "currentColor"
            }),
            name: l,
            innerClassName: en.avatarWithText
          }), u]
        }))
      })
    })
  };

function eg(e) {
  let {
    channel: t,
    isGDMFacepileEnabled: o,
    selected: s = false,
    user: f,
    activities: b,
    applicationStream: O,
    voiceChannel: j,
    isTyping: G,
    status: B,
    isMobile: Y,
    nameplate: W,
    ref: q,
    "aria-posinset": Q,
    "aria-setsize": J
  } = e, [$, er] = i.useState(false), [eo, el] = i.useState(false), [em, eg] = i.useState(false), eE = i.useRef(null), eb = i.useRef(null), ey = null != q ? q : eb, {
    avatarSrc: eO,
    avatarDecorationSrc: ev,
    eventHandlers: eI
  } = (0, H.Z)({
    userId: null == f ? true : f.id,
    size: h.EFr.SIZE_32,
    animateOnHover: !(s || $ || eo || em)
  }), eT = t.isMultiUserDM(), eS = t.isSystemDM(), eA = (0, V.Q)(), eC = !eT && !eS && t.type === ee.d4z.DM, eN = eC && (null == f ? true : f.primaryGuild) != null, eR = (0, p.e7)([X.ZP], () => X.ZP.isChannelMuted(t.getGuildId(), t.id)), {
    ignored: eP,
    blocked: ew
  } = (0, p.cj)([z.Z], () => ({
    ignored: z.Z.isIgnored(t.getRecipientId()),
    blocked: z.Z.isBlocked(t.getRecipientId())
  })), eD = eC && eP, ex = eC && ew, eL = (eR || eD || ex) && !(s || $), ej = (0, p.e7)([K.ZP], () => K.ZP.getMentionCount(t.id) > 0), eM = (0, N.ZP)(t), ek = (0, p.e7)([L.Z], () => L.Z.isFavorite(t.id)), {
    dotsInsteadOfCloseButton: eU,
    rearrangeContextMenu: eG
  } = M.Z.useExperiment({
    location: "private_channel"
  }, {
    autoTrackExposure: true
  }), eB = i.useRef(null), eZ = null != W && (s || $ || em), eF = () => {
    er(true)
  }, eV = () => {
    er(false)
  }, eH = () => {
    el(true)
  }, eY = () => {
    el(false)
  }, eW = function(e) {
    let n = arguments.length > 1 && true !== arguments[1] && arguments[1];
    null != e && (e.preventDefault(), e.stopPropagation()), m.Z.closePrivateChannel(t.id, s, n)
  }, eK = () => {
    m.Z.preload(ee.ME, t.id)
  }, ez = e => {
    e.stopPropagation()
  }, eq = e => {
    if (e.target === e.currentTarget) {
      var t;
      null == (t = eE.current) || t.click()
    }
  }, eX = e => {
    let i = "contextmenu" === e.type,
      o = eG && !i,
      a = i ? u.ImpressionNames.DM_LIST_RIGHT_CLICK_MENU_SHOWN : u.ImpressionNames.DM_LIST_KEBAB_MENU_SHOWN;
    eg(true), t.isMultiUserDM() ? (0, g.jW)(e, async () => {
      let {
        default: e
      } = await Promise.all([n.e("79695"), n.e("90508"), n.e("70205"), n.e("57789"), n.e("30470")]).then(n.bind(n, 354741));
      return n => (0, r.jsx)(e, ea(ei({}, n), {
        channel: t,
        selected: s
      }))
    }, {
      impressionName: a,
      noBlurEvent: true,
      onClose: () => eg(false)
    }) : (0, g.jW)(e, async () => {
      let {
        default: e
      } = o ? await Promise.all([n.e("90508"), n.e("56167")]).then(n.bind(n, 416951)) : await Promise.all([n.e("79695"), n.e("90508"), n.e("70205"), n.e("98783"), n.e("57789"), n.e("56826"), n.e("40412")]).then(n.bind(n, 131404));
      return n => (0, r.jsx)(e, ea(ei({}, n), {
        user: f,
        channel: t,
        channelSelected: s
      }))
    }, {
      impressionName: a,
      onClose: () => eg(false)
    })
  }, eQ = e => {
    e.preventDefault(), e.stopPropagation();
    let i = et.intl.formatToPlainString(et.t.hJ5Ap6, {
        name: eM
      }),
      o = et.intl.format(et.t.SSIVOj, {
        name: eM
      });
    t.isManaged() && (i = et.intl.formatToPlainString(et.t.hVGjER, {
      name: eM
    }), o = et.intl.format(et.t.IK1Qvr, {
      name: eM
    })), (0, h.ZDy)(async () => {
      let {
        default: e
      } = await n.e("14604").then(n.bind(n, 960670));
      return t => (0, r.jsx)(e, ei({
        header: i,
        body: o,
        onSubmit: eW
      }, t))
    })
  }, eJ = () => t.isSystemDM() ? (0, r.jsx)("div", {
    className: en.subtext,
    children: (0, S.Z)(t.id) ? et.intl.string(et.t.FL5T09) : et.intl.string(et.t.NnY5lZ)
  }) : t.isMultiUserDM() ? (0, r.jsx)("div", {
    className: en.subtext,
    children: et.intl.format(et.t.CxSA5O, {
      members: t.recipients.length + 1
    })
  }) : (0, y.Z)({
    activities: b,
    status: B,
    applicationStream: O,
    voiceChannel: j
  }) ? (0, r.jsx)(v.Z, {
    location: "PrivateChannel",
    user: f,
    activities: b,
    voiceChannel: j,
    applicationStream: O,
    animateEmoji: $ || eo || em,
    textClassName: en.activityStatusText,
    iconClassName: eL ? en.mutedIcon : true
  }) : null, e$ = () => {
    let e = h.EFr.SIZE_32;
    if (t.isMultiUserDM())
      if (t.recipients.length >= 2 && o && null == t.icon) return (0, r.jsx)(k.Z, {
        "aria-hidden": true,
        recipients: t.recipients,
        size: e,
        isTyping: G,
        status: B
      });
      else return (0, r.jsx)(ec, ea(ei({}, eI), {
        src: (0, C.x)(t),
        "aria-hidden": true,
        size: e,
        status: G ? ee.Skl.ONLINE : B,
        isTyping: G
      }));
    l()(null != f, "PrivateChannel.renderAvatar: Invalid prop configuration - no user or channel");
    let n = null;
    return f.isSystemUser() || (n = (0, E.Z)(b) ? ee.Skl.STREAMING : B), (0, r.jsx)(ec, ea(ei({}, eI), {
      size: h.EFr.SIZE_32,
      src: eO,
      avatarDecoration: ev,
      status: n,
      isMobile: Y,
      isTyping: G,
      "aria-label": f.username,
      statusTooltip: true
    }))
  }, e0 = (0, r.jsx)(x.Z, {
    userName: eM,
    displayNameStyles: null == f ? true : f.displayNameStyles,
    effectDisplayType: $ || s || em ? D.F.ANIMATED : D.F.PLAIN,
    loop: $,
    boldFontOpacity: .9
  }), e1 = eN ? (0, r.jsxs)(r.Fragment, {
    children: [e0, (0, r.jsx)(U.ZP, {
      primaryGuild: null == f ? true : f.primaryGuild,
      userId: null == f ? true : f.id,
      inline: true,
      disableGuildProfile: true,
      className: a()(en.clanTag, {
        [en.clanTagMuted]: eL
      })
    })]
  }) : e0, e2 = i.useRef(null), e3 = (0, w.Y)({
    location: "PrivateChannel"
  }) && (null == f ? true : f.displayNameStyles) != null, e4 = (0, _.Z)();
  return (0, r.jsx)(d.mh, {
    id: t.id,
    children: e => {
      var {
        role: n
      } = e, i = es(e, ["role"]);
      return (0, r.jsxs)(R.Z, {
        className: a()(en.channel, en.dm),
        role: n,
        focusProps: ea(ei({}, eu), {
          focusTarget: eE,
          ringTarget: ey
        }),
        ref: ey,
        onMouseEnter: eF,
        onMouseLeave: eV,
        onMouseDown: eK,
        onFocus: eH,
        onBlur: eY,
        onContextMenu: eX,
        "aria-setsize": J,
        "aria-posinset": Q,
        children: [ej ? (0, r.jsx)("div", {
          className: a()(en.unreadPill, {
            [en.muted]: eL
          })
        }) : null, (0, r.jsxs)(h.vjx, {
          className: a()(en.interactive, {
            [en.interactiveSystemDM]: eA && eS,
            [en.interactiveSelected]: s || em
          }),
          as: "div",
          onClick: eq,
          muted: eL,
          selected: s,
          children: [(0, r.jsx)(F.Z, {
            nameplate: eZ ? W : true,
            selected: s,
            hovered: $,
            content: e2,
            placement: Z.i.CHANNEL
          }), (0, r.jsx)(c.rU, ea(ei({
            innerRef: eE,
            to: ee.Z5c.CHANNEL(ee.ME, t.id),
            className: en.link,
            "aria-label": (0, A.ZP)({
              channel: t,
              unread: ej
            })
          }, i), {
            children: (0, r.jsx)(T.Z, {
              ref: e2,
              avatar: e$(),
              highlighted: ej && !eL,
              muted: eL,
              subText: eJ(),
              name: (0, r.jsx)(P.Z, {
                className: a()(en.overflowTooltip, {
                  [en.withDisplayNameStyles]: e3
                }),
                children: e1
              }),
              decorators: t.isSystemDM() ? (0, r.jsx)(I.Z, {
                className: en.decorator,
                type: I.Z.Types.SYSTEM_DM,
                verified: true
              }) : null,
              withDisplayNameStyles: e3
            })
          })), (0, r.jsxs)("div", {
            className: a()(en.iconsContainer, {
              [en.nameplated]: eZ
            }),
            children: [ek && !em ? (0, r.jsx)(e_, {}) : null, eD ? (0, r.jsx)(ep, {}) : null, ex ? (0, r.jsx)(eh, {}) : null, eU && !e4 ? (0, r.jsx)(h.ua7, {
              text: et.intl.string(eT ? et.t.Et8qws : et.t["d+e27u"]),
              delay: ed,
              targetElementRef: eB,
              children: e => {
                var {
                  onClick: t
                } = e, n = es(e, ["onClick"]);
                return (0, r.jsx)(ef, ea(ei({}, n), {
                  icon: h.xhG,
                  "aria-label": et.intl.string(et.t.PdRCRk),
                  onClick: e => {
                    null == t || t(), eX(e)
                  },
                  onMouseDown: ez,
                  nameplate: W,
                  forceShow: em,
                  visibleElementRef: eB
                }))
              }
            }) : (0, r.jsx)(ef, {
              icon: h.Dio,
              "aria-label": eT ? et.intl.string(et.t["26C4oq"]) : et.intl.string(et.t.jsvgc3),
              onClick: eT ? eQ : eW,
              onMouseDown: ez,
              nameplate: W,
              reducedClickTarget: true,
              visibleElementRef: eB
            })]
          })]
        })]
      })
    }
  })
}
let eE = e => {
  var {
    channel: t,
    selected: n
  } = e, i = es(e, ["channel", "selected"]);
  let o = (0, p.e7)([Q.default], () => Q.default.getUser(t.getRecipientId())),
    {
      isStatusIndicatorEnabled: a,
      isTypingIndicatorEnabled: s,
      isFacepileEnabled: l
    } = j.Z.useExperiment({
      location: "private_channel"
    }, {
      autoTrackExposure: true
    }),
    c = null == o ? true : o.id,
    u = (0, p.cj)([W.Z, Y.Z], () => {
      let e;
      if (t.isMultiUserDM()) {
        if (a) {
          let n = W.Z.getState().statuses;
          t.recipients.some(e => n[e] === ee.Skl.ONLINE) && (e = ee.Skl.ONLINE)
        }
      } else null != c && (e = W.Z.getStatus(c));
      return {
        status: e,
        activities: null != c ? W.Z.getActivities(c) : null,
        applicationStream: null != c ? Y.Z.getAnyStreamForUser(c) : null,
        isMobile: null != c && W.Z.isMobileOnline(c)
      }
    }, [t, c, a]),
    {
      voiceActivityStatusEnabled: d
    } = (0, b.U)({
      location: "PrivateChannel"
    }),
    {
      voiceChannel: f
    } = (0, O.Z)({
      userId: c
    }),
    _ = (0, G.K)({
      user: o
    }),
    h = (0, p.e7)([Q.default, q.Z], () => {
      if (t.isMultiUserDM())
        if (s) return J.default.keys(q.Z.getTypingUsers(t.id)).some(e => {
          var t;
          return e !== (null == (t = Q.default.getCurrentUser()) ? true : t.id)
        });
        else returnfalse;
      return null != o && q.Z.isTyping(t.id, t.getRecipientId())
    }, [t, o, s]);
  return t.isMultiUserDM() ? (0, r.jsx)(eg, ei({
    channel: t,
    selected: n,
    isTyping: h,
    status: u.status === ee.Skl.ONLINE ? ee.Skl.ONLINE : true,
    isGDMFacepileEnabled: l
  }, i)) : (0, r.jsx)(eg, ei({
    channel: t,
    selected: n,
    user: o,
    voiceChannel: d ? f : true,
    isTyping: h,
    nameplate: _
  }, i, u))
}