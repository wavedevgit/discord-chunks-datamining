...
let m = new i.Z("ImageEditor"),
  ...
  v = e => {
    let {
      ...
      x: 0,
      y: 0
    }), j = a.useRef(null), [E, N] = a.useState(1), [I, D] = a.useState(null), [S, y] = a.useState({
      top: 0,
      bottom: 0,
      left: 0,
      right: 0
    }), [C, w] = a.useState(0), [A, M] = a.useState({
      x: 0,
      y: 0
    }), [k, R] = a.useState(!1), T = "image/gif" === t.type, [L, P] = a.useState(null);
    a.useEffect(() => {
      (0, c.Z)()
    }, []);
    ...
    let F = a.useRef(null),
      V = a.useCallback(async () => {
        ...
        let n = Date.now(),
          ...
          s = null;
        if (null != F.current && (F.current(), F.current = null), T) try {
          let n = O({
              file: t,
              image: r,
              cropDimensions: f,
...
function _(e, t, n) {
  let {
    width: r,
    height: a
  } = x(e, t), l = Math.abs(256 - r) / 2, s = Math.abs(256 - a) / 2;
...
let O = (e, t, n) => {
  let {
    height: r,
    width: a
  } = x(t, n), l = (r = Math.min(r, 256)) / (a = Math.min(a, 256)), s = {
    height: r,
    width: a
