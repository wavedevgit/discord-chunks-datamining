...
function b(e) {
  ...
  return null == t ? <s.EO4 size={"sm"} color={"currentColor"} /> : <l.Z animated={false} emojiId={t.id} emojiName={n} />
}
let y = function(e) {
  let {
    channel: t
  ...
  let R = i.useCallback(() => {
    ...
    D = i.useRef(null);
  return <div className={E.bar}>{<p.Z channel={t} shouldShow={N} onRequestClose={R} setEmojiConfetti={P} positionRef={D} position={"top"} align={"left"}>{() => (0, r.jsxs)(s.P3F, {
        innerRef: D,
        onClick: () => C(!N),
      ...
      })}</p.Z>}{<div className={E.end}>{<a.zx look={a.zx.Looks.LINK} size={a.zx.Sizes.SMALL} onClick={w} color={a.zx.Colors.TRANSPARENT} className={E.learnMoreButton}><o.xv variant={"text-sm/medium"}>{A}</o.xv></a.zx>}{<div className={E.separator} aria-hidden={true} />}{<c.B onClick={S} />}</div>}</div>
}