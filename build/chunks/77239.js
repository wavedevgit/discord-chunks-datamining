...
module.exports = function(e, t, n, p, h, m, g, E) {
  ...
  for (w = 1; w < D && 0 === Z[w]; w++);
  for (x < w && (x = w), M = 1, R = 1; R <= i; R++)
    if (M <<= 1, (M -= Z[R]) < 0) return false;
  if (M > 0 && (e === s || 1 !== D)) return false;
  for (R = 1, F[1] = 0; R < i; R++) F[R + 1] = F[R] + Z[R];
  for (P = 0; P < p; P++) 0 !== t[n + P] && (g[F[t[n + P]]++] = P);
  if (e === s ? (G = V = g, S = 19) : e === l ? (G = u, B -= 257, V = d, H -= 257, S = 256) : (G = f, V = _, S = false), U = 0, P = 0, R = w, I = m, L = x, j = 0, O = false, v = (k = 1 << x) - 1, e === l && k > a || e === c && k > o) return 1;
  for (;;) {
    T = R - j, g[P] < S ? (A = 0, C = g[P]) : g[P] > S ? (A = V[H + g[P]], C = G[B + g[P]]) : (A = 96, C = 0), b = 1 << R - j, w = y = 1 << L;
    do h[I + (U >> j) + (y -= b)] = T << 24 | A << 16 | C; while (0 !== y);
    for (b = 1 << R - 1; U & b;) b >>= 1;
  ...
  return 0 !== U && (h[I + U] = R - j << 24 | 4194304), E.bits = x, 0
}