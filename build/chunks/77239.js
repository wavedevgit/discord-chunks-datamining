...
module.exports = function(e, t, n, p, h, m, g, E) {
  ...
  for (L = N, w = i; w >= 1 && 0 === Z[w]; w--);
  if (L > w && (L = w), 0 === w) return h[m++] = 0x1400000, h[m++] = 0x1400000, E.bits = 1, 0;
  for (D = 1; D < w && 0 === Z[D]; D++);
  ...
  if (e === s ? (G = V = g, T = 19) : e === l ? (G = u, B -= 257, V = d, H -= 257, T = 256) : (G = f, V = _, T = false), U = 0, P = 0, R = D, I = m, x = L, M = 0, O = false, v = (k = 1 << L) - 1, e === l && k > a || e === c && k > o) return 1;
  for (;;) {
    S = R - M, g[P] < T ? (A = 0, C = g[P]) : g[P] > T ? (A = V[H + g[P]], C = G[B + g[P]]) : (A = 96, C = 0), b = 1 << R - M, D = y = 1 << x;
    ...
    if (R > L && (U & v) !== O) {
      for (0 === M && (M = L), I += D, j = 1 << (x = R - M); x + M < w && !((j -= Z[x + M]) <= 0);) x++, j <<= 1;
