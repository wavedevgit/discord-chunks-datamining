...
module.exports = function(e, t, n, p, h, m, g, E) {
  ...
  for (L < w && (L = w), k = 1, R = 1; R <= i; R++)
    if (k <<= 1, (k -= V[R]) < 0) return false;
  if (k > 0 && (e === s || 1 !== D)) return false;
  for (R = 1, F[1] = 0; R < i; R++) F[R + 1] = F[R] + V[R];
  for (P = 0; P < p; P++) 0 !== t[n + P] && (g[F[t[n + P]]++] = P);
  if (e === s ? (G = Z = g, T = 19) : e === l ? (G = u, B -= 257, Z = d, H -= 257, T = 256) : (G = f, Z = _, T = false), U = 0, P = 0, R = w, I = m, x = L, M = 0, O = false, v = (j = 1 << L) - 1, e === l && j > a || e === c && j > o) return 1;
  for (;;) {
