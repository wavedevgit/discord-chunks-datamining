...
function h(e) {
  ...
  return <div>{<o.X6q color={"header-primary"} variant={"text-md/semibold"}>{g.intl.string(g.t["N+GnPz"])}</o.X6q>}{<o.Text color={"text-secondary"} variant={"text-sm/medium"} className={p.bodyText}>{g.intl.format(g.t["Y/jYws"], {
        helpdeskArticle: d.Z.getArticleURL(m.BhN.MEMBER_APPLICATIONS)
      })}</o.Text>}{<o.njP selectedItem={t} onItemSelect={n} orientation={"horizontal"} className={p.joinOptions}>{b.map(e => {
        let n = e.id === t;
