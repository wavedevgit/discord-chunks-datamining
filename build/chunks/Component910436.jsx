/** Chunk was on 1272 **/
/** chunk id: 910436, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  Z: () => I
}), require("./539854.js"), require("./388685.js");
var Chunk54381 = require("./54381.js");
require("./473749.js");
var Chunk287734 = require("./287734.js"),
  Chunk872810 = require("./872810.js"),
  Chunk122810 = require("./122810.js"),
  Chunk841784 = require("./841784.js"),
  Chunk503438 = require("./503438.js"),
  Chunk802856 = require("./802856.js"),
  Chunk420660 = require("./420660.js"),
  Chunk49436 = require("./49436.js"),
  Chunk968843 = require("./968843.js"),
  Chunk862657 = require("./862657.js"),
  Chunk759479 = require("./759479.js"),
  Chunk364833 = require("./364833.jsx"),
  Chunk81063 = require("./81063.js"),
  Chunk761282 = require("./761282.jsx"),
  Chunk358924 = require("./358924.jsx"),
  Chunk868854 = require("./868854.js"),
  Chunk817915 = require("./817915.js"),
  Chunk981631 = require("./981631.js");

function I(e) {
  let {
    party: t,
    onChannelContextMenu: n,
    quest: I
  } = e, y = (0, E.L)(), {
    voiceChannels: C,
    currentActivities: S,
    partiedMembers: T,
    applicationStreams: N,
    guildContext: j
  } = t, P = (0, p.z1)(I), x = [], Z = e => {
    let {
      length: t
    } = x;
    if (0 === t) return void x.push(e);
    x.push(e)
  };
  for (let {
      activity: e
    }
    of(C.length > 0 && C.forEach(e => {
      let {
        members: t,
        channel: i,
        guild: l
      } = e;
      Z((0, r.jsx)(_.Z.VoiceSection, {
        guild: l,
        channel: i,
        members: t,
        partySize: function(e, t) {
          let n = e.length;
          return {
            knownSize: n,
            unknownSize: 0,
            totalSize: n + 0
          }
        }(t),
        onChannelContextMenu: n
      }, "voice-".concat(i.id)))
    }), t.currentActivities))
    if (null != e) {
      let t = b.ct(e);
      if (null != t) {
        Z(t);
        break
      }
    } return N.length > 0 && y && N.forEach(e => {
    let {
      stream: t,
      streamUser: n,
      activity: a
    } = e;
    Z((0, r.jsx)(_.Z.ApplicationStreamingSection, {
      guildId: null == j ? true : j.id,
      user: n,
      activity: a,
      applicationStream: t,
      onPreviewClick: () => {
        i.default.selectVoiceChannel(t.channelId), (0, l.iV)(t)
      }
    }, "application-stream-".concat(t.ownerId)))
  }), S.forEach((e, t) => {
    var n, i, l, p, b, E, y, N, x;
    let {
      activity: A,
      game: w,
      playingMembers: L,
      activityUser: R
    } = e;
    if (null == A || null == A.type) return null;
    if (A.type === v.IIU.PLAYING)
      if (S.length > 1 && !(0, a.Z)(A) && null != w) Z((0, r.jsx)(_.Z.GameSection, {
        icon: w.getIconURL(O.Z),
        name: w.name,
        partySize: {
          knownSize: L.length,
          unknownSize: 0,
          totalSize: L.length
        },
        members: L,
        activity: A
      }, "game-".concat(null != (n = A.session_id) ? n : t, "-").concat(null != (i = A.application_id) ? i : t)));
      else if ((0, o.Z)(A)) {
      let e = new Set(L.map(e => e.id)),
        t = null == (l = C.find(e => null != e)) ? true : l.channel;
      null != t && Z((0, r.jsx)(_.Z.EmbeddedActivitySection, {
        activity: A,
        participants: e,
        channel: t,
        guildId: t.guild_id
      }, "embedded-activity-".concat(A.application_id)))
    } else(0, c.Z)(A) ? Z((0, r.jsx)(_.Z.XboxSection, {
      title: w.name
    }, "xbox-".concat(null != (p = A.session_id) ? p : t))) : Z((0, r.jsx)(_.Z.RichPresenceSection, {
      activity: A,
      getAssetImage: m.xF,
      user: R
    }, "rich-presence-".concat(null != (b = A.session_id) ? b : t, "-").concat(R.id)));
    else if ((0, u.Z)(A)) {
      let e = C.length > 0 && C[0].members.length > 1,
        n = L.length > 1;
      Z((0, r.jsx)(_.Z.TwitchSection, {
        guildId: null == j ? true : j.id,
        activity: A,
        user: e || n ? R : null,
        getAssetImage: m.xF
      }, "streaming-".concat(null != (E = A.session_id) ? E : t)))
    } else A.type === v.IIU.LISTENING && ((0, s.Z)(A) ? Z((0, r.jsx)(_.Z.SpotifySection, {
      activity: A,
      isSolo: 1 === T.length,
      partySize: {
        knownSize: L.length,
        unknownSize: 0,
        totalSize: L.length
      },
      getAssetImage: m.xF,
      members: L
    }, "spotify-".concat(null != (y = A.session_id) ? y : t, "-").concat(R.id))) : (null != A.assets || (0, a.Z)(A)) && Z((0, r.jsx)(_.Z.RichPresenceSection, {
      activity: A,
      getAssetImage: m.xF,
      user: R
    }, "rich-presence-".concat(null != (N = A.session_id) ? N : t, "-").concat(R.id))));
    P && null != I && null != w && (0, g._D)(A, I) && (null == I.userStatus || !(0, f.zE)(I.userStatus, d.jn.ACTIVITY_PANEL)) && Z((0, r.jsx)(h.Z, {
      quest: I
    }, "quest-".concat(I.id, "-").concat(null != (x = A.session_id) ? x : t)))
  }), x.length > 0 ? (0, r.jsx)(_.Z.Body, {
    children: x
  }) : null
}