...
function x(e) {
  ...
  return i.useEffect(() => {
    if (x) {
      ...
      null == (e = C.current) || e.focus()
  ...
  }, [x]), <div>{<u.Ee src={null == O ? n(26230) : n(935227)} className={o()(_.marginBottom20, _.marginTop8)} />}{<u.Dx>{m.intl.string(m.t["IfBQ5+"])}</u.Dx>}{null != O && "" !== O ? <a.Text variant={"text-sm/normal"} color={"text-danger"}>{O}</a.Text> : null}{<u.gO className={o()(_.marginBottom20, _.marginTop20)}>{<u.II name={"password"} type={"password"} label={m.intl.string(m.t["8dM4FB"])} setRef={C} className={_.marginBottom20} value={N} onChange={y} error={j} autoComplete={"new-password"} maxLength={72} placeholder={m.intl.string(m.t["yY/PXV"])} />}{<u.zx className={_.marginTop8} onClick={() => {
          if (!E) {
            if (0 === N.length) {
        ...
        }} submitting={E} disabled={E}>{m.intl.string(m.t.ezv91d)}</u.zx>}</u.gO>}{<div className={g.buttonContainer}><u.zx color={u.zx.Colors.PRIMARY} onClick={() => {
          y(""), l(p.n.START)
        }}>{m.intl.string(m.t.rzxnQ0)}</u.zx></div>}</div>
}