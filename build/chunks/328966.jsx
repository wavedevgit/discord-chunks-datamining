/** Chunk was on web.js **/
/** chunk id: 328966, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  H: () => s,
  Z: () => c
});
var Chunk255367 = require("./255367.js");
require("./73800.js");
var Chunk318713 = require("./318713.jsx"),
  Chunk388032 = require("./388032.js"),
  Chunk848697 = require("./848697.js"),
  s = function(e) {
    return e[e.WITH_CONTENT = 0] = "WITH_CONTENT", e[e.AFTER_ACCESSORIES = 1] = "AFTER_ACCESSORIES", e
  }({});

function l(e, t) {
  return t || null != e.content && 0 !== e.content.length ? 0 : 1
}

function c(e) {
  let {
    message: t,
    compact: n,
    location: s
  } = e;
  return t.isEdited() && null != t.editedTimestamp && s === l(t, n) ? <r.Fragment>{" "}{<i.Z timestamp={t.editedTimestamp} isEdited={true} isInline={false}><span className={a.edited}>{"("}{o.intl.string(o.t.C8sXIC)}{")"}</span></i.Z>}</r.Fragment> : null
}