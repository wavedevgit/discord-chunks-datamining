...
function T(e) {
  ...
  if (t.type === b.IIU.PLAYING) {
    ...
    return <div className={v.activityRow}>{<div className={v.activityRowContent}>{_ || null == e ? <s.IMN size={"custom"} width={I} height={I} /> : <img src={e} alt={t.name} onError={() => p(true)} />}{<div>{<s.Text variant={"text-sm/medium"} color={"text-tertiary"}>{(0, u.Z)(t)}</s.Text>}{<s.Text variant={"text-md/medium"}>{t.name}</s.Text>}</div>}</div>}{<s.zxk loading={m} disabled={f || T} fullWidth={true} size={"sm"} text={A()} onClick={S} />}</div>
  }
}
...
function S(e) {
  ...
  return null != l && g ? <f.Z.Provider value={n.guild_id}><div className={v.root}>{<div className={v.userHeader}>{<c.Z user={l} status={d} size={s.EFr.SIZE_24} />}{<s.Text variant={"text-md/medium"}>{h}</s.Text>}</div>}{<div className={v.activityList}>{y.map(e => (0, r.jsx)(T, {
          activity: e,
          channel: n,
          userId: t
        }, "".concat(e.session_id, "-").concat(e.application_id, "-").concat(e.name)))}</div>}</div></f.Z.Provider> : null
}