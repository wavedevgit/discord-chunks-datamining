...
let q = (e, t, n) => {
    ...
    if (null == s) return <strong>{l}</strong>;
    ...
    return [<img alt={""} className={G.displayAvatar} src={h} />, <span className={G.displayedNick}>{p}</span>, <span className={G.displayUsername}>{N.ZP.getUserTag(s, {
  ...
  X = (e, t, n) => {
    ...
    return null == i ? <strong>{o}</strong> : <div className={G.resultChannel}>{<x.MC channel={i} />}{<x.GQ channel={i} />}{<x.P channel={i} />}</div>
  },
  Q = e => {
    let t, n, {
      id: i,
    ...
    if (m) {
      var E, b;
      t = <span className={G.filter}>{null != (b = null == (E = D.ZP[l]) ? true : E.key) ? b : "addme:"}</span>
    }
    return n = null != g ? g(o, l, s) : <strong>{s.text}</strong>, <p.P3F tag={"li"} className={a()(G.option, c)} onClick={_} onFocus={h} id={i} role={u} tabIndex={d} aria-selected={f}>{<div className={G.content}>{t}{n}</div>}{<p.qJs size={"sm"} color={"currentColor"} className={G.plusIcon} />}</p.P3F>
  },
  J = e => <Q{...V(Z({}, e), {
    className: G.user,
    renderResult: q
  ...
  let f = "",
    _ = i.useMemo(() => ee(o.text), [o.text]).map(e => {
      ...
      let n = j.TNx.test(e.type),
        i = j.KA4.test(e.type),
        o = !n && !i;
      return f += t, <span className={a()(G.searchHistoryRow, {
          [G.filter]: n,
          [G.answer]: i,
          [G.nonText]: o
        })}>{t}</span>
...
let en = {
  ...
  [Chunk981631.rtL.HISTORY]: {
    ...
    groupTip(e) {
      ...
      return <p.ua7 text={U.intl.string(U.t.dwAvX1)} position={"left"}>{e => {
          let {
            onMouseEnter: n,
            onMouseLeave: i
          } = e;
  ...
  [Chunk981631.rtL.SEARCH_OPTIONS]: {
    ...
    groupTip: () => <Chunk481060.ua7 text={Chunk388032.intl.string(Chunk388032.t.hvVgAQ)} position={"left"}>{e => (0, r.jsx)("div", V(Z({
...
class er extends Chunk73800.PureComponent {
  ...
  render() {
    ...
    return <Chunk542051.Z numResults={Chunk73800} query={require.query} navId={exports} hideQuery={!this.shouldShowSearchQuery()} focusedIndex={module} renderCustomResults={this.renderAutocompletes} renderInitialState={this.renderAutocompletes} onHighlightQuery={() => this.focusOption(false)} onSelectQuery={this.performSearch} onSelectSearchEverywhere={() => this.performSearch({
        searchEverywhere: true
      })} renderNoResults={() => null} searchFavorites={Chunk120356.type === Chunk981631.aib.FAVORITES && (0, Chunk607802.X$)()} showSearchInSelectedChannel={this.shouldShowSearchInSelectedChannel()} channel={a} onSelectSearchInSelectedChannel={() => this.handleSearchInChannel({
  ...
  constructor(...e) {
    ...
    super(...e), t = this, B(this, "state", {
    ...
    }), B(this, "renderDatePicker", () => <div className={G.datePicker}>{<K onSelect={this.handleDateChange} maxDate={u()().local()} minDate={W} />}{<div className={G.datePickerHint}>{<span className={G.hint}>{U.intl.string(U.t.fmtCi4)}{"\xa0"}</span>}{<p.P3F tag={"span"} className={G.hintValue} onClick={this.handleHintClick}>{this.state.dateHint}</p.P3F>}</div>}</div>), B(this, "handleHintClick", () => {
      ...
      return i.map(i => {
        ...
        null != f.titleText && (d = "".concat(i.group, "-header"), u = <div id={d} className={G.header}>{f.titleText()}</div>);
        let _ = null != (l = f.groupTip) ? l : null,
          p = null != _ ? <_ searchContext={n} /> : null,
          m = null != (c = f.component) ? c : Q,
          g = o.type === j.Sap.FILTER_ALL;
        return <ul role={"group"} aria-labelledby={d} className={G.resultsGroup}>{u}{p}{i.results.map(o => {
            var s, l;
...
let ei = Chunk73800.forwardRef((e, t) => {
  ...
  return <er ref={t} navId={i} onSelectedIndexChanged={o} searchContext={n} resultsState={s} totalResults={l} channel={a} />
})