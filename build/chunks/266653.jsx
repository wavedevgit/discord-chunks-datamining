...
function N(e) {
  ...
  return i.useEffect(() => {
    if (C) j(<c.Wn messageType={c.QYI.ERROR} className={g.infoMessage}>{f.intl.format(f.t.HAuRSE, {
        maxNumAccounts: h.$H
      })}</c.Wn>), E(_.lds);
    else if (null != A) {
      let e = d.default.getUser(A);
      null != e && j(<c.Wn messageType={c.QYI.POSITIVE} className={g.infoMessage}>{f.intl.format(f.t["09qidX"], {
          username: e.username
        })}</c.Wn>), p(false)
    }
  }, [A, C]), i.useEffect(() => {
  ...
  }), <l.Modal onClose={s} transitionState={t} size={"md"} aria-label={f.intl.string(f.t.WbFpq6)} title={f.intl.string(f.t.WbFpq6)} subtitle={f.intl.string(f.t["+1Uk3d"])} actions={[{
    ...
    }]}>{v}{<m.Z actionText={f.intl.string(f.t.Wf421N)} onAction={(e, t) => {
        switch (e) {
