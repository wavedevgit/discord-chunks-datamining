...
function G(e) {
  ...
  let N = n.items.some(e => {
      ...
      return !Y.dJ.has(t)
    })) != null ? <p.zxk variant={"primary"} text={W.intl.string(W.t.PDTjLC)} onClick={() => s(z.R.PREVIEW)} /> : <p.zxk variant={"critical-primary"} text={l ? W.intl.string(W.t["cY+Ooa"]) : W.intl.formatToPlainString(W.t["V3+Rpa"], {
        planPremiumType: O.ZP.getDisplayPremiumType(n.planId)
      })} disabled={_} onClick={async () => {
        await V({
          setHasError: C,
          onClose: o,
          premiumSubscription: n,
    ...
    I = <m.zx look={m.zx.Looks.LINK} color={(0, d.ap)(j) ? m.zx.Colors.PRIMARY : m.zx.Colors.WHITE} onClick={o}>{l ? W.intl.string(W.t.h9tkAA) : W.intl.string(W.t["ETE/oK"])}</m.zx>;
  return <i.Fragment>{l ? <B.Z premiumType={a} className={X.cancellationHeader} onClose={o} /> : <p.xBx data-migration-pending={true} separator={false}>{<p.vwX tag={p.RB0.H4}>{W.intl.format(W.t.ZpiGy8, {
          planPremiumType: O.ZP.getDisplayPremiumType(n.planId)
        })}</p.vwX>}{<p.olH data-migration-pending={true} onClick={o} />}</p.xBx>}{<p.hzk data-migration-pending={true} className={X.body}>{g ? <p.kzN className={X.errorBlock}>{W.intl.string(W.t["5mlOCQ"])}</p.kzN> : null}{<div className={X.cancelImage} />}{<div>{v}</div>}</p.hzk>}{<p.mzw data-migration-pending={true} justify={h.Z.Justify.START}>{N}{I}</p.mzw>}</i.Fragment>
}
...
function K(e) {
  ...
  if (null == s || null == o) return <p.$jN className={X.loading} />;
  let {
    intervalType: l,
    intervalCount: c
  } = O.ZP.getIntervalForInvoice(s);
  return <div className={X.__invalid_bodyText}>{<div className={X.renewalInvoiceDate}>{0 !== s.total ? W.intl.format(W.t.ycyeBw, {
        renewalDate: s.subscriptionPeriodStart,
        rate: (0, P.og)((0, P.T4)(s.total, s.currency), l, c)
      }) : W.intl.format(W.t["+y0Tj4"], {
        renewalDate: s.subscriptionPeriodStart
      })}</div>}{<R.aO>{<R.Z9>{W.intl.string(W.t.iqhIp6)}</R.Z9>}{<R.B1 label={W.intl.formatToPlainString(W.t.r3jVZm, {
          planName: O.ZP.getDisplayName(o.id)
        })} value={(0, O.PK)(o)} className={X.invoiceCancelRow} />}{<R.UN />}{<U.nd premiumSubscription={r} renewalInvoice={s} isUpdate={true} />}</R.aO>}</div>
}
...
function J(e) {
  ...
  return <i.Fragment>{<B.Z premiumType={n} onClose={o} />}{<p.hzk data-migration-pending={true} className={s()(X.previewStep, X.body)}>{d ? <p.kzN className={X.errorBlock}>{W.intl.string(W.t["5mlOCQ"])}</p.kzN> : null}{<K premiumSubscription={t} />}</p.hzk>}{<p.mzw data-migration-pending={true} align={h.Z.Align.CENTER} justify={h.Z.Justify.BETWEEN}>{<p.zxk variant={"critical-primary"} text={W.intl.format(W.t["V3+Rpa"], {
          planPremiumType: O.ZP.getDisplayPremiumType(t.planId)
        })} disabled={c} onClick={async () => {
          await V({
            setHasError: m,
            onClose: o,
            premiumSubscription: t,
        ...
        }} />}{<M.Z onClick={a} />}</p.mzw>}</i.Fragment>
}
...
let ee = e => {
  ...
  switch (w) {
    case z.R.PAUSE_SELECT:
      n = <A.of premiumSubscription={a} premiumType={v} setStep={M} onClose={() => $(w)} pauseDuration={V} setPauseDuration={K} footer={(0, i.jsxs)("div", {
    ...
    case z.R.PAUSE_CONFIRM:
      ...
      n = <A.Sz premiumSubscription={a} premiumType={v} setStep={M} onClose={() => $(w)} analyticsLocation={_} pauseDuration={V} />;
      break;
    case z.R.WHAT_YOU_LOSE:
      n = <L.Z premiumType={v} titleText={W.intl.string(W.t.PWq8TE)} subtitleText={W.intl.format(W.t.nsGVzs, {})} subtitleClassName={true} footer={(0, i.jsxs)("div", {
          className: X.whatYouLoseButtonContainer,
          children: [(0, i.jsx)(p.zxk, {
            variant: "primary",
        ...
        })} onClose={() => $(w)} onDiscountClaim={() => M(z.R.CONFIRM_DISCOUNT)} onContinue={() => M(z.R.CONFIRM)} isLoading={ee && en} churnUserDiscountOffer={et} analyticsLocations={R} />;
      break;
    case z.R.CONFIRM:
      n = <G premiumSubscription={a} premiumType={v} setStep={M} onClose={() => $(w)} whatYouLoseExperienceEnabled={T} analyticsLocation={_} />;
      break;
    case z.R.PREVIEW:
      n = <J premiumSubscription={a} premiumType={v} onBack={() => M(z.R.CONFIRM)} onClose={() => $(w)} analyticsLocation={_} />;
      break;
    case z.R.CONFIRM_DISCOUNT:
      n = <D.Z premiumSubscription={a} premiumType={v} onClose={() => $(w)} onConfirm={() => M(z.R.DISCOUNT_APPLIED)} userDiscountOffer={et} />;
      break;
    case z.R.DISCOUNT_APPLIED:
      n = <k.D premiumSubscription={a} premiumType={v} onClose={() => $(w)} confettiCanvas={g} userDiscountOffer={et} />;
      break;
    default:
      throw Error("Unexpected step: ".concat(w))
  }
  return <x.Gt value={R}>{<o.O_ ref={E} className={X.confettiCanvas} environment={h.current} />}{<p.Y0X data-migration-pending={true} transitionState={s} parentComponent={"PremiumSubscriptionCancellationModal"}>{n}</p.Y0X>}</x.Gt>
}