/** Chunk was on 99133 **/
/** chunk id: 810123, original params: A,t,e (module,exports,require) **/
require.d(exports, {
  E: () => i,
  Z: () => a
});
var r, Chunk255367 = require("./255367.js"),
  Chunk120356 = require("./120356.js"),
  n = require.n(Chunk120356),
  Chunk471445 = require("./471445.js"),
  Chunk686546 = require("./686546.jsx"),
  Chunk601964 = require("./601964.js"),
  Chunk292895 = require("./292895.js"),
  i = ((r = {}).SMALL_32 = "SMALL_32", r);
let o = {
    SMALL_32: 24
  },
  C = {
    SMALL_32: [13, 11, 10, 10, 10, 10, 10]
  },
  a = function(A) {
    let {
      className: t,
      iconClassName: e,
      size: r,
      channel: v,
      guild: i,
      locked: a,
      hasActiveThreads: w
    } = A;
    if (null == i) return null;
    let s = (0, h.EB)(i, 48),
      Q = C[r],
      H = o[r],
      c = (0, f.KS)(v, i, {
        locked: a,
        hasActiveThreads: w
      });
    if (null == c) return null;
    let D = null != i ? (0, h.gM)(i) : "";
    return <div role={"img"} className={t}>{<B.ZP mask={B.QS.GUILD_ICON_WITH_CHANNEL_TYPE}>{null != s ? (0, g.jsx)("img", {
          alt: "",
          src: s,
          className: l.channelGuildIcon,
          style: {
            width: H,
            height: H
          }
        }) : (0, g.jsx)("div", {
          className: n()(l.channelGuildIcon, l.acronym),
          style: {
            fontSize: D.length - 1 > Q.length ? Q[Q.length - 1] : Q[D.length - 1],
            width: H,
            height: H
          },
          children: D
        })}</B.ZP>}{<c className={n()(l.icon, l.iconWithGuildIcon, e)} color={"currentColor"} />}</div>
  }