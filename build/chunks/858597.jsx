...
function f(t) {
  ...
  return <div className={j.actionGroup}>{e}</div>
}
...
function N(t) {
  ...
  return <div className={j.actionRow}>{<div className={j.actionButtonDetails}>{<c.Text variant={"text-md/semibold"} color={"header-primary"}>{e}</c.Text>}{<c.Text variant={"text-xs/medium"} color={"header-secondary"}>{n}</c.Text>}</div>}{<c.zxk size={"sm"} variant={s} text={r} onClick={l} disabled={a} />}</div>
}
...
function y(t) {
  ...
  return <c.P3F className={l()(j.actionRow, j.clickableActionRow)} onClick={r}>{<div className={j.actionButtonDetails}>{<c.Text variant={"text-md/semibold"} color={"header-primary"}>{e}</c.Text>}{null != n ? <c.Text variant={"text-xs/medium"} color={"header-secondary"}>{n}</c.Text> : null}</div>}{<c.Fbu size={"md"} color={o.Z.colors.INTERACTIVE_NORMAL} />}</c.P3F>
}
let C = function(t) {
  let {
    otherUserId: e,
    ...
    warningType: o,
    transitionToSlide: j
  } = t, C = (0, S.o)(), L = (0, S.Q)(), R = (0, b.q)(s), v = null != (0, O.M)(s), F = (0, a.e7)([_.Z], () => _.Z.isIgnored(e), [e]), M = (0, a.e7)([_.Z], () => _.Z.isBlocked(e), [e]), Z = r.useCallback(t => {
    (0, E.qc)({
      channelId: s,
      warningId: l,
      senderId: e,
    ...
    u.Z.unblockUser(e, {
      location: A.DL
    }), Z(E.NM.USER_SAFETY_TOOLS_UNBLOCK)
  }, [e, Z]), U = r.useCallback(() => {
    ...
    disabled: M,
    onPress: () => {
      F ? (u.Z.unignoreUser(e, A.cH, s), Z(E.NM.USER_SAFETY_TOOLS_UNIGNORE)) : (u.Z.ignoreUser(e, A.cH, s), Z(E.NM.USER_SAFETY_TOOLS_IGNORE))
    }
  }, {
    type: 0,
    ...
    onPress: () => {
      M ? (k(), x.Z.showUnblockSuccessToast(e, s)) : (0, c.ZDy)(async () => {
        ...
        return n => {
          ...
          return <t transitionState={r} onBlock={I} onBlockAndReport={U} onCancel={() => {
              null == l || l(), Z(E.NM.USER_SAFETY_TOOLS_BLOCK_CANCEL)
            }} onClose={l} userId={e} channelId={s} />
        }
      })
    }
  }, {
    ...
    if (0 === t.type) {
      ...
      return <N type={e} title={n} subTitle={r} buttonText={s} variant={o} onPress={l} disabled={a} />
    }
    if (1 === t.type) {
      let {
        type: e,
      ...
      return <y type={e} title={n} subTitle={r} onPress={s} />
    }
    return null
  };
  return <p.Z>{<f>{P.map(t => B(t))}</f>}{<f>{Y.map(t => B(t))}</f>}{<f>{w.map(t => B(t))}</f>}</p.Z>
}