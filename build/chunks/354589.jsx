...
let W = (0, Chunk108843.Z)((0, Chunk883385.Z)(function(e) {
  ...
  return <s.v2r navId={"user-context"} onClose={o.Zy} aria-label={L.intl.string(L.t.liqwPD)} onSelect={B} onInteraction={J}>{!eI && <t.Fragment>{<s.kSQ>{ex}</s.kSQ>}{<s.kSQ>{q && X}{u && Y}{ee}{en}{!ew && $}{q && er}{q && et}{ef}</s.kSQ>}{G && <s.kSQ>{ei}</s.kSQ>}{ey && <s.kSQ>{!ew && es}{eO}</s.kSQ>}{<s.kSQ>{q && ec}</s.kSQ>}{<s.kSQ>{G && eo}{G && el}{G && ea}{e_}{q && ed}{eu}{em}{eh}{q && eg}{q && eb}{G && ev}</s.kSQ>}{W && <s.kSQ>{ej}{eZ}</s.kSQ>}</t.Fragment>}{<s.kSQ>{ep}</s.kSQ>}</s.v2r>
}, {
