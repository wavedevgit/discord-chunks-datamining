...
var Chunk255367 = require("./255367.js"),
  ...
  Chunk807989 = require("./807989.js"),
  Chunk627799 = require("./627799.js"),
  Chunk260430 = require("./260430.js"),
  Chunk668185 = require("./668185.js"),
  Chunk407546 = require("./407546.js"),
  Chunk41259 = require("./41259.jsx"),
  Chunk28752 = require("./28752.jsx"),
  Chunk787025 = require("./787025.js"),
  Chunk397394 = require("./397394.js"),
  Chunk574384 = require("./574384.js"),
...
function el() {
  ...
  if (Chunk73800.useEffect(() => {
    ...
    let e;
    return e = true === Chunk149765 ? <Chunk255367.Fragment>{<Chunk388905.Dx>{Chunk388032.intl.string(Chunk388032.t.csrAMD)}</Chunk388905.Dx>}{<Chunk388905.DK>{Chunk120356 ? Chunk388032.intl.string(Chunk388032.t["m1+IBg"]) : Chunk388032.intl.string(Chunk388032.t.kRzrSE)}</Chunk388905.DK>}{<Chunk680018.z onClick={() => Chunk373793(true)} variant={"primary"} text={Chunk388032.intl.string(Chunk388032.t.fIv16O)} />}</Chunk255367.Fragment> : <Chunk255367.Fragment>{<Chunk388905.Dx>{Chunk388032.intl.string(Chunk388032.t["Z+hCVV"])}</Chunk388905.Dx>}{<Chunk922770.$ />}</Chunk255367.Fragment>, <Chunk787025.G removeChildWrapper={true}><div className={Chunk453214.deepLinkContainer}>{module}</div></Chunk787025.G>
  }
  return <Chunk787025.G removeChildWrapper={true}><ec{...es(eo({
...
function ec(e) {
  ...
  }), h = null != o ? <div className={a()(er.fakeHeader, {
      [er.moveUp]: !e.hideHeader
    })}>{o}</div> : true, m = null != o || null != s || null != u ? <div className={er.authorize}>{<div className={a()(er.content, d ? er.contentBackground : null, f ? er.noPadding : null)}>{s}{null != c || p.enabled ? null : u}</div>}{null == c && p.enabled ? <div className={a()(er.content, d ? er.contentBackground : null, f ? er.noPadding : null)}>{u}</div> : null}</div> : null;
  return <Q.j{...es(eo({}, e), {
    onClose: () => {
      var t;
...
function eu() {
  return <div className={Chunk453214.loadingContainer}><Chunk922770.$ className={Chunk453214.spinner} /></div>
}
...
function ed(e) {
  ...
  if (ex instanceof Error) return eS ? {
    body: <J.Lk message={ex.message} />
  } : eA ? {
    label: en.intl.string(en.t.j2d6Ki),
    header: <g.Text variant={"text-lg/bold"} color={"header-primary"}>{en.intl.string(en.t.j2d6Ki)}</g.Text>,
    body: <g.Text variant={"text-md/normal"} color={"header-secondary"}>{ex.message}</g.Text>,
    actions: [{
      onClick: eI,
  ...
  switch (eD) {
    case null:
      return {
        label: en.intl.string(en.t.ZTNur6), body: <eu />
      };
    case "SELECT_INSTALL_TYPE":
      ...
      h = <X.Z application={e1} onSelect={tu} />, tp = false, th = false, tm = false, tg = true;
      break;
    case "AUTHORIZE_SCOPES":
      ...
      m = <r.Fragment>{<Y.Z application={eP.application} accountScopes={e6} />}{<W.Z application={eP.application} accountScopes={e6} requestedScopes={e4} integrationType={eJ} errors={tE} isTrustedName={eb || eX} noDivider={!tv && !tO && !ty && e5.enabled} />}{tv ? <g.j7V hideBorder={true} onChange={ez} className={er.mobilePushContainer} note={en.intl.string(en.t.hw1nKS)} value={eK}><g.Text variant={"text-sm/medium"}>{en.intl.string(en.t["Pkw//v"])}</g.Text></g.j7V> : null}{<div className={er.intObserver} ref={tc} />}{tO ? <z.Z error={(null != (a = null != (o = tE[u.x.BOT]) ? o : tE[u.x.APPLICATIONS_COMMANDS]) ? a : [])[0]} selectedGuildId={eZ} onGuildChange={eF} guilds={null != tb ? tb : []} disabled={"" !== eZ && null != eZ && true === em} /> : null}{ty ? <$.Z error={(null != (l = tE[u.x.WEBHOOK_INCOMING]) ? l : [])[0]} selectedChannelId={eV} selectedGuildId={eZ} onChannelChange={eH} /> : null}</r.Fragment>, e4.includes(u.x.BOT) && !d.fS(e8, k.Hn) && (N = "AUTHORIZE_BOT_PERMISSIONS"), ta.length > 1 && (v = "SELECT_INSTALL_TYPE"), t_ = tO && null == eQ || ty && null == eV, tf = true;
      break;
    case "AUTHORIZE_BOT_PERMISSIONS":
      ...
      m = <K.Z application={eP.application} permissions={e8} deniedPermissions={eY} onPermissionsChange={td} guild={eQ} />, v = "AUTHORIZE_SCOPES", tf = true
  }
  let tI = [];
  if (tp && null != eP) {
    let e = null == (_ = eP.bot) ? true : _.approximate_guild_count;
    C = <r.Fragment>{<H.Z application={eP.application} scopes={e4} disclosures={e9} redirectUri={null != (p = eP.redirect_uri) ? p : null} approximateGuildCount={true !== e ? e : null} isEmbeddedFlow={ey} />}{<div className={er.intObserver} ref={tc} />}</r.Fragment>
  }
  return th && null != eP && null != eq && (h = <q.Z user={eq} application={eP.application} bot={eP.bot} accountScopes={e6} showLogout={eg || false} location={eC} scopes={e4} />), tm && (tI.push(null != v ? {
    variant: "secondary",
    onClick: () => eL(v),
...
function ef(e, t) {
  ...
  l && a === ee.Z5c.OAUTH2_AUTHORIZED ? (0, _.h7)(e => <J.jO{...eo({
    guild: t.guild,
    ...
    return Array.isArray(i) && (i = i[0]), <Q.j{...es(eo({}, e), {
