...
function T(e) {
  ...
  return <a.P3F className={N.channelResult} onClick={() => n(t.record.id)}>{<div className={N.channelResultInfo}>{<img alt={""} src={null != i ? (0, p.EB)(i, 32) : true} className={N.guildIcon} />}{<div>{<a.Text className={N.channelName} variant={"text-md/medium"} color={"interactive-normal"}>{<a.gj8 size={"xs"} color={"currentColor"} />}{" "}{t.record.name}</a.Text>}{<a.Text variant={"text-xs/medium"} color={"text-muted"}>{null == i ? true : i.name}</a.Text>}</div>}</div>}{<u.Z users={l} maxUsers={3} hideOverflowCount={true} />}</a.P3F>
}
...
function E(e) {
  ...
  return <div className={s()(N.root, n)}>{<a.E1j className={N.searchBar} placeholder={w.intl.string(w.t["3jvv+/"])} query={u} onChange={e => l({
        query: e,
        resultTypes: [c.h8.VOICE_CHANNEL]
      })} onClear={() => l({
        query: "",
        resultTypes: [c.h8.VOICE_CHANNEL]
      })} autoFocus={true} />}{R.length > 0 ? <a.aVo renderListHeader={function() {
        return (0, r.jsx)(a.Text, {
          variant: "text-xs/semibold",
          color: "text-muted",
          children: w.intl.string(Z.default.FZ9Fl5)
        })
      }} sections={[R.length]} sectionHeight={0} rowHeight={48} renderRow={function(e) {
        let {
          rowIndex: n
        } = e, i = R[n];
        return (0, r.jsx)(T, {
...
function P() {
  return <div className={Chunk152965.emptyState}>{<Chunk481060._Ve className={Chunk152965.emptyIcon} size={"md"} color={Chunk481060.TVs.colors.ICON_MUTED} />}{<Chunk481060.Text className={Chunk152965.emptyHeader} variant={"text-lg/medium"} color={"header-primary"}>{Chunk388032.intl.string(Chunk676462.default.sWGfr6)}</Chunk481060.Text>}{<Chunk481060.Text variant={"text-sm/medium"} color={"text-secondary"}>{Chunk388032.intl.string(Chunk676462.default.nBRtt7)}</Chunk481060.Text>}</div>
}