...
let y = "MULTIPLE_CHOICE",
  v = e => {
    ...
    return l.useLayoutEffect(() => (v(h), j(C(b)), () => {
      v(null), C(null)
    }), [v, C, j]), <div ref={b} className={i()(x.draggableInputContainer, {
        [x.dragging]: m
      })} data-dnd-name={t}>{<div className={x.radioIconEmptyContainer}><g.Z height={20} width={20} className={x.radioIconEmpty} /></div>}{<div className={x.inputWrapper}>{<u.Is autoFocus={true} onChange={e => a(e)} placeholder={f.intl.formatToPlainString(f.t["Ep/pbG"], {
            index: n + 1
          })} value={t} />}{<div className={x.clearButton}><d.hU icon={d.k$p} size={"sm"} aria-label={f.intl.string(f.t.VkKicX)} variant={"icon-only"} onClick={s} /></div>}</div>}{<div ref={h} className={x.dragContainer} data-dnd-name={t}><d.Vni size={"xs"} color={"currentColor"} className={x.dragIcon} /></div>}</div>
  },
  j = function(e) {
    var t, n, a, i;
    let {
