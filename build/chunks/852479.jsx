/** Chunk was on 6850 **/
/** chunk id: 852479, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  Z: () => h
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk913527 = require("./913527.js"),
  i = require.n(Chunk913527),
  Chunk399606 = require("./399606.js"),
  Chunk755721 = require("./755721.js"),
  Chunk481060 = require("./481060.js"),
  Chunk740900 = require("./740900.js"),
  Chunk241559 = require("./241559.js"),
  Chunk893966 = require("./893966.js"),
  Chunk527379 = require("./527379.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk350381 = require("./350381.js"),
  Chunk93163 = require("./93163.js");

function h(e) {
  let {
    guild: t,
    onSubmit: o
  } = e, h = (0, a.e7)([C.Z], () => C.Z.getMembersCountByGuildId(t.id, u.R_.NEW_GUILD_MEMBER), [t.id]), j = (0, a.e7)([C.Z], () => {
    let e = C.Z.getNewMemberTimestamp(t.id);
    return i()(e).format("h:mm A")
  }, [t.id]), g = (0, a.e7)([C.Z], () => C.Z.hasDefaultSearchStateByGuildId(t.id), [t.id]), x = l.useCallback(() => {
    (0, m.YO)(t.id), null == o || o()
  }, [t.id, o]), v = l.useCallback(() => {
    g ? x() : (0, c.ZDy)(async () => {
      let {
        default: e
      } = await n.e("24478").then(n.bind(n, 319481));
      return t => {
        var n, l;
        return <e{...n = function(e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = null != arguments[t] ? arguments[t] : {},
              r = Object.keys(n);
            "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
              return Object.getOwnPropertyDescriptor(n, e).enumerable
            }))), r.forEach(function(t) {
              var r;
              r = n[t], t in e ? Object.defineProperty(e, t, {
                value: r,
                enumerable: true,
                configurable: true,
                writable: true
              }) : e[t] = r
            })
          }
          return e
        }({}, t), l = l = {
          onConfirm: x
        }, Object.getOwnPropertyDescriptors ? Object.defineProperties(n, Object.getOwnPropertyDescriptors(l)) : (function(e, t) {
          var n = Object.keys(e);
          if (Object.getOwnPropertySymbols) {
            var r = Object.getOwnPropertySymbols(e);
            n.push.apply(n, r)
          }
          return n
        })(Object(l)).forEach(function(e) {
          Object.defineProperty(n, e, Object.getOwnPropertyDescriptor(l, e))
        }), n} />
      }
    })
  }, [x, g]), y = new Intl.NumberFormat(b.intl.currentLocale).format(h), O = (0, d.nX)(t.id), H = (0, d.xC)(t.id), _ = l.useMemo(() => 1 + +!!O + +!!H, [O, H]);
  return 0 === h ? null : <tr className={f.newMembersNotice} onClick={v}>{<td colSpan={3}><c.Text variant={"text-sm/normal"} color={"always-white"}>{b.intl.format(b.t["/i5uJy"], {
          count: y,
          date: j
        })}</c.Text></td>}{<td colSpan={_} />}{<td colSpan={2}><div className={p.actionCell}><s.zx size={s.zx.Sizes.SMALL} color={s.zx.Colors.TRANSPARENT} look={s.zx.Looks.LINK} className={f.memberNoticeActionButton}><c.Text variant={"text-sm/normal"} color={"always-white"} className={f.memberNoticeAction}>{<div>{b.intl.string(b.t.rkyOzM)}</div>}{<c.tBG size={"custom"} color={"currentColor"} width={20} height={20} className={f.memberNoticeActionIcon} />}</c.Text></s.zx></div></td>}</tr>
}