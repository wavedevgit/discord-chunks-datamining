/** Chunk was on 58354 **/
"use strict";

function n(e) {
  let t = function(e) {
    var t;
    let r, n, o, {
        PI: l,
        min: i,
        max: u,
        cos: a,
        round: s
      } = Math,
      c = e[0] | e[1] << 8 | e[2] << 16,
      d = e[3] | e[4] << 8,
      f = (63 & c) / 63,
      p = (c >> 6 & 63) / 31.5 - 1,
      b = (c >> 12 & 63) / 31.5 - 1,
      v = c >> 23,
      O = d >> 15,
      g = u(3, O ? v ? 5 : 7 : 7 & d),
      m = u(3, O ? 7 & d : v ? 5 : 7),
      h = v ? (15 & e[5]) / 15 : 1,
      y = (e[5] >> 4) / 15,
      w = v ? 6 : 5,
      j = 0,
      P = (t, r, n) => {
        let o = [];
        for (let l = 0; l < r; l++)
          for (let i = +!l; i * r < t * (r - l); i++) o.push(((e[w + (j >> 1)] >> ((1 & j++) << 2) & 15) / 7.5 - 1) * n);
        return o
      },
      S = P(g, m, (c >> 18 & 31) / 31),
      x = P(3, 3, (d >> 3 & 63) / 63 * 1.25),
      D = P(3, 3, (d >> 9 & 63) / 63 * 1.25),
      I = v && P(5, 5, y),
      E = (r = (t = e)[3], n = 128 & t[2], ((o = 128 & t[4]) ? n ? 5 : 7 : 7 & r) / (o ? 7 & r : n ? 5 : 7)),
      k = s(E > 1 ? 32 : 32 * E),
      C = s(E > 1 ? 32 / E : 32),
      _ = new Uint8Array(k * C * 4),
      Z = [],
      M = [];
    for (let e = 0, t = 0; e < C; e++)
      for (let r = 0; r < k; r++, t += 4) {
        let n = f,
          o = p,
          s = b,
          c = h;
        for (let e = 0, t = u(g, v ? 5 : 3); e < t; e++) Z[e] = a(l / k * (r + .5) * e);
        for (let t = 0, r = u(m, v ? 5 : 3); t < r; t++) M[t] = a(l / C * (e + .5) * t);
        for (let e = 0, t = 0; e < m; e++)
          for (let r = +!e, o = 2 * M[e]; r * m < g * (m - e); r++, t++) n += S[t] * Z[r] * o;
        for (let e = 0, t = 0; e < 3; e++)
          for (let r = +!e, n = 2 * M[e]; r < 3 - e; r++, t++) {
            let e = Z[r] * n;
            o += x[t] * e, s += D[t] * e
          }
        if (v)
          for (let e = 0, t = 0; e < 5; e++)
            for (let r = +!e, n = 2 * M[e]; r < 5 - e; r++, t++) c += I[t] * Z[r] * n;
        let d = n - 2 / 3 * o,
          O = (3 * n - d + s) / 2,
          y = O - s;
        _[t] = u(0, 255 * i(1, O)), _[t + 1] = u(0, 255 * i(1, y)), _[t + 2] = u(0, 255 * i(1, d)), _[t + 3] = u(0, 255 * i(1, c))
      }
    return {
      w: k,
      h: C,
      rgba: _
    }
  }(e);
  return function(e, t, r) {
    let n = 4 * e + 1,
      o = 6 + t * (5 + n),
      l = [137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, e >> 8, 255 & e, 0, 0, t >> 8, 255 & t, 8, 6, 0, 0, 0, 0, 0, 0, 0, o >>> 24, o >> 16 & 255, o >> 8 & 255, 255 & o, 73, 68, 65, 84, 120, 1],
      i = [0, 0x1db71064, 0x3b6e20c8, 0x26d930ac, 0x76dc4190, 0x6b6b51f4, 0x4db26158, 0x5005713c, -0x12477ce0, -0xff06cbc, -0x29295c18, -0x349e4c74, -0x649b3d50, -0x792c2d2c, -0x5ff51d88, -0x42420de4],
      u = 1,
      a = 0;
    for (let e = 0, o = 0, i = n - 1; e < t; e++, i += n - 1)
      for (l.push(e + 1 < t ? 0 : 1, 255 & n, n >> 8, 255 & ~n, n >> 8 ^ 255, 0), a = (a + u) % 65521; o < i; o++) {
        let e = 255 & r[o];
        l.push(e), a = (a + (u = (u + e) % 65521)) % 65521
      }
    for (let [e, t] of(l.push(a >> 8, 255 & a, u >> 8, 255 & u, 0, 0, 0, 0, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66, 96, 130), [
        [12, 29],
        [37, 41 + o]
      ])) {
      let r = -1;
      for (let n = e; n < t; n++) r ^= l[n], r = (r = r >>> 4 ^ i[15 & r]) >>> 4 ^ i[15 & r];
      r = ~r, l[t++] = r >>> 24, l[t++] = r >> 16 & 255, l[t++] = r >> 8 & 255, l[t++] = 255 & r
    }
    return "data:image/png;base64," + btoa(String.fromCharCode(...l))
  }(t.w, t.h, t.rgba)
}
r.d(t, {
  xS: () => n
})