...
function r(e) {
  let t = function(e) {
    ...
    let {
      ...
      cos: a,
      round: c
    } = Math, d = e[0] | e[1] << 8 | e[2] << 16, u = e[3] | e[4] << 8, f = (63 & d) / 63, h = (d >> 6 & 63) / 31.5 - 1, m = (d >> 12 & 63) / 31.5 - 1, x = d >> 23, p = u >> 15, g = o(3, p ? x ? 5 : 7 : 7 & u), _ = o(3, p ? 7 & u : x ? 5 : 7), v = x ? (15 & e[5]) / 15 : 1, j = (e[5] >> 4) / 15, b = x ? 6 : 5, C = 0, S = (t, n, r) => {
      let l = [];
      for (let i = 0; i < n; i++)
        for (let s = +!i; s * n < t * (n - i); s++) l.push(((e[b + (C >> 1)] >> ((1 & C++) << 2) & 15) / 7.5 - 1) * r);
      return l
    }, N = S(g, _, (d >> 18 & 31) / 31), y = S(3, 3, (u >> 3 & 63) / 63 * 1.25), Z = S(3, 3, (u >> 9 & 63) / 63 * 1.25), w = x && S(5, 5, j), O = (n = (t = e)[3], r = 128 & t[2], ((l = 128 & t[4]) ? r ? 5 : 7 : 7 & n) / (l ? 7 & n : r ? 5 : 7)), I = c(O > 1 ? 32 : 32 * O), P = c(O > 1 ? 32 / O : 32), T = new Uint8Array(I * P * 4), E = [], R = [];
    for (let e = 0, t = 0; e < P; e++)
      for (let n = 0; n < I; n++, t += 4) {
        let r = f,
          ...
          c = m,
          d = v;
        for (let e = 0, t = o(g, x ? 5 : 3); e < t; e++) E[e] = a(i / I * (n + .5) * e);
        for (let t = 0, n = o(_, x ? 5 : 3); t < n; t++) R[t] = a(i / P * (e + .5) * t);
        ...
        let u = r - 2 / 3 * l,
          p = (3 * r - u + c) / 2,
          j = p - c;
        T[t] = o(0, 255 * s(1, p)), T[t + 1] = o(0, 255 * s(1, j)), T[t + 2] = o(0, 255 * s(1, u)), T[t + 3] = o(0, 255 * s(1, d))
      }
    return {
      w: I,
      h: P,
