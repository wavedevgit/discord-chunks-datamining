...
function r(e) {
  let t = function(e) {
    ...
    let {
      ...
      cos: a,
      round: c
    } = Math, d = e[0] | e[1] << 8 | e[2] << 16, u = e[3] | e[4] << 8, f = (63 & d) / 63, m = (d >> 6 & 63) / 31.5 - 1, h = (d >> 12 & 63) / 31.5 - 1, x = d >> 23, g = u >> 15, p = o(3, g ? x ? 5 : 7 : 7 & u), _ = o(3, g ? 7 & u : x ? 5 : 7), v = x ? (15 & e[5]) / 15 : 1, j = (e[5] >> 4) / 15, b = x ? 6 : 5, C = 0, N = (t, n, r) => {
      let l = [];
      for (let i = 0; i < n; i++)
        for (let s = +!i; s * n < t * (n - i); s++) l.push(((e[b + (C >> 1)] >> ((1 & C++) << 2) & 15) / 7.5 - 1) * r);
      return l
    }, S = N(p, _, (d >> 18 & 31) / 31), y = N(3, 3, (u >> 3 & 63) / 63 * 1.25), Z = N(3, 3, (u >> 9 & 63) / 63 * 1.25), w = x && N(5, 5, j), O = (n = (t = e)[3], r = 128 & t[2], ((l = 128 & t[4]) ? r ? 5 : 7 : 7 & n) / (l ? 7 & n : r ? 5 : 7)), I = c(O > 1 ? 32 : 32 * O), P = c(O > 1 ? 32 / O : 32), T = new Uint8Array(I * P * 4), E = [], R = [];
    for (let e = 0, t = 0; e < P; e++)
      for (let n = 0; n < I; n++, t += 4) {
        ...
        for (let e = 0, t = 0; e < _; e++)
          for (let n = +!e, l = 2 * R[e]; n * _ < p * (_ - e); n++, t++) r += S[t] * E[n] * l;
