...
let I = e => {
  ...
  return <div className={I}>{O && <div className={E.previewTitleContainer}>{<c.vwX className={E.previewTitle}>{g.intl.string(g.t.PpoJzs)}</c.vwX>}{D && <c.yRy targetElementRef={R} preload={() => (0, f.Z)(S.id, S.getAvatarURL(null, 80))} renderPopout={e => (0, r.jsx)(d.Z, v(y({}, e), {
          user: S,
        ...
        }))} align={"center"} position={"right"}>{e => (0, r.jsx)(c.P3F, v(y({}, e), {
        ...
        }))}</c.yRy>}</div>}{<c.P3F tag={"div"} onClick={L} className={a()(E.previewContainer, {
        [E.previewContainerSelected]: n && null == A,
        [E.previewContainerError]: n && null != A
      })}>{<div className={E.giftInfoContainer}>{<m.O product={T} fallbackLabel={null} />}{<div className={E.previewTextContainer}>{<c.Text variant={"text-md/semibold"}>{C}</c.Text>}{<c.X6q variant={"heading-sm/medium"} color={"header-secondary"}>{w()}</c.X6q>}</div>}{<c.Text variant={"text-md/semibold"}>{o}</c.Text>}</div>}{n && null != A && <div className={E.recipientError}><c.Text variant={"text-sm/normal"} color={"status-danger"}>{A}</c.Text></div>}</c.P3F>}</div>
}