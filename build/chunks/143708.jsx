...
function m(e) {
  ...
  return v ? (t = p.intl.string(p.t["F/OLvL"]), m = p.intl.string(p.t.C89OLC)) : (m = p.intl.string(p.t["YVub5+"]), E ? t = p.intl.string(p.t["57nBt7"]) : null != O && I ? (t = p.intl.formatToPlainString(p.t["9Akp1t"], {
    username: O
  }), n = <a.Z.Button onClick={T}>{p.intl.string(p.t.P8tvKC)}</a.Z.Button>) : t = p.intl.string(p.t["SMJr+f"])), <a.Z>{<a.Z.Header text={m} />}{<a.Z.Body>{<div className={h.headerLine}>{<a.Z.Icon expired={true} />}{<a.Z.Info expired={true} title={p.intl.string(p.t["Jhx/ub"])}>{t}</a.Z.Info>}</div>}{n}</a.Z.Body>}</a.Z>
}