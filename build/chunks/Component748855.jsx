/** Chunk was on 81985 **/
/** chunk id: 748855, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  L: () => g
}), require("./388685.js");
var Chunk54381 = require("./54381.js"),
  Chunk473749 = require("./473749.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk442837 = require("./442837.js"),
  Chunk692547 = require("./692547.js"),
  Chunk681715 = require("./681715.js"),
  Chunk481060 = require("./481060.js"),
  Chunk106301 = require("./106301.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk227426 = require("./227426.js"),
  Chunk564546 = require("./564546.js");

function g(e) {
  let {
    label: t,
    icon: n,
    setStatus: l,
    isFavorited: g,
    onFavoriteClick: m
  } = e, [b, y] = i.useState(false), v = (0, o.e7)([d.Z], () => d.Z.getFavoritedStatuses().length >= d.o), O = null != m, j = i.useCallback(e => {
    e.stopPropagation(), null == m || m()
  }, [m]), x = g ? u.r7p : u.vxU;
  return (0, r.jsxs)(u.P3F, {
    onClick: l,
    onMouseEnter: () => {
      y(true)
    },
    onMouseLeave: () => {
      y(false)
    },
    className: a()(h.item, h.labelContainer, f.container),
    children: [(0, r.jsx)("div", {
      children: n
    }), (0, r.jsx)(u.Text, {
      variant: "text-md/normal",
      className: f.itemText,
      children: t
    }), b && O && (0, r.jsx)(c.u, {
      text: v && !g ? p.intl.string(p.t.YSDH9n) : true,
      children: (0, r.jsx)(u.P3F, {
        className: a()(f.centerAlign, f.favoriteIcon),
        onClick: j,
        children: (0, r.jsx)(x, {
          className: f.closeIcon,
          color: g ? s.Z.colors.ICON_FEEDBACK_WARNING : "currentColor"
        })
      })
    })]
  })
}