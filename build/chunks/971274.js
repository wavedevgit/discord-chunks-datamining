...
function u(e) {
  ...
  return {
    ...
    focusNextRow() {
      h.days ? this.focusNextPage() : (h.weeks || h.months || h.years) && G(R.add({
        weeks: 1
      }))
    },
    focusPreviousRow() {
      h.days ? this.focusPreviousPage() : (h.weeks || h.months || h.years) && G(R.subtract({
        weeks: 1
      }))
    },
    focusNextPage() {
    ...
    focusNextSection(e) {
      ...
      h.days ? this.focusNextPage() : h.weeks ? G(R.add({
        months: 1
      })) : (h.months || h.years) && G(R.add({
        years: 1
      }))
    },
    focusPreviousSection(e) {
      if (!e && !h.days) return void G(R.subtract(d(h)));
      h.days ? this.focusPreviousPage() : h.weeks ? G(R.subtract({
        months: 1
      })) : (h.months || h.years) && G(R.subtract({
        years: 1
      }))
    },
    selectFocusedDate() {
