/** Chunk was on 49152 **/
/** chunk id: 901434, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  Z: () => v
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk442837 = require("./442837.js"),
  Chunk755721 = require("./755721.js"),
  Chunk481060 = require("./481060.js"),
  Chunk287734 = require("./287734.js"),
  Chunk358221 = require("./358221.js"),
  Chunk305325 = require("./305325.js"),
  Chunk281956 = require("./281956.js"),
  Chunk27457 = require("./27457.jsx"),
  Chunk546247 = require("./546247.jsx"),
  Chunk823379 = require("./823379.js"),
  Chunk5192 = require("./5192.js"),
  Chunk501655 = require("./501655.js"),
  Chunk856393 = require("./856393.jsx"),
  Chunk388032 = require("./388032.jsx"),
  Chunk761325 = require("./761325.js");
let O = (e, t) => b.ZP.getName(e.getGuildId(), e.id, t.user),
  v = function(e) {
    let {
      participants: t,
      channel: n,
      hasConnectPermission: l
    } = e, b = (0, p.J)(n.guild_id), v = i.useCallback(() => {
      b ? (0, h.hk)(n.guild_id, () => u.default.selectVoiceChannel(n.id)) : u.default.selectVoiceChannel(n.id)
    }, [n.id, n.guild_id, b]), C = t.filter(e => e.type === y.Ui.VOICE), E = 4 === C.length ? 2 : 3, S = (0, s.Wu)([d.Z], () => C.map(e => d.Z.getParticipant(n.id, e.id)).filter(m.lm), [n.id, C]);
    return <div className={j.container}>{<g.Z />}{<div className={j.tiles} style={{
          maxWidth: 168 * E
        }}>{S.slice(0, 5).map(e => (0, r.jsx)(f.ZP, {
          participant: e,
          channel: n,
          className: j.tile,
          inCall: true,
          noVideoRender: true,
          inPopout: false,
          width: 48
        }, e.id))}</div>}{<c.X6q className={j.channelName} variant={"heading-xxl/normal"}>{n.name}</c.X6q>}{<div className={j.participantsRow}><c.Text tag={"div"} color={"header-secondary"} variant={"heading-lg/normal"}>{0 === C.length ? _.intl.string(_.t.FUVhyM) : 1 === C.length ? _.intl.formatToPlainString(_.t.EQwZlJ, {
            a: O(n, C[0])
          }) : 2 === C.length ? _.intl.formatToPlainString(_.t.zBcKoK, {
            a: O(n, C[0]),
            b: O(n, C[1])
          }) : C.length > 2 ? _.intl.formatToPlainString(_.t["3AqFaG"], {
            a: O(n, C[0]),
            b: O(n, C[1]),
            n: C.length - 2
          }) : true}</c.Text></div>}{<div className={j.buttonContainer}>{<o.zx disabled={!l} className={a()(j.joinButton, {
            [j.roundButton]: false
          })} color={l ? o.zx.Colors.WHITE : o.zx.Colors.PRIMARY} onClick={v} size={o.zx.Sizes.MEDIUM}>{l ? _.intl.string(_.t["7vb2cX"]) : _.intl.string(_.t.TVBCKS)}</o.zx>}{<x.e channel={n} />}</div>}</div>
  }