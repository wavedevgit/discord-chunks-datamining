...
async function C() {
  ...
  n > h.Z.Millis.HOUR && g.Z.addBreadcrumb({
...
function j() {
  if (P || null != T && T !== _.hes.DISCONNECTED && T !== _.hes.RTC_DISCONNECTED) try {
    C()
  } catch (e) {
    g.Z.captureException(e)
  } else v && (v = !1, g.Z.addBreadcrumb({
    category: O,
...
function A() {
  ...
  }), d.Z.addChangeListener(x), l.Z.subscribe("WINDOW_FOCUS", w), l.Z.subscribe("APP_STATE_UPDATE", L), l.Z.subscribe("LOGIN_SUCCESS", Z), j()
}
