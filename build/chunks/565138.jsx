...
class R extends Chunk73800.PureComponent {
  renderAcronym() {
    ...
    return null != module.icon || null != exports ? null : <div className={Chunk812737.acronym}>{(0, Chunk601964.gM)(module)}</div>
  }
  renderBadge() {
    let {
      showBadge: e,
    ...
    return module ? <Chunk372769.Z className={Chunk812737.guildIconBadge} guild={exports} badgeStrokeColor={require} tooltipColor={r} tooltipDelay={Chunk73800} /> : null
  }
  renderIcon() {
    var e, t;
    let n = this.props,
  ...
  renderTooltip() {
    ...
    return exports ? <Chunk481060.ua7 text={module.name} position={require} aria-label={false}>{e => o.cloneElement(o.Children.only(this.renderIcon()), O({}, e))}</Chunk481060.ua7> : this.renderIcon()
  }
  render() {
    let {
      to: e,
    ...
    return null != module ? <Chunk688642.rU aria-hidden={Chunk73800} to={{
        pathname: module,
        state: null != require ? {
          analyticsSource: require
        } : null
      }} aria-label={exports.name} tabIndex={r}>{this.renderTooltip()}</Chunk688642.rU> : this.renderTooltip()
  }
}
...
})((0, Chunk891819.N)(e => <R{...O({}, e)} />));
class w extends(r = Chunk73800.PureComponent) {
  render() {
    return <P{...O({}, this.props)} />
  }
}
