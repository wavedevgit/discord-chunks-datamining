...
var Chunk255367 = require("./255367.js"),
  ...
  Chunk384298 = require("./384298.js"),
  Chunk386019 = require("./386019.js"),
...
function W(e) {
  ...
  let eh = () => {
      ...
      return null == ep && ed ? <div className={V.preview}><l.$jN /></div> : null == ep || ec ? <l.P3F className={a()(V.preview, V.overlay, es ? V.clickable : V.disabled)} onClick={e} aria-label={(0, y.gR)(el)} aria-disabled={!es}>{<img alt={""} src={Q === B.BRd.LIGHT ? Y : H} />}{X !== G.lY.MODAL && X !== G.lY.MODAL_V2 && <r.Fragment>{<l.Text className={V.overlayText} variant={"text-sm/medium"} color={"always-white"}>{(0, y.P9)(el)}</l.Text>}{<l.Text variant={"text-sm/medium"}>{ef ? Z.intl.string(Z.t.uQZTBQ) : Z.intl.string(Z.t.pgUTZG)}</l.Text>}</r.Fragment>}</l.P3F> : <l.P3F className={a()(V.preview, V.overlay, es ? V.clickable : V.disabled)} onClick={e} aria-label={(0, y.gR)(el)} aria-disabled={!es}>{<img alt={""} src={ep} className={V.image} />}{X !== G.lY.MODAL && X !== G.lY.MODAL_V2 && <r.Fragment>{<l.Text className={V.overlayText} variant={"text-sm/medium"} color={"always-white"}>{(0, y.P9)(el)}</l.Text>}{<l.gw7 size={"xs"} color={"currentColor"} className={V.clyde} />}</r.Fragment>}</l.P3F>
    },
    em = () => X !== G.lY.MODAL && X !== G.lY.MODAL_V2 ? null : <div className={F.details}>{<M.Z channel={et} guild={ee} onAction={eo} onClose={K} />}{<E.Z users={en} guildId={ee.id} channelId={et.id} maxUsers={k.W} size={l.EFr.SIZE_16} onClickOverflow={e => {
          null == e || e.stopPropagation(), null == eo || eo({
            action: "PRESS_VOICE_CHANNEL_AVATARS"
          })
        }} onUserClick={e => e.stopPropagation()} disableUserPopout={e => e === t.id} overflowCountColor={"text-muted"} aria-label={Z.intl.string(Z.t.jNqDh4)} />}{X === G.lY.MODAL_V2 && eE()}</div>,
    eg = () => X === G.lY.MODAL || X === G.lY.MODAL_V2 ? null : <k.Z user={t} guild={ee} channel={et} onAction={eo} onClose={K} />,
    eE = () => t.id === n.id ? null : <div className={F.actions}><j.Z channel={et} onAction={eo} onClose={K} /></div>,
    eb = eu,
    ey = J === G.n_.ACTIVITY && $ === eb,
    eO = null != er ? Z.intl.formatToPlainString(Z.t["4CQq9f"], {
      name: er.name
    }) : Z.intl.string(Z.t["Jpkr/v"]);
  return <m.Gt value={ei}><D.Z targetElementRef={q} sourceType={G.n_.ACTIVITY} user={t}><R.Z user={t} guildId={z} themeType={X} sourceId={eb} sourceDetails={eO} sourceType={G.n_.ACTIVITY} onAction={eo}>{() => (0, r.jsx)(P.Z, {
          ref: q,
          user: t,
