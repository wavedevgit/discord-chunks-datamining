...
let w = 40,
  D = e => {
    ...
    if (x === o) return <div className={a()(A.popoutWrapper, {
        [A.mounted]: R
      })}><I.v currentStatus={t} channel={N} /></div>;
    ...
    return <l.m position={"right"} targetElementRef={D}>{<div className={A.contentContainer}>{<div className={A.statusGroup}>{<c.Text variant={"text-xs/medium"} color={"text-muted"}>{S.intl.string(S.t["74vS//"])}</c.Text>}{<c.Text variant={"text-sm/semibold"} className={A.statusText}>{(0, y.O8)(t)}</c.Text>}</div>}{<div className={A.iconGroup}>{<v.Z className={A.statusIcon} hangStatusActivity={t} />}{<div className={A.avatarWrapper}>{<c.qEK className={A.avatar} size={c.EFr.SIZE_40} src={B} aria-hidden={true} />}{<div className={A.outline} />}</div>}</div>}</div>}{!k && j ? <c.zxk size={"sm"} text={S.intl.string(S.t["B/dHXF"])} onClick={Z} /> : null}{k && U ? <c.zxk size={"sm"} text={S.intl.string(S.t.xcVcFR)} onClick={F} /> : null}</l.m>
  }