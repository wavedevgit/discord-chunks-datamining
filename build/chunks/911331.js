...
var s = function(e, t) {
  ...
  for (var g = n.visualViewport ? n.visualViewport.width : innerWidth, E = n.visualViewport ? n.visualViewport.height : innerHeight, b = window.scrollX || pageXOffset, y = window.scrollY || pageYOffset, O = e.getBoundingClientRect(), v = O.height, I = O.width, T = O.top, S = O.right, A = O.bottom, N = O.left, C = "start" === s || "nearest" === s ? T : "end" === s ? A : T + v / 2, R = "center" === l ? N + I / 2 : "end" === l ? S : N, P = [], w = 0; w < h.length; w++) {
    var D = h[w],
      ...
      U = L.bottom,
      G = L.left;
    if ("if-needed" === i && T >= 0 && N >= 0 && A <= E && S <= g && T >= M && A <= U && N >= G && S <= j) break;
    ...
    if (p === D) Y = "start" === s ? C : "end" === s ? C - E : "nearest" === s ? o(y, y + E, E, V, H, y + C, y + C + v, v) : C - E / 2, W = "start" === l ? R : "center" === l ? R - g / 2 : "end" === l ? R - g : o(b, b + g, g, F, Z, b + R, b + R + I, I), Y = Math.max(0, Y + y), W = Math.max(0, W + b);
    else {
      Y = "start" === s ? C - M - V : "end" === s ? C - U + H + z : "nearest" === s ? o(M, U, x, V, H + z, C, C + v, v) : C - (M + x / 2) + z / 2, W = "start" === l ? R - G - F : "center" === l ? R - (G + k / 2) + K / 2 : "end" === l ? R - j + Z + K : o(G, j, k, F, Z + K, R, R + I, I);
      var Q = D.scrollLeft,
        J = D.scrollTop;
      C += J - (Y = Math.max(0, Math.min(J + Y / X, D.scrollHeight - x / X + z))), R += Q - (W = Math.max(0, Math.min(Q + W / q, D.scrollWidth - k / q + K)))
    }
    P.push({
      el: D,
      top: Y,
