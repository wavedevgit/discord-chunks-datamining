...
var s = function(e, t) {
  ...
  for (var g = n.visualViewport ? n.visualViewport.width : innerWidth, E = n.visualViewport ? n.visualViewport.height : innerHeight, b = window.scrollX || pageXOffset, y = window.scrollY || pageYOffset, v = e.getBoundingClientRect(), O = v.height, I = v.width, S = v.top, T = v.right, A = v.bottom, N = v.left, C = "start" === s || "nearest" === s ? S : "end" === s ? A : S + O / 2, P = "center" === l ? N + I / 2 : "end" === l ? T : N, R = [], w = 0; w < h.length; w++) {
    ...
    if (p === D) Y = "start" === s ? C : "end" === s ? C - E : "nearest" === s ? o(y, y + E, E, F, H, y + C, y + C + O, O) : C - E / 2, W = "start" === l ? P : "center" === l ? P - g / 2 : "end" === l ? P - g : o(b, b + g, g, V, Z, b + P, b + P + I, I), Y = Math.max(0, Y + y), W = Math.max(0, W + b);
    else {
      Y = "start" === s ? C - k - F : "end" === s ? C - U + H + z : "nearest" === s ? o(k, U, x, F, H + z, C, C + O, O) : C - (k + x / 2) + z / 2, W = "start" === l ? P - G - V : "center" === l ? P - (G + M / 2) + K / 2 : "end" === l ? P - j + Z + K : o(G, j, M, V, Z + K, P, P + I, I);
      var X = D.scrollLeft,
        J = D.scrollTop;
      C += J - (Y = Math.max(0, Math.min(J + Y / Q, D.scrollHeight - x / Q + z))), P += X - (W = Math.max(0, Math.min(X + W / q, D.scrollWidth - M / q + K)))
    }
    R.push({
      el: D,
      top: Y,
