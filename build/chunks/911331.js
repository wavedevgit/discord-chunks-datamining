...
var s = function(e, t) {
  ...
  for (var g = n.visualViewport ? n.visualViewport.width : innerWidth, E = n.visualViewport ? n.visualViewport.height : innerHeight, b = window.scrollX || pageXOffset, y = window.scrollY || pageYOffset, O = e.getBoundingClientRect(), v = O.height, I = O.width, S = O.top, T = O.right, A = O.bottom, C = O.left, N = "start" === s || "nearest" === s ? S : "end" === s ? A : S + v / 2, R = "center" === l ? C + I / 2 : "end" === l ? T : C, P = [], w = 0; w < h.length; w++) {
    var D = h[w],
      ...
      U = x.bottom,
      G = x.left;
    if ("if-needed" === i && S >= 0 && C >= 0 && A <= E && T <= g && S >= M && A <= U && C >= G && T <= k) break;
    ...
    if (p === D) Y = "start" === s ? N : "end" === s ? N - E : "nearest" === s ? o(y, y + E, E, F, H, y + N, y + N + v, v) : N - E / 2, W = "start" === l ? R : "center" === l ? R - g / 2 : "end" === l ? R - g : o(b, b + g, g, Z, V, b + R, b + R + I, I), Y = Math.max(0, Y + y), W = Math.max(0, W + b);
      ...
      Y = "start" === s ? N - M - F : "end" === s ? N - U + H + z : "nearest" === s ? o(M, U, L, F, H + z, N, N + v, v) : N - (M + L / 2) + z / 2, W = "start" === l ? R - G - Z : "center" === l ? R - (G + j / 2) + K / 2 : "end" === l ? R - k + V + K : o(G, k, j, Z, V + K, R, R + I, I);
      var Q = D.scrollLeft,
        J = D.scrollTop;
