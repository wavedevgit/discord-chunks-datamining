...
class P extends Chunk73800.PureComponent {
  render() {
    ...
    return <span onContextMenu={exports} className={Chunk677234.userHook}>{<Chunk481060.Text variant={"text-md/normal"}>{Chunk51144.ZP.getUserTag(module, {
          mode: "username"
        })}</Chunk481060.Text>}{"0" !== module.discriminator && <Chunk481060.Text variant={"text-xs/normal"} className={Chunk677234.discrim}>{"#"}{module.discriminator}</Chunk481060.Text>}</span>
  }
}
let w = Chunk73800.forwardRef((e, t) => {
  ...
  n = B === F ? <d.Text className={E.timestamp} variant={"text-sm/normal"}>{B}</d.Text> : <d.Text className={E.timestamp} variant={"text-sm/normal"}>{B}{"—"}{F}</d.Text>;
  let H = G ? D : N.dG4;
  return <s.mh id={l.id}>{e => (0, r.jsxs)("div", {
      ref: t,
