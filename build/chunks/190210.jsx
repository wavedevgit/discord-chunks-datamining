...
function ea(e, t) {
  ...
  return <I.f6 value={eX}><b.Gt value={eD}>{<div ref={eL} className={a()(T, Q.channelTextArea)} onMouseDown={tr}>{<div ref={ek} onScroll={e5} className={a()(Q.announcementScrollableContainer, {
            [Q.themedBackground]: !eT
          })}><div className={a()(Q.inner, X.inner)}>{<div className={X.body}>{<c.animated.div className={X.headerInput} style={{
                  height: tu,
                  paddingTop: td
                }}>{null != eR ? <ec file={eR} onRemoveHeroImage={tE} /> : null}{null != eR ? null : <ed channel={j} onImageUploaded={e => (0, v.c)(j.id, {
                    heroFile: e
                  })} onFocus={() => tl(true)} />}{<input onFocus={() => tl(true)} onBlur={() => tl(false)} maxLength={en} className={X.titleInput} placeholder={q.intl.string(q.t.Z8fYjI)} value={eC} onChange={e => (0, v.c)(j.id, {
                    title: e.target.value
                  })} />}</c.animated.div>}{<div className={X.inputWrapper}><_.tEY ringTarget={eL} ringClassName={Q.focusRing}><B.Z ref={eM} id={A} focused={z} useSlate={eH} textValue={f} richValue={m} disabled={eZ} placeholder={eE} required={N} accessibilityLabel={M} isPreviewing={(eG || eB) && eV} channel={j} type={G.Ie.CREATE_ANNOUNCEMENT_POST} canPasteFiles={eF} uploadPromptCharacterCount={Y.en1} maxCharacterCount={null != eg ? eg : eW} allowNewLines={true} aria-describedby={eb} onChange={J} onResize={e6} onBlur={ea} onFocus={es} onKeyDown={ef} onSubmit={e$} onTab={e8} onEnter={e7} onMoveSelection={e9} onSelectionChanged={eQ} onMaybeShowAutocomplete={e2} onHideAutocomplete={e3} promptToUpload={ep} fontSize={eK} spellcheckEnabled={ez} canOnlyUseTextCommands={false} aria-labelledby={ey} /></_.tEY></div>}</div>}{<div className={X.attachmentsContainer}><H.Z channelId={j.id} type={V} canAttachFiles={eF} ignoreFile={null == eR ? true : eR.name} smallAttachments={true} /></div>}</div></div>}{<div className={X.divider} />}{<div className={X.footer}>{<div className={X.footerPart}>{<eu channel={j} />}{<Z.Z type={G.Ie.CREATE_ANNOUNCEMENT_POST} disabled={eZ} channel={j} handleSubmit={e0} isEmpty={0 === f.trim().length} showAllButtons={true} />}</div>}{<div className={X.footerPart}><div ref={tf} className={X.sendButtonContainer} onMouseEnter={tm} onMouseLeave={tg}><_.yRy targetElementRef={tf} renderPopout={() => (0, r.jsx)(el, {
                  channelId: j.id,
                  canCreateThread: eq
                })} shouldShow={t_} autoInvert={true} nudgeAlignIntoViewport={true} position={"top"} align={"right"}>{e => (0, r.jsx)(_.zxk, et($({}, e), {
                  onClick: () => {
                ...
                }))}</_.yRy></div></div>}</div>}{<O.Z targetRef={eL} ref={e1} channel={j} canMentionRoles={eh} canMentionChannels={em} useNewSlashCommands={eH} canOnlyUseTextCommands={false} canSendStickers={true} textValue={f} focused={z} expressionPickerView={te} type={V} editorRef={eM} onSendMessage={e$} onSendSticker={() => {}} onVisibilityChange={tn} editorHeight={e4} setValue={(e, t) => null == J ? true : J(null, e, t)} position={ev} />}{eI}</div>}{tt ? null : <S.Z positionTargetRef={eL} type={V} onSelectGIF={to} onSelectEmoji={ti} onSelectSticker={ta} channel={j} closeOnModalOuterClick={eS} parentModalKey={eA} position={"top"} align={"right"} positionLayerClassName={Q.expressionPickerPositionLayer} />}</b.Gt></I.f6>
}
...
function el(e) {
  ...
  return <_.v2r aria-label={q.intl.string(q.t["9WnJys"])} navId={"send-announcement-options"} onClose={Y.dG4} onSelect={Y.dG4}>{<_.S89 id={"create-thread"} label={q.intl.string(q.t.rBIGBA)} checked={o && s} disabled={!o} action={() => {
        (0, v.c)(i, {
          createThread: !s
        })
      }} />}{<_.S89 id={"send-and-publish"} label={q.intl.string(q.t["MFGE5+"])} checked={l} action={() => {
        (0, v.c)(i, {
          publish: !l
        })
      }} />}</_.v2r>
...
function ec(e) {
  ...
  return null == o ? null : <div onMouseEnter={c} onMouseLeave={u} className={X.heroImageWrapper} aria-hidden={true}>{<img src={o} alt={q.intl.string(q.t["2ePvR0"])} className={X.heroImage} />}{s ? <T.ZP className={X.heroOverlay}><V.Z tooltip={q.intl.string(q.t.VjC219)} onClick={n} dangerous={true}><_.XHJ /></V.Z></T.ZP> : null}</div>
}
...
function eu(e) {
  ...
  return <A.Z aria-label={q.intl.string(q.t["/IBYAg"])} className={X.attachButton} size={"icon"} color={"transparent"} look={"blank"} onChange={n}><_.xD0 size={"custom"} width={20} height={20} color={f.Z.colors.INTERACTIVE_NORMAL} /></A.Z>
}
...
function ed(e) {
  ...
  return <_.DY3 text={q.intl.string(q.t["/IBYAg"])} position={"top"} className={X.heroImageButton}><_.P3F className={X.heroImageButton} onMouseOver={d} onMouseOut={h} onFocus={o}>{<C.ZP ref={a} onChange={s} aria-hidden={true} tabIndex={false} maxFileSizeBytes={z.B} onFileSizeError={() => (0, N.Z)(z.B)} />}{<_.FmF size={"md"} color={l ? f.Z.colors.INTERACTIVE_ACTIVE : f.Z.colors.INTERACTIVE_NORMAL} />}</_.P3F></_.DY3>
}