/** Chunk was on 63141 **/
/** chunk id: 876205, original params: e,t,n (module,exports,require) **/
require.d(exports, {
  Z: () => u
});
var Chunk255367 = require("./255367.js");
require("./73800.js");
var Chunk442837 = require("./442837.js"),
  Chunk481060 = require("./481060.js"),
  Chunk556296 = require("./556296.js"),
  Chunk13140 = require("./13140.js"),
  Chunk981631 = require("./981631.js"),
  Chunk42124 = require("./42124.js");
let d = {
  [Chunk981631.kg4.TOGGLE_MUTE]: {
    slash: Chunk481060.nRN,
    default: Chunk481060.S6n
  }
};

function u(e) {
  let {
    value: t,
    action: n,
    shouldShow: a
  } = e, u = (0, r.e7)([l.ZP], () => l.ZP.getKeybindForAction(n, false, true));
  if (null == u || !a) return null;
  let h = s.BB(u.shortcut),
    p = d[n],
    f = t ? p.slash : p.default;
  return <div className={c.keybindIndicator}>{<f className={c.keybindIcon} size={"xs"} color={"currentColor"} />}{<o.Text variant={"text-xxs/semibold"} className={c.keybindIndicatorText}>{h}</o.Text>}</div>
}