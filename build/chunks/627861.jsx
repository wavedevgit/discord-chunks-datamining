...
function ei(e) {
  ...
  return <div className={en.partyStatusWrapper}>{u.length > 0 && <Q.ZP guildId={o} users={u} max={i > 0 ? Math.min(i, er) : er} size={Q.u8.SIZE_16} dimEmptyUsers={true} />}{<l.Text variant={"text-xs/medium"} color={"none"}>{c}</l.Text>}</div>
}
...
function e_(e) {
  ...
  if (O) {
    let e = <l.Text variant={"text-xs/medium"} className={en.description} color={"none"} lineClamp={1}>{et.intl.string(et.t["84qx9v"])}</l.Text>;
    return <_.W header={S} title={c.name} iconSrc={C.r9.getWhiteIconURL()} info={e} actions={v} onClickContent={T} trackingConfig={{
        id: c.id,
    ...
    let e = null != g && null != g.details && null != g.state ? et.intl.formatToPlainString(et.t.JCvHt7, {
      ...
      i = null != t ? <div className={en.timestampContainer}>{<l.RZG size={"xxs"} color={"currentColor"} />}{<E.x3 entry={{
            start: t,
            end: null == g || null == (o = g.timestamps) ? true : o.end
          }} textColor={"currentColor"} textTabularNumbers={false} textFontCode={false} />}</div> : null,
      u = <div className={en.info}>{<l.Text variant={"text-xs/normal"} className={en.tagline} color={"none"} lineClamp={1}>{i}</l.Text>}{d ? null : m}</div>;
    return <_.W header={S} title={e} iconSrc={null != (s = (0, H.Z)(g, c.id)) ? s : true} info={u} actions={I} onClickContent={T} trackingConfig={{
        id: c.id,
        linkType: p.Un.RICH_PRESENCE_INVITE,
        onView: h
      }} />
...
function eh(e) {
  let {
    ...
    switch (e) {
      case q.Ol.MOBILE:
        return <l.AtH size={"xxs"} color={"currentColor"} />;
      case q.Ol.ANDROID:
        return <J.j width={a.Z.xxs} height={a.Z.xxs} color={"currentColor"} />;
      case q.Ol.IOS:
        return <l.gLQ size={"xxs"} color={"currentColor"} />;
      case q.Ol.PLAYSTATION:
        return <l.Tsp size={"xxs"} color={"currentColor"} />;
      case q.Ol.XBOX:
        return <l.Mko size={"xxs"} color={"currentColor"} />;
      default:
        return null
    }
  }).filter(B.lm), [n]);
  return 0 === o.length ? null : <div className={en.footer}>{<div className={en.footerSupportedPlatformIconsContainer}>{o.map((e, t) => (0, r.jsx)("div", {
        className: en.footerSupportedPlatformIconContainer,
        children: e
      }, t))}</div>}{<l.Text variant={"text-sm/medium"} color={"currentColor"} className={en.footerSupportedPlatformText}>{et.intl.string(et.t["4dGUPz"])}</l.Text>}</div>
}
...
function em(e) {
  ...
  let {
    ...
    icon: <l.v3n size={"refresh_sm"} color={"currentColor"} />,
  ...
  }, [em, Z, e_]), ey = (0, V.dQ)(I.name, null == (t = C.activity) ? true : t.type), eO = (0, v.N)(I.id).some(e => (0, b.ig)(e) === o.o.GLOBAL) ? <r.Fragment>{<l.YqE size={"xxs"} color={"currentColor"} />}{et.intl.string(et.t.TsWCdX)}</r.Fragment> : null, ev = <eh presenceActivity={w} />;
  if (Z) {
    let e = <l.Text variant={"text-xs/medium"} className={en.description} color={"none"} lineClamp={3}>{(0, V.wR)(C, I, T, S)}</l.Text>;
    return <_.W header={ey} title={I.name} staticBannerSrc={j} videoBannerSrc={U} onClickBanner={eE} bannerAspectRatio={B} iconSrc={null != k ? k : true} info={e} actions={eb} onClickContent={eE} trackingConfig={{
        id: I.id,
        linkType: p.Un.RICH_PRESENCE_INVITE,
        onView: R
      }} />
  ...
  let eI = null != (h = null == w || null == (n = w.timestamps) ? true : n.start) ? h : null == w ? true : w.created_at,
    eT = null != eI ? <div className={en.timestampContainer}>{<l.iWm size={"xxs"} color={"currentColor"} />}{<E.x3 entry={{
          start: eI,
          end: null == w || null == (a = w.timestamps) ? true : a.end
        }} textColor={"currentColor"} textTabularNumbers={false} textFontCode={false} />}</div> : null,
    eS = <l.Text variant={"text-xs/normal"} className={en.tagline} color={"none"} lineClamp={2}>{F ? (0, V.$v)(C, I, T, S, Z) : eT}{F ? null : eO}</l.Text>,
    eA = <div className={en.info}>{eS}{N || F ? null : P}</div>;
  return <_.W header={ey} title={I.name} staticBannerSrc={j} videoBannerSrc={U} onClickBanner={eE} bannerAspectRatio={B} iconSrc={null != k ? k : true} info={eA} actions={eb} onClickContent={eE} trackingConfig={{
      id: I.id,
      linkType: p.Un.RICH_PRESENCE_INVITE,
      onView: R
    }} footer={ev} />
...
function eg(e) {
  ...
  }), [g]), O = (0, ee.Ps)(null == p || null == (t = p.party) ? true : t.id) || f.id === C.r9.id, v = <ei partyMembers={y} partySize={E} maxPartySize={b} guildId={l.guild_id} activityActionType={null == (n = c.activity) ? true : n.type} />;
  return O ? <e_ application={f} currentUserPresenceActivity={m} hideParty={u} message={c} onView={d} partyStatusElement={v} presenceActivity={p} /> : ea(c) ? <S.Z analyticsLocations={o} application={f} channel={l} currentUserId={_} message={c} /> : <em analyticsLocations={o} application={f} channel={l} currentUserId={_} currentUserPresenceActivity={m} hideParty={u} message={c} onView={d} partyStatusElement={v} presenceActivity={p} />
}