...
function w(e) {
  let {
    ...
    return <s.ua7 text={e}>{e => (0, r.jsx)(s.d3s, P(C({}, e), {
        size: "xxs"
      }))}</s.ua7>
  }, B = () => <div className={A.actions}><v.Z channel={i} onAction={L} onClose={R} /></div>, Z = i.isDM() || i.isGroupDM() ? S.intl.string(S.t["9FaEzs"]) : i.isGuildStageVoice() ? S.intl.string(S.t.QygGCA) : S.intl.string(S.t.msxteH);
  return <c.Gt value={D}><g.Z ref={x} className={o()(A.card, N)} onAction={L} onClose={R} aria-label={"".concat(Z, ", ").concat(j)}>{<E.Z text={Z} tags={G()} contextMenu={(0, r.jsx)(I.Z, {
          display: "voice",
          user: t,
          onClose: R
        })} />}{<div className={A.body}><div className={A.content}>{<y.Z users={k} channel={i} />}{<div className={A.details}>{<b.Z channel={i} guild={M} onAction={L} onClose={R} />}{U && i.isGuildVoice() && <O.Z voiceChannel={i} onAction={L} onClose={R} />}{w === T.lY.MODAL_V2 && B()}</div>}{w === T.lY.MODAL && B()}</div></div>}{w !== T.lY.MODAL && w !== T.lY.MODAL_V2 && B()}</g.Z></c.Gt>
}