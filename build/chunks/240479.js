/** Chunk was on web.js **/
"use strict";
n.d(t, {
  Z: () => A
}), n(388685), n(457542), n(953529);
var r = n(255367),
  i = n(73800),
  a = n(831209),
  o = n(793030),
  s = n(442837),
  l = n(481060),
  c = n(749210),
  u = n(231053),
  d = n(183023),
  _ = n(524444),
  f = n(208049),
  p = n(697426),
  h = n(294206),
  m = n(430824),
  g = n(767714),
  E = n(125900),
  b = n(767434),
  y = n(474936),
  O = n(981631),
  v = n(710111),
  I = n(388032),
  T = n(367753);

function S(e) {
  let {
    discoverableGuildId: t,
    closePopout: n,
    buttonType: a
  } = e, o = i.useCallback(async () => {
    if (n(), null != t) try {
      await c.Z.joinGuild(t), c.Z.transitionToGuildSync(t)
    } catch (e) {}
  }, [n, t]);
  return a === b.y.GET_NITRO ? (0, r.jsx)(g.Z, {
    fullWidth: !0,
    showGradient: !0,
    premiumModalAnalyticsLocation: {
      section: O.jXE.PREMIUM_SOUNDMOJI_GUILD_INFO_POPOUT
    },
    subscriptionTier: y.Si.TIER_2,
    size: l.zxk.Sizes.SMALL,
    color: l.zxk.Colors.CUSTOM,
    onClick: n,
    textOptions: {
      textOverride: I.intl.string(I.t.pj0XBA)
    }
  }) : a === b.y.JOIN_GUILD ? (0, r.jsx)(l.zxk, {
    size: l.zxk.Sizes.SMALL,
    fullWidth: !0,
    onClick: o,
    children: I.intl.string(I.t.riu2R0)
  }) : null
}

function A(e) {
  let {
    sound: t,
    channel: n,
    closePopout: c,
    refreshPosition: g
  } = e, y = t.guildId === v.X8, O = (0, s.e7)([m.Z], () => m.Z.getGuild(t.guildId)), A = !y && null != O, [N, C] = i.useState(), R = (0, E.V2)({
    location: "SoundmojiGuildInfo"
  }), P = y || A || null != N || !R, [w, D] = i.useState(!P);
  i.useEffect(() => {
    P || (D(!0), (0, f.xU)(t.soundId, t.guildId).then(e => {
      C(e)
    }).finally(() => {
      D(!1), g()
    }))
  }, [g, P, t.guildId, t.soundId]);
  let {
    buttonType: L,
    description: x
  } = (0, b.Z)(t, n, A, N), M = L === b.y.JOIN_GUILD, k = !y && w, j = i.useMemo(() => A ? u.JO.createFromGuildRecord(O) : null != N ? u.JO.createFromDiscoverableGuild(N) : void 0, [O, A, N]);
  return k ? (0, r.jsx)(_.SE, {}) : (0, r.jsxs)("div", {
    className: T.infoContainer,
    children: [(0, r.jsxs)(_.W_, {
      children: [(0, r.jsxs)("div", {
        className: T.infoExpandedSoundContainer,
        children: [(0, r.jsx)(h.ZP, {
          buttonOverlay: p.Pb.NONE,
          sound: t,
          channel: void 0,
          isSoundmoji: !0,
          onSelectItem: () => {}
        }), (0, r.jsx)(o.xv, {
          variant: "text-sm/normal",
          children: x
        })]
      }), null != j && (0, r.jsxs)("div", {
        className: T.infoExpandedGuildContainer,
        children: [(0, r.jsx)(o.xv, {
          variant: "eyebrow",
          color: "header-muted",
          className: T.infoExpandedGuildTitle,
          children: A ? I.intl.string(I.t.tGDabm) : I.intl.string(I.t.rnOmOT)
        }), (0, r.jsx)("div", {
          className: T.infoExpandedGuildInfo,
          children: (0, r.jsx)(d.Oe, {
            expressionSourceGuild: j,
            hasJoinedExpressionSourceGuild: A,
            isDisplayingJoinGuildButtonInPopout: M
          })
        }), (0, r.jsx)(S, {
          buttonType: L,
          discoverableGuildId: null == N ? void 0 : N.id,
          closePopout: c
        })]
      })]
    }), (0, r.jsx)(l.IGR, {
      text: "BETA",
      color: a.Z.BG_BRAND,
      className: T.betaBadge
    })]
  })
}