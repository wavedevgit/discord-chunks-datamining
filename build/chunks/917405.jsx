...
function x(e) {
  ...
  return <div className={g.rtcConnectionStatusWrapper}>{<div className={o()(g.rtcConnectionStatus, N[t], y)}>{j && <s.ua7 text={t !== h.IE4.UNKNOWN && null != n ? "".concat(n.toFixed(0), " ms") : null}>{e => (0, r.jsx)(b, p({
          quality: t,
          largePing: m
        }, e))}</s.ua7>}{<div className={g.labelWrapper}>{<s.yRy targetElementRef={T} renderPopout={I} position={"top"}>{e => {
            let {
              onClick: t
            } = e;
          ...
          }}</s.yRy>}{m ? x : null}</div>}</div>}{m ? null : x}</div>
}