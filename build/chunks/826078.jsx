...
let _ = function(e) {
  ...
  return <div className={b.centeringWrapper}><div className={b.timelineContainer}>{<div className={b.playPauseButtonWrapper}><c.P3F tabIndex={0} onClick={() => {
            var e, t;
            return R ? null == (e = S.current) ? true : e.pause() : null == (t = S.current) ? true : t.play()
          }} className={b.playPauseButton}>{R ? (0, a.jsx)(c.fpf, {
            size: "md",
            color: "currentColor",
            ...
            color: "currentColor",
            className: b.playPauseIcon
          })}</c.P3F></div>}{<div className={i()(b.timeline, {
          [b.timelineDragging]: null != F
        })} onMouseDown={er}>{<canvas className={b.hiddenCanvas} ref={J} />}{<canvas className={b.timelineBackground} ref={G} />}{<u.t><div tabIndex={0} ref={Z} className={b.playhead} style={{
              left: null != es ? "".concat(es.currentTime / es.duration * 100, "%") : 0
            }} /></u.t>}{<p.Z preload={"auto"} onLoadedMetadata={function() {
            let e = T.current;
            if (null != e) {
          ...
          }} className={b.timelineVideo} ref={T} src={O} muted={true} />}{<div className={b.dragBox} style={{
            left: null != I ? "".concat(N.start / I * 100, "%") : "0",
            right: null != I ? "".concat((I - N.end) / I * 100, "%") : "0"
          }}>{<div className={b.timePillContainer}><div ref={B} className={b.timePillBackground}><c.Text variant={"text-sm/normal"} className={b.timePillText} color={"always-white"}>{w(eo)}{<span className={b.slashCharacter}>{" / "}</span>}{w(X)}</c.Text></div></div>}{<u.t><button className={i()(b.dragHandleLeft, {
                [b.dragging]: "start" === F
              })} onMouseDown={er} onKeyDown={en} role={"slider"} tabIndex={0} aria-valuemin={0} aria-valuenow={N.start} aria-valuetext={C(N.start)} aria-valuemax={N.end - g.Hp} aria-label={x.intl.string(x.t["+BTvw8"])}><div className={b.cropLeftArrow} /></button></u.t>}{<u.t><button className={i()(b.dragHandleRight, {
                [b.dragging]: "end" === F
              })} onMouseDown={er} onKeyDown={ea} role={"slider"} tabIndex={0} aria-valuemin={N.start + g.Hp} aria-valuenow={N.end} aria-valuetext={C(N.end)} aria-valuemax={null == (P = S.current) || null == (_ = P.videoElement) ? true : _.duration} aria-label={x.intl.string(x.t.bBgBYm)}><div className={b.cropRightArrow} /></button></u.t>}</div>}</div>}</div></div>
};
