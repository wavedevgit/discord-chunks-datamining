/** Chunk was on web.js **/
/** chunk id: 415079, original params: e,t,n (module,exports,re quire) **/
"use strict";
let r, i, a, o;
var s, Chunk555256 = require("./555256.js"),
  c = Object.defineProperty,
  u = (e, t, n) => t in e ? c(e, t, {
    enumerable: true,
    configurable: true,
    writable: true,
    value: n
...
let k = "sprigReplayIframeLoaded",
  ...
  Z = new class {
    ...
    release() {
      ...
      module && this.activeCount <= this.capacity ? module() : this.activeCount--
    }
    async execute(e) {
      try {
  ...
  F = async ({
    ...
    var u;
    if (!c && !n && !r) return void l.b.error("UploadErr", {
      isMobile: c,
      uploadId: n,
      etags: r
    });
    l.b.info("MarkUploadComplete", {
      surveyId: t
    });
    let d = await (0, l.s)(`${e}/sdk/1/completeSessionReplay`, {
      ...
      body: JSON.stringify({
        ...
        replayDuration: o,
        eventDigest: s,
        userAgent: null == (u = null == window ? true : window.navigator) ? true : u.userAgent
      }),
      headers: i,
      shouldRetryRequest: true
    });
    return l.b.info("MarkUploadDone", {
      surveyId: t
    }), d
  }, V = e => {
    ...
    for (let n = e; n && !(n instanceof Document) && null !== n; n = n instanceof Attr ? n.ownerElement : n.parentElement) {
      let e = t[t.length] = {
        name: true,
        position: null
      };
    ...
    let n = {
      ...
      r = e.parentElement;
    return r && K.includes(null == (t = r.tagName) ? true : t.toLowerCase()) && Object.assign(n, J(r)), n
  }, ee = (e, t) => {
    var n, r, i;
    let a = t.target;
    t.target === (null == (n = window.document) ? true : n.body) && window.Sprig.pointerDownTarget && (a = window.Sprig.pointerDownTarget), null != (r = (i = {
      x: t.x,
      y: t.y,
      type: e,
      elementAttributes: $(a),
  ...
  }, 750), ea = (s = "left_click", e => ee(s, e)), eo = e => {
    ...
    scrollEventUuids: {},
    stopRecording: () => {}
  }, ec = () => window.indexedDB && window.IDBKeyRange && window.CompressionStream, eu = (() => {
    let e = Chunk555256.a.getItem("sprig.sessionId");
    if (module) return Chunk555256.b.info("SessionIDFound", {
      savedSessionId: module
    }), Chunk555256.a.removeItem("sprig.sessionId"), module;
    let t = (0, Chunk555256.v)();
    return Chunk555256.b.info("GeneratedSessionID", {
      uuid: exports
    }), exports
  })(), ed = () => {
...
let ep = (e, t) => {
  ...
  eh = async e => {
    let {
      ...
      xPath: n,
      y: r
    } = e, i = el.scrollEventUuids[n];
    if (i) return ey(async () => {
      var n, a, o, s;
      let l = await ev.openDB(),
        c = await l.get("events", i);
      if (null != c && c.event) {
        let i = JSON.parse(c.event),
          u = t > (null == (a = null == (n = i.data) ? true : n.payload) ? true : a.x),
  ...
  }, eg = ["did not allow mutations", "called in an invalid security context"], eE = (e, t, {
    reportError: n = true,
    extraInfo: r = {}
  }) => {
    if (!ef() && t instanceof Error) {
      if (ed(), "VersionError" === t.name) return l.b.error("VersionErr", {
        message: e
      }), void ev.deleteDB();
      (e => {
        if (!e) returntrue;
        for (let t of eg)
          if (e.toLowerCase().includes(t)) returnfalse;
        returntrue
      })(null == t ? true : t.message) && (n && window.UserLeap.reportError(e, t, r), ev.clearAll())
    }
  }, eb = (e, t, {
    reportError: n
  } = {
...
let ev = new class {
    openDB() {
      return function(e, t, {
        ...
        let o = indexedDB.open(e, 1),
          s = v(o);
        return r && (o.onupgradeneeded = e => {
          r(v(o.result), e.oldVersion, e.newVersion, v(o.transaction), e)
        }), n && (o.onblocked = e => n(e.oldVersion, e.newVersion, e)), s.then(e => {
          a && (e.onclose = () => a()), i && (e.onversionchange = e => i(e.oldVersion, e.newVersion, e))
        }).catch(() => {}), s
      }("sprigReplay", 0, {
        upgrade: (e, t, n) => {
          if (0 === n && l.a.setItem("sprig.pendingCount", "0"), !e.objectStoreNames.contains("events")) {
            let t = e.createObjectStore("events", {
              keyPath: "uuid"
            });
            t.createIndex("sessionId", "sessionId"), t.createIndex("timestamp", "timestamp"), t.createIndex("[sessionId+timestamp]", ["sessionId", "timestamp"])
          }
          if (!e.objectStoreNames.contains("chunkUploads")) {
            let t = e.createObjectStore("chunkUploads", {
              keyPath: "uuid"
            });
            t.createIndex("sessionId", "sessionId"), t.createIndex("timestamp", "timestamp"), t.createIndex("[sessionId+status]", ["sessionId", "status"]), t.createIndex("[uploadId+status]", ["uploadId", "status"]), t.createIndex("[sessionId+status+uploadId]", ["sessionId", "status", "uploadId"])
          }
          if (!e.objectStoreNames.contains("pendingCaptures")) {
            let t = e.createObjectStore("pendingCaptures", {
              keyPath: "uuid"
            });
            t.createIndex("sessionId", "sessionId"), t.createIndex("timestamp", "timestamp"), t.createIndex("[sessionId+targetTimestamp]", ["sessionId", "targetTimestamp"])
          }
        }
      })
    }
    ...
    async clearAll() {
      ...
      return Promise.all([module.objectStore("events").clear(), module.objectStore("chunkUploads").clear(), module.objectStore("pendingCaptures").clear()])
    }
    ...
    async deleteRowsBefore(e, t, n = () => true) {
      let r = IDBKeyRange.upperBound(t, true),
    ...
    async getEventsBetween(e, t = Date.now()) {
      ...
      let n = IDBKeyRange.bound([eu, e], [eu, t], false, true);
    ...
    async updateEventsExpiredAt(e, t, n = 30) {
      let r = new Date,
        ...
        s = IDBKeyRange.bound([eu, e], [eu, t], false, true);
      for await (let e of o.iterate(s)) await e.update({
        ...e.value,
        expiredAt: i
      });
      await a.done
    ...
    async getPendingCaptures(e = {}) {
      return (await (await this.openDB()).getAllFromIndex("pendingCaptures", "sessionId", eu)).filter(t => !e.beforePresent || t.targetTimestamp < Date.now()).filter(t => !e.isBeforeType || "before" === t.captureParams.replayParams.replayDurationType).filter(t => !e.isHeatmap || (t.captureParams.isHeatmap ?? false))
    }
    async markPendingCaptureToCanUpload(e) {
      ...
      for await (let n of t.store.index("sessionId").iterate(eu)) {
        let t = n.value;
        t.captureParams.responseGroupId === e && await n.update({
          ...t,
          canUpload: true
        })
      }
    ...
    async markPendingHeatmapsReady(e) {
      ...
      for await (let r of n.store.index("sessionId").iterate(eu)) {
        let n = r.value;
        n.captureParams.isHeatmap && (!e || e.includes(n.uuid)) && await r.update({
          ...n,
          targetTimestamp: t,
          captureParams: {
            ...n.captureParams,
            ...
            replayParams: {
              ...
              replayDurationSeconds: Math.floor((t - n.timestamp) / 1e3)
            }
          }
        })
      }
  ...
  eZ = e => e.some(eB),
  eF = async () => {
    if (!eM()) return void window.clearInterval(eR);
    if (eV(), !eZ(eI)) return;
    ...
    ej.splice(0, module.length).forEach(t => t(e.shift())), module.forEach(e => eP.push(e)), ek = false
  }, eH = () => {
    ...
    return Chunk555256.b.info("Fetched more always-on upload urls", {
      body: require,
      urls: i
    }), i
  }, eq = async () => {
    ...
    eM() && !eL && (e || eI.length) && (e && eI.length && (async () => {
      let e = eI.splice(0);
      if (!eZ(e)) return;
...
window.addEventListener("beforeunload", async () => {
  eL = true, eM() && (Chunk555256.b.info("Always On handle page unload"), (() => {
    let e;
    ...
    Chunk555256.b.info("Storing session state on unload", exports), Chunk555256.a.setItem("sprig.alwayson.info", JSON.stringify(exports))
  })())
});
let eJ = async (e, t) => {
  let n, r = performance.now();
  try {
    n = await e()
  } finally {
    let e = performance.now() - r,
      n = l.P[t];
    n || (n = (0, l.r)(t)), n.report(e / 1e3)
  }
  return n
}, e$ = (e, t) => {
  ...
  el.cleanupInterval = window.setInterval(() => {
    let r = Date.now();
    eJ(() => ey(async () => {
      ef() || await Promise.all([ev.deleteRowsBefore("events", r - 1e3 * e, t => true === t.expiredAt || t.expiredAt < r - 1e3 * e), ev.deleteRowsBefore("chunkUploads", r - 1e3 * t), ev.deleteRowsBefore("pendingCaptures", r - 1e3 * n, e => !e.canUpload)])
    }, "Error deleting table rows"), "sdk_replay_cleanup_seconds"), l.b.debug("CleanupComplete")
  }, 3e4)
}, e9 = () => {
  ...
  if (!(null != o && o.length)) return l.b.debug("NoEventsFound"), {
    validStartFound: r,
    events: []
  };
  if (!r) {
    ...
    let e = false;
    return null == o || o.forEach((n, r) => {
      if (!n.isValidStart) return;
      let i = n.timestamp <= t;
      (e < 0 || i) && (e = r)
    }), e < 0 ? (l.b.debug("ValidStartNotFound"), {
      validStartFound: r,
      events: []
    }) : {
      validStartFound: true,
      events: null == o ? true : o.slice(e)
    }
  }
  return {
    validStartFound: r,
  ...
  let t = await (async e => Z.execute(async () => {
    ...
    let r = null == (t = n.headers) ? true : t.get("ETag");
    if (!r) throw Error(`Upload response did not include etag for upload ${e.uploadId}, part ${e.chunkIndex}`);
    return r
  }))(e);
  return await ev.updatePartial("chunkUploads", e.uuid, {
    data: null,
    etag: t,
  ...
  if (!(null != t && t.length)) return void l.b.info("NoChunksForUpload", {
    uploadId: e
  });
  ...
  ey(async () => {
    ...
    if (!(null != module && module.length)) return;
    let t = await tr(module);
    null != exports && exports.length && await Promise.all(exports.map(e => {
      if (e) return ti(e)
    }))
  ...
  return (async (e, t) => {
    let n = new TextEncoder,
      ...
      [c, u] = [0, 0],
      d = [];
    for (let r = e - 35e3; r < t; r += e1) {
      if ({
          validStartFound: s,
          events: d
        } = await tn(r, e, t, s, e1), !(null != d && d.length)) {
  ...
  let {
    ...
    replayParams: o,
    triggerTimestamp: s
  } = e, c = await to(1e3 * o.replayDurationSeconds, s);
  if (!(null != c && c.length)) return void l.b.info("FileDataEmpty", {
    surveyId: t
  });
  let u = ((e, t, n) => {
      let r = e.length,
    ...
    d = await Promise.all(u.map(async (e, c) => {
      let d = (0, l.v)(),
        f = {
          apiUrl: i,
          chunkIndex: c + 1,
      ...
      return await (await ev.openDB()).add("chunkUploads", {
        ...f,
  ...
  let {
    ...
    setTimeout(() => l.e.removeListener(l.S.QuestionAnswered, s), 0), ey(async () => {
      "before" === i.replayDurationType ? await ts(e) : await ev.markPendingCaptureToCanUpload(o)
    }, "Error in schedule/capture callback")
  };
  ey(async () => {
    if ("after" === i.replayDurationType || "beforeAndAfter" === i.replayDurationType) return r || n || l.e.on(l.S.QuestionAnswered, s), void await td(e);
    if (r || n || t) await ts(e), n && tc();
    else {
      let e = a - 1e3 * (35 + i.replayDurationSeconds),
        t = a;
  ...
  let {
    isHeatmap: t,
    surveyId: n
  } = e, r = await ev.getPendingCaptures(), i = null == r ? true : r.filter(e => e.captureParams.surveyId === n);
  ...
  checkPendingHeatmapsUrl: () => ef() ? Chunk555256.b.debug("ReplayDisabled-PendingHeatmaps") : ey(async () => {
    let e = (await ev.getPendingCaptures({
      isHeatmap: true
    })).map(e => ({
    ...
    return Chunk555256.b.info("PendingHeatmapsToComplete", {
      count: module.length
    }), module.length && (await ev.markPendingHeatmapsReady(module), Chunk555256.b.info("MarkedPendingHeatmapsReady")), module.length
  ...
  initializeReplay: async ({
    ...
    if (await (async () => {
        var e;
        if (!ec()) returntrue;
        if (null != (e = window.navigator.storage) && e.estimate) try {
          let {
            quota: e = 0,
            usage: t = 0
          } = await window.navigator.storage.estimate(), n = (e - t) / 0x40000000;
          return l.b.info("Storage", {
            availableGb: n
          }), n < .5
        } catch {
          returntrue
    ...
    try {
      ...
      return l.b.error("ReplayOpenErr", {
        name: e.name
      }), "VersionError" === e.name && ev.deleteDB(), ed()
    }
    ey(async () => {
      await tt(true)
    }, "Error uploading ready pending captures");
    let o = Math.max(e ?? 0, 30 * !!eM());
