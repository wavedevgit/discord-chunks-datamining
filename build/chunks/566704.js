...
let s = 30 * Chunk927813.A.Millis.SECOND,
  o = 5 * Chunk927813.A.Millis.SECOND;
...
function c(e) {
  ...
  r.useEffect(() => {
    ...
    let n = true,
      ...
      a = 0,
      i = () => setTimeout(() => {
        t && (r = new AbortController, (0, l.cq)(e, false, r.signal).then(() => a = 0).catch(() => a = Math.min(a + 1, 4)).finally(() => {
          n && (c = i())
        }))
      }, s * Math.pow(2, a) + Math.random() * o),
      c = i();
    return () => {
      n = false, null == r || r.abort(), clearTimeout(c)
    }
  }, [e, t])
}