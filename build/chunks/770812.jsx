...
function v(e) {
  ...
  return <div className={y.container}>{<div className={y.background} />}{<div className={y.content}>{<s.zx look={s.zx.Looks.BLANK} size={s.zx.Sizes.ICON} className={y.closeButton} onClick={t}><l.Dio size={"xs"} color={"currentColor"} aria-label={b.intl.string(b.t.cpT0Cg)} className={y.closeButtonIcon} /></s.zx>}{<img alt={""} src={c} className={y.upsellImage} />}{<l.LZC size={22} />}{<l.X6q color={"header-primary"} variant={"heading-lg/extrabold"} className={y.header}>{b.intl.string(b.t.cBjkc3)}</l.X6q>}{<l.LZC size={4} />}{<l.Text color={"header-secondary"} variant={"text-md/normal"} className={y.description}>{A}</l.Text>}{<l.LZC size={24} />}{<m.Z onClick={I}>{N}</m.Z>}</div>}</div>
}