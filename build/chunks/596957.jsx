...
function D(e) {
  ...
  return <div className={x.emptyStateContainer}><y.Z type={t} onClick={l} /></div>
}
let k = function(e) {
  let {
    titleId: t
  ...
  let K = i.useMemo(() => {
    ...
    J = i.useCallback(e => {
      ...
      return u === P.pJs.PENDING && 0 === e ? <div className={x.sectionTitle}>{<S.Z id={t} title={n} />}{X && <s.zx look={s.iL.LINK} color={s.Tt.LINK} className={x.clearButton} size={s.zx.Sizes.TINY} onClick={Q} aria-label={j.intl.string(j.t.O8k7Oz)}>{j.intl.string(j.t.O8k7Oz)}</s.zx>}</div> : <div className={x.sectionTitle}><S.Z id={t} title={n} /></div>
    }, [K, u, t, X, Q]);
  if (i.useEffect(() => {
      u === P.pJs.ALL && (0, m.d$)()
    }, [u]), 0 === z.length && "" === U[u]) return <D section={u} showSpamCta={F} />;
  let $ = "" !== U[u],
    ee = 0 === z.length && $;
  return <h.Gt value={n}><p.Z section={P.jXE.FRIENDS_LIST}>{M && <C.R />}{<c.E1j className={a()(x.searchBar, ee ? x.searchEmptyState : null)} query={U[u]} onChange={H} onClear={B} />}{<I.Z rows={K} renderRow={V} renderSection={J} sectionFilter={u} isVirtualizedList={k >= T.nG} hasSearchQuery={$} footer={F && !ee ? (0, r.jsx)(s.zx, {
        ...
        }) : null} />}{ee && <div className={x.emptyStateContainer}><y.Z type={y.j.SECTION_NO_RESULTS} /></div>}</p.Z></h.Gt>
}