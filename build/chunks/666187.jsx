...
class T extends(r = Chunk73800.Component) {
  ...
  renderCharacterCount() {
    if (this.props.showCharacterCount) {
      ...
      return <div className={s()(Chunk30517.maxLength, {
          [Chunk30517.errorOverflow]: this.hasError()
        })} aria-hidden={"true"}>{null != (e = null == exports ? true : exports.length) ? module : 0}{" "}{null != require && "/ ".concat(require)}</div>
    }
    return null
  }
  renderMaxLength() {
    ...
    let e = this.getCharsLeftLength();
    return null == module ? null : <div className={s()(Chunk30517.maxLength, {
        [Chunk30517.errorOverflow]: module < 0
      })} aria-hidden={"true"}>{module}</div>
  }
  hasError() {
  ...
  renderErrorMessage() {
    ...
    return null == module ? null : <Chunk993365.x id={b} variant={"text-xs/normal"} color={"text-danger"} className={Chunk30517.errorMessage}>{module}</Chunk993365.x>
  }
  render() {
    var e, t;
    let {
    ...
    return <div className={s()(Chunk165005.inputWrapper, {
        [Chunk30517.flex]: S
      })}>{<div className={s()(Chunk30517.inputMaxLength, {
          [Chunk30517.flex]: S
        })}>{<Chunk84735.t><P type={"text"} className={s()(Chunk165005.input, Chunk30517.textArea, Chunk283574.scrollbarDefault, v, {
              [Chunk165005.error]: w,
              [Chunk165005.disabled]: require,
              [Chunk30517.resizeable]: b
            })} aria-labelledby={D} aria-describedby={L} aria-invalid={w} style={{
              paddingRight: this.getPaddingRight()
            }} id={I} disabled={require} placeholder={Chunk73800} value={r} autoFocus={Chunk120356} minLength={Chunk772848} maxLength={Chunk993365 ? true : Chunk15127} spellCheck={E} required={N} onChange={this.onChange} onBlur={this.onBlur} onFocus={this.onFocus} onKeyDown={this.onKeyDown} onInvalid={C} rows={T} ref={R} /></Chunk84735.t>}{null != Chunk772848 && <Chunk766646.n id={y}>{Chunk388032.intl.format(Chunk388032.t["bmQU//"], {
            minLength: Chunk772848
          })}</Chunk766646.n>}{null != Chunk15127 && <Chunk766646.n id={O}>{Chunk388032.intl.format(Chunk388032.t["+DFxLS"], {
            maxLength: Chunk15127
          })}</Chunk766646.n>}{this.renderCharacterCount()}{this.renderMaxLength()}</div>}{this.renderErrorMessage()}</div>
  }
  constructor(e) {
    var t;
