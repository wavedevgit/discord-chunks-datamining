/** Chunk was on web.js **/
/** chunk id: 438784, original params: e,t,n (module,exports,re quire) **/
"use strict";
require.d(exports, {
  I: () => I,
  b: () => T
}), require("./388685.js");
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk120356 = require("./120356.js"),
  a = require.n(Chunk120356),
  Chunk126663 = require("./126663.js"),
  Chunk481060 = require("./481060.js"),
  Chunk58178 = require("./58178.js");

function u(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: true,
    configurable: true,
    writable: true
  }) : e[t] = n, e
}

function d(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = null != arguments[t] ? arguments[t] : {},
      r = Object.keys(n);
    "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
      return Object.getOwnPropertyDescriptor(n, e).enumerable
    }))), r.forEach(function(t) {
      u(e, t, n[t])
    })
  }
  return e
}

function f(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t && (r = r.filter(function(t) {
      return Object.getOwnPropertyDescriptor(e, t).enumerable
    })), n.push.apply(n, r)
  }
  return n
}

function _(e, t) {
  return t = null != t ? t : {}, Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : f(Object(t)).forEach(function(n) {
    Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n))
  }), e
}

function p() {
  return new Promise(e => setTimeout(e, 1e3))
}

function h() {
  return document.hasFocus()
}
let m = .25,
  g = 4,
  E = 1,
  b = 2.8,
  y = {
    config: {
      friction: 50,
      tension: 900,
      mass: 1
    },
    unique: true,
    initial: {
      dotPosition: 1
    },
    from: {
      dotPosition: 0
    },
    enter: {
      dotPosition: 1
    },
    leave: {
      dotPosition: 0
    }
  },
  O = {
    config: {
      duration: 2400
    },
    from: {
      dotCycle: 2.8
    },
    reset: true
  };

function v(e) {
  let t = e % 2;
  return t > 1 ? 1 - (t - 1) : t
}
let I = Chunk73800.memo(function(e) {
    let {
      dotRadius: t,
      dotPosition: n,
      fill: o = "currentColor",
      spacing: a = 2.5
    } = e, c = i.useRef(h()), u = i.useRef(true);
    i.useEffect(() => () => void(u.current = false), []);
    let [f] = (0, l.q_F)(() => _(d({}, O), {
      to: async e => {
        let t = b;
        for (; u.current;) c.current = h(), c.current ? (t += E * g, await e({
          dotCycle: t,
          immediate: false
        })) : t !== b ? (t = b, await e({
          dotCycle: t,
          immediate: true
        })) : await p()
      }
    }), "animate-always"), y = (2 * t * 3 + t / 4 * 2) / 2;
    return <r.Fragment>{[0, 1, 2].map(e => {
        let i = m * e,
          l = t + t * a * e;
        return (0, r.jsx)(s.animated.circle, {
          cx: n ? n.to([0, 1], [y, l]) : l,
          cy: t,
          r: f.dotCycle.to(e => v(e - i)).to([0, .4, .8, 1], [.8 * t, .8 * t, t, t]).to(e => c.current ? e : t),
          fill: o,
          style: {
            opacity: f.dotCycle.to(e => v(e - i)).to([0, .4, .8, 1], [.3, .3, 1, 1]).to(e => c.current ? e : 1)
          }
        }, e)
      })}</r.Fragment>
  }),
  T = Chunk73800.memo(Chunk73800.forwardRef(function(e, t) {
    let {
      dotRadius: n,
      x: i,
      y: o,
      hide: u = false,
      themed: f = false,
      className: p
    } = e;
    return (0, l.Yzy)(u, _(d({}, y), {
      key: e => e ? "true" : "false"
    }), h() ? "animate-always" : "animate-never")((e, l, u) => {
      let {
        dotPosition: d
      } = e, {
        key: _
      } = u;
      return l ? null : <svg ref={t} x={i} y={o} width={2 * n * 3 + n / 2 * 2} height={2 * n} className={a()(p, c.dots, f ? c.themed : null)}><s.animated.g style={{
            opacity: d.to(e => Math.min(1, Math.max(e, 0)))
          }}><I dotRadius={n} dotPosition={d} /></s.animated.g></svg>
    })
  }))