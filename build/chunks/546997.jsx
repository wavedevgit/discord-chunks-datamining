...
function f(e) {
  ...
  })), b = <i.Fragment>{<c.F setting={m.s6.VOICE_AND_VIDEO_ADVANCED_SCREENSHARE_VIDEO_HOOK}><a.j7V value={n} onChange={e => l.Z.setVideoHook(e)} note={p.intl.string(p.t["Fj/xn5"])} hideBorder={t}>{p.intl.string(p.t.GmWk2N)}</a.j7V></c.F>}{o && u && <c.F setting={m.s6.VOICE_AND_VIDEO_ADVANCED_SCREENSHARE_EXPERIMENTAL_SOUNDSHARE}><a.j7V value={s} onChange={e => l.Z.setExperimentalSoundshare(e)} hideBorder={t}>{p.intl.string(p.t["4I0qzc"])}</a.j7V></c.F>}{<c.F setting={m.s6.VOICE_AND_VIDEO_ADVANCED_SCREENSHARE_SYSTEM_PICKER}><a.j7V value={f} onChange={h} hideBorder={t}>{p.intl.string(p.t.ie1mgY)}</a.j7V></c.F>}</i.Fragment>;
  return <c.F setting={m.s6.VOICE_AND_VIDEO_ADVANCED_SCREENSHARE}>{t ? b : (0, i.jsx)(a.hjN, {
