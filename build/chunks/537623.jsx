...
function x(e) {
  ...
  return <s.Y0X transitionState={x} size={s.CgR.SMALL} parentComponent={"AutomodRaidResolveModal"}>{<s.xBx separator={false} className={f.center}><s.X6q color={"header-primary"} variant={"heading-xl/bold"}>{p.intl.string(p.t["1zmw/P"])}</s.X6q></s.xBx>}{<s.hzk className={f.center}>{<s.Text variant={"text-sm/normal"} color={"header-secondary"} className={f.textCenter}>{p.intl.string(p.t.nF79oK)}</s.Text>}{<div className={f.options}>{g.map(e => {
          let {
            text: t,
            value: n
          } = e;
        ...
        })}</div>}</s.hzk>}{<s.mzw className={f.__invalid_modalFooter}>{<div className={f.button}><s.zxk variant={"primary"} text={p.intl.string(p.t.Gh3A0N)} onClick={j} /></div>}{<s.zxk variant={"secondary"} text={p.intl.string(p.t["ETE/oK"])} onClick={b} />}</s.mzw>}</s.Y0X>
}