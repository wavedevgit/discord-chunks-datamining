...
function A(e) {
  ...
  if (n) t = <S />;
    ...
    t = <u.Z emojiId={i.id} emojiName={e} animated={i.animated} />
  }
  return null != t ? <div className={b.imageContainer}>{t}</div> : null
}
...
function C(e) {
  ...
  switch (n) {
    case "NO_VOTES":
      t = <l.Text variant={"text-sm/semibold"} color={"text-default"} scaleFontToUserSetting={true}>{E.intl.string(E.t["a+lSZ2"])}</l.Text>;
      break;
    case "VICTOR":
      let {
        victorAnswerText: a, victorVotePercentage: s
      } = i;
      t = <r.Fragment>{<div className={b.victorAnswer}>{<l.Text variant={"text-sm/semibold"} color={"text-default"} className={b.victorAnswerText} scaleFontToUserSetting={true}>{a}</l.Text>}{<m.ZY size={T} className={b.victorIcon} isVictor={true} isExpired={true} />}</div>}{<l.Text variant={"text-xs/medium"} color={o} scaleFontToUserSetting={true}>{E.intl.string(E.t.ufIDIy)}{" • "}{s}{"%"}</l.Text>}</r.Fragment>;
      break;
    case "TIE":
      let {
        tiedVotePercentage: c
      } = i;
      t = <r.Fragment>{<l.Text variant={"text-sm/semibold"} color={"text-default"} scaleFontToUserSetting={true}>{E.intl.string(E.t.kPN9sr)}</l.Text>}{<l.Text variant={"text-xs/medium"} color={o} scaleFontToUserSetting={true}>{c}{"%"}</l.Text>}</r.Fragment>
  }
  return <div className={b.description}>{t}</div>
}
...
function R(e) {
  ...
  return <div className={a()(b.container, {
      [b.containerWithImage]: c
    }, t)}>{<A hasNoVotes={"NO_VOTES" === s.type} victorEmoji={n.victorEmoji} />}{<C{...O({}, s)} />}{null != o && <l.zxk size={"sm"} onClick={o} variant={"secondary"} text={E.intl.string(E.t.Jw7VbW)} />}</div>
}
...
function P(e) {
  ...
  return null == m ? null : <r.Fragment>{<p.Z iconNode={(0, r.jsx)(l.QDj, {
        size: "xs"
      })} timestamp={n.timestamp} compact={a}>{E.intl.format(E.t["VJcK4+"], {
        username: O.nick,
        usernameHook: v(O),
        title: y,
        titleOnClick: T
      })}</p.Z>}{<R className={b.embed} data={m} onClickPollLink={u ? true : T} />}</r.Fragment>
}