/** Chunk was on 20087 **/
/** chunk id: 426642, original params: t,e,r (module,exports,require) **/
require.d(exports, {
  Z: () => f,
  d: () => u
}), require("./953529.js");
var n, Chunk255367 = require("./255367.js");
require("./73800.js");
var Chunk692547 = require("./692547.js"),
  Chunk481060 = require("./481060.js"),
  Chunk752657 = require("./752657.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk135340 = require("./135340.js"),
  u = ((n = {})[n.TOO_BIG = 0] = "TOO_BIG", n[n.RATE_LIMIT = 1] = "RATE_LIMIT", n[n.TOO_MANY_EMOJI = 2] = "TOO_MANY_EMOJI", n[n.TOO_MANY_ANIMATED_EMOJI = 3] = "TOO_MANY_ANIMATED_EMOJI", n[n.WRONG_TYPE = 4] = "WRONG_TYPE", n[n.RESIZE_GIF = 5] = "RESIZE_GIF", n);
let f = t => {
  let {
    error: e
  } = t, {
    type: r,
    filename: n
  } = e, {
    icon: u,
    description: f,
    color: d,
    tooltip: h
  } = {
    0: {
      icon: o.gm1,
      description: s.intl.string(s.t.YbdEFB),
      color: l.Z.unsafe_rawColors.RED_400.css,
      tooltip: s.intl.string(s.t.PanA4O)
    },
    4: {
      icon: o.LB9,
      description: s.intl.string(s.t["01Papq"]),
      color: l.Z.unsafe_rawColors.RED_400.css,
      tooltip: s.intl.string(s.t.PanA4O)
    },
    5: {
      icon: a.Z,
      description: s.intl.string(s.t.sp16MT),
      color: l.Z.unsafe_rawColors.RED_400.css,
      tooltip: s.intl.string(s.t.PanA4O)
    },
    1: {
      icon: o.EmW,
      description: s.intl.string(s.t["4rjikp"]),
      color: l.Z.unsafe_rawColors.ORANGE_345.css,
      tooltip: s.intl.string(s.t.dq9aTk)
    },
    2: {
      icon: o.d3s,
      description: s.intl.string(s.t["5VmbbG"]),
      color: l.Z.unsafe_rawColors.RED_400.css,
      tooltip: s.intl.string(s.t.PanA4O)
    },
    3: {
      icon: o.d3s,
      description: s.intl.string(s.t.rjFOZ2),
      color: l.Z.unsafe_rawColors.RED_400.css,
      tooltip: s.intl.string(s.t.PanA4O)
    }
  } [r];
  return <div className={c.emojiErrorItem}>{<u className={c.fileIcon} color={l.Z.unsafe_rawColors.PRIMARY_300.css} />}{<o.hjN className={c.fileInfo}>{<o.vwX className={c.fileName}>{n}</o.vwX>}{<o.R94 type={o.geA.DESCRIPTION}>{f}</o.R94>}</o.hjN>}{<o.DY3 position={"right"} text={h}><o.Mgn size={"custom"} width={20} height={20} color={d} /></o.DY3>}</div>
}