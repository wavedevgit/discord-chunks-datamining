...
let I = e => {
  ...
  E = e => {
    ...
    return <div className={x.embedContainer}><d.BC className={x.sizedToParent} iframeWrapperClassName={x.sizedToParent} href={null} thumbnail={t} video={n} provider={r} maxHeight={c.r.SMALL.height} maxWidth={f.b.SMALL} onPlay={l} playable={true} volume={s} onVolumeChange={o} autoMute={m} onMute={p} autoPlay={h} renderImageComponent={O} renderVideoComponent={I} renderLinkComponent={g.iT} /></div>
  };
class N extends(r = Chunk73800.PureComponent) {
  ...
  render() {
    ...
    return <Chunk453499.Z items={module.slice(0, 12)} delay={exports} initialPaused={r || Chunk120356} renderItem={this.renderItem} className={s()(Chunk365664.smallCarousel, require)} themedPagination={Chunk73800} onIntentionalChange={this.handleIntentionalChange} onChangeItem={this.handleChangeItem} />
  }
  constructor(...e) {
    super(...e), v(this, "state", {
      ...
      switch (e.type) {
        ...
        case _.s9s.VIDEO:
          n = <E item={e} onPlay={this.handlePlay} isMuted={r} volume={i} onVolumeChange={this.handleVolumeChange} onMute={this.handleMute} autoPlay={l} />;
          break;
        case _.s9s.IMG:
          if ("errored" === this.state.status.get(e.src)) {
            let e = <o.X6q variant={"heading-xl/semibold"}>{b.intl.string(b.t.UvDfMz)}</o.X6q>;
            return <div className={x.errorContainer}>{true !== this.props.errorComponent ? this.props.errorComponent : e}</div>
          } {
            var c;
            let t = null != (c = e.alt) ? c : b.intl.string(b.t.X4IxWF);
            n = <a.Fragment>{<o.$jN className={s()(x.spinner, {
                  [x.hidden]: true !== this.state.status.get(e.src)
                })} type={o.$jN.Type.SPINNING_CIRCLE} />}{<img onError={this.handleOnErrorImg} onLoad={this.handleOnLoadImg} className={s()(x.smallCarouselImage, {
                  [x.hidden]: "loaded" !== this.state.status.get(e.src)
                })} src={e.src} alt={t} />}</a.Fragment>
          }
          break;
        default:
          throw Error("Unexpected MediaType")
      }
      return <o.P3F className={x.smallCarouselItem} onClick={() => this.handleCurrentItemClick(e, t)}>{n}</o.P3F>
    })
  }
}
v(N, "defaultProps", {
