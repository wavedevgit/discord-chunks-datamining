...
function w(e) {
  ...
  return l.useEffect(() => {
    null != w && (u()(null != r.current, "Input is submitting when not mounted"), r.current.value = "", r.current.focus())
      ...
      f.Z.sendRequest({
        ...
        return d({
    ...
    }} autoComplete={"off"}>{<div className={b.addFriendWumpusWrapper}>{<c.R94 id={E} type={c.R94.Types.DESCRIPTION}><c.Text tag={"div"} variant={"heading-md/normal"}>{g.intl.string(g.t["Rn/sLi"])}</c.Text></c.R94>}{<img src={O} alt={"Wumpus Waving"} />}</div>}{<c.tEY focusTarget={r} ringTarget={i} ringClassName={b.ring}><div ref={i} className={s()(b.addFriendInputWrapper, {
          [b.success]: w,
          [b.error]: C
        })}>{<o.Is id={j} inputRef={r} className={b.addFriendInput} inputClassName={b.input} onKeyPress={e => {
          ...
          }} maxLength={37} autoFocus={true} autoComplete={"off"} name={"add-friend"} data-form-type={"other"} data-lpignore={true} data-1p-ignore={true} placeholder={t} aria-label={t} aria-invalid={null != C || true} aria-describedby={null != C ? S : E} />}{null != _ && <div className={b.addFriendHint} aria-hidden={true}>{_}</div>}{<c.zxk variant={"primary"} size={"sm"} text={g.intl.string(g.t["PMsq/f"])} disabled={!m} type={"submit"} />}</div></c.tEY>}{null != C && <c.R94 role={"alert"} id={S} type={c.R94.Types.ERROR} className={h.marginTop8}>{C}</c.R94>}{null != w && <c.R94 role={"status"} type={c.R94.Types.SUCCESS} className={h.marginTop8}>{w}</c.R94>}</form>
}