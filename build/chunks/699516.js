...
function P() {
  let {
    [u.OGo.PENDING_INCOMING]: e = 0, [u.OGo.PENDING_OUTGOING]: t = 0, [u.OGo.FRIEND]: n = 0
  } = a().countBy(Array.from(m.values()));
  A = t, N = n, C = b.size, R = O.size, S = Math.max(e - C - R, 0), I++
}

function w(e) {
  m.clear(), g = {}, E = {}, y = new Set, b = new Set, O = new Set, v = {}, T = {}, e.relationships.forEach(e => {
...
function L(e) {
  ...
  m.set(e.relationship.id, e.relationship.type), null != e.relationship.nickname && (g = p(f({}, g), {
    [e.relationship.id]: e.relationship.nickname
  })), null != e.relationship.since && (E = p(f({}, E), {
    [e.relationship.id]: e.relationship.since
  })), null != e.relationship.originApplicationId && (v = p(f({}, v), {
    [e.relationship.id]: e.relationship.originApplicationId
  })), e.relationship.isSpamRequest ? b.add(e.relationship.id) : b.delete(e.relationship.id), e.relationship.userIgnored ? (y.add(e.relationship.id), e.relationship.type === u.OGo.PENDING_INCOMING ? O.add(e.relationship.id) : e.relationship.type === u.OGo.FRIEND && O.delete(e.relationship.id)) : (y.delete(e.relationship.id), O.delete(e.relationship.id)), P(), e.relationship.type === u.OGo.FRIEND && t === u.OGo.PENDING_OUTGOING && s.Z.dispatch({
...
class U extends(r = o.ZP.Store) {
  ...
  getFriendIDs() {
    return Array.from(m.entries()).filter(e => {
      let [t, n] = e;
