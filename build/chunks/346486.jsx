...
class f extends(r = Chunk73800.PureComponent) {
  ...
  defaultRender(e) {
    ...
    let l = s()(o).map(e => e < 10 ? "0".concat(e) : e).map((e, t) => [t > 0 && !n ? ":" : " ", <span>{e}{n ? a[o.length - t - 1] : null}</span>]).flatten().value();
    return <span className={r}>{l}</span>
  }
  render() {
    let {
      deadline: e,
    ...
    if (module === 1 / 0) return <span className={require} aria-label={Chunk388032.intl.string(Chunk388032.t.PqEzn5)}>{"∞"}</span>;
