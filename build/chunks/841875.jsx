...
function C() {
  ...
  return <div className={Chunk590305.filterBar}>{<div className={i()(Chunk590305.filterSection, Chunk590305.itemType)}>{<Chunk481060.Text variant={"text-md/semibold"} className={Chunk590305.filterTitle}>{Chunk388032.intl.string(Chunk388032.t.Qk6r1d)}</Chunk481060.Text>}{Chunk215023.xg.map(e => <E filter={e} trackFilterAction={d} />)}{Chunk335818 && <Chunk481060.XZJ value={exports} onChange={() => {
          Chunk410030("filter orb eligible ".concat(false === exports ? "on" : "off")), module()
        }}><Chunk481060.Text variant={"text-md/normal"}>{Chunk388032.intl.string(Chunk388032.t.AHHHgI)}</Chunk481060.Text></Chunk481060.XZJ>}</div>}{<S trackFilterAction={Chunk410030} />}{<j trackFilterAction={Chunk410030} />}{Chunk990705 && <Chunk481060.zxk variant={"secondary"} onClick={() => {
        Chunk410030("filter reset"), require()
      }} text={Chunk388032.intl.string(Chunk388032.t.jwH6KS)} fullWidth={true} />}</div>
}
let E = e => {
    var t;
    let {
  ...
  S = e => {
    ...
    return <div className={O.filterSection}>{<u.Text variant={"text-md/semibold"} className={O.filterTitle}>{v.intl.string(v.t.K1xGoK)}</u.Text>}{<y colors={n} trackFilterAction={t} />}{<y colors={a} trackFilterAction={t} />}</div>
  },
  y = e => {
    let {
      colors: t,
    ...
    return <div className={O.colorSwatchRow}>{t.map(e => {
        let {
          color: t,
          label: i,
          enum: o
  ...
  j = e => {
    let {
      ...
      icon: <u.wGt size={"xs"} color={p(c.h.ANIME)} />,
      enum: c.h.ANIME
    }, {
      name: v.intl.string(v.t["3WoZBQ"]),
      icon: <u.iWm size={"xs"} color={p(c.h.GAMING)} />,
      enum: c.h.GAMING
    }, {
      name: v.intl.string(v.t.yuEmLi),
      icon: <u.h_8 size={"xs"} color={p(c.h.CUTE_COZY)} />,
      enum: c.h.CUTE_COZY
    }, {
      name: v.intl.string(v.t.mMvCHh),
      icon: <u.lqV size={"xs"} color={p(c.h.SCI_FI)} />,
      enum: c.h.SCI_FI
    }, {
      name: v.intl.string(v.t.TlhOQE),
      icon: <u.ED8 size={"xs"} color={p(c.h.FOOD_DRINKS)} />,
      enum: c.h.FOOD_DRINKS
    }, {
      name: v.intl.string(v.t["4IaUIC"]),
      icon: <u.HI3 size={"xs"} color={p(c.h.FANTASY)} />,
      enum: c.h.FANTASY
    }, {
      name: v.intl.string(v.t.w0nSGx),
      icon: <u.Ffc size={"xs"} color={p(c.h.ANIMALS_PETS)} />,
      enum: c.h.ANIMALS_PETS
    }, {
      name: v.intl.string(v.t.cJng7u),
      icon: <u.hg2 size={"xs"} color={p(c.h.NATURE)} />,
      enum: c.h.NATURE
    }, {
      name: v.intl.string(v.t["5mUvyM"]),
      icon: <u.Roe size={"xs"} color={p(c.h.MOVIES_TV_SHOWS)} />,
      enum: c.h.MOVIES_TV_SHOWS
    }, {
      name: v.intl.string(v.t.MB9H5e),
      icon: <u.Z6G size={"xs"} color={p(c.h.DARK_MOODY)} />,
      enum: c.h.DARK_MOODY
    }], [p]);
    return <div className={O.filterSection}>{<u.Text variant={"text-md/semibold"} className={O.filterTitle}>{v.intl.string(v.t.t1Ztrq)}</u.Text>}{<div className={O.themeRow}>{g.map(e => {
          let {
            name: l,
            icon: o,
            enum: c
