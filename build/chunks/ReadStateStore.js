...
function ej(e) {
  ...
  if (!(n && !e._isJoinedThread) && !F.Z.isAtBottom(e.channelId)) return eL(e, "Not at bottom"), false;
...
class ek {
  ...
  _shouldAck(e, t, n) {
    var r;
    return !!e || !!n || !!t || !this.isManualAck && (this.type !== ei.W.CHANNEL || !!this.loadedMessages || !!(null == (r = V.Z.getChannel(this.channelId)) ? true : r.isForumLikeChannel()))
  }
  _ack(e, t) {
    let {
      outgoingAck: r
...
function e$(e) {
  ...
  for (let e of (null != o && (ek.get(o, ei.W.NOTIFICATION_CENTER).lastMessageId = G.default.fromTimestamp(Date.now())), eB(r), eM(i), n)) eM("full_sync" === e.channels.op ? e.channels.items : e.channels.writes), null != e.channelTimestampUpdates && e3(e.channelTimestampUpdates), eG(e);
...
function e5(e) {
  ...
  null != a && (r.length > 0 && 1 === G.default.compare(r[0].id, i.ackMessageId) && 0 === i.unreadCount || a.hasPresent() || a.jumpTargetId === i.ackMessageId ? i.rebuildChannelState() : n && null != i.ackMessageId && a.has(i.ackMessageId, true) && (i.unreadCount += r.length, null == i.oldestUnreadMessageId && i.rebuildChannelState())), eU(r.map(e => e.thread).filter(M.lm))
}
...
function tI(e) {
  let {
    channelId: t
  } = e, n = Z.ZP.getCurrentSidebarChannelId(t), r = V.Z.getChannel(t);
  if (null != r) {
    var i;
    let e = ek.get(r.id);
...
function tq(e) {
  ...
  for (let e in t) {
    ...
    let i = t[e],
      a = ek.getIfExists(e);
    if (null != a)
      for (let e of [null == (n = i.new_messages) ? true : n[0]].concat(null != (r = i.modified_messages) ? r : []).filter(M.lm)) 1 === G.default.compare(e.id, a.lastMessageId) && (a.lastMessageId = e.id)
  }
}
