...
function Z(e) {
  ...
  }, [n]), <div className={y.qrCodeContainer}>{"" !== t && n ? (0, r.jsxs)(r.Fragment, {
...
let P = e => {
  ...
  return <u.Text variant={"text-md/normal"} color={"text-danger"} className={t}>{n}</u.Text>
};
...
function R(e) {
  ...
  switch (t.step) {
    case 0:
    case 1:
      return <r.Fragment>{<Z text={1 === t.step ? "https://discord.com/ra/".concat(t.fingerprint) : ""} />}{<m.Dx className={C.marginBottom8}>{N.intl.string(N.t.UPiHaG)}</m.Dx>}{null != i ? <P>{i}</P> : <m.DK>{N.intl.format(N.t["Qq+A6u"], {})}</m.DK>}{<c.zx size={c.Ph.LARGE} look={c.iL.LINK} color={c.Tt.LINK} disabled={s} onClick={() => (function(e) {
          ...
          })(o)}>{N.intl.string(N.t["/kpMDg"])}</c.zx>}</r.Fragment>;
    case 3:
    case 2: {
      let {
        user: e
      } = t;
      return <r.Fragment>{<h.Z className={y.qrAvatar} user={e} size={u.EFr.SIZE_120} isMobile={true} status={S.Sk.ONLINE} />}{<m.Dx className={C.marginBottom8}>{N.intl.string(N.t.apGCUV)}</m.Dx>}{<m.DK>{N.intl.format(N.t.Cbl5JC, {
            username: "".concat(j.ZP.getUserTag(e))
          })}</m.DK>}{<c.zx look={c.zx.Looks.BLANK} color={c.zx.Colors.LINK} size={c.zx.Sizes.MIN} onClick={n} className={y.startOverButton}>{N.intl.string(N.t.nOOhws)}</c.zx>}</r.Fragment>
    }
    case 4:
    case 5:
      return <u.$jN type={u.$jN.Type.WANDERING_CUBES} />
  }
}
...
function w(e) {
  ...
  }, [c, t, d, m]), <r.Fragment>{<div className={y.verticalSeparator} />}{<u.qBt fillParent={true} className={y.qrLogin} step={g} steps={[0, 1]}><div className={y.qrLoginInner}><R state={c} cancel={h} conditionalMediationAbortController={n} isPasswordlessActive={l} /></div></u.qBt>}</r.Fragment>
}