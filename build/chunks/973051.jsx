...
function g(e) {
  ...
  return (l.useEffect(() => {
    ...
    return null != t && null != t.lastElementChild && e.observe(t.lastElementChild), () => {
      null != t && null != t.lastElementChild && e.unobserve(t.lastElementChild)
  ...
  }, [Z, P]), null == C) ? null : <div className={p.prompt}><div className={p.promptContent} ref={P}>{<o.h21 className={p.scrollerContent} ref={O}>{<o.Text variant={"text-sm/medium"} color={"text-muted"}>{f.intl.string(f.t["b1JC+v"])}</o.Text>}{<o.X6q variant={"heading-xl/semibold"}>{f.intl.string(f.t.prJqwc)}</o.X6q>}{<div className={p.termsFieldBody} ref={Z}>{C.values.map((e, t) => (0, r.jsxs)("div", {
          ...
          }, "term-".concat(t)))}</div>}</o.h21>}{<div className={p.overlay} />}{<div className={p.navButtons}>{<div className={p.leftButtons}>{false !== b && (0, r.jsx)(o.zxk, {
            variant: "secondary",
            size: "md",
            ...
            icon: o.j9r,
            iconPosition: "start"
          })}</div>}{<div className={p.rightButtons}>{<o.Text className={p.helpText} variant={"text-xs/normal"} color={"text-muted"}>{y ? f.intl.string(f.t.arAe3N) : f.intl.string(f.t.D0CVAQ)}</o.Text>}{<o.zxk variant={"primary"} size={"md"} text={"".concat(f.intl.string(f.t["8SuVoK"]), " \uD83C\uDF89")} onClick={I} disabled={!y} />}</div>}</div>}</div></div>
}