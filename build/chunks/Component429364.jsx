...
let h = Chunk64700.createContext(null);
...
function p(e) {
  ...
  let w, O, A, I, T, L, R, {
    ...
    } = (w = P.length / 1e3, O = null != (x = null == (C = P.editMetadata) ? true : C.start) ? x : 0, A = null != (y = null == (k = P.editMetadata) ? true : k.end) ? y : 0, I = 0 !== O || 0 !== A && A !== w, {
      initialDuration: w,
      initialCropStart: O,
      initialCropEnd: A,
      isCropped: I,
    ...
    [K, V] = a.useState({
      ...
      cropStart: z,
      cropEnd: 0 === G ? D : G,
      voiceAudioEnabled: null == (p = null == (b = P.editMetadata) ? true : b.voiceAudio) || p,
      applicationAudioEnabled: null == (v = null == (g = P.editMetadata) ? true : g.applicationAudio) || v,
      soundboardAudioEnabled: null == (f = null == (j = P.editMetadata) ? true : j.soundboardAudio) || f
    }),
    {
      clipName: F,
      isTemporary: W,
    ...
    Y = a.useRef(B),
    [Z, ee] = a.useState(D),
    [et, el] = a.useState(false),
    [en, ea] = a.useState(false),
    er = a.useRef(new Set),
    ei = a.useRef(false),
  ...
  (0, i.A)(() => {
    var e;
    let t = null == (e = Q.current) ? true : e.videoElement;
    if (null == t || !ei.current) return;
    let l = t.currentTime;
    Y.current !== l && (Y.current = l, er.current.forEach(e => {
      var t;
      return null == (t = e.onTimeUpdate) ? true : t.call(e, l)
    }))
  }), a.useEffect(() => {
    if (null == es) return;
    let e = () => {
        el(true), er.current.forEach(e => {
          var t;
          return null == (t = e.onPlay) ? true : t.call(e)
        })
      },
      t = () => {
        el(false), er.current.forEach(e => {
          var t;
      ...
      n = () => {
        var e;
        ea(true), ee(es.duration), null == Q || null == (e = Q.current) || e.seek(B)
      },
      a = () => {
        ei.current = true
      };
    if (es.addEventListener("play", e), es.addEventListener("pause", t), es.addEventListener("durationchange", l), es.addEventListener("loadedmetadata", n), es.addEventListener("seeked", a), es.duration > 0 && ee(es.duration), es.readyState >= 1) {
      var r;
      ea(true), null == Q || null == (r = Q.current) || r.seek(B)
    }
    return el(!es.paused), () => {
