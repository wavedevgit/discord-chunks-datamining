...
class D {
  ...
  _check() {
    ...
    let t = [h.ABu.TWITCH],
      n = Date.now();
    S <= n && (t.push(h.ABu.YOUTUBE), S = n + y), Promise.allSettled(e.filter(e => t.includes(e.type)).map(e => e.type === h.ABu.TWITCH ? this._checkTwitch(e) : this._checkYouTube(e))).then(e => {
      if (this._started) {
        var t;
