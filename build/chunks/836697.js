/** Chunk was on 53745 **/
n.d(t, {
  Z: () => S
}), n(388685), n(539854);
var r = n(200651),
  i = n(192379),
  l = n(120356),
  o = n.n(l),
  s = n(442837),
  a = n(481060),
  c = n(110924),
  u = n(540059),
  d = n(693546),
  h = n(863249),
  p = n(937111),
  f = n(914010),
  g = n(289090),
  m = n(727258),
  b = n(234383),
  y = n(179809),
  _ = n(588275),
  O = n(388032),
  v = n(315268);

function C(e) {
  let {
    onActivate: t,
    children: n
  } = e, l = (0, s.e7)([p.Z], () => p.Z.hasJoinRequestCoackmark()), o = i.useCallback(() => {
    h.ZP.clearCoachmark()
  }, []), u = (0, c.Z)(l), d = i.useRef(null);
  return i.useEffect(() => {
    if (l && l !== u) {
      var e, n, r, i, o;
      t(), null == (o = d.current) || null == (i = o.ref) || null == (r = i.current) || null == (n = r.layerRef) || null == (e = n.current) || e.updatePosition()
    }
  }, [d, l, u, t]), (0, r.jsxs)("div", {
    className: v.container,
    children: [(0, r.jsx)(a.yRy, {
      ref: d,
      shouldShow: l,
      renderPopout: () => (0, r.jsxs)("div", {
        className: v.popoutContainer,
        children: [(0, r.jsxs)("div", {
          className: v.popoutContent,
          children: [(0, r.jsxs)("div", {
            className: v.coachmarkTextContainer,
            children: [(0, r.jsx)(a.X6q, {
              variant: "heading-md/medium",
              children: O.intl.string(O.t["0YV0YG"])
            }), (0, r.jsx)(a.Text, {
              variant: "text-xs/normal",
              color: "text-muted",
              children: O.intl.string(O.t["0UQVDA"])
            })]
          }), (0, r.jsx)(a.zxk, {
            fullWidth: !0,
            size: a.PhG.SMALL,
            color: a.Ttl.BRAND,
            onClick: o,
            children: O.intl.string(O.t["4r+amZ"])
          })]
        }), (0, r.jsx)("div", {
          className: v.popoutCaretLeft
        })]
      }),
      position: "right",
      animationPosition: "right",
      align: "top",
      spacing: 16,
      children: e => {
        var t, n;
        return (0, r.jsx)("div", (t = function(e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = null != arguments[t] ? arguments[t] : {},
              r = Object.keys(n);
            "function" == typeof Object.getOwnPropertySymbols && (r = r.concat(Object.getOwnPropertySymbols(n).filter(function(e) {
              return Object.getOwnPropertyDescriptor(n, e).enumerable
            }))), r.forEach(function(t) {
              var r;
              r = n[t], t in e ? Object.defineProperty(e, t, {
                value: r,
                enumerable: !0,
                configurable: !0,
                writable: !0
              }) : e[t] = r
            })
          }
          return e
        }({}, e), n = n = {
          className: v.popoutAnchor
        }, Object.getOwnPropertyDescriptors ? Object.defineProperties(t, Object.getOwnPropertyDescriptors(n)) : (function(e, t) {
          var n = Object.keys(e);
          if (Object.getOwnPropertySymbols) {
            var r = Object.getOwnPropertySymbols(e);
            n.push.apply(n, r)
          }
          return n
        })(Object(n)).forEach(function(e) {
          Object.defineProperty(t, e, Object.getOwnPropertyDescriptor(n, e))
        }), t))
      }
    }), n]
  })
}

function S(e) {
  let {
    onActivate: t
  } = e, n = (0, u.Q3)("PendingGuildJoinRequestsFolderNode"), [l, S] = i.useState(!1), j = (0, s.e7)([f.Z], () => f.Z.getGuildId()), E = (0, s.e7)([p.Z], () => p.Z.hasFetchedRequestToJoinGuilds), x = (0, b.Z)(), I = (0, c.Z)(j), P = i.useMemo(() => {
    let e = (0, m.qQ)({
      folderId: g.S.PENDING_JOIN_REQUESTS_FOLDER,
      folderName: O.intl.string(O.t["scsU+v"]),
      expanded: l,
      guildIds: []
    });
    for (let t of x) e.children.push((0, m.Mg)(t, e.id));
    return e
  }, [x, l]);
  i.useEffect(() => {
    l && !E && d.Z.fetchRequestToJoinGuilds()
  }, [l, E]);
  let w = null != j && x.includes(j);
  return (i.useEffect(() => {
    !l && w && I !== j && S(!0)
  }, [l, w, I, j]), 0 === x.length) ? null : (0, r.jsx)(C, {
    onActivate: t,
    children: (0, r.jsx)("div", {
      className: o()({
        [v.folderWrapperCollapsed]: !l
      }),
      children: (0, r.jsx)(y.Z, {
        folderNode: P,
        expanded: l,
        useCircleMask: !l && !w,
        selected: w,
        draggable: !1,
        sorting: !1,
        onExpandCollapse: () => {
          S(!l), h.ZP.clearCoachmark()
        },
        folderIconContentClassName: v.folderIconContent,
        folderIconContent: (0, r.jsx)(a.wGF, {
          size: n ? "sm" : "md",
          color: "currentColor",
          className: v.pendingIcon
        }),
        renderChildNode: function(e, t, n, i) {
          return e.type !== m.eD.GUILD ? null : (0, r.jsx)(_.Z, {
            guildNode: e,
            height: i,
            "aria-setsize": n,
            "aria-posinset": t
          }, e.id)
        }
      })
    })
  })
}