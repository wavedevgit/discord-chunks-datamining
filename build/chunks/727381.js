/** Chunk was on 16318 **/
n.d(t, {
  Z: () => x
});
var r = n(255367);
n(73800);
var i = n(120356),
  l = n.n(i),
  a = n(399606),
  o = n(481060),
  s = n(271383),
  c = n(594174),
  u = n(777288),
  d = n(753042),
  p = n(787469),
  h = n(477718),
  f = n(791176),
  m = n(908474),
  g = n(467014),
  b = n(388032),
  _ = n(649986);

function C() {
  return (0, r.jsxs)("div", {
    className: _.header,
    children: [(0, r.jsxs)("div", {
      className: _.headerTitle,
      children: [(0, r.jsx)(u.Z, {
        width: 16,
        height: 16
      }), (0, r.jsx)(o.X6q, {
        variant: "heading-md/semibold",
        children: b.intl.string(b.t.kj3tz8)
      })]
    }), (0, r.jsx)("div", {
      children: (0, r.jsx)(o.M2$, {
        shortcut: "esc",
        keyClassName: _.keyComboInner,
        className: _.keyComboContainer
      })
    })]
  })
}

function x(e) {
  let {
    userId: t,
    guildId: n,
    location: i,
    className: u,
    onNavigate: b
  } = e, x = (0, a.e7)([c.default], () => c.default.getUser(t), [t]), y = (0, a.e7)([s.ZP], () => s.ZP.getMember(n, t), [n, t]);
  return null == x || null == y ? null : (0, r.jsxs)("div", {
    className: l()(_.container, u),
    children: [(0, r.jsx)(C, {}), (0, r.jsxs)(o.Ttm, {
      className: _.innerContainer,
      children: [(0, r.jsx)(m.Z, {
        member: y
      }), (0, r.jsx)(g.Z, {
        userId: t,
        guildId: n
      }), (0, r.jsx)(f.Z, {
        member: y,
        onNavigate: b
      }), (0, r.jsx)(p.Z, {
        member: y
      }), (0, r.jsx)(d.Z, {
        member: y
      }), (0, r.jsx)(h.Z, {
        member: y
      })]
    })]
  })
}