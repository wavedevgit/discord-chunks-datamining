...
function j(t) {
  ...
  return <a.kL8 className={r()(f.container, {
      [f.canceled]: Z,
      [f.clickable]: null != x,
      [f.active]: y
    })} onClick={t => {
      t.stopPropagation(), Z || null == x || x(e)
    }} onContextMenu={w} aria-label={""}>{<m.z startTime={k.toISOString()} status={null != S ? S : _} eventType={C.entity_type} guildEventId={C.id} recurrenceId={e} className={f.timeStatus} />}{Z && <a.Text variant={"text-sm/semibold"} color={"text-danger"} className={f.canceledStatus}>{v.intl.string(v.t.fyBVRk)}</a.Text>}{<a.ua7 text={v.intl.string(v.t.UKOtz8)} position={"top"} aria-label={v.intl.string(v.t.bt75u7)}>{t => {
        var e, n;
