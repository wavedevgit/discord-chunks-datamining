...
function b(e) {
  ...
  return <div className={o()(g.applicationDetails, {
      [g.noPadding]: N.enabled
    })}>{null != n ? <y icon={l.xPt} text={m.intl.format(m.t["5k5OKC"], {
        origin: n
      })} /> : null}{<y icon={l.mBM} text={(0, _.i)(i)} />}{<y icon={l.T39} text={m.intl.formatToPlainString(m.t["+1bjc3"], {
        date: T
      })} />}{s.includes(a.x.BOT) && null != O ? <y icon={l.wGt} text={m.intl.formatToPlainString(m.t.UHGHSE, {
        guildCount: O
      })} /> : null}{<y icon={l.b7C} text={S} />}{null != I ? I.map(e => {
      let t = (0, u.PM)(e),
        n = E(e);
      return null != n && null != t ? <y icon={n} text={t} /> : null
    }) : null}</div>
}
...
function y(e) {
  ...
  return <div className={g.entry}>{<t className={g.entryIcon} color={"currentColor"} />}{<s.x variant={"text-xs/normal"} className={g.entryInner}>{n}</s.x>}</div>
}