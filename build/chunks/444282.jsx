...
function E(e) {
  ...
  return null != O ? <p.Z channelId={t} upload={O} keyboardModeEnabled={o} draftType={d.d.SlashCommand} label={(0, r.jsxs)(i.Fragment, {
    ...
    })} canEdit={false} /> : <_.Z id={n.name} channelId={t} keyboardModeEnabled={o} onKeyDown={I} className={a()(m.emptyOption, {
      [m.emptyOptionActive]: b
    })} draftType={d.d.SlashCommand} ref={v}>{<span className={a()(m.optionName, {
        [m.optionNameActive]: b
      })}>{n.name}</span>}{<s.P3F className={m.clickContainer} onClick={() => {
        var e;
        return null == (e = E.current) ? true : e.activateUploadDialogue()
      }}><div className={m.commandOptionContainer}>{<img src={g} className={m.optionIcon} alt={""} />}{<s.Text className={m.optionHelp} variant={"text-sm/normal"}>{"Drag and drop or click to upload file"}</s.Text>}{<c.Z ref={E} onChange={N} multiple={false} tabIndex={false} aria-hidden={true} className={m.fileInput} />}</div></s.P3F>}</_.Z>
}