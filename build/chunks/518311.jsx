...
function eb(e) {
  ...
  return <w.Z align={w.Z.Align.CENTER} justify={w.Z.Justify.CENTER} direction={w.Z.Direction.VERTICAL} className={s()(eo.errorState, t)}>{<div className={s()(eo.errorStateIcon, ea.marginBottom20)} />}{n}</w.Z>
}
let ey = e => {
    var t;
    let {
    ...
    return <p.P3F onClick={() => {
        (0, k.XU)(er.ME, n.id), r()
      }}><div className={eo.confirmChannelItemContainer}>{<p.qEK src={(0, R.x)(n)} size={p.EFr.SIZE_24} aria-label={o} />}{<span className={eo.confirmChannelName}>{o}</span>}{<span className={eo.lastActiveTimestamp}>{u()(J.default.extractTimestamp(a)).fromNow()}</span>}</div></p.P3F>
  },
  eO = e => {
    var {
      onConfirm: t,
...
class ev extends(r = Chunk73800.PureComponent) {
  ...
  renderCreateGroupButton() {
    ...
    return <Chunk481060.zxk variant={"primary"} fullWidth={true} disabled={r} onClick={this.handleInviteUsers} text={Chunk73800} />
  }
  renderAddUsersButton() {
    ...
    return <div className={Chunk382314.addButton}><Chunk481060.zxk variant={"primary"} size={"sm"} text={Chunk388032.intl.string(Chunk388032.t.OYkgVl)} disabled={r} onClick={this.handleInviteUsers} /></div>
  }
  renderSubtitle() {
    ...
    return <Chunk481060.Text variant={"text-xs/normal"} className={s()(Chunk382314.subtitle, {
        [Chunk382314.subtitleWarning]: module < 0
      }, Chunk20493.marginTop4)}>{exports}</Chunk481060.Text>
  }
  renderSearchBar() {
    var e;
    let {
    ...
    }), <div className={Chunk382314.searchBar}>{<Chunk794433.Z ref={this.searchBarRef} className={Chunk382314.searchBarComponent} autoFocus={true} placeholder={0 === Chunk120356.size ? Chunk388032.intl.string(Chunk388032.t.Wxnb6u) : ""} disabled={this.isPartyFull()} size={Chunk794433.Z.Sizes.MEDIUM} maxHeight={this.props.inBornThisNamedExperiment ? eE : true} query={exports} selectedRow={Chunk73800} sections={[null != (e = null == r ? true : r.length) ? module : 0]} tags={s} onSelect={this.handleSelect} onSelectionChange={this.handleSelectionChange} onQueryChange={this.handleQueryChange} onRemoveTag={this.handleRemoveUser} onHeightChange={this.scrollToCounterSearchBarHeightChange} inputProps={{
          "aria-labelledby": em,
          "aria-controls": eg,
          "aria-expanded": true,
          "aria-activedescendant": "user-row-".concat(Chunk73800),
          autoComplete: "off"
        }} />}{this.renderAddUsersButton()}</div>
  }
  renderHeader() {
    ...
    return <Chunk481060.xBx direction={Chunk600164.Z.Direction.VERTICAL} align={Chunk600164.Z.Align.STRETCH} className={Chunk382314.header} separator={module}>{Chunk873546.tq ? this.renderMobileCloseButton() : null}{<Chunk481060.X6q id={em} variant={"heading-lg/semibold"}>{exports}</Chunk481060.X6q>}{this.renderSubtitle()}{<Chunk481060.olH className={Chunk382314.closeButton} onClick={this.props.onClose} />}</Chunk481060.xBx>
  }
  renderBody() {
    let {
      channel: e,
    ...
    if (this.isNotFriends()) {
      ...
      return <eb className={Chunk382314.notFriends}>{<Chunk481060.Text variant={"text-sm/medium"} color={"text-secondary"}>{Chunk388032.intl.format(Chunk388032.t["eg+R9/"], {
            username: r
          })}</Chunk481060.Text>}{<div data-button-hoisted-classname-wrapper={true} className={Chunk20493.marginTop20}><Chunk481060.zxk variant={"primary"} size={"sm"} text={Chunk73800 ? Chunk388032.intl.string(Chunk388032.t.xMH6vL) : Chunk388032.intl.string(Chunk388032.t["PMsq/f"])} disabled={Chunk73800} onClick={() => null != require ? this.handleAddFriend(require) : null} /></div>}</eb>
    }
    return exports ? this.isPartyFull() ? <eb className={Chunk382314.partyFull}>{<div>{Chunk388032.intl.string(Chunk388032.t.OtTQDw)}</div>}{<div>{Chunk388032.intl.format(Chunk388032.t.xYr009, {
          number: this.getMaxParticipants()
        })}</div>}</eb> : 0 === require.length ? <eb className={s()(Chunk382314.noResults, Chunk20493.marginBottom20)}><div>{Chunk388032.intl.string(Chunk388032.t.SV4DJy)}</div></eb> : <Chunk481060.aVo ref={this.scrollerRef} sections={[require.length]} className={Chunk382314.scroller} innerClassName={Chunk382314.scrollerInner} renderRow={this.renderRow} rowHeight={this.getRowHeight} renderSection={this.renderSection} sectionHeight={0} onScroll={this.handleScroll} paddingTop={0} paddingBottom={14} fade={true} role={true} innerRole={"listbox"} innerId={eg} innerAriaMultiselectable={true} innerAriaOrientation={"vertical"} /> : <eb className={Chunk382314.noFriends}>{<div>{Chunk388032.intl.string(Chunk388032.t["7orY6O"])}</div>}{<div data-button-hoisted-classname-wrapper={true} className={Chunk20493.marginTop20}><Chunk481060.zxk variant={"primary"} size={"sm"} text={Chunk388032.intl.string(Chunk388032.t.w5uwoK)} autoFocus={true} fullWidth={true} onClick={this.handleAddFriendNavigation} /></div>}</eb>
  }
  renderSection() {
    return null
  }
  renderInviteLink() {
    ...
    return <Chunk73800.Fragment>{<Chunk481060.vwX tag={"h5"} className={Chunk20493.marginBottom8}>{Chunk388032.intl.string(Chunk388032.t.t3O2BQ)}</Chunk481060.vwX>}{<Chunk481060.kO8 supportsCopy={Chunk572004.wS} placeholder={(0, Chunk366980.Z)(Chunk388032.intl.string(Chunk388032.t.lPVBqK))} value={Chunk392711 ? require ? Chunk388032.intl.string(Chunk388032.t["6HzNgY"]) : Chunk120356 : ""} buttonColor={Chunk755721.zx.Colors.BRAND} text={Chunk392711 ? r ? Chunk388032.intl.string(Chunk388032.t.q30c5u) : Chunk388032.intl.string(Chunk388032.t.OpuAlJ) : Chunk388032.intl.string(Chunk388032.t.qzxqUV)} mode={r ? Chunk481060.uA3.SUCCESS : Chunk481060.uA3.DEFAULT} onCopy={() => {
          Chunk392711 ? this.handleCopyInvite(Chunk120356) : this.createInvite()
        }} />}{Chunk120356.length > 0 ? <Chunk481060.Text variant={"text-xs/normal"} className={s()(Chunk20493.marginTop8, Chunk382314.footerText)}>{Chunk388032.intl.format(Chunk388032.t.ZVdJMz, {
          numHours: "".concat(eh)
        })}</Chunk481060.Text> : null}</Chunk73800.Fragment>
  }
  renderFooter() {
    let {
      hasFriends: e,
    ...
    if (null == exports) return <Chunk481060.mzw className={Chunk382314.footer}>{require.size > 1 && <eI previewIcon={this.state.previewIcon} selectedUsers={require} channelName={this.state.newChannelName} onChange={this.handleChannelNameChange} onIconChange={this.handleIconChange} onIconRemove={this.handleIconRemove} />}{<Chunk481060.hE2 fullWidth={true} direction={"horizontal"}>{<Chunk481060.zxk variant={"secondary"} text={Chunk388032.intl.string(Chunk388032.t["ETE/oK"])} onClick={this.props.onClose} />}{this.renderCreateGroupButton()}</Chunk481060.hE2>}</Chunk481060.mzw>;
    let r = exports.isDM() ? this.renderCreateGroupButton() : this.renderInviteLink();
    if (null != r) return <Chunk481060.mzw className={Chunk382314.footer}>{r}</Chunk481060.mzw>
  }
  render() {
    let {
      transitionState: e,
    ...
    return <Chunk481060.Y0X transitionState={null != module ? module : Chunk481060.Dvm.ENTERED} className={s()(Chunk382314.popout, {
        [Chunk382314.showAsModal]: exports,
        [Chunk382314.showAsPopout]: !exports
      })} fullscreenOnMobile={false} parentComponent={"PrivateChannelRecipientsInvite"}>{this.renderHeader()}{this.renderSearchBar()}{this.renderBody()}{this.renderFooter()}</Chunk481060.Y0X>
  }
  handleAddFriend(e) {
  ...
  constructor(...e) {
    super(...e), es(this, "state", {
    ...
    }), es(this, "renderMobileCloseButton", () => <div className={eo.mobileToolsContainer} ref={this._mobileCloseRef}><p.JcV containerRef={this._mobileCloseRef}><div><v.Z className={eo.mobileToolsCloseIcon} closeAction={this.props.onClose} keybind={"ESC"} /></div></p.JcV></div>), es(this, "getRowHeight", (e, t) => {
      if (e > 0) return 0;
      let {
        results: n
      } = this.props;
      ...
      return <en.Z row={n} user={c} hideDiscriminator={s} comparator={u} checked={d} disabled={f} selected={n === a} onClick={this.handleClick} onMouseEnter={this.focusResult} aria-posinset={n + 1} aria-setsize={r.length} inlineUsername={!this.props.inBornThisNamedExperiment} />
    }), es(this, "forceFocus", () => {
...
function eI(e) {
  ...
  return <div className={eo.customizationContainer}>{<x.B className={eo.iconSelector} channel={null} previewIcon={r} onIconChange={o} onIconRemove={a} analyticsLocations={l} petite={true} allowRemovingIcon={false} />}{<p.Text className={eo.channelNameLabel} variant={"text-sm/medium"} color={"header-muted"}>{ei.intl.string(ei.t.YynaLC)}</p.Text>}{<_.Is aria-label={ei.intl.string(ei.t.YynaLC)} className={eo.channelNameInput} inputClassName={eo.channelNameInputInner} name={"create-gdm-channel-name"} placeholder={c} value={n} onChange={s} />}</div>
}
...
function eS(e) {
  ...
  let {
    ...
    location: m
  } = e, g = null != l ? l : null == r ? p.kL_ : p.ejJ, E = (0, L.a)(m), b = "channel-invite-modal-".concat(null == r ? true : r.id), y = (0, p.VXO)(b), O = o.useCallback(() => (0, p.ZDy)(() => Promise.resolve(e => <eT{...eu(el({}, e), {
    channel: r,
    inBornThisNamedExperiment: E
  })} />), {
    modalKey: b
  }), [r, b, E]), v = o.useRef(null), [I, S] = o.useState(null != r && r.isGroupDM() && 0 === r.recipients.length), A = o.useCallback(() => {
    let e = () => y ? (0, p.Mr3)(b) : O(),
      t = () => S(e => !e);
    return E ? e() : t()
  }, [E, y, b, O]);
  o.useEffect(() => (h && X.S.subscribe(er.CkL.TOGGLE_DM_CREATE, A), () => {
    X.S.unsubscribe(er.CkL.TOGGLE_DM_CREATE, A)
  }), [h, A, E]);
  let N = (0, f.e7)([K.default], () => K.default.getUser(null == r ? true : r.getRecipientId())),
    C = e => <eT{...eu(el({}, e), {
  ...
  return ((0, T.ZP)(() => {
    (0, j._)()
  }), null != (t = null == N ? true : N.bot) && t || null != (n = null == N ? true : N.isProvisional) && n) ? null : E ? <et.ZP.Icon ref={v} onClick={O} icon={g} className={s} iconClassName={a} tooltip={c} tooltipPosition={u} /> : <p.yRy targetElementRef={v} renderPopout={C} position={d} shouldShow={I} nudgeAlignIntoViewport={true} autoInvert={true} align={_} onRequestClose={() => S(false)} animation={p.yRy.Animation.NONE} ignoreModalClicks={true} clickTrap={true}>{e => (0, i.jsx)(et.ZP.Icon, eu(el({}, e), {
      ref: v,
      onClick: A,
      icon: g,
      className: s,
...
function eA(e) {
  ...
  let a = (0, L.a)("PrivateChannelRecipientsInviteTextButton"),
    ...
    f = o.useCallback(() => (0, p.ZDy)(() => Promise.resolve(e => <eT{...eu(el({}, e), {
      channel: t,
      inBornThisNamedExperiment: a
    })} />)), [t, a]);
  return a ? <p.zxk onClick={f} variant={"primary"} buttonRef={s} text={ei.intl.string(ei.t["6Qgren"])} aria-label={ei.intl.string(ei.t["6Qgren"])} icon={p.ejJ} /> : <p.yRy targetElementRef={s} renderPopout={d} position={n} shouldShow={l} nudgeAlignIntoViewport={true} autoInvert={true} align={r} onRequestClose={() => c(false)} animation={p.yRy.Animation.NONE} ignoreModalClicks={true} clickTrap={true}>{e => (0, i.jsx)(p.zxk, eu(el({}, e), {
      onClick: u,
      variant: "primary",
      buttonRef: s,
...
function eN(e) {
  let t = eC(e);
  return new Set((0, l.chain)(B.Z.getMutablePrivateChannels()).values().filter(e => (0, G.bc)(e.type)).filter(e => eC(e.recipients) === t).map(e => e.id).value())
}
