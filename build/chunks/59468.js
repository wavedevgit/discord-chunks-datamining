...
async function d(e) {
  let {
    ...
    width: t,
    height: n
  }), m = (0, a._)(), h = await m, x = await f, p = x.filter(e => e.id.startsWith(i.vA.SCREEN)).map(e => {
    var t, n;
    let r = e.name;
  ...
  }(x.filter(e => e.id.startsWith(i.vA.WINDOW)), h), _ = [];
  return d.includes(i.vA.CAMERA) && (_ = Object.entries(u).filter(e => {
    let [t, n] = e;
    return !n.disabled
  }).map((e, t) => {
    let [n, i] = e, l = c[t % c.length], s = Uint8Array.from(atob(l), e => e.charCodeAt(0)), a = (0, r.xS)(s);
    return {
      id: "camera:" + i.id,
      name: i.name,
      url: a
