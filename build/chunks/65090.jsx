...
let E = {
  ...
  renderResults(e) {
    let t, n, {
      ...
      b = a.map((e, t) => <o.ZP.User guildId={p.guild_id} onClick={E} onHover={g} selected={_ === t} index={t} user={e.user} nick={e.nick} status={e.status} hidePersonalInformation={m.hidePersonalInformation} />),
      y = s.map((e, t) => <o.ZP.Generic onClick={E} onHover={g} selected={_ === t + a.length} index={a.length + t} text={e.text} description={m.hideMentionDescription ? null : e.description} aria-label={e.text} badge={"@game" === e.text ? "new" : true} />),
      O = l.map((e, t) => <o.ZP.Role onClick={E} onHover={g} selected={_ === t + a.length + s.length} index={a.length + s.length + t} role={e} hideDescription={m.hideMentionDescription} guildId={p.guild_id} />);
    return m.mentions.user === u.h3.DENY ? (t = f.t.MLiD1d, n = f.intl.string(f.t.LPJmLy)) : (t = f.t.rPNimp, n = f.intl.string(f.t["9Oq93t"])), <i.Fragment>{(0, c.gm)({
        titleWithQuery: t,
        titleWithoutQuery: n,
        query: h,
        getQuery: e => "".concat(d.ME).concat(e)
      })}{b}{a.length > 0 && s.length > 0 ? <o.ZP.Divider /> : null}{y}{a.length > 0 && l.length > 0 || s.length > 0 && l.length > 0 ? <o.ZP.Divider /> : null}{O}</i.Fragment>
  },
  onSelect(e) {
    let {
      results: {
