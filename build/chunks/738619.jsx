...
class U extends Chunk73800.PureComponent {
  ...
  render() {
    ...
    return <div className={a()(Chunk486850.activityInviteEducation, {
        [Chunk486850.activityInviteEducationFadeIn]: this.state.fadeIn
      })}>{<div className={Chunk486850.activityInviteEducationArrow} />}{<span>{Chunk388032.intl.format(Chunk388032.t["i/MoCg"], {
          game: module.name,
...
function G(e) {
  ...
  if (i.useLayoutEffect(() => {
    ...
    }, [p]), 0 === o.length && !E && !_) return null != t ? <U activity={t} isFocused={n} /> : null;
  let [I, T, S] = o, A = "";
  ...
  return <div className={a()(P.typing, {
      "stop-animation": !n,
      [P.isComboing]: f && _,
      [P.inTextChannel]: p
    }, s)}>{<div className={P.typingDots} ref={b}>{o.length > 0 && false !== u && <d.bbz className={P.ellipsis} dotRadius={3.5} themed={true} />}{<span className={P.text} aria-live={"polite"} aria-atomic={true}>{N}</span>}{<span className={P.text} style={{
          position: "absolute",
          visibility: "hidden"
        }} aria-hidden={true} ref={y}>{A}</span>}</div>}{<h.Z channel={l} isThreadCreation={c} />}{f && _ && <g.Z channelId={l.id} />}</div>
}
