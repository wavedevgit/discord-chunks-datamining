...
function w(e) {
  ...
  let eS = () => {
      if (eh) return <p.Z analyticsLocation={x} guild={D.guild} onClose={Y} />;
      ...
      return <y.Z className={es} showGradient={eo || ea} premiumModalAnalyticsLocation={x} subscriptionTier={et} size={l.zx.Sizes.SMALL} color={eo || ea ? l.zx.Colors.CUSTOM : l.zx.Colors.GREEN} onClick={() => {
          null == W || W(), Y()
        }} onSubscribeModalClose={K} textOptions={{
          textOverride: null != Q ? Q : e
        }} />
    },
    eA = ee ? a()(A.artContainer, A.artContainerBoxShadow, B) : a()(A.artContainer, B),
    eN = null;
  return null != ep.artURL ? eN = <img className={a()(Z, A.art)} alt={""} src={ep.artURL} /> : null != ep.artElement && (eN = ep.artElement), <c.Y0X data-migration-pending={true} className={a()(A.root, !eE && U)} aria-label={R} transitionState={H} parentComponent={"PremiumUpsellModal"}>{null != eN && <div className={eA}>{eN}{J ? <img className={A.sparkleBadge} alt={""} src={N} /> : null}</div>}{<c.hzk data-migration-pending={true} scrollbarType={"none"} className={a()(eo ? A.enhancedContent : A.content, !eE && G)}>{en ? (0, r.jsx)(c.$jN, {}) : eE ? (0, r.jsx)(O.ZP, {
        onClose: Y,
        type: w,
      ...
      })}</c.hzk>}{<c.mzw data-migration-pending={true} className={a()(A.footer, e_, {
        [A.enhancedFooter]: eo
      })}>{<div className={a()(A.primaryActions, {
          [A.enhancedPrimaryActions]: eo
        })}>{null != q ? <l.zx className={a()(A.secondaryAction, X, {
            [A.enhancedSecondaryAction]: eo
          })} onClick={z} size={l.zx.Sizes.SMALL} color={eo ? l.zx.Colors.CUSTOM : l.zx.Colors.PRIMARY} look={l.zx.Looks.LINK}>{q}</l.zx> : null}{eS()}</div>}{!er && !eo && <c.zxk variant={"secondary"} size={"sm"} text={null != ei ? ei : S.intl.string(S.t["13/7kZ"])} onClick={Y} />}</c.mzw>}{eo ? <c.olH onClick={Y} className={A.closeButton} /> : null}{ed}</c.Y0X>
}