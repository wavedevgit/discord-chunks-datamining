...
class N extends i.PureComponent {
  ...
  renderPartyStatus() {
    ...
    }, "state")] : this.isActionType(C.mFx.LISTEN) ? x.intl.string(x.t.gXYoq6) : x.intl.string(x.t.eyKDl5) : this.isActionType(C.mFx.JOIN_REQUEST) ? this.isPartyFull() ? x.intl.string(x.t.jfrMtr) : this.hasPartySize() && 0 !== u ? x.intl.formatToPlainString(x.t.XWapnZ, {
      number: u - c
    }) : null : this.isInParty() ? x.intl.string(x.t.KC26NT) : t || n || i || this.isEmbeddedActivityApplication() ? this.isPartyFull() ? x.intl.string(x.t.jfrMtr) : this.hasPartySize() && 0 !== u ? x.intl.formatToPlainString(x.t.XWapnZ, {
      number: u - c
  ...
  renderParty() {
    let {
      ...
      hideParty: n,
      guildId: i
    } = this.props, [l, a] = this.getPartySize(), o = this.hasPartySize() || this.isActionType(C.mFx.LISTEN) || this.isActionType(C.mFx.WATCH);
  ...
  render() {
    let {
      ...
      name: n,
      activity: i
    } = this.props, l = this.isActionType(C.mFx.JOIN) || this.isActionType(C.mFx.JOIN_REQUEST), o = this.isDeadInvite(), c = null;
