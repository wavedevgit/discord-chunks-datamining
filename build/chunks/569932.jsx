/** Chunk was on 10667 **/
/** chunk id: 569932, original params: t,n,e (module,exports,require) **/
require.d(exports, {
  A: () => u
});
var Chunk255367 = require("./255367.js"),
  Chunk73800 = require("./73800.js"),
  Chunk481060 = require("./481060.js"),
  Chunk63063 = require("./63063.js"),
  Chunk921869 = require("./921869.js"),
  Chunk981631 = require("./981631.js"),
  Chunk388032 = require("./388032.jsx"),
  Chunk660025 = require("./660025.js");

function u(t) {
  let {
    platformType: n,
    onContinue: e,
    onClose: u
  } = t, p = i.useMemo(() => [{
    label: l.intl.string(l.t["+eJP7u"]),
    subLabel: l.intl.string(l.t["+0VIUl"]),
    icon: r.gj8
  }, {
    label: l.intl.string(l.t.ZH4QFR),
    icon: r.iWm
  }], []), _ = s.Z.getArticleURL(c.BhN.PS_CONNECTION), b = l.intl.format(l.t.kqZQNT, {
    helpdeskArticleUrl: _
  });
  return <a.m platformType={n} headerConnect={l.intl.string(l.t.xAWHOz)} headerReconnect={l.intl.string(l.t["ZJ/vBg"])} body={b} onClose={u} onContinue={e} img={(0, o.jsx)("img", {
      src: d.Z,
      width: 230,
      height: 160,
      alt: ""
    })} valueProps={p} />
}