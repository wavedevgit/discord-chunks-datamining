...
let es = e => {
    ...
    return <div className={ei.friendsButtonContainer}>{<X.Qj{...ea(el({
        onClick: () => {
          let e;
      ...
      })} />}{f && !u && <k.Z className={ei.confetti} wind={0} sprites={ee.CA} spriteColors={ee.Br} firing={h} confettiConfig={{
          opacity: {
            type: "static",
            value: 1
          },
  ...
  eu = () => {
    ...
    return module > 0 ? <Chunk481060.mAB count={module} /> : null
  },
  ed = e => {
    let {
      selected: t
  ...
  ef = e => {
    ...
    return t ? <X.bU onClick={() => {
      ...
      }} aria-label={en.intl.string(en.t.cpT0Cg)} icon={g.Dio} /> : i > 0 ? <div className={ei.familyCenterPendingBadge}><g.mAB count={i} /></div> : <I.ZP contentTypes={[p.z.FAMILY_CENTER_NEW_BADGE]}>{e => {
        let {
          visibleContent: t
        } = e;
...
let em = Chunk73800.memo(function(e) {
  ...
  return <nav className={ei.privateChannels} aria-label={en.intl.string(en.t.ZH9aPz)}>{<G.Z childRef={m} tutorialId={"direct-messages"} position={"right"} offsetX={false}><div ref={m} className={a()(ei.searchBar, {
          [ei.themedHeaderMobile]: u.tq
        })}><h.zx grow={true} color={h.zx.Colors.PRIMARY} size={h.zx.Sizes.SMALL} fullWidth={true} onClick={eh}>{en.intl.string(en.t.LzcpeX)}</h.zx></div></G.Z>}{<Q.Z{...ea(el({}, e), {
      version: g,
      selectedChannelId: f,
      showDMHeader: true,
      listScrollerRef: _,
...
function eb() {
  return <div className={Chunk416350.sectionDivider} />
}
...
function e_() {
  ...
  return <em theme={Chunk73800} showLibrary={require} hasLibraryApplication={Chunk120356} storeLink={a} homeLink={module} premiumTabSelected={exports} shouldShowNitroTab={Chunk873546} isReferralProgramPopoverShowable={Chunk704215} shouldShowMessageRequestsRow={Chunk952265} shouldShowFamilyCenterRow={Chunk755721} selectedChannelId={Chunk990547} path={Chunk91192} />
}