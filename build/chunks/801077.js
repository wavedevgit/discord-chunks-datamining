...
function ee(e) {
  ...
  return (null == t ? void 0 : t.channelId) != null && Z.Z.canWithPartialContext(M.Plq.VIEW_CHANNEL, {
...
function en(e, t, n) {
  var l, r, a, o, d, f;
  let h, g = w.default.getCurrentUser(),
  ...
  for (let e of t) {
    ...
    let E = function(e) {
      ...
      }), null) : e === b.XB ? b.r9 : e.startsWith(C.H) ? X(e.slice(C.H.length)) : e.startsWith(N._) ? J(e.slice(N._.length)) : (q(e), null)
    }(h);
    if ((0, c.Z)(f)) {
      let t = (0, s.a)();
      if ((0, u.ZP)({
    ...
    let j = [];
    j = null != f && null != f.party && null != f.party.id ? Array.from(null != (o = T.Z.getParty(f.party.id)) ? o : []).reduce((e, t) => {
    ...
    }), (j = i().orderBy(j, [et], ["desc"])).length !== t.length && (A = !0), k.add(E.id), M.push({
      game: E,
      activity: f,
      activityUser: e,
  ...
  for (let e of t) {
    ...
    if (F.has(r) && B.has(n) || null == l || null == a || l.id === a.afkChannelId) null == l && (h = null, G = !0);
      ...
      o.filter(e => !E.includes(e.id)).forEach(e => t.push(e)), G ? F.has(r) || (h = null) : (h = a, G = !0), F.add(r), B.add(n), V.push({
        channel: l,
        guild: a,
        members: o,
        voiceStates: e
  ...
  ! function() {
    ...
    if (er()) {
      if (z.clear(), F = (B = (function(e) {
        ...
        }))).values().orderBy([el, e => e.partiedMembers.length > 1, e => e.applicationStreams.length > 0, e => e.voiceChannels.length > 0, e => e.currentActivities.length > 0, e => e.isSpotifyActivity, e => e.priorityMembers.map(e => e.user.username.toLowerCase()).join(" ")], ["asc", "desc", "desc", "desc", "desc", "asc", "asc"]).value().filter(e => {
